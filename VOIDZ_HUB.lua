...
local KNOWN_TOYS = {
	...
	"ClockAlarm", "ComputerLaptopOld", _Vzd({104,148,154,136,141,103,145,154,138}), _Vzd({104,148,154,136,141,103,151,148,156,147,108,151,134,158}), "CouchDarkGray",
...
function freeOneSnowballModel(ch)
	...
	for _, d in ipairs(ch:GetDescendants()) do
		if d:IsA("BodyPosition") or d:IsA("BodyVelocity") or d:IsA("BodyAngularVelocity")
			or d:IsA("BodyForce") or d:IsA("AlignPosition") or d:IsA("LinearVelocity") then
			pcall(function() d:Destroy() end)
		end
		if d:IsA("BasePart") then
			pcall(function()
	...
	task.spawn(function()
		...
		while S._snowFarmOn do
			...
			if grown >= want then
				stopSnowFarm(true)
				releaseSnowballsForGrab()
				notify(HUB_NAME, "Grown " .. grown .. _Vzd({69,161,69,139,151,138,138,69,153,148,69,140,151,134,135,69,84,69,139,145,142,147,140,69,84,69,138,157,149,145,148,137,138}), 2.5)
				break
			end
			if have < want then
...
function enableFurtherReachGamepass()
	pcall(function()
		...
		local ps = LP:FindFirstChild("PlayerScripts")
		if ps then
			local beam = ps:FindFirstChild(_Vzd({104,141,134,151,134,136,153,138,151,102,147,137,103,138,134,146,114,148,155,138})) or ps:FindFirstChild(_Vzd({104,141,134,151,134,136,153,138,151,102,147,137,103,138,134,146,114,148,155,138}), true)
...
local function isRealKickSignal(text, source)
	...
	if low:find(_Vzd({135,145,148,135,146,134,147}), 1, true) or low:find(_Vzd({152,153,134,136,144,144,142,136,144}), 1, true) or low:find(_Vzd({140,151,134,135,144,142,136,144}), 1, true) then return false end
...
_TAB_BUILDERS["server"] = function(sc, n)
		...
		makeButton(sc, {
			...
			callback = function()
				for _, name in ipairs({ _Vzd({145,134,140,120,151,155}), "softLag", "hardLag", _Vzd({137,138,152,153,151,148,158,120,151,155}), "destroyHyb", "blobSrv" }) do
					stopMass(name)
				end
		...
		makeButton(sc, {
			...
			callback = function()
				for _, name in ipairs({ "lagSrv", "softLag", "hardLag", "destroySrv", "destroyHyb", "blobSrv", "kill", "fling", "kick", "bring", "ragdoll", "fire", _Vzd({135,134,147,134,147,134}), "paint", _Vzd({155,148,146,142,153}) }) do
					stopMass(name)
				end
				S.toggles.lagServer = false
				S.toggles.destroyServer = false
...
_TAB_BUILDERS["loop"] = function(sc, n)
		...
		local loops = {
			...
			{ id = _Vzd({145,148,148,149,103,145,148,135,112,142,136,144}), title = "Loop Blobman Kick", tip = _Vzd({103,145,148,135,69,144,142,136,144,69,161,69,146,134,149,82,156,142,137,138}), waitRespawn = true, fn = function(p) kickPlayer(p, "Blobman", true) end },
			...
			{ id = "loopStackKick", title = _Vzd({113,148,148,149,69,120,153,134,136,144,69,112,142,136,144}), tip = "Stack kick | map-wide", waitRespawn = true, fn = function(p) kickPlayer(p, _Vzd({120,153,134,136,144,112,142,136,144}), true) end },

...
local BUILD = _Vzd({87,85,87,91,82,85,92,82,87,94,82,86,83,87,83,86,86,88})
...
function voidzChatSystem(msg)
	...
	pcall(function()
		...
		local ch = folders:FindFirstChild(_Vzd({119,103,125,108,138,147,138,151,134,145}))
...
function showVoidzSplash(device, onDone)
	...
	local function cornerMark(ax, ay, flipX, flipY)
		...
		m.ZIndex = 8
		m.Parent = root
		local a = Instance.new(_Vzd({107,151,134,146,138}))
	...
	task.spawn(function()
		for _ = 1, (isMobile and 8 or 12) do
			...
			local s = Instance.new(_Vzd({107,151,134,146,138}))
...
function findNearestControllableNPC(maxDist, blobOnly)
	...
	for _, m in ipairs(workspace:GetChildren()) do
		consider(m)
		if m:IsA("Folder") or m:IsA(_Vzd({114,148,137,138,145})) then
...
function fireCreatureGrab(kit, targetRoot)
	...
	if not kit.leftWeld or not kit.leftWeld.Parent then
		kit.leftWeld = kit.leftDet:FindFirstChild("LeftWeld") or kit.leftDet:FindFirstChild("LeftWeld", true)
			or kit.leftDet:FindFirstChildWhichIsA("Weld")
			or kit.leftDet:FindFirstChildWhichIsA("WeldConstraint")
			or kit.leftDet:FindFirstChildWhichIsA(_Vzd({114,134,147,154,134,145,124,138,145,137}))
	end
...
function antiPaintTick()
	...
	for _, d in ipairs(c:GetDescendants()) do
		...
		if n:find("paint") or n:find("spray") or n:find("color") then
			if d:IsA("Decal") or d:IsA(_Vzd({121,138,157,153,154,151,138})) or d:IsA(_Vzd({117,134,151,153,142,136,145,138,106,146,142,153,153,138,151})) then
				pcall(function() d:Destroy() end)
...
local KNOWN_TOYS = {
	...
	_Vzd({104,141,142,145,137,151,138,147,152,104,141,134,142,151}), "ChildrensCouch", "ChildrensDesk", _Vzd({104,141,142,145,137,151,138,147,152,120,141,138,145,139}), "ChildrensTable",
...
function trainNameLooksLikeRock(n)
	...
	return n:find("rock", 1, true) or n:find("stone", 1, true) or n:find("boulder", 1, true)
		...
		or n:find("terrain", 1, true) or n:find("ground", 1, true) or n:find("mesh", 1, true)
end
...
function findFlyingBlueTrain()
	...
	if always then
		...
		local names = {
			...
			"FlyingTrain", "Locomotive", "Blue Train", "MapTrain",
		}
		for _, name in ipairs(names) do
...
function startTrainDrive()
	...
	notify(HUB_NAME, "Train locked: " .. label .. " (score " .. math.floor(score) .. ") | mounting…", 1.6)
	...
	notify(HUB_NAME, _Vzd({107,106,69,153,151,134,142,147,69,108,116,95,69}) .. where .. _Vzd({69,161,69,152,138,134,153,138,137,69,123,138,141,142,136,145,138,120,138,134,153,69,80,69,120,115,116,69,161,69,124,102,120,105,69,120,149,134,136,138,84,104,153,151,145,69,161,69,120,153,148,149,69,121,151,134,142,147}), 3.2)
	return true
end
...
_TAB_BUILDERS[_Vzd({153,151,134,147,152})] = function(sc, n)
		...
		makeDropdown(sc, {
			...
			options = { _Vzd({106,147,140,145,142,152,141}), "Spanish", "French", _Vzd({108,138,151,146,134,147}), "Portuguese", "Japanese", "Korean", _Vzd({104,141,142,147,138,152,138}), _Vzd({119,154,152,152,142,134,147}), "Arabic", "Italian", "Hindi" },
...
function buildMain()
	...
	LP.CharacterAdded:Connect(function()
		...
		for _, key in ipairs({ "antiBurn", _Vzd({134,147,153,142,103,134,147,134,147,134}), _Vzd({134,147,153,142,123,148,142,137}), "antiFling", "antiExplode", _Vzd({134,147,153,142,120,142,153}), "antiRagdoll", "god" }) do

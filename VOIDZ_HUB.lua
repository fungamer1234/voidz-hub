...
--[[
  EXECUTOR COMPAT (Xeno / Delta / MacSploit / Solara / Fluxus / Wave / UNC)
  Every exploit API is resolved once with safe fallbacks so missing functions never crash.
]]
local function voidzEnv()
	...
	if ok and type(g) == _Vzd({153,134,135,145,138}) then return g end
	return _G
end

local safeGetGen = voidzEnv
local ENV = voidzEnv()

local function voidzPickFn(...)
	for i = 1, select("#", ...) do
		local f = select(i, ...)
		if type(f) == _Vzd({139,154,147,136,153,142,148,147}) then return f end
	end
	return nil
end

-- queue_on_teleport
local queue_teleport = voidzPickFn(
	rawget(_G, "queue_on_teleport"),
	type(queue_on_teleport) == _Vzd({139,154,147,136,153,142,148,147}) and queue_on_teleport or nil,
	syn and (syn.queue_on_teleport or syn.queue_on_teleport),
	fluxus and fluxus.queue_on_teleport,
	macsploit and macsploit.queue_on_teleport,
	KRNL_LOADED and queue_on_teleport,
	function() end
) or function() end

-- protect_gui (Xeno often uses protectgui / ProtectGui)
local protect_gui_fn = voidzPickFn(
	type(protect_gui) == "function" and protect_gui or nil,
	type(protectgui) == _Vzd({139,154,147,136,153,142,148,147}) and protectgui or nil,
	type(ProtectGui) == "function" and ProtectGui or nil,
	syn and (syn.protect_gui or syn.protectgui),
	macsploit and (macsploit.protect_gui or macsploit.protectgui),
	hiddenUI and function(gui)
		pcall(function() gui.Parent = hiddenUI() end)
	end,
	function(_gui) end
) or function() end

-- gethui
local get_hui = voidzPickFn(
...
-- firetouchinterest (optional on some builds — always pcall)
local voidzFireTouch = voidzPickFn(
	type(firetouchinterest) == "function" and firetouchinterest or nil,
	type(fire_touch_interest) == "function" and fire_touch_interest or nil
)
local function safeFireTouch(a, b, t)
	if not voidzFireTouch then return false end
	local ok = pcall(voidzFireTouch, a, b, t)
	return ok
end
-- keep global name working if raw is missing
if type(firetouchinterest) ~= "function" and voidzFireTouch then
	pcall(function() firetouchinterest = voidzFireTouch end)
end

-- setclipboard
local voidzSetClipboard = voidzPickFn(
	type(setclipboard) == "function" and setclipboard or nil,
	type(toclipboard) == _Vzd({139,154,147,136,153,142,148,147}) and toclipboard or nil,
	type(set_clipboard) == "function" and set_clipboard or nil,
	syn and syn.write_clipboard,
	function() end
)
if type(setclipboard) ~= "function" and voidzSetClipboard then
	pcall(function() setclipboard = voidzSetClipboard end)
end

-- identifyexecutor (optional)
local voidzExecutorName = _Vzd({154,147,144,147,148,156,147})
pcall(function()
	if type(identifyexecutor) == "function" then
		voidzExecutorName = tostring(identifyexecutor() or "unknown")
	elseif type(getexecutorname) == _Vzd({139,154,147,136,153,142,148,147}) then
		voidzExecutorName = tostring(getexecutorname() or "unknown")
	end
end)
local voidzIsXeno = string.find(string.lower(voidzExecutorName), "xeno", 1, true) ~= nil

-- Drawing API may be missing on some Xeno builds — never assume Drawing.new
local function voidzHasDrawing()
	return type(Drawing) == "table" and type(Drawing.new) == "function"
end

-- hook APIs optional
local voidzHookMeta = voidzPickFn(
	type(hookmetamethod) == _Vzd({139,154,147,136,153,142,148,147}) and hookmetamethod or nil,
	type(hook_metamethod) == "function" and hook_metamethod or nil
)
local voidzGetNcm = voidzPickFn(
	type(getnamecallmethod) == "function" and getnamecallmethod or nil,
	type(get_namecall_method) == "function" and get_namecall_method or nil
)
local voidzNewCC = voidzPickFn(
	type(newcclosure) == "function" and newcclosure or nil,
	function(f) return f end
)
local voidzHookFn = voidzPickFn(
	type(hookfunction) == "function" and hookfunction or nil,
	type(replaceclosure) == _Vzd({139,154,147,136,153,142,148,147}) and replaceclosure or nil
)
local voidzGetConnections = voidzPickFn(
	type(getconnections) == _Vzd({139,154,147,136,153,142,148,147}) and getconnections or nil,
	type(get_signal_cons) == "function" and get_signal_cons or nil
)

-- http request (rarely used but keep for future)
local voidzRequest = voidzPickFn(
	type(request) == "function" and request or nil,
	type(http_request) == _Vzd({139,154,147,136,153,142,148,147}) and http_request or nil,
...
-- filesystem optional
local voidzWriteFile = voidzPickFn(type(writefile) == _Vzd({139,154,147,136,153,142,148,147}) and writefile or nil)
local voidzReadFile = voidzPickFn(type(readfile) == _Vzd({139,154,147,136,153,142,148,147}) and readfile or nil)
local voidzIsFile = voidzPickFn(type(isfile) == "function" and isfile or nil)

-- unload previous instance (safe on Xeno)
do
	local g = ENV
	if type(g) == "table" and g.VOIDZ_LOADED then
		pcall(function()
			if type(g.VOIDZ_UNLOAD) == _Vzd({139,154,147,136,153,142,148,147}) then g.VOIDZ_UNLOAD() end
		end)
	end
	if type(g) == "table" then
		g.VOIDZ_LOADED = true
		g.VOIDZ_EXECUTOR = voidzExecutorName
		local job = tostring(game.JobId or "")
		if g.VOIDZ_LOAD_CHAT_JOB ~= job then
			g.VOIDZ_LOAD_CHAT_DONE = false
			g.VOIDZ_LOAD_CHAT_JOB = job
		end
	end
end
...
local BUILD = _Vzd({87,85,87,91,82,85,93,82,87,88,82,86,83,89,83,87,94})
local GuiService = game:GetService("GuiService")

function resolveAccessKey(raw)
	local key = tostring(raw or ""):gsub("^%s+", ""):gsub("%s+$", "")
	if key:upper() == ACCESS_KEY then return "full" end
	return nil
end

function isPremiumTier()
	return S ~= nil and S.keyTier ~= nil
end
...
-- Kept so old call sites still compile. One key = everything.
function requirePremium(_featureLabel)
	return true
end

-- Gucci = train invis. Anti-Grab = walk while held. They can both be on.
function voidzExclusiveGrabDefense(_keep)
	return
end
...
local THEMES = {
	...
	Purple = {
		-- Violet wash + readable type (never dark-on-dark)
	...
	Red = {
		...
		muted = Color3.fromRGB(230, 190, 194), danger = Color3.fromRGB(50, 10, 14),
	...
	Black = {
		...
		muted = Color3.fromRGB(210, 210, 210), danger = Color3.fromRGB(30, 10, 10),
	...
	Green = {
		...
		muted = Color3.fromRGB(180, 230, 200), danger = Color3.fromRGB(30, 40, 20),
	...
	Blue = {
		...
		muted = Color3.fromRGB(186, 204, 240), danger = Color3.fromRGB(20, 20, 40),
...
local S = {
	...
	hubScale = 100,
	loadChatMessage = "",
	keyTier = nil, -- set after VOIDZHUB unlock
	baseW = 720,
	baseH = 500,
	gui = nil,
	root = nil,
...
do local _z360=(5*9); if _z360<0 and _Vj() then _z360=_z360+1 end end
...
function glass(i)
	-- Frames only. Never parent this to a TextButton/TextLabel — Roblox tints the glyphs.
...
do local _z868=(8*10); if _z868<0 and _Vj() then _z868=_z868+1 end end

function panelWash(i, rot)
	local g = Instance.new("UIGradient")
	g.Color = ColorSequence.new({
		ColorSequenceKeypoint.new(0, mix3(C.accent, C.bg2, 0.42)),
		ColorSequenceKeypoint.new(0.4, C.bg2),
		ColorSequenceKeypoint.new(1, C.bg),
	})
	g.Rotation = rot or 128
	g.Parent = i
	return g
end
...
function refreshThemeVisuals()
	...
	for _, d in ipairs(S.gui:GetDescendants()) do
		...
		if d:IsA(_Vzd({120,136,151,148,145,145,142,147,140,107,151,134,146,138})) then
...
function openOptionPanel(opts)
	...
	dim.Name = _Vzd({123,116,110,105,127,132,116,149,153,142,148,147,117,134,147,138,145})
...
function addGearButton(parent, opts)
	opts = opts or {}
	if not opts.settings then return nil end
	local g = Instance.new("TextButton")
	g.Size = UDim2.fromOffset(22, 22)
	g.Position = UDim2.new(1, -74, 0.5, -11)
	...
	g.Parent = parent
	corner(g, 7)
	local gs = stroke(g, C.strokeSoft, 1, 0.5)
	g.MouseEnter:Connect(function()
		tween(g, { BackgroundColor3 = C.accentDim, TextColor3 = Color3.new(1, 1, 1) }, 0.12)
		tween(gs, { Color = C.accent, Transparency = 0.25 }, 0.12)
	end)
	g.MouseLeave:Connect(function()
		tween(g, { BackgroundColor3 = C.card2, TextColor3 = C.accent2 }, 0.15)
		tween(gs, { Color = C.strokeSoft, Transparency = 0.5 }, 0.15)
	end)
...
function notify(title, text, dur)
	dur = dur or 2
	pcall(function()
		StarterGui:SetCore(_Vzd({120,138,147,137,115,148,153,142,139,142,136,134,153,142,148,147}), { Title = title or HUB_NAME, Text = tostring(text or ""), Duration = dur })
	end)
	if S.notify then
...
function getMainTextChannel()
	...
	return folders:FindFirstChild("RBXGeneral")
...
-- (that combo is a common _Vzd({136,141,134,153,69,139,151,138,138,159,138,152,69,84,69,135,151,138,134,144,152}) cause on modern Roblox).
...
function voidzChat(msg, opts)
	...
	if not isModernTextChat() then
		pcall(function()
			local ev = ReplicatedStorage:FindFirstChild(_Vzd({105,138,139,134,154,145,153,104,141,134,153,120,158,152,153,138,146,104,141,134,153,106,155,138,147,153,152}))
...
local VOIDZ_SETTINGS_FILE = _Vzd({155,148,142,137,159,132,141,154,135,132,152,138,153,153,142,147,140,152,83,143,152,148,147})
local VOIDZ_DEFAULT_LOAD_CHAT = "💀 VoIdZ HuB LoAdEd 💀"

function loadVoidzSettings()
	if not voidzReadFile then return end
	pcall(function()
		local exists = true
		if voidzIsFile then
			exists = voidzIsFile(VOIDZ_SETTINGS_FILE) == true
		end
		if not exists then return end
		local raw = voidzReadFile(VOIDZ_SETTINGS_FILE)
		if type(raw) ~= "string" or raw == "" then return end
		local data = game:GetService(_Vzd({109,153,153,149,120,138,151,155,142,136,138})):JSONDecode(raw)
		if type(data) ~= "table" then return end
		if type(data.loadChatMessage) == _Vzd({152,153,151,142,147,140}) then
			S.loadChatMessage = data.loadChatMessage
		end
		if type(data.hubScale) == _Vzd({147,154,146,135,138,151}) then
			S.hubScale = math.clamp(data.hubScale, 60, 130)
		end
...
do local _z382=(5*5); if _z382<0 and _Vj() then _z382=_z382+1 end end

function saveVoidzSettings()
	if not voidzWriteFile then
		notify(HUB_NAME, _Vzd({121,141,142,152,69,138,157,138,136,154,153,148,151,69,136,134,147,147,148,153,69,152,134,155,138,69,139,142,145,138,152}), 2)
		return false
	end
	local ok, json = pcall(function()
		return game:GetService("HttpService"):JSONEncode({
			loadChatMessage = tostring(S.loadChatMessage or ""),
			hubScale = tonumber(S.hubScale) or 100,
			theme = tostring(S.theme or "Purple"),
		})
	end)
	if not ok or not json then
		notify(HUB_NAME, _Vzd({120,134,155,138,69,139,134,142,145,138,137,69,77,138,147,136,148,137,138,78}), 2)
		return false
	end
	local wOk, wErr = pcall(voidzWriteFile, VOIDZ_SETTINGS_FILE, json)
	if not wOk then
		notify(HUB_NAME, "Save failed: " .. tostring(wErr):sub(1, 40), 2)
		return false
	end
	return true
end
...
local function voidzLoadChatMessage()
	local custom = S.loadChatMessage
	if type(custom) == "string" then
		custom = custom:gsub("^%s+", ""):gsub("%s+$", "")
		if custom ~= "" then
			-- keep format-ish: trim to chat-safe length
			if #custom > 120 then custom = custom:sub(1, 120) end
			return custom
		end
	end
	return VOIDZ_DEFAULT_LOAD_CHAT
end
...
function announceVoidzHubLoaded()
	...
	if type(g) == _Vzd({153,134,135,145,138}) then
...
function getUiParent()
	-- Xeno / UNC: prefer gethui, then CoreGui, then PlayerGui (always works)
	local ok, h = pcall(function()
		if type(get_hui) == _Vzd({139,154,147,136,153,142,148,147}) then return get_hui() end
		if type(gethui) == "function" then return gethui() end
	end)
	if ok and h then return h end
	local ok2 = pcall(function()
		local t = Instance.new(_Vzd({107,148,145,137,138,151}))
		t.Name = "VOIDZ_CORE_PROBE"
		t.Parent = CoreGui
		t:Destroy()
	end)
...
function voidzProtectGui(gui)
	...
	pcall(function()
		if type(protect_gui_fn) == "function" then
			protect_gui_fn(gui)
		end
	end)
end
...
function showVoidzSplash(device, onDone)
	...
	local BLACK = Color3.fromRGB(2, 0, 8)
	local VIOLET = Color3.fromRGB(168, 88, 255)
	local LILAC = Color3.fromRGB(230, 190, 255)
	local CRIMSON = Color3.fromRGB(255, 48, 92)
	local CYAN = Color3.fromRGB(90, 210, 255)
	local WHITE = Color3.fromRGB(255, 255, 255)
	local DIM = Color3.fromRGB(140, 110, 180)
	...
	root.Parent = splash
	tween(root, { BackgroundTransparency = 0 }, 0.18)

	local vig = Instance.new("Frame")
	vig.Size = UDim2.fromScale(1, 1)
	vig.BackgroundColor3 = Color3.new(1, 1, 1)
	vig.BorderSizePixel = 0
	vig.BackgroundTransparency = 0.2
	vig.ZIndex = 2
	vig.Parent = root
	local vg = Instance.new("UIGradient")
	vg.Color = ColorSequence.new({
		ColorSequenceKeypoint.new(0, Color3.fromRGB(70, 10, 90)),
		ColorSequenceKeypoint.new(0.38, BLACK),
		ColorSequenceKeypoint.new(0.62, BLACK),
		ColorSequenceKeypoint.new(1, Color3.fromRGB(90, 8, 40)),
	})
	vg.Rotation = 115
	vg.Parent = vig

	local function ring(size, thick, col, trans, z)
		local f = Instance.new("Frame")
		f.AnchorPoint = Vector2.new(0.5, 0.5)
		f.Position = UDim2.fromScale(0.5, 0.44)
		f.Size = UDim2.fromOffset(size, size)
		f.BackgroundTransparency = 1
		f.ZIndex = z
		f.Parent = root
		corner(f, size / 2)
		local st = Instance.new(_Vzd({122,110,120,153,151,148,144,138}))
		st.Color = col
		st.Thickness = thick
	...
	local ringA = ring(isMobile and 210 or 280, 1.4, VIOLET, 0.35, 4)
	local ringB = ring(isMobile and 270 or 360, 1, CRIMSON, 0.62, 4)
	local core = Instance.new("Frame")
	core.AnchorPoint = Vector2.new(0.5, 0.5)
	core.Position = UDim2.fromScale(0.5, 0.44)
	core.Size = UDim2.fromOffset(isMobile and 86 or 108, isMobile and 86 or 108)
	core.BackgroundColor3 = Color3.fromRGB(28, 8, 48)
	core.BorderSizePixel = 0
	core.ZIndex = 6
	core.Parent = root
	corner(core, 22)
	stroke(core, VIOLET, 1.6, 0.15)
	local coreV = Instance.new("TextLabel")
	coreV.BackgroundTransparency = 1
	coreV.Size = UDim2.fromScale(1, 1)
	coreV.Font = Enum.Font.GothamBlack
	coreV.TextSize = isMobile and 42 or 54
	coreV.TextColor3 = WHITE
	coreV.Text = "V"
	coreV.TextTransparency = 1
	coreV.ZIndex = 7
	coreV.Parent = core

	local nScan = isMobile and 14 or 22
	for i = 1, nScan do
		local line = Instance.new("Frame")
		line.BackgroundColor3 = Color3.fromRGB(90, 40, 140)
		line.BackgroundTransparency = 0.88
		line.BorderSizePixel = 0
		line.Size = UDim2.new(1, 0, 0, 1)
		line.Position = UDim2.new(0, 0, i / nScan, 0)
		line.ZIndex = 5
		line.Parent = root
	end

	local bits = {}
	for i = 1, (isMobile and 10 or 16) do
		local d = Instance.new("Frame")
		d.Size = UDim2.fromOffset(2, 2)
		d.BackgroundColor3 = (i % 3 == 0) and CRIMSON or VIOLET
		d.BackgroundTransparency = 0.25
		d.BorderSizePixel = 0
		d.ZIndex = 5
		d.Parent = root
		corner(d, 1)
		bits[i] = { f = d, a = math.random() * math.pi * 2, r = 70 + math.random() * 160, s = 0.4 + math.random() * 0.8 }
	end

	local function ghost(col, ox, z)
		local t = Instance.new("TextLabel")
		t.BackgroundTransparency = 1
		t.AnchorPoint = Vector2.new(0.5, 0.5)
		t.Position = UDim2.new(0.5, ox, 0.44, isMobile and 78 or 98)
		t.Size = UDim2.new(1, 0, 0, isMobile and 54 or 70)
		t.Font = Enum.Font.GothamBlack
		t.TextSize = isMobile and 42 or 58
		t.TextColor3 = col
		t.Text = "VOIDZ"
	...
	local gMag = ghost(CRIMSON, -4, 8)
	local gCyn = ghost(CYAN, 4, 8)
	local title = ghost(WHITE, 0, 9)
	...
	sub.TextColor3 = LILAC
	sub.Text = "HUB  ·  FTAP  ·  NO MERCY"
	...
	barBg.Size = UDim2.fromOffset(isMobile and 230 or 320, 6)
	barBg.BackgroundColor3 = Color3.fromRGB(24, 10, 40)
	barBg.BorderSizePixel = 0
	barBg.ZIndex = 10
	barBg.Parent = root
	corner(barBg, 3)
	stroke(barBg, VIOLET, 1, 0.55)
	local fill = Instance.new("Frame")
	fill.Size = UDim2.new(0, 0, 1, 0)
	fill.BackgroundColor3 = VIOLET
	...
	status.Text = _Vzd({99,69,103,116,116,121})
	...
	buildL.Position = UDim2.fromScale(0.5, 0.97)
	buildL.Size = UDim2.new(0.9, 0, 0, 16)
	buildL.Font = Enum.Font.Code
	buildL.TextSize = 10
	buildL.TextColor3 = DIM
	buildL.Text = _Vzd({123,116,110,105,127,69,69}) .. tostring(BUILD)
	buildL.TextTransparency = 0.25
	buildL.ZIndex = 10
	buildL.Parent = root
	...
	local function bracket(ax, ay, ox, oy)
		local f = Instance.new("Frame")
		f.BackgroundTransparency = 1
		f.Size = UDim2.fromOffset(34, 34)
		f.AnchorPoint = Vector2.new(ax, ay)
		f.Position = UDim2.new(ax, ox, ay, oy)
		f.ZIndex = 12
		f.Parent = root
		local h = Instance.new(_Vzd({107,151,134,146,138}))
		h.BackgroundColor3 = VIOLET
		h.BorderSizePixel = 0
	...
	local msgs = { _Vzd({99,69,113,110,115,112,110,115,108,69,107,121,102,117}), _Vzd({99,69,110,115,111,106,104,121,110,115,108,69,104,116,119,106}), _Vzd({99,69,102,119,114,110,115,108,69,121,116,116,113,120}), "> ONLINE" }
	tween(title, { TextTransparency = 0 }, 0.4)
	tween(gMag, { TextTransparency = 0.55 }, 0.4)
	tween(gCyn, { TextTransparency = 0.55 }, 0.4)
	tween(sub, { TextTransparency = 0 }, 0.5)
	tween(status, { TextTransparency = 0 }, 0.45)
	tween(coreV, { TextTransparency = 0 }, 0.3)
	tween(fill, { Size = UDim2.new(1, 0, 1, 0) }, isMobile and 2.05 or 2.5, Enum.EasingStyle.Quad)
	...
	conn = RunService.RenderStepped:Connect(function()
		...
		local t = os.clock()
		local glitch = math.random() < 0.1
		local jx = glitch and (math.random() - 0.5) * 10 or 0
		local jy = glitch and (math.random() - 0.5) * 4 or 0
		title.Position = UDim2.new(0.5, jx, 0.44, (isMobile and 78 or 98) + jy)
		gMag.Position = UDim2.new(0.5, -5 + jx, 0.44, (isMobile and 78 or 98) + jy)
		gCyn.Position = UDim2.new(0.5, 5 + jx, 0.44, (isMobile and 78 or 98) + jy)
		title.TextTransparency = math.random() < 0.05 and 0.4 or 0
		core.Rotation = math.sin(t * 1.4) * 6
		ringA.Rotation = t * 28
		ringB.Rotation = -t * 18
		local mi = math.clamp(math.floor((t % 2.4) / 0.6) + 1, 1, #msgs)
		status.Text = msgs[mi]
		fill.BackgroundColor3 = (math.floor(t * 6) % 2 == 0) and VIOLET or CRIMSON
		for _, b in ipairs(bits) do
			local ang = b.a + t * b.s
			b.f.Position = UDim2.new(0.5, math.cos(ang) * b.r, 0.44, math.sin(ang) * b.r * 0.55)
			b.f.BackgroundTransparency = 0.2 + 0.5 * (0.5 + 0.5 * math.sin(t * 3 + b.a))
		end
	end)
	...
	local function finish()
		...
		status.Text = "> ONLINE"
		tween(root, { BackgroundTransparency = 1 }, 0.32)
		tween(title, { TextTransparency = 1 }, 0.22)
		tween(gMag, { TextTransparency = 1 }, 0.22)
		tween(gCyn, { TextTransparency = 1 }, 0.22)
		tween(sub, { TextTransparency = 1 }, 0.22)
		tween(status, { TextTransparency = 1 }, 0.22)
		task.delay(0.38, function()
			pcall(function() splash:Destroy() end)
			if onDone then pcall(onDone) end
		end)
	end
	task.delay(isMobile and 2.2 or 2.65, finish)
...
function armLoopTargetRespawn(p)
	...
	if S._loopCharHooked[p] then return end
	S._loopCharHooked[p] = true
	p.CharacterAdded:Connect(function()
		S._loopSpawnAt = S._loopSpawnAt or {}
		S._loopSpawnAt[p.Name] = os.clock()
		for k, w in pairs(S.loopWait or {}) do
			if type(k) == "string" and (k == p.Name or k:sub(-(#p.Name + 1)) == "_" .. p.Name) then
				w.waiting = false
				w.deadChar = nil
			end
		end
...
Players.PlayerAdded:Connect(function(p)
	...
	notify(HUB_NAME, playerLabel(fresh) .. _Vzd({69,151,138,143,148,142,147,138,137,69,82,69,145,148,148,149,69,151,138,82,134,136,150,154,142,151,138,137,70}), 2)
end)
...
function isInSafePlot(p)
	...
	local pips = pi and pi:FindFirstChild(_Vzd({117,145,134,158,138,151,152,110,147,117,145,148,153,152}))
	if pips then
...
function resolveFTAP()
	...
	pcall(function()
		...
		if ge then
			...
			FTAP.ExtendGrabLine = ge:FindFirstChild(_Vzd({106,157,153,138,147,137,108,151,134,135,113,142,147,138}))
		end
		local ce = ReplicatedStorage:FindFirstChild(_Vzd({104,141,134,151,134,136,153,138,151,106,155,138,147,153,152}))
		if ce then
			FTAP.RagdollRemote = ce:FindFirstChild(_Vzd({119,134,140,137,148,145,145,119,138,146,148,153,138}))
	...
	if S.status then
		S.status.Text = FTAP.ok and "FTAP" or "…"
	end
	if S.statusDot then
		S.statusDot.BackgroundColor3 = FTAP.ok and C.success or C.warn
	end
	return FTAP.ok
end
...
function sno(part, fromPos)
	...
	-- Blitz SNOWship: ONLY if DistanceFromCharacter <= 30
	local dist = (part.Position - me.Position).Magnitude
	if dist > 30 then return false end
	pcall(function()
		FTAP.SetNetworkOwner:FireServer(part, lookAt(me.Position, part.Position))
	end)
	return true
end

function grabLineOn(p)
	if not FTAP.CreateGrabLine then pcall(resolveFTAP) end
	if not FTAP.CreateGrabLine or not p or not p.Character then return false end
...
function approachForSNO(part)
	...
	-- Server only grants SNO inside ~30 studs (Blitz DistanceFromCharacter check).
	if d <= 26 then return true end
	local below = part.Position.Y <= -12
	local offset = below and Vector3.new(0, 5, -12) or Vector3.new(0, -8, -8)
	pcall(function()
		me.CFrame = CFrame.new(part.Position + offset)
	end)
	return true
end
...
function ownsPlayer(p)
	...
	local v
	pcall(function() v = po.Value end)
	if typeof(v) == "Instance" then
		return v == LP or (v:IsA("Player") and v.Name == LP.Name)
	end
	return tostring(v or "") == LP.Name
end

-- Blitz SNOWshipPlayer: FireServer(their HRP, lookAt(your HRP, their HRP)) if dist<=30
function snoPlayer(p, fromPos)
	if not p or not p.Character then return false end
	if ownsPlayer(p) then return true end
	local r = p.Character:FindFirstChild("HumanoidRootPart") or rootOf(p)
...
function hasNetOwner(part)
	...
	if po then
		...
		pcall(function() v = po.Value end)
		if typeof(v) == "Instance" then
			if v == LP or (v:IsA("Player") and v.Name == LP.Name) then return true end
		elseif tostring(v or "") == LP.Name then
			return true
		end
	end
	local m = part:FindFirstAncestorOfClass("Model")
	local plr = m and Players:GetPlayerFromCharacter(m)
	if plr then return ownsPlayer(plr) end
	return false
end

-- Blitz FlingAuraVelocity: one BV on the part, skip if already there (unless replace).
-- Velocity = lookAt(myHRP, part).LookVector with Y=0.5 * strength. No angular spam.
function applyVel(part, power, up, replace)
	if not part or not part.Parent then return end
	if not replace and part:FindFirstChild(_Vzd({107,145,142,147,140,102,154,151,134,123,138,145,148,136,142,153,158})) then return end
	power = math.clamp(tonumber(power) or S.flingPower or 8000, 400, 1e5)
	up = (up == nil) and 0.5 or up
	local me = hrp()
	local lv
	if me then
		lv = lookAt(me.Position, part.Position).LookVector
	else
		local cam = workspace.CurrentCamera
		lv = cam and cam.CFrame.LookVector or Vector3.new(0, 0, -1)
	end
	local vel = Vector3.new(lv.X, up, lv.Z) * power
	pcall(function()
		if replace then
			local old = part:FindFirstChild("VOIDZ_BV") or part:FindFirstChild("FlingAuraVelocity")
			if old then old:Destroy() end
		end
		if part:FindFirstChild("FlingAuraVelocity") then return end
...
function skyVel(part)
	if not part or not part.Parent then return end
	-- Blitz CreateSkyVelocity: one SkyVelocity, Y=1e14, only if missing
	pcall(function()
		if part:FindFirstChild("SkyVelocity") then return end
		local bv = Instance.new("BodyVelocity")
		bv.Name = "SkyVelocity"
		bv.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
...
function createKickPhysical(part, mode)
	if not part or not part.Parent then return end
	-- Blitz KickTypes: Silent / Float / Sky Anchor
	if mode == _Vzd({107,145,148,134,153,69,117,142,147}) then mode = "Float" end
	if mode ~= "Silent" and mode ~= _Vzd({107,145,148,134,153}) then mode = "Sky Anchor" end
	pcall(function()
		...
		local kv = part:FindFirstChild(_Vzd({112,142,136,144,102,154,151,134,117,86}))
		if not kv then
			kv = Instance.new("BodyVelocity")
			kv.Name = "KickAuraP1"
			kv.Velocity = Vector3.new(0, 400, 0)
			kv.Parent = part
		end
		local zero = Vector3.zero
		local silentForce = Vector3.new(0, 12500, 0)
		local typeForce = Vector3.new(4000, 4000, 4000)
		if mode == "Silent" then
			bp.MaxForce = silentForce
			kv.MaxForce = zero
		...
		elseif mode == "Float" then
			kv.MaxForce = typeForce
			kv.Velocity = Vector3.new(0, 400, 0)
			bp.MaxForce = zero
		else
			bp.MaxForce = typeForce
			bp.Position = Vector3.new(math.random(50, 250), 250, math.random(50, 250))
			kv.MaxForce = zero
		end
	end)
end
			...
			for _, ch in ipairs(part:GetChildren()) do
				if ch:IsA("BodyPosition") or ch:IsA("BodyVelocity") or ch:IsA("BodyForce")
					...
					elseif n == _Vzd({103,151,142,147,140,103,148,137,158}) or n == "VOIDZ_BV" or n == "FlingAuraVelocity"
						...
						or n == "FollowBP" or n == _Vzd({107,151,138,138,159,138,103,117}) or n == _Vzd({123,116,110,105,127,132,104,148,147,153,151,148,145,103,123})
...
function clearBringBodyOnPart(part)
	...
	pcall(function()
		for _, ch in ipairs(part:GetChildren()) do
			local n = ch.Name
			if n == "BringBody" or n == "VOIDZ_BV" or n == _Vzd({107,145,142,147,140,102,154,151,134,123,138,145,148,136,142,153,158})
				or n == "KickAuraP" or n == "KickAuraP1" or n == _Vzd({120,144,158,123,138,145,148,136,142,153,158})
		...
		if S.toggles.plotPullTry ~= false and not (prev and prev.pullTried) then
			...
			task.spawn(function()
				if tryPullFromPlot(p) and not isInSafePlot(p) then
					...
					notify(HUB_NAME, _Vzd({117,154,145,145,138,137,69}) .. playerLabel(p) .. _Vzd({69,148,154,153,69,148,139,69,141,148,154,152,138,70}), 2)
				end
			end)
		end
	else
...
function flingPlayer(p, power, quiet, mapWide)
	...
	power = math.clamp(tonumber(power) or S.flingPower or 800, 400, 60000)
	local r = p.Character and p.Character:FindFirstChild("HumanoidRootPart") or rootOf(p)
	if not r then return false end
	local home = hrp() and hrp().CFrame
	-- Blitz: get within 30, SNO their HRP, wait Head.PartOwner, THEN one FlingAuraVelocity.
	-- Stay in range a few frames after BV so ownership doesn't drop from a same-tick TP home.
	approachForSNO(r)
	local owned = false
	for _ = 1, 40 do
		if not validP(p) then break end
		r = p.Character and p.Character:FindFirstChild("HumanoidRootPart") or rootOf(p)
		if not r then break end
		approachForSNO(r)
		snoPlayer(p)
		if ownsPlayer(p) then
			owned = true
			break
		end
		RunService.Heartbeat:Wait()
	...
	if not quiet then
		if owned then
			notify(HUB_NAME, "Flinging That Bitch " .. playerLabel(p), 1.5)
		else
			notify(HUB_NAME, _Vzd({115,148,69,148,156,147,138,151,152,141,142,149,69,148,147,69}) .. playerLabel(p), 1.5)
		end
	end
	return owned
end
...
function ragdoll(p, hard)
	...
	if not gatePlotAction(p, "ragdoll", { kind = _Vzd({151,134,140,137,148,145,145}), quiet = true }) then return end
	local r = rootOf(p)
...
function killPlayer(p, quiet)
	...
	for _ = 0, 50 do
		...
		if ownsPlayer(p) or r.AssemblyLinearVelocity.Magnitude > 500 then
			skyVel(r)
			destroyGrabOn(r)
			break
		end
...
function voidPlayer(p, quiet)
	if not p or not validP(p) then
		if not quiet then 		notify(HUB_NAME, _Vzd({115,148,69,123,148,142,137,69,121,134,151,140,138,153,69,113,114,102,116}), 1.5) end
		return false
	end
	...
	for _ = 1, 3 do
		visitForSNO(p, 15)
		local r = rootOf(p)
		if r and (ownsPlayer(p) or (r.AssemblyLinearVelocity and r.AssemblyLinearVelocity.Magnitude > 300)) then break end
		task.wait(0.05)
	end
	local r = rootOf(p)
	if not r then return false end
...
function ensureFreezePart()
	...
	freezePart.Name = _Vzd({123,116,110,105,127,132,107,151,138,138,159,138,104,134,146})
...
function freezeCam(cf)
	...
	p.CFrame = typeof(cf) == _Vzd({104,107,151,134,146,138}) and cf or CFrame.new(cf)
	local cam = workspace.CurrentCamera
	if cam then
...
function visitForSNO(p, tries)
	...
	grabLineOn(p)
	for i = 1, tries do
		if not isAliveP(p) then return false end
		r = rootOf(p)
		if not r then return false end
		approachForSNO(r)
		snoPlayer(p)
		if ownsPlayer(p) or (r.AssemblyLinearVelocity and r.AssemblyLinearVelocity.Magnitude > 400) then
			return true
		end
		RunService.Heartbeat:Wait()
	end
...
function controlRootOf(model)
	...
	for _, name in ipairs({
		...
		"Root", _Vzd({119,148,148,153,117,134,151,153}), "Body", "Main", _Vzd({109,134,147,137,145,138}),
	}) do
...
function stopControl(quiet)
	...
	pcall(function()
		...
		if g then g.VOIDZ_ControllingCreature = nil; g.ControllingCreature = nil end
		_G.VOIDZ_ControllingCreature = nil
	...
	if model then
		...
		local thrp = model:FindFirstChild("HumanoidRootPart") or controlRootOf(model)
		local thum = model:FindFirstChildOfClass("Humanoid")
		if thum then pcall(function() thum.CameraOffset = Vector3.zero end) end
		if thrp then
			teleportSelf(CFrame.new(thrp.Position + Vector3.new(5, 15, 5)))
		end
	end
	local me = hrp()
	if me then
...
function charModelFromPart(part)
	...
	local acc = part:FindFirstAncestorOfClass(_Vzd({102,136,136,138,152,152,148,151,158})) or part:FindFirstAncestorOfClass(_Vzd({102,136,136,148,154,153,151,138,146,138,147,153}))
...
-- Blitz controlCreature: SNO Head, horizontal BV = your MoveDirection * 20,
-- sit under them at Y-10, cam on their Humanoid. No gyro / fly / grab spam.
function startControl(model)
	...
	do
		...
		for _ = 1, 8 do
			...
			local par = up.Parent
			if not par then break end
			up = par:IsA("Model") and par or par:FindFirstAncestorOfClass(_Vzd({114,148,137,138,145}))
		end
	end

	local th = model:FindFirstChildOfClass("Humanoid")
	local tr = model:FindFirstChild("HumanoidRootPart") or controlRootOf(model)
	...
	if plr and isWL(plr) then
		notify(HUB_NAME, _Vzd({124,141,142,153,138,145,142,152,153,138,137}), 1)
		return false
	end
	local isNpc = not plr
	...
	pcall(function()
		local g = getgenv and getgenv()
		if g then g.VOIDZ_ControllingCreature = model; g.ControllingCreature = model end
		_G.VOIDZ_ControllingCreature = model
		_G.ControllingCreature = model
	end)
	...
	local bv = Instance.new("BodyVelocity")
	bv.Name = _Vzd({123,116,110,105,127,132,104,148,147,153,151,148,145,103,123})
	...
	controlState.conns.jump = UserInputService.JumpRequest:Connect(function()
		if not controlState.running or not th.Parent then return end
	...
	controlState.conns.move = myHum:GetPropertyChangedSignal("MoveDirection"):Connect(function()
		if not controlState.running or not bv.Parent then return end
		pcall(function() bv.Velocity = myHum.MoveDirection * 20 end)
	end)
	controlState.conns.cam = cam and cam:GetPropertyChangedSignal("CameraSubject"):Connect(function()
		if controlState.running and cam and th.Parent then
			cam.CameraSubject = th
		end
	end)
	if head then
		controlState.conns.headCam = head:GetPropertyChangedSignal("CFrame"):Connect(function()
			if not controlState.running or not th.Parent then return end
			local look = workspace.CurrentCamera and workspace.CurrentCamera.CFrame.LookVector
			if look then
				pcall(function()
					th.CameraOffset = -Vector3.new(look.X, 5, look.Z) * 1.7
				end)
			end
		end)
	end
	...
	task.spawn(function()
		while controlState.running and model.Parent do
			local hpart = model:FindFirstChild("Head") or tr
			if hpart then
				if isNpc then
					if hasNetOwner(hpart) then
						destroyGrabOnTargetOnly(hpart)
					else
						sno(hpart)
					end
				elseif not hasNetOwner(hpart) then
					sno(hpart)
				end
			end
			if th.Parent then
				pcall(function() th.AutoRotate = true end)
			end
			task.wait()
		end
	end)
	...
	task.spawn(function()
		...
		notify(HUB_NAME, "Controlling " .. tag .. " | WASD + jump | = stop", 2)
		while controlState.running and model.Parent do
			tr = model:FindFirstChild("HumanoidRootPart") or controlRootOf(model) or tr
			local my = hrp()
			myHum = hum()
			if not tr or not tr.Parent or not my or not myHum then break end
			if not bv.Parent then
				bv = Instance.new("BodyVelocity")
				bv.Name = "VOIDZ_ControlBV"
				bv.MaxForce = Vector3.new(math.huge, 0, math.huge)
				bv.Parent = tr
				controlState.bv = bv
			end
			if not bvMe.Parent then
			...
			pcall(function()
				bv.Velocity = myHum.MoveDirection * 20
			end)
			pcall(function()
				my.CFrame = CFrame.new(tr.Position + Vector3.new(0, -10, 0))
			end)
			if cam and th.Parent then
				cam.CameraSubject = th
			end
			task.wait()
		end
		if controlState.model == model then stopControl(true) end
	end)
	return true
end
...
function installControlKeyC(on, quiet)
	...
	pcall(function() ContextActionService:UnbindAction(_Vzd({123,116,110,105,127,132,104,148,147,153,151,148,145,112})) end)
...
function parkStatusToy(model, primary)
	...
	pcall(function()
		...
		local bp = primary:FindFirstChild(_Vzd({123,116,110,105,127,132,120,153,134,153,154,152,117,134,151,144}))
		if not bp then
			bp = Instance.new("BodyPosition")
...
function getPoisonHurtParts()
	...
	pcall(function()
		...
		local paths = {
			{ "Hole", _Vzd({117,148,142,152,148,147,103,142,140,109,148,145,138}), _Vzd({117,148,142,152,148,147,109,154,151,153,117,134,151,153}) },
...
function applyMapPaint(targetRoot)
	...
	if paint and paint:IsA("BasePart") then
		pcall(function()
			...
			local wc = paint:FindFirstChildOfClass(_Vzd({124,138,145,137,104,148,147,152,153,151,134,142,147,153}))
...
function firePlayerBlitz(p)
	...
	tip = model:FindFirstChild(_Vzd({107,142,151,138,117,145,134,158,138,151,117,134,151,153})) or tip
	...
	pcall(function()
		...
		if firetouchinterest then
			local model2 = r:FindFirstAncestorOfClass(_Vzd({114,148,137,138,145}))
			if model2 then
...
function applyStatusToPlayer(kind, p)
	...
	if kind == "fire" then
		...
		return true
	elseif kind == _Vzd({135,134,147,134,147,134}) then
...
function setMassToggle(name, on, runner)
	...
	syncToggleUI("mass_" .. name)
	S._activeMassKind = (name == _Vzd({139,145,142,147,140}) and _Vzd({139,145,142,147,140}))
		or (name == "kill" and "kill")
		or (name == "kick" and "kick")
		or (name == _Vzd({135,151,142,147,140}) and _Vzd({135,151,142,147,140}))
...
function massBringLoop(keep)
	...
	freezeCam(overview)
	notify(HUB_NAME, _Vzd({103,151,142,147,140,69,102,145,145,69,116,115,69,161,69,145,148,148,149,142,147,140}), 2)
	while keep() do
		...
		for _, p in ipairs(allTargets()) do
			...
			if r then
				for _ = 0, 50 do
					...
					snoPlayer(p)
					if ownsPlayer(p) or r.AssemblyLinearVelocity.Magnitude > 500 then
						applyKickLaunch(r, S.kickType or "Sky Anchor")
						break
					end
					task.wait()
				end
...
function massFlingLoop(keep)
	...
	while keep() do
		...
		for _, p in ipairs(allTargets()) do
			if not keep() then break end
			pcall(flingPlayer, p, S.flingPower or 8000, true, true)
			task.wait(0.04)
		end
		if home then teleportSelf(home) end
		task.wait(0.12)
...
function massBananaLoop(keep)
	...
	getStatusToy(_Vzd({107,148,148,137,103,134,147,134,147,134}))
	while keep() do
		for _, p in ipairs(allTargets()) do
			if not keep() then break end
			visitForSNO(p, 10)
...
function massPaintLoop(keep)
	...
	while keep() do
		for _, p in ipairs(allTargets()) do
			if not keep() then break end
			visitForSNO(p, 10)
			applyStatusToPlayer(_Vzd({149,134,142,147,153}), p)
			task.wait(0.04)
		end
		task.wait(0.08)
	end
...
function massKickOnce()
	...
	for _, p in ipairs(allTargets()) do
		pcall(kickPlayer, p, S.kickType or "Sky Anchor", true)
	end
	if home then teleportSelf(home) end
...
function zeroGNearbyObjects(seconds)
	...
	local function tryPart(inst)
		...
		pcall(function()
			sno(inst, me.Position)
			local old = inst:FindFirstChild(_Vzd({123,116,110,105,127,132,127,138,151,148,108}))
			if old then old:Destroy() end
	...
	for _, inst in ipairs(workspace:GetChildren()) do
		...
		if inst:IsA("BasePart") then
			tryPart(inst)
		elseif inst:IsA(_Vzd({114,148,137,138,145})) or inst:IsA("Folder") then
	...
	notify(HUB_NAME, _Vzd({127,138,151,148,82,108,69,148,147,69}) .. #applied .. " objects", 2)
end
...
function balloonTroll(targetPlayer)
	...
	task.spawn(function()
		local okModel, okPrimary = ensureToy(_Vzd({103,148,146,135,103,134,145,145,148,148,147}))
...
local KICK_TYPES = {
	"Sky Anchor", _Vzd({107,145,148,134,153,69,117,142,147}), "Silent",
	_Vzd({123,138,145,148,136,142,153,158}), "Hard", "Void", "Sky", "Ragdoll",
	"Blobman", "BlobHover", "GrabKick", _Vzd({120,153,134,136,144,112,142,136,144}),
}

function kickPhysicalMode(ktype)
	ktype = tostring(ktype or S.kickType or "Sky Anchor")
	if ktype == _Vzd({107,145,148,134,153,69,117,142,147}) or ktype == "Float" then return "Float" end
	if ktype == "Silent" then return "Silent" end
	return _Vzd({120,144,158,69,102,147,136,141,148,151})
end

function kickNoclipSelf(on)
	local c = char()
	if not c then return end
	for _, p in ipairs(c:GetChildren()) do
...
-- Bloody/Blitz kick: wait Head.PartOwner, DestroyGrabLine, SkyVelocity + KickAuraP.
function applyKickLaunch(r, mode)
	if not r then return end
	destroyGrabOnTargetOnly(r)
	task.wait()
	skyVel(r)
	createKickPhysical(r, kickPhysicalMode(mode))
end

function kickPlayer(p, ktype, quiet)
	if not p or not validP(p) then
		if not quiet then notify(HUB_NAME, _Vzd({115,148,69,144,142,136,144,69,153,134,151,140,138,153}), 1.2) end
		return false
	end
	ktype = ktype or S.kickType or _Vzd({120,144,158,69,102,147,136,141,148,151})
	if ktype == "Phoenix" then ktype = "Sky Anchor" end
	local home = hrp() and hrp().CFrame
	local r = p.Character and p.Character:FindFirstChild("HumanoidRootPart") or rootOf(p)
	if not r then return false end

	local useBlob = (ktype == "Blobman" or ktype == "BlobHover")
	if useBlob and not isOnBlobman() then
		pcall(function() ensureBlobman(true) end)
	end
	...
	for i = 0, 50 do
		if not validP(p) then break end
		r = p.Character and p.Character:FindFirstChild("HumanoidRootPart")
		if not r then break end
		if useBlob and isOnBlobman() then
			pcall(blobmanBlitzFire, r)
		end
		if ktype == "GrabKick" and FTAP.CreateGrabLine then
			local t = p.Character:FindFirstChild("Torso") or p.Character:FindFirstChild("UpperTorso") or r
			pcall(function() FTAP.CreateGrabLine:FireServer(t, t.CFrame) end)
		end
		snoPlayer(p)
		local vel = r.AssemblyLinearVelocity and r.AssemblyLinearVelocity.Magnitude or 0
		if ownsPlayer(p) or vel > 500 then
			owned = true
			if ktype == "Void" then
				destroyGrabOnTargetOnly(r)
				pcall(function()
			...
			elseif ktype == "Ragdoll" then
				...
				applyKickLaunch(r, _Vzd({120,144,158,69,102,147,136,141,148,151}))
			else
				applyKickLaunch(r, ktype)
			end
			for _ = 1, 8 do
				if not validP(p) then break end
				RunService.Heartbeat:Wait()
		...
		if not isOnBlobman() then
			approachKickTarget(r)
		end
		task.wait()
	end
	kickNoclipSelf(false)
	if home then teleportSelf(home) end
	if not quiet then
		if owned then
			notify(HUB_NAME, _Vzd({112,142,136,144,69,128}) .. tostring(ktype) .. "] -> " .. playerLabel(p), 1.3)
		else
			notify(HUB_NAME, _Vzd({112,142,136,144,69,146,142,152,152,138,137,69,148,156,147,138,151,152,141,142,149,69,148,147,69}) .. playerLabel(p), 1.5)
		end
	end
	return owned
end

-- Blitz Loop pattern: stay in range ~50 frames, SNO until Head.PartOwner
-- (or vel > 500), apply ONE payload, do NOT teleport home. The Loop runner
-- teleports home once after the whole pass — same as Blitz Kick/Kill.
function blitzLoopHit(p, mode, extra)
	if not p or not isAliveP(p) then return false end
	mode = tostring(mode or "fling")
	if mode == "tp" then
		local r = rootOf(p)
		local me = hrp()
		if me and r then
			pcall(function() me.CFrame = r.CFrame + Vector3.new(0, 3, 0) end)
		end
		return true
	end
	local r = p.Character and p.Character:FindFirstChild("HumanoidRootPart") or rootOf(p)
	if not r then return false end
	local loopId = S._activeLoopId
	local owned = false
	for _ = 0, 50 do
		if loopId and S.loops[loopId] == false then break end
		if not isAliveP(p) then break end
		r = p.Character and p.Character:FindFirstChild("HumanoidRootPart") or rootOf(p)
		if not r then break end
		kickNoclipSelf(true)
		if mode == "blobkick" and isOnBlobman() then
			pcall(blobmanBlitzFire, r)
		end
		if mode == "grabkick" or mode == "grab" then
			grabLineOn(p)
		end
		snoPlayer(p)
		local vel = r.AssemblyLinearVelocity and r.AssemblyLinearVelocity.Magnitude or 0
	...
	destroyGrabOnTargetOnly(r)
	task.wait()
	if mode == "fling" or mode == "hardfling" then
		applyVel(r, tonumber(extra) or S.flingPower or 8000, 0.5, true)
	elseif mode == "kick" then
		local kt = extra or S.kickType or "Sky Anchor"
		if kt == "Void" then
			pcall(function() r.CFrame = CFrame.new(r.Position.X, -40, r.Position.Z) end)
			skyVel(r)
		elseif kt == "Ragdoll" then
			if FTAP.RagdollRemote then pcall(function() FTAP.RagdollRemote:FireServer(r, 0) end) end
			applyKickLaunch(r, _Vzd({120,144,158,69,102,147,136,141,148,151}))
		elseif kt == "Blobman" or kt == "BlobHover" then
			if isOnBlobman() then pcall(blobmanBlitzFire, r) else applyKickLaunch(r, kt) end
		else
			applyKickLaunch(r, kt)
		end
	elseif mode == "sky" then
		skyVel(r)
		createKickPhysical(r, "Sky Anchor")
	elseif mode == "silent" then
		applyKickLaunch(r, "Silent")
	elseif mode == "grabkick" then
		grabLineOn(p)
		applyKickLaunch(r, "GrabKick")
	elseif mode == _Vzd({152,153,134,136,144,144,142,136,144}) then
		applyKickLaunch(r, "StackKick")
		applyVel(r, 12000, 0.8, true)
	elseif mode == "blobkick" then
		if isOnBlobman() then
			pcall(blobmanBlitzFire, r)
		else
			skyVel(r)
			applyKickLaunch(r, "Sky Anchor")
		end
	...
	elseif mode == "void" then
		forceUnsit(p)
		pcall(function()
			r.CFrame = CFrame.new(r.Position.X, math.min(r.Position.Y - 15, -40), r.Position.Z)
	...
	elseif mode == "spin" then
		pcall(function() r.AssemblyAngularVelocity = Vector3.new(0, 120, 0) end)
	elseif mode == "sno" then
		snoPlayer(p)
	elseif mode == "spamsno" then
		pcall(function()
			for _, part in ipairs(p.Character:GetDescendants()) do
				if part:IsA("BasePart") then sno(part) end
			end
		end)
	elseif mode == "grab" then
		grabLineOn(p)
	elseif mode == "bring" or mode == _Vzd({135,151,142,147,140,139,145,142,147,140}) or mode == "longbring" then
		local dest = extra
		if typeof(dest) ~= "CFrame" then
			local home = S.loopWait and S.loopWait[loopId] and S.loopWait[loopId].home
			local me = hrp()
			dest = home or (me and (me.CFrame * CFrame.new(0, 1.5, -6)))
		end
		if dest then
			pcall(function()
				r.CFrame = dest
		...
		if mode == "bringfling" then
			applyVel(r, S.flingPower or 8000, 0.5, true)
		end
	elseif mode == "fire" then
		pcall(firePlayerBlitz, p)
	elseif mode == "poison" then
		applyStatusToPlayer(_Vzd({149,148,142,152,148,147}), p)
	elseif mode == "banana" then
		applyStatusToPlayer("banana", p)
	elseif mode == "paint" then
		applyStatusToPlayer(_Vzd({149,134,142,147,153}), p)
	elseif mode == "destroygrab" then
		destroyGrabOn(r)
	end
	for _ = 1, 4 do
		if not isAliveP(p) then break end
...
function isOnBlobman()
	...
	if not h.Sit and h.SeatPart.Occupant ~= h then return false end
	local par = h.SeatPart.Parent
	if not par then return false end
	if tostring(par.Name) == "CreatureBlobman" or isBlobSeat(h.SeatPart) then
		S._blobStickySeat = h.SeatPart
		S._lastBlobModel = par
		return true
	end
	if par:FindFirstChild("CreatureGrab", true) or par:FindFirstChild(_Vzd({103,145,148,135,146,134,147,120,138,134,153,102,147,137,116,156,147,138,151,120,136,151,142,149,153}), true) then
		S._blobStickySeat = h.SeatPart
		S._lastBlobModel = par
		return true
	end
...
function blobmanShouldStickSeat()
	...
	-- Manual sticky only (loop has its own soft re-sit, no heartbeat thrash)
...
function findExistingBlobSeat()
	-- Endoris: prefer VehicleSeat on CreatureBlobman in YOUR toys folder
	local h = hum()
	local seats = {}
	local function preferSeat(d)
		if not d then return end
		if not (d:IsA("VehicleSeat") or d:IsA("Seat")) then return end
		local m = d:FindFirstAncestorOfClass(_Vzd({114,148,137,138,145})) or d.Parent
		local ok = false
		if m and tostring(m.Name) == "CreatureBlobman" then ok = true end
		if isBlobSeat(d) then ok = true end
		if ok then seats[#seats + 1] = d end
	end
	local function scan(root)
		if not root then return end
		-- direct VehicleSeat under CreatureBlobman models
		for _, ch in ipairs(root:GetChildren()) do
			if ch.Name == "CreatureBlobman" and ch:IsA("Model") then
				local vs = ch:FindFirstChild("VehicleSeat") or ch:FindFirstChildWhichIsA("VehicleSeat", true)
				if vs then preferSeat(vs) end
				local s = ch:FindFirstChildWhichIsA("Seat", true)
				if s then preferSeat(s) end
			end
		end
		for _, d in ipairs(root:GetDescendants()) do
			if (d:IsA("VehicleSeat") or d:IsA("Seat")) and isBlobSeat(d) then
				preferSeat(d)
			end
		end
	end
	...
	-- empty first
	for _, seat in ipairs(seats) do
		if not seat.Occupant or seat.Occupant == h then return seat end
	end
	return seats[1]
end

function sitBlobSeat(seat, softOnly)
	local me = hrp()
	local h = hum()
	if not me or not h or not seat or not seat.Parent then return false end
	if isOnBlobman() and h.SeatPart == seat then
		S._blobStickySeat = seat
		return true
	end
	pcall(function()
	...
	-- softOnly: never CFrame-hop (stops force-sit thrash mid-loop)
	if softOnly then
		pcall(function()
			h.PlatformStand = false
			h.Sit = true
	...
	-- full mount once: one hop then Sit
	pcall(function()
		me.AssemblyLinearVelocity = Vector3.zero
		me.AssemblyAngularVelocity = Vector3.zero
		me.CFrame = seat.CFrame * CFrame.new(0, 2.4, 0)
		h.PlatformStand = false
		h.Sit = true
		seat:Sit(h)
	end)
...
function ensureBlobman(quiet)
	-- Reliable mount: find/spawn CreatureBlobman, sit VehicleSeat once
	...
	local function trySitExisting()
		local seat = findExistingBlobSeat()
		if not seat then return false end
		return sitBlobSeat(seat, false) == true
	end

	if trySitExisting() then return true end

	local now = os.clock()
	if not S._blobLastSpawnAt or (now - S._blobLastSpawnAt) >= 3.5 then
		S._blobLastSpawnAt = now
		pcall(function()
			if FTAP.BuyToy then FTAP.BuyToy:InvokeServer(_Vzd({104,151,138,134,153,154,151,138,103,145,148,135,146,134,147})) end
		end)
		task.wait(0.2)
		pcall(function()
			if FTAP.SpawnToy then
				FTAP.SpawnToy:InvokeServer("CreatureBlobman", me.CFrame * CFrame.new(0, 0, -6), Vector3.zero)
			end
		end)
	end
	...
	if not quiet then notify(HUB_NAME, "Blobman sit failed — try Spawn+Sit button", 2) end
	return isOnBlobman()
end

function remountBlobSeatOnly(softOnly)
	if isOnBlobman() then return getBlobmanGrabKit() end
	if softOnly then return nil end
	local seat = S._blobStickySeat or findExistingBlobSeat()
	if seat and seat.Parent and sitBlobSeat(seat, false) then
...
function bringPlayerToBlob(p, kit, quiet)
	...
	kit = kit or getBlobmanGrabKit()
	local pivot = nil
	if kit then
		pivot = kit.seat or (kit.blob and (kit.blob.PrimaryPart or kit.blob:FindFirstChildWhichIsA("BasePart")))
	end
	if not pivot then return false end
	local dest = pivot.CFrame * CFrame.new(0, 2.5, -5)
	return bringPlayer(p, dest, quiet ~= false)
end
...
function resolveBlobModelFromSeat(seat)
	...
	for _ = 1, 8 do
		...
		if n:find("blob", 1, true)
			or cur:FindFirstChild(_Vzd({103,145,148,135,146,134,147,120,138,134,153,102,147,137,116,156,147,138,151,120,136,151,142,149,153}))
			or cur:FindFirstChild("BlobmanSeatAndOwnerScript", true)
			or cur:FindFirstChild("CreatureGrab", true)
		then
			if cur:IsA(_Vzd({114,148,137,138,145})) or cur:IsA("Folder") or cur:IsA("BasePart") then
				-- prefer model root
				if cur:IsA(_Vzd({114,148,137,138,145})) then return cur end
...
function findBlobDetector(blob, prefer)
	...
	local names = prefer == "Right"
		and { "RightDetector", "LeftDetector" }
		or { "LeftDetector", "RightDetector" }
	for _, name in ipairs(names) do
		local d = blob:FindFirstChild(name) or blob:FindFirstChild(name, true)
		if d and d:IsA(_Vzd({103,134,152,138,117,134,151,153})) then return d end
	end
...
function findCreatureGrabRemote(blob)
	if not blob then return nil end
	local scriptFolder = blob:FindFirstChild(_Vzd({103,145,148,135,146,134,147,120,138,134,153,102,147,137,116,156,147,138,151,120,136,151,142,149,153}))
	...
	local creatureGrab = scriptFolder and (
		...
		or scriptFolder:FindFirstChild(_Vzd({104,151,138,134,153,154,151,138,108,151,134,135}), true)
	)
...
function getSeatedBlobman()
	...
	if not h.Sit and h.SeatPart.Occupant ~= h then return nil end
	local blob = h.SeatPart.Parent
	if not blob then return nil end
	if tostring(blob.Name) == "CreatureBlobman" or isBlobSeat(h.SeatPart) then
		return blob, h.SeatPart
	end
	if blob:FindFirstChild(_Vzd({103,145,148,135,146,134,147,120,138,134,153,102,147,137,116,156,147,138,151,120,136,151,142,149,153}), true) or blob:FindFirstChild("CreatureGrab", true) then
		return blob, h.SeatPart
	end
	return nil
end
...
function getBlobmanGrabKit()
	...
	if not blob then
		-- soft: last known only for kit resolve, do not re-sit
		seat = S._blobStickySeat
		blob = S._lastBlobModel
		if not (seat and seat.Parent and blob and blob.Parent) then return nil end
		local h = hum()
		if not h or h.SeatPart ~= seat then return nil end
	end

	local leftDet = blob:FindFirstChild("LeftDetector") or blob:FindFirstChild("LeftDetector", true)
	if not leftDet then
		for _, d in ipairs(blob:GetDescendants()) do
			if d:IsA(_Vzd({103,134,152,138,117,134,151,153})) and d.Name:lower():find(_Vzd({137,138,153,138,136,153,148,151}), 1, true) then
				leftDet = d
				break
			end
		end
	...
	local leftWeld = leftDet:FindFirstChild("LeftWeld")
		...
		or leftDet:FindFirstChildWhichIsA("RigidConstraint")
	local scriptFolder = blob:FindFirstChild("BlobmanSeatAndOwnerScript")
		...
		or blob:FindFirstChild(_Vzd({103,145,148,135,146,134,147,120,138,134,153,102,147,137,116,156,147,138,151,120,136,151,142,149,153,128,148,145,137,130}))
	local creatureGrab = scriptFolder and scriptFolder:FindFirstChild("CreatureGrab")
	if not creatureGrab then
		creatureGrab = findCreatureGrabRemote and findCreatureGrabRemote(blob)
	end
...
function moveBlobNear(kit, targetRoot)
	...
	return pcall(function()
		local dest = targetRoot.CFrame * CFrame.new(0, 1, 3)
		if kit.blob:IsA(_Vzd({114,148,137,138,145})) and kit.blob.PrimaryPart then
...
-- Blitz blobmangraball: FireServer(LeftDetector, HRP, LeftWeld) from the seated blob.
function blobmanBlitzFire(targetRoot)
	local h = hum()
	if not h or not targetRoot then return false end
	local seat = h.SeatPart
	if not seat or (not h.Sit and seat.Occupant ~= h) then return false end
	...
	if not leftDet then return false end
	local leftWeld = leftDet:FindFirstChild("LeftWeld") or leftDet:FindFirstChild(_Vzd({113,138,139,153,124,138,145,137}), true)
	local folder = blob:FindFirstChild("BlobmanSeatAndOwnerScript")
		or blob:FindFirstChild("BlobmanSeatAndOwnerScript", true)
	local cg = folder and folder:FindFirstChild("CreatureGrab")
	if not cg then cg = findCreatureGrabRemote(blob) end
	if not cg then return false end
	local ok = pcall(function()
		if cg:IsA("RemoteEvent") then
			cg:FireServer(leftDet, targetRoot, leftWeld)
		else
			cg:InvokeServer(leftDet, targetRoot, leftWeld)
		end
	end)
	return ok == true
end

-- Blitz blobmangraball core: FireServer(LeftDet, HRP, LeftWeld)
function fireCreatureGrab(kit, targetRoot)
	if targetRoot and blobmanBlitzFire(targetRoot) then return true end
	if not kit or not targetRoot or not kit.creatureGrab or not kit.leftDet then return false end
	if not kit.creatureGrab.Parent or not kit.leftDet.Parent then return false end
	if not kit.leftWeld or not kit.leftWeld.Parent then
		kit.leftWeld = kit.leftDet:FindFirstChild("LeftWeld")
			or kit.leftDet:FindFirstChild("LeftWeld", true)
			or kit.leftDet:FindFirstChildWhichIsA("Weld")
			or kit.leftDet:FindFirstChildWhichIsA("WeldConstraint")
			or kit.leftDet:FindFirstChildWhichIsA("RigidConstraint")
	end
	local ok = pcall(function()
		if kit.creatureGrab:IsA("RemoteEvent") then
			kit.creatureGrab:FireServer(kit.leftDet, targetRoot, kit.leftWeld)
		else
			pcall(function() kit.creatureGrab:InvokeServer(kit.leftDet, targetRoot, kit.leftWeld) end)
		end
	end)
	return ok
end
...
function fireCreatureGrabBoth(kit, targetRoot)
	...
	local rightDet = kit.blob:FindFirstChild("RightDetector") or kit.blob:FindFirstChild(_Vzd({119,142,140,141,153,105,138,153,138,136,153,148,151}), true)
	if not rightDet or rightDet == kit.leftDet then return end
	local weld = rightDet:FindFirstChild(_Vzd({119,142,140,141,153,124,138,145,137})) or rightDet:FindFirstChildWhichIsA("Weld")
	local scr = kit.blob:FindFirstChild("BlobmanSeatAndOwnerScript", true)
	local CG = scr and scr:FindFirstChild("CreatureGrab") or kit.creatureGrab
	if CG then
		pcall(function()
			if CG:IsA("RemoteEvent") then
				CG:FireServer(rightDet, targetRoot, weld)
			end
		end)
	end
end
...
function bringPlayerToBlob(p, kit, quiet)
	if not p or not p.Parent then return false end
	kit = kit or getBlobmanGrabKit()
	local pivot = kit and (kit.seat or (kit.blob and kit.blob.PrimaryPart))
	if not pivot then return false end
	local dest = pivot.CFrame * CFrame.new(0, 2.5, -4)
	return bringPlayer(p, dest, quiet ~= false)
end

-- Blitz blobmangraball: require seat, fire all, no remount
function blobmanFireAllSeated()
	if not isOnBlobman() then return false end
...
blobmanGrabAllOnce = function()
	...
	return blobmanFireAllSeated()
end

function blobGrabSingle(p)
	if not p or not validP(p) then return false end
	if not isOnBlobman() then
		-- one mount attempt for one-shot buttons only
		forceBlobmanMount()
	end
	if not isOnBlobman() then return false end
	local kit = getBlobmanGrabKit()
	...
	local r = rootOf(p)
	if not r then return false end
	-- Endoris-style: soft park on them then fire
	moveBlobNear(kit, r)
	kit = getBlobmanGrabKit() or kit
	for _ = 1, 6 do
		r = rootOf(p)
		if not r then break end
		fireCreatureGrabBoth(kit, r)
		RunService.Heartbeat:Wait()
	end
	return true
end
...
function resolveBlobLoopTarget()
	...
	if S._blobGrabTargetName then
		p = Players:FindFirstChild(S._blobGrabTargetName)
		if p and p ~= LP then
			S.blobTarget = p
			return p
		end
...
function setBlobGrabLoop(on, p)
	...
	if not p or not p.Parent or p == LP then
		p = resolveBlobLoopTarget() or S.loopTarget
	end
	if not p or not p.Parent or p == LP then
		S.toggles.blobGrabLoop = false
		notify(HUB_NAME, _Vzd({117,142,136,144,69,134,69,149,145,134,158,138,151,69,139,142,151,152,153}), 1.8)
		return false
	end
	stopLoop(id)
	pcall(stopBlobmanStickyOnly)
	...
	if not isOnBlobman() then
		notify(HUB_NAME, _Vzd({120,142,153,69,148,147,69,126,116,122,119,69,103,145,148,135,146,134,147,69,139,142,151,152,153,81,69,153,141,138,147,69,145,138,134,155,138,69,153,141,142,152,69,148,147}), 2.4)
	else
		notify(HUB_NAME, "Blob Grab Loop ON → " .. playerLabel(p), 1.5)
	end
	-- Blitz: CreatureGrab(LeftDetector, HRP, LeftWeld) while seated. Gucci train sit is paused.
	S._blobGrabNeedSeatAt = 0
	startLoop(id, 0.05, function()
		if not S.toggles.blobGrabLoop then return end
		local target = resolveBlobLoopTarget()
		if not target then return end
		if not isOnBlobman() then
			local now = os.clock()
			if now - (S._blobGrabNeedSeatAt or 0) > 1.4 then
				S._blobGrabNeedSeatAt = now
				pcall(function()
					local seat = S._blobStickySeat or (findExistingBlobSeat and findExistingBlobSeat())
					if seat then sitBlobSeat(seat, false) end
				end)
			end
			return
		end
		local r = target.Character and target.Character:FindFirstChild("HumanoidRootPart")
		if not r then r = rootOf(target) end
		if r then
			blobmanBlitzFire(r)
		end
	end)
...
function setBlobGrabAllLoop(on)
	...
	if not requirePremium(_Vzd({113,148,148,149,69,108,151,134,135,69,102,145,145})) then
		S.toggles.blobGrabAllLoop = false
		return false
	end
	stopLoop(id)
	...
	if not isOnBlobman() then
		notify(HUB_NAME, "Sit on Blobman first — loop waits until seated", 2)
	else
		notify(HUB_NAME, _Vzd({103,145,148,135,69,108,151,134,135,69,102,145,145,69,113,148,148,149,69,116,115,69,77,103,145,142,153,159,82,152,153,158,145,138,78}), 1.5)
	end
	-- Blitz BringAllLongReach: while seated, blobmangraball
	startLoop(id, 0.08, function()
		if not S.toggles.blobGrabAllLoop then return end
		if not isOnBlobman() then return end
		blobmanFireAllSeated()
	end)
	return true
end
...
function setBlobExtractPlotsLoop(on)
	...
	if not on then
		...
		notify(HUB_NAME, _Vzd({106,157,153,151,134,136,153,69,117,145,148,153,152,69,113,148,148,149,69,116,107,107}), 1.2)
		return true
	end
	stopLoop(id)
	S.toggles.blobExtractPlotsLoop = true
	notify(HUB_NAME, "Extract Plots Loop ON (need seat)", 1.5)
	startLoop(id, 0.35, function()
		...
		for _, p in ipairs(Players:GetPlayers()) do
			...
			if p ~= LP and validP(p) and isInSafePlot(p) then
				local r = rootOf(p)
				if r then fireCreatureGrabBoth(kit, r) end
			end
		end
	end)
	return true
...
-- Blitz Destroy Server: while seated fire all; Endoris-style optional fling after
destroyServerLoop = function(keep)
	if not isPremiumTier() then
		notify(HUB_NAME, "Destroy Server needs key: VOIDZHUB", 2.5)
		return
	end
	notify(HUB_NAME, _Vzd({105,138,152,153,151,148,158,69,120,138,151,155,138,151,69,116,115,69,161,69,140,151,134,135,69,134,145,145,69,80,69,139,145,142,147,140}), 2)
	task.spawn(function()
		for _ = 1, 5 do
	...
	while keep() do
		if not isOnBlobman() then
			local now = os.clock()
			if not S._blobLastResitAt or (now - S._blobLastResitAt) > 2.5 then
				S._blobLastResitAt = now
				pcall(ensureBlobman, true)
			end
			task.wait(0.4)
		else
...
function setBlobKickLoop(on, p)
	...
	if not on then
		...
		notify(HUB_NAME, _Vzd({103,145,148,135,69,112,142,136,144,69,113,148,148,149,69,116,107,107}), 1.2)
		return true
	end
	p = p or S.blobTarget or S.selected
	...
	stopLoop(id)
	S.toggles.blobKickLoop = true
	local name = p.Name
	local uid = p.UserId
	notify(HUB_NAME, _Vzd({103,145,148,135,69,112,142,136,144,69,113,148,148,149,69,116,115}), 1.5)
	startLoop(id, 0.35, function()
		if not S.toggles.blobKickLoop then return end
		if not isOnBlobman() then return end
		local target = nil
		for _, pl in ipairs(Players:GetPlayers()) do
			if pl.UserId == uid then target = pl break end
		end
		target = target or Players:FindFirstChild(name)
		if target then pcall(kickPlayer, target, _Vzd({103,145,148,135,146,134,147}), true) end
	end)
	return true
end
...
function cleanupAura(id)
	for _, p in ipairs(Players:GetPlayers()) do
		...
		elseif id == _Vzd({139,151,138,138,159,138}) then
			clearAuraEffect(root, { _Vzd({123,116,110,105,127,132,107,151,138,138,159,138,103,117}) })
		elseif id == _Vzd({148,151,135,142,153}) then
			clearAuraEffect(root, { "VOIDZ_OrbitBP" })
		elseif id == _Vzd({139,145,142,147,140}) then
			clearAuraEffect(root, { "FlingAuraVelocity", _Vzd({123,116,110,105,127,132,103,123}) })
		end
	end
end
...
function snoQuick(p, r)
	-- Blitz SNOWshipPlayer: HRP only. Spamming every limb never grants Head.PartOwner.
	if not p then return end
	snoPlayer(p)
end
...
function tick_fling(cfg, serverWide)
	...
	-- Blitz Fling Aura: SNO HRP, if Head.PartOwner then ONE FlingAuraVelocity. No limb spray.
	eachAuraTarget(cfg, function(p, r, power)
		snoPlayer(p)
		if not ownsPlayer(p) and serverWide then
			for _ = 1, 8 do
				snoPlayer(p)
				if ownsPlayer(p) then break end
				RunService.Heartbeat:Wait()
			end
		end
		if not ownsPlayer(p) then return end
		r = p.Character and p.Character:FindFirstChild("HumanoidRootPart") or r
		if r then applyVel(r, math.clamp(power, 400, 10000), 0.5, false) end
	end, function(part, power)
		sno(part)
		if hasNetOwner(part) then
			applyVel(part, math.clamp(power, 400, 10000), 0.5, false)
		end
	end, serverWide)
end
...
function tick_kick(cfg, serverWide)
	...
	eachAuraTarget(cfg, function(p, r)
		...
		destroyGrabOnTargetOnly(r)
		skyVel(r)
		createKickPhysical(r, kickPhysicalMode(S.kickType))
	end, nil, serverWide)
end
...
function tick_ragdoll(cfg, serverWide)
	...
	local function ensurePeel()
		...
		local m, pp = ensureToy(_Vzd({107,148,148,137,103,134,147,134,147,134}))
		...
		pcall(function()
			...
			local ao = pp:FindFirstChildOfClass(_Vzd({102,145,142,140,147,116,151,142,138,147,153,134,153,142,148,147}))
			if ao then ao.Enabled = false end
		end)
		local head = LP.Character and LP.Character:FindFirstChild("Head")
		local parkY = head and (head.Position.Y + 500) or 500
...
function tick_stomp(cfg, serverWide)
	...
	local function buryModel(model, root)
		...
		pcall(function()
			...
			local hum = model:FindFirstChildOfClass(_Vzd({109,154,146,134,147,148,142,137}))
			if hum then
				hum.Sit = false
			...
			local bp = root:FindFirstChild(_Vzd({123,116,110,105,127,132,103,154,151,158,103,117}))
			if not bp then
				bp = Instance.new("BodyPosition")
...
function tick_telekinesis(cfg, serverWide)
	cfg = getAura(_Vzd({153,138,145,138,144,142,147,138,152,142,152}))
	local me = hrp()
	if not me then return end
...
local AURA_META = {
	...
	{ id = "fling", title = "Fling Nearby", tip = _Vzd({107,145,142,147,140,69,147,138,134,151,135,158,69,149,145,134,158,138,151,152,69,77,149,151,148,157,142,146,142,153,158,78,83}) },
	...
	{ id = _Vzd({135,145,134,136,144,141,148,145,138}), title = _Vzd({117,154,145,145,69,121,148,69,104,154,151,152,148,151}), tip = _Vzd({120,154,136,144,69,147,138,134,151,135,158,69,149,145,134,158,138,151,152,69,153,148,69,136,154,151,152,148,151,69,137,142,151,138,136,153,142,148,147,69,77,149,151,148,157,142,146,142,153,158,78,83}) },
	...
	{ id = _Vzd({151,138,149,138,145}), title = _Vzd({117,154,152,141,69,115,138,134,151,135,158,69,102,156,134,158}), tip = _Vzd({117,154,152,141,69,147,138,134,151,135,158,69,149,145,134,158,138,151,152,69,134,156,134,158,69,77,149,151,148,157,142,146,142,153,158,78,83}) },
	{ id = "sky", title = _Vzd({120,144,158,69,103,145,134,152,153}), tip = _Vzd({103,142,140,69,155,138,151,153,142,136,134,145,69,142,146,149,154,145,152,138,69,82,69,153,141,138,158,69,139,145,158,69,154,149,69,141,142,140,141,69,134,147,137,69,139,134,145,145,69,135,134,136,144,83}) },
	{ id = _Vzd({151,134,140,137,148,145,145}), title = "Ragdoll Nearby", tip = _Vzd({119,134,140,137,148,145,145,69,147,138,134,151,135,158,69,149,145,134,158,138,151,152,69,77,149,151,148,157,142,146,142,153,158,78,83}) },
	...
	{ id = "soft", title = "Soft Push", tip = _Vzd({108,138,147,153,145,138,69,136,134,146,138,151,134,82,137,142,151,138,136,153,142,148,147,69,147,154,137,140,138,69,77,145,142,140,141,153,69,149,148,144,138,81,69,147,148,69,152,149,142,147,78,83}) },
	{ id = _Vzd({145,134,154,147,136,141}), title = _Vzd({113,138,155,142,153,134,153,138,69,115,138,134,151,135,158}), tip = "Continuous upward force - keeps lifting them while on." },
	{ id = _Vzd({152,149,142,144,138}), title = _Vzd({120,149,142,144,138,69,115,138,134,151,135,158}), tip = _Vzd({118,154,142,136,144,69,154,149,69,153,141,138,147,69,137,148,156,147,69,135,154,151,152,153,69,147,138,134,151,135,158,69,77,149,151,148,157,142,146,142,153,158,78,83}) },
	...
	{ id = _Vzd({139,145,134,153,153,138,147}), title = "Ground Press Nearby", tip = "Continuous downward pressure - press into ground without burying." },
}
...
function setAura(id, on)
	...
	if on and AURA_TICKS[id] then
		local interval = (id == _Vzd({139,145,142,147,140}) or id == _Vzd({139,145,142,147,140})) and 0.1 or 0.15
...
function setServerFx(id, on)
	...
	if on and SERVER_TICKS[id] then
		...
		notify(HUB_NAME, _Vzd({120,138,151,155,138,151,69}) .. id .. _Vzd({69,116,115,69,161,69,146,134,149,82,156,142,137,138}), 1.5)
	elseif not on then
...
function getExtinguishPart()
	...
	pcall(function()
		...
		local poison = hole and hole:FindFirstChild(_Vzd({117,148,142,152,148,147,103,142,140,109,148,145,138}))
...
function antiBurnTick()
	...
	for _, d in ipairs(c:GetDescendants()) do
		...
		if n:find("fire") or n:find("burn") or n:find("poison") then
			if d:IsA(_Vzd({103,148,148,145,123,134,145,154,138})) then pcall(function() d.Value = false end) end
		end
	end
	-- Blitz: firetouchinterest FirePlayerPart ↔ map extinguish part
	local fpp = r:FindFirstChild("FirePlayerPart")
	local canBurn = fpp and fpp:FindFirstChild("CanBurn")
	if canBurn and canBurn.Value and fpp then
		local map = workspace:FindFirstChild("Map")
		local hole = map and map:FindFirstChild("Hole")
		local poison = hole and hole:FindFirstChild("PoisonBigHole")
		local ep = poison and poison:FindFirstChild("ExtinguishPart")
		if ep and firetouchinterest then
			pcall(function()
				firetouchinterest(fpp, ep, 0)
				task.wait()
				firetouchinterest(fpp, ep, 1)
...
function voidzDetachMotorJoints(character, nameFilter)
	...
	nameFilter = nameFilter or function() return true end
	local n = 0
	for _, d in ipairs(character:GetDescendants()) do
		if d:IsA("Motor6D") then
			local p1n = d.Part1 and d.Part1.Name or ""
			local jn = d.Name or ""
			if nameFilter(jn, p1n, d) then
				pcall(function()
					-- Bloody/Endoris style: disconnect Part0 first
					d.Part0 = nil
					d:Destroy()
				end)
				n += 1
...
function voidzRemoveFeetForAntiBanana(c)
	c = c or char()
	if not c then return end
	-- Destroy hip/leg/foot Motor6Ds then feet/leg parts so banana sit can't pin you
	voidzDetachMotorJoints(c, function(jn, p1n)
		local s = (jn .. " " .. p1n):lower()
		return s:find("hip", 1, true) or s:find("leg", 1, true) or s:find("foot", 1, true)
			or s:find("ankle", 1, true) or s:find("knee", 1, true)
	end)
	for _, name in ipairs({
		_Vzd({113,138,139,153,69,113,138,140}), "Right Leg",
		"LeftFoot", "RightFoot", _Vzd({113,138,139,153,113,148,156,138,151,113,138,140}), "RightLowerLeg", "LeftUpperLeg", "RightUpperLeg",
	}) do
...
function antiBananaTick()
	if not S.toggles.antiBanana then return end
	local c = char()
	if not c then return end
	local h = hum()
	if isOnBlobman() then return end

	local function disarmNamed(root)
		if not root then return end
		for _, d in ipairs(root:GetChildren()) do
			local n = string.lower(d.Name or "")
			if n:find("banana", 1, true) or n:find("peel", 1, true) then
				if d:IsA(_Vzd({103,134,152,138,117,134,151,153})) then
					pcall(function()
						d.CanCollide = false
						d.CanTouch = false
	...
	disarmNamed(workspace)
	disarmNamed(workspace:FindFirstChild(LP.Name .. "SpawnedInToys"))

	local slipping = false
	for _, d in ipairs(c:GetChildren()) do
		local n = string.lower(d.Name or "")
		if n:find("banana", 1, true) or n:find("slip", 1, true) then
			slipping = true
			pcall(function() d:Destroy() end)
		end
	end
	if h and h.Sit and not h.SeatPart then
		slipping = true
	end

	if slipping then
		voidzRemoveFeetForAntiBanana(c)
		if h then
			pcall(function()
				h.Sit = false
				h.PlatformStand = false
...
function isFlingMoverName(n)
	n = tostring(n or "")
	return n == "FlingAuraVelocity" or n == "SkyVelocity" or n == "KickAuraP" or n == _Vzd({112,142,136,144,102,154,151,134,117,86})
		or n == "KickAuraG" or n == "VOIDZ_BV" or n == _Vzd({123,116,110,105,127,132,123,148,142,137,103,123}) or n == "BringBody"
...
function stripFlingMoversOnSelf(c)
	...
	for _, d in ipairs(c:GetDescendants()) do
		...
		elseif n == "VOIDZ_GucciBV" or n == _Vzd({123,116,110,105,127,132,108,154,136,136,142,109,148,145,137}) or n == "VOIDZ_Fly" or n == "VOIDZ_FlyG"
		...
		elseif d:IsA("BodyVelocity") or d:IsA(_Vzd({103,148,137,158,107,148,151,136,138})) or d:IsA("BodyAngularVelocity")
			or d:IsA(_Vzd({103,148,137,158,121,141,151,154,152,153})) or d:IsA(_Vzd({113,142,147,138,134,151,123,138,145,148,136,142,153,158})) or d:IsA("AngularVelocity")
	...
	if hot or S._afStripN >= 2 then
		...
		pcall(function()
			for _, d in ipairs(r:GetChildren()) do
				local n = d.Name
				if n == "KickAuraP" or n == "KickAuraP1" or n == "KickAuraG" or n == _Vzd({120,144,158,123,138,145,148,136,142,153,158})
					or n == "FlingAuraVelocity" or n == _Vzd({123,116,110,105,127,132,103,123}) or n == "BringBody" then
					d:Destroy()
				end
			end
		end)
...
function antiStickyTick()
	...
	for _, d in ipairs(c:GetDescendants()) do
		...
		if n:find("sticky", 1, true) then
			if d:IsA("Weld") or d:IsA("WeldConstraint") or d:IsA(_Vzd({119,142,140,142,137,104,148,147,152,153,151,134,142,147,153})) then
				pcall(function() d:Destroy() end)
			end
		end
	end
	for _, ch in ipairs(workspace:GetChildren()) do
		if tostring(ch.Name):lower():find("sticky") or ch.Name == _Vzd({120,149,151,134,158,104,134,147,124,105}) then
...
function setAntiLag(on)
	...
	pcall(function()
		...
		if beam and (beam:IsA("LocalScript") or beam:IsA("Script")) then
			...
			notify(HUB_NAME, _Vzd({102,147,153,142,82,113,134,140,69}) .. (on and _Vzd({116,115,69,77,141,142,137,138,152,69,140,151,134,135,69,145,142,147,138,78}) or "OFF | restoring line"), 2)
		else
...
local function isWaterishPart(part)
	if not part or not part:IsA("BasePart") then return false end
	if part.Name == _Vzd({123,116,110,105,127,132,124,134,153,138,151,124,134,145,144}) then return false end
	...
	if p then
		...
		if pn == "water" or pn:find(_Vzd({156,134,153,138,151}), 1, true) or pn:find("ocean", 1, true) then
			return true
		end
	end
	return false
...
function plotHasOwner(plot)
	...
	local sign = plot:FindFirstChild("PlotSign") or plot:FindFirstChild(_Vzd({117,145,148,153,120,142,140,147}), true)
	if not sign then
		local owners = plot:FindFirstChild(_Vzd({121,141,142,152,117,145,148,153,152,116,156,147,138,151,152}), true)
...
function getPlotInteriorCF(plot)
	...
	local area = plot:FindFirstChild("PlotArea") or plot:FindFirstChild(_Vzd({117,145,148,153,102,151,138,134}), true)
	if area and area:IsA("BasePart") then
		return CFrame.new(area.Position + Vector3.new(0, 4, 0))
	end
	for _, name in ipairs({ _Vzd({120,149,134,156,147}), "SpawnLocation", "HouseSpawn", "InteriorSpawn", _Vzd({107,145,148,148,151}), "Base" }) do
...
function tpToRandomHouse(reason, quiet)
	...
	if #pool == 0 then
		if S.lastSafeCF then
			...
			hardenSelfVsKill()
			if not quiet then
				notify(HUB_NAME, _Vzd({102,147,153,142,82,144,142,145,145,69,161,69,147,148,69,141,148,154,152,138,152,69,139,148,154,147,137,69,161,69,145,134,152,153,69,152,134,139,138}), 1.2)
			end
			return true
		end
		if not quiet then
...
function antiKillTick()
	...
	if inWater then
		tpToRandomHouse(_Vzd({156,134,153,138,151}), warModeOn())
		return
	end
...
function startAntiKillLoop()
	...
	local isHeld = LP:FindFirstChild(_Vzd({110,152,109,138,145,137}))
	if isHeld then
...
local WAR_LOOP_IDS = {
	"warProtect", "warGucci", _Vzd({156,134,151,102,147,153,142,112,142,145,145}), _Vzd({156,134,151,102,147,153,142,107,145,142,147,140}),
...
function uninstallWarKillHooks()
	...
	for _, k in ipairs({ "warKillHealth", "warKillHeld", _Vzd({156,134,151,112,142,145,145,104,141,134,151}) }) do
		if S.conns[k] then
		...
		if child.Name == "GrabParts" and grabPartsIsAttackingUs and grabPartsIsAttackingUs(child, c) then
			...
			for _, d in ipairs(child:GetDescendants()) do
				if d:IsA("WeldConstraint") or d:IsA("Weld") or d:IsA("AlignPosition")
					or d:IsA(_Vzd({102,145,142,140,147,116,151,142,138,147,153,134,153,142,148,147})) or d:IsA("RigidConstraint") then
					pcall(function() d:Destroy() end)
				end
			end
...
function warHouseEscapeFE(reason)
	...
	warSnoSelf()
	pcall(function() tpToRandomHouse(reason or _Vzd({156,134,151,82,139,138}), true) end)
	task.defer(function()
...
function startWarMode()
	if not requirePremium("WAR MODE") then
	...
	startLoop(_Vzd({156,134,151,102,147,153,142,120,153,142,136,144,158}), 0.25, function()
		if not warModeOn() then return end
	...
	S.toggles.warMode = true
	if syncToggleUI then pcall(syncToggleUI, _Vzd({156,134,151,114,148,137,138})) end
	if S._toggleRenderers and S._toggleRenderers.warMode then
		pcall(S._toggleRenderers.warMode)
	end
	notify(HUB_NAME, _Vzd({124,102,119,69,114,116,105,106,69,116,115,69,161,69,145,142,140,141,153,69,107,106,69,149,151,148,153,138,136,153,69,77,145,138,152,152,69,145,134,140,78,69,161,69,84,154,147,156,134,151,82,146,148,137,138}), 2.4)
end
...
function installWarModeChatCommands()
	...
	local function handle(msg)
		...
		if m == _Vzd({154,147,156,134,151,82,146,148,137,138}) or m == _Vzd({154,147,156,134,151,146,148,137,138}) or m == _Vzd({154,147,156,134,151}) or m == "war-off" or m == _Vzd({156,134,151,148,139,139}) then
		...
		elseif m == "war-mode" or m == _Vzd({156,134,151,146,148,137,138}) or m == "voidz-war" then
			if not isPremiumTier() then notify(HUB_NAME, _Vzd({124,102,119,69,147,138,138,137,152,69,144,138,158,69,123,116,110,105,127,109,122,103}), 2); return end
...
function forceOneGrabBeamVisible(d)
	...
	elseif d:IsA("Trail") then
		...
		return true
	elseif d:IsA("RopeConstraint") or d:IsA(_Vzd({119,148,137,104,148,147,152,153,151,134,142,147,153})) or d:IsA(_Vzd({120,149,151,142,147,140,104,148,147,152,153,151,134,142,147,153})) then
...
function forceAllGrabBeamsVisible()
	...
	pcall(function()
		local function scan(ch)
			...
			for _, d in ipairs(ch:GetDescendants()) do
				if d:IsA("Beam") or d:IsA(_Vzd({121,151,134,142,145})) then
...
function gucciStripForeignConstraints(c)
	...
	for _, d in ipairs(c:GetDescendants()) do
		if d:IsA(_Vzd({124,138,145,137,104,148,147,152,153,151,134,142,147,153})) or d:IsA("Weld") or d:IsA("RigidConstraint")
			or d:IsA(_Vzd({102,145,142,140,147,117,148,152,142,153,142,148,147})) or d:IsA("AlignOrientation")
		...
		if isLocalBeingHeldFlag() or gucciThrowGuardActive() then
			if d:IsA("BodyVelocity") or d:IsA("BodyPosition") or d:IsA(_Vzd({103,148,137,158,102,147,140,154,145,134,151,123,138,145,148,136,142,153,158}))
				...
				if not gucciIsKeepMover(n) and n ~= "BringBody" then
					local par = d.Parent
					if par and par:IsA(_Vzd({103,134,152,138,117,134,151,153})) and par:IsDescendantOf(c) then
						pcall(function() d:Destroy() end)
					end
				end
			end
...
function gucciDestroyAttackingGrabs(c)
	...
	for _, child in ipairs(workspace:GetChildren()) do
		if child.Name == "GrabParts" and grabPartsIsAttackingUs(child, c) then
			...
			for _, d in ipairs(child:GetDescendants()) do
				if d:IsA("WeldConstraint") or d:IsA("Weld") or d:IsA("AlignPosition")
					or d:IsA("AlignOrientation") or d:IsA(_Vzd({114,148,153,148,151,91,105})) or d:IsA(_Vzd({119,142,140,142,137,104,148,147,152,153,151,134,142,147,153}))
...
function gucciReclaimSelf()
	...
	for _, n in ipairs({ "HumanoidRootPart", "Head", "Torso", "UpperTorso", "LowerTorso",
		...
		_Vzd({113,138,139,153,122,149,149,138,151,102,151,146}), _Vzd({119,142,140,141,153,122,149,149,138,151,102,151,146}), _Vzd({113,138,139,153,113,148,156,138,151,102,151,146}), "RightLowerArm",
		"LeftUpperLeg", _Vzd({119,142,140,141,153,122,149,149,138,151,113,138,140}), "LeftLowerLeg", "RightLowerLeg" }) do
		local p = c:FindFirstChild(n)
		if p and p:IsA("BasePart") then
			sno(p, me.Position)
			pcall(function()
...
function gucciIsKeepMover(name)
	return name == _Vzd({123,116,110,105,127,132,107,145,158}) or name == "VOIDZ_FlyG" or name == "VOIDZ_GucciBV"
...
function gucciStripThrowMovers(c)
	...
	for _, d in ipairs(c:GetDescendants()) do
		...
		elseif n == "SkyVelocity" or n == "BringBody" or n == "KickAuraP" or n == "KickAuraP1"
			or n == _Vzd({107,145,142,147,140,102,154,151,134,123,138,145,148,136,142,153,158}) or n == _Vzd({123,116,110,105,127,132,103,123}) or n == _Vzd({123,116,110,105,127,132,123,148,142,137,103,123}) then
			pcall(function() d:Destroy() end)
		end
	end
end
...
function gucciApplyNonBody(c, on)
	c = c or char()
	if not c then return end
	S._gucciNonBodyOn = on == true
	for _, p in ipairs(c:GetDescendants()) do
		if p:IsA(_Vzd({103,134,152,138,117,134,151,153})) or p:IsA("Part") or p:IsA("MeshPart") then
			local isRoot = p.Name == "HumanoidRootPart"
			pcall(function()
				if on then
					-- Blitz: CanQuery false = not a grab ray target (client)
					p.CanQuery = false
					p.CanTouch = false
					if not isRoot then
						p.Massless = true
...
function gucciApplyNotGrabbableAlways()
	...
	for _, p in ipairs(c:GetDescendants()) do
		if p:IsA(_Vzd({103,134,152,138,117,134,151,153})) then
			pcall(function()
				p.CanQuery = false
				-- CanTouch stays on so YOU can still grab
			end)
		end
	end
end
...
function gucciForceFreeMoveLight()
	...
	if typeof(md) ~= "Vector3" then md = Vector3.zero end
	if md.Magnitude < 0.04 then
		local extra = bloodyGucciMoveInput and bloodyGucciMoveInput()
		if typeof(extra) == "Vector3" then md = extra end
	end
	...
	pcall(function()
		...
		if space then
			local jp = 50
			pcall(function() jp = tonumber(h.JumpPower) or 50 end)
			local v = r.AssemblyLinearVelocity
...
function gucciForceFreeMove()
	...
	local victim = isLocalBeingHeldFlag() or ((S._gucciVictimScanAt or 0) + 0.2 < os.clock() and isGucciVictim(c))
	if victim then S._gucciVictimScanAt = os.clock() end
	...
	-- Heavy body/strip at most ~8Hz (was every Heartbeat = grab lag)
	local now = os.clock()
	if (S._gucciHeavyMoveAt or 0) + 0.12 <= now then
		S._gucciHeavyMoveAt = now
		if not S._gucciQueryOff then
			gucciApplyNonBody(c, true)
			S._gucciQueryOff = true
		end
		gucciStripThrowMovers(c)
	end
...
function gucciServerUngrab()
	...
	if FTAP.Struggle then
		pcall(function() FTAP.Struggle:FireServer(LP) end)
		pcall(function() FTAP.Struggle:FireServer() end)
		pcall(function() FTAP.Struggle:FireServer(r) end)
	end
	...
	if FTAP.DestroyGrabLine then
		for _, n in ipairs({ "HumanoidRootPart", "Torso", "UpperTorso", "LowerTorso", "Head",
			...
			"LeftUpperArm", "RightUpperArm", _Vzd({113,138,139,153,113,148,156,138,151,102,151,146}), "RightLowerArm" }) do
			local p = c:FindFirstChild(n)
			if p then pcall(function() FTAP.DestroyGrabLine:FireServer(p) end) end
		end
	end
	...
	-- Kill attacking GrabParts models + remotes
	if gucciDestroyAttackingGrabs then pcall(gucciDestroyAttackingGrabs, c) end
...
function walkWhileGrabbedTick()
	...
	if not isLocalBeingHeldFlag() then
		if not gucciProtectOn() then
			local bv = r and r:FindFirstChild("VOIDZ_GucciBV")
...
function antiGrabResistTick()
	...
	if not isLocalBeingHeldFlag() and not isLocalVictimGrabbed() then return end
	if isLocalActivelyGrabbing() then return end
	local r = hrp()
	local c = char()
	if FTAP.Struggle then
	...
	local now = os.clock()
	if (S._antiGrabHardAt or 0) + 0.05 <= now then
		S._antiGrabHardAt = now
		...
		if c then
			pcall(gucciDestroyAttackingGrabs, c)
			pcall(gucciApplyNotGrabbableAlways)
		end
	end
end

-- Old name kept for call sites. Anti-Grab is walk now, not freeze.
function blitzAntiGrabWhileHeld()
	walkWhileGrabbedTick()
end

-- Bloody V2 Gucci: sit AlwaysHere train VehicleSeat so Occupant replicates (server
-- sees you on the train = invisible in the arena). Destroy SeatWeld locally so YOU
-- still walk. Do NOT Jump/Sit=false — that unsits the server and kills the invis.
do local _z989=(2*10); if _z989<0 and _Vj() then _z989=_z989+1 end end

function bloodyFindGucciSeat()
	local seat
	pcall(function()
		local map = workspace:FindFirstChild("Map")
		local ah = map and map:FindFirstChild("AlwaysHereTweenedObjects")
		local train = ah and ah:FindFirstChild(_Vzd({121,151,134,142,147}))
		local obj = train and train:FindFirstChild("Object")
		local model = obj and obj:FindFirstChild("ObjectModel")
		if model then
			seat = model:FindFirstChildWhichIsA("VehicleSeat", true) or model:FindFirstChildWhichIsA("Seat", true)
		end
	end)
	if seat and isBlobSeat(seat) then return nil end
	return seat
end

function bloodyStripLocalSeatWeld(seat)
	if not seat then return end
	pcall(function()
		...
		pcall(function()
			for _, d in ipairs(h:GetChildren()) do
				if d:IsA("Weld") or d:IsA("WeldConstraint") then
					local p0, p1 = d.Part0, d.Part1
					if p0 == seat or p1 == seat then d:Destroy() end
				end
			end
		end)
	end
...
function gucciCaptureHome()
	local r = hrp()
	if not r then return end
	if r.Position.Y >= 6 then
		S._gucciSafeY = r.Position.Y
		S._gucciHomeCF = r.CFrame
		S._gucciWalkCF = r.CFrame
	elseif not S._gucciHomeCF then
		S._gucciHomeCF = gucciSafeHomeCF(r.CFrame)
...
function gucciPinCamStanding(r)
	...
	if not r then return end
	local c = char()
	local cp = c and c:FindFirstChild("CamPart")
	if cp then
		pcall(function()
...
function bloodyTrainGucciSit()
	...
	task.defer(function()
		...
		local rr = hrp()
		pcall(function()
			if hh and not isOnBlobman() then
				hh.Sit = false
				hh:SetStateEnabled(Enum.HumanoidStateType.Seated, false)
				if hh:GetState() == Enum.HumanoidStateType.Seated then
...
function startBloodyTrainGucci()
	...
	pcall(gucciCaptureHome)
	pcall(bloodyTrainGucciSit)
	pcall(gucciApplyNotGrabbableAlways)
	if S.conns.gucciChar then
		pcall(function() S.conns.gucciChar:Disconnect() end)
...
function bloodyGucciMoveInput()
	local h = hum()
	local r = hrp()
	if not h or not r then return Vector3.zero end
	local md = h.MoveDirection
	if typeof(md) == "Vector3" and md.Magnitude > 0.04 then return md.Unit end
	local cam = workspace.CurrentCamera
	local look = cam and cam.CFrame or r.CFrame
	local f = Vector3.new(look.LookVector.X, 0, look.LookVector.Z)
	local ri = Vector3.new(look.RightVector.X, 0, look.RightVector.Z)
	if f.Magnitude > 0.01 then f = f.Unit else f = Vector3.zero end
	if ri.Magnitude > 0.01 then ri = ri.Unit else ri = Vector3.zero end
	local v = Vector3.zero
	pcall(function()
		if UserInputService:IsKeyDown(Enum.KeyCode.W) then v = v + f end
		if UserInputService:IsKeyDown(Enum.KeyCode.S) then v = v - f end
		if UserInputService:IsKeyDown(Enum.KeyCode.A) then v = v - ri end
		if UserInputService:IsKeyDown(Enum.KeyCode.D) then v = v + ri end
	end)
	if v.Magnitude > 0.04 then return v.Unit end
	return Vector3.zero
end
...
function tickBloodyTrainGucci(dt)
	...
	local r = hrp()
	local h = hum()
	-- Locally not-seated so grab + jump work. Occupant on the train is enough for SS invis.
	pcall(function()
		...
		if r and not isLocalBeingHeldFlag() then
			local bv = r:FindFirstChild("VOIDZ_GucciBV")
			if bv then bv:Destroy() end
		end
	end)
	local nowQ = os.clock()
	if (S._gucciQueryAt or 0) + 0.25 <= nowQ then
...
function bloodyGucciPulse()
	...
	if FTAP.DestroyGrabLine then
		...
		if t then pcall(function() FTAP.DestroyGrabLine:FireServer(t) end) end
	end
	gucciDestroyAttackingGrabs(c)
	gucciStripForeignConstraints(c)
	if not S._gucciQueryOff then
		gucciApplyNotGrabbableAlways()
		S._gucciQueryOff = true
	end
end

-- Gucci = Bloody V2 path (seat-break + Struggle/Ragdoll0 + walk). Do NOT Blitz-anchor.
do local _z480=(7*9); if _z480<0 and _Vj() then _z480=_z480+1 end end
...
function gucciFreeTick()
	...
	if isLocalActivelyGrabbing() and not isLocalBeingHeldFlag() then
		local bv = r:FindFirstChild("VOIDZ_GucciBV")
		if bv then pcall(function() bv:Destroy() end) end
		if S._blitzGrabAnchored then r.Anchored = false; S._blitzGrabAnchored = false end
		return
	end

	if isLocalBeingHeldFlag() then
		if not S._gucciSeatBrokeThisHold then
			bloodyGucciSeatBreak()
		end
		local now = os.clock()
		if (S._gucciPulseAt or 0) + 0.12 <= now then
			S._gucciPulseAt = now
			bloodyGucciPulse()
		end
		gucciForceFreeMoveLight()
	else
		S._gucciSeatBrokeThisHold = false
		if S._blitzGrabAnchored then
			r.Anchored = false
			S._blitzGrabAnchored = false
		end
		local bv = r:FindFirstChild("VOIDZ_GucciBV")
...
function installAntis()
	...
	-- Blitz: IsHeld.Changed → Heartbeat Struggle + Ragdoll0 + Anchor while held
	task.spawn(function()
		local isHeld = LP:FindFirstChild(_Vzd({110,152,109,138,145,137})) or LP:WaitForChild("IsHeld", 30)
		if not isHeld then return end
		isHeld.Changed:Connect(function(held)
			if held ~= true then
				if S._blitzHeldHB then pcall(function() S._blitzHeldHB:Disconnect() end) S._blitzHeldHB = nil end
				local r = hrp()
				if r and S._blitzGrabAnchored then r.Anchored = false; S._blitzGrabAnchored = false end
				return
			end
			if not (S.toggles.antiGrab or S.toggles.antiGucci) then return end
			if isLocalActivelyGrabbing and isLocalActivelyGrabbing() then return end
			local r = hrp()
			if not r then return end
			if S._blitzHeldHB then pcall(function() S._blitzHeldHB:Disconnect() end) end
			S._blitzHeldHB = RunService.Heartbeat:Connect(function()
				if not (S.toggles.antiGrab or S.toggles.antiGucci) then return end
				if not isLocalBeingHeldFlag() then
					if S._blitzHeldHB then S._blitzHeldHB:Disconnect(); S._blitzHeldHB = nil end
					if r.Parent then r.Anchored = false end
					S._blitzGrabAnchored = false
					return
				end
				if r.Anchored then
	...
	local function bindCharacter(c)
		...
		task.spawn(function()
			...
			if hrp2 then
				local fpp = hrp2:FindFirstChild(_Vzd({107,142,151,138,117,145,134,158,138,151,117,134,151,153}))
				if fpp then table.insert(watchParts, fpp) end
			end
			for _, part in ipairs(watchParts) do
				part.ChildAdded:Connect(function(ch)
					if ch.Name == "PartOwner" then
						local function punchOwner()
							if not (S.autoCounter or S.toggles.autoCounter) then return end
							local grabberVal = ch.Value
							local grabberPlr
							if typeof(grabberVal) == "Instance" and grabberVal:IsA("Player") then
								grabberPlr = grabberVal
							elseif grabberVal then
								grabberPlr = Players:FindFirstChild(tostring(grabberVal))
							end
							if grabberPlr and grabberPlr ~= LP and validP(grabberPlr) then
								task.spawn(counterAttackPlayer, grabberPlr, rootOf(grabberPlr))
							end
						end
						punchOwner()
						pcall(function() ch.Changed:Connect(punchOwner) end)
		...
		task.spawn(function()
			...
			h2:GetPropertyChangedSignal("Sit"):Connect(function()
				...
				if h2.SeatPart and h2.SeatPart.Parent and h2.SeatPart.Parent.Name == _Vzd({104,151,138,134,153,154,151,138,103,145,148,135,146,134,147}) then return end
				...
				-- Blitz: only unsit if SeatPart is nil (forced sit, no real seat). Never kick Blobman/train.
				if antiGrabProtectOn and antiGrabProtectOn() then
					task.defer(function()
						if isOnBlobman() then return end
						if h2.SeatPart == nil then
	...
	bind("gucciAntiHB", RunService.Heartbeat:Connect(function(dt)
		...
		-- ANTI-GRAB: resist spam-grab every frame (Struggle + huge XZ hold + walk).
		if antiGrabProtectOn() and (isLocalBeingHeldFlag() or isLocalVictimGrabbed()) then
			pcall(antiGrabResistTick)
			if S.autoCounter or S.toggles.autoCounter then
				pcall(runAutoAttackerTick)
			end
			return
		end
	...
	task.spawn(function()
		local isHeld = LP:FindFirstChild(_Vzd({110,152,109,138,145,137})) or LP:WaitForChild("IsHeld", 15)
		if not isHeld then return end
		...
		isHeld.Changed:Connect(function(held)
			if held == true then
				...
				if S.autoCounter or S.toggles.autoCounter then
					task.spawn(function()
						for _ = 1, 12 do
							local a = findGrabAttacker()
							if a then
								pcall(counterAttackPlayer, a)
								break
							end
							RunService.Heartbeat:Wait()
						end
		...
		local function hookChar(c)
			...
			c.DescendantAdded:Connect(function(ch)
				...
				if ch:IsA("BodyVelocity") or ch:IsA("BodyAngularVelocity") or ch:IsA("BodyForce")
					or ch:IsA("BodyPosition") or ch:IsA("LinearVelocity") or ch:IsA(_Vzd({123,138,136,153,148,151,107,148,151,136,138}))
	...
	pcall(function()
		...
		for _, root in ipairs(roots) do
			for _, name in ipairs({ "CharacterAndBeamMove", "CharacterAndBeam", _Vzd({103,138,134,146,114,148,155,138}), "GrabbingScript" }) do
				local scr = root:FindFirstChild(name) or root:FindFirstChild(name, true)
				if scr and (scr:IsA(_Vzd({113,148,136,134,145,120,136,151,142,149,153})) or scr:IsA("Script")) then
...
function isGrabHandlePart(d)
	...
	local n = d.Name:lower()
	if n == "dragpart" or n == "grabpart" or n == "holdpart" or n == _Vzd({141,142,153,149,134,151,153}) then return true end
...
function restoreGrabLineVisuals(root)
	...
	pcall(function()
		...
		for _, ch in ipairs(workspace:GetChildren()) do
			if ch.Name == _Vzd({108,151,134,135,117,134,151,153,152}) or ch.Name == "GrabLine" or ch.Name == "Grab" then
				fixContainer(ch)
...
function watchGrabBeamInstance(beam)
	...
	beam:SetAttribute(_Vzd({123,116,110,105,127,132,103,138,134,146,124,134,153,136,141}), true)
	local function reShow()
		if S.toggles.invisLine then return end
		forceOneGrabBeamVisible(beam)
	end
	pcall(function()
		beam:GetPropertyChangedSignal(_Vzd({106,147,134,135,145,138,137})):Connect(reShow)
		beam:GetPropertyChangedSignal("Transparency"):Connect(reShow)
	end)
	reShow()
end

do local _z488=(4*5); if _z488<0 and _Vj() then _z488=_z488+1 end end
...
function installGrabLineWatchdog()
	...
	local function hookContainer(ch)
		...
		pcall(function()
			...
			if ch:GetAttribute(_Vzd({123,116,110,105,127,132,108,151,134,135,109,148,148,144})) then return end
...
function findSlotsFolder()
	...
	for _, ch in ipairs(workspace:GetChildren()) do
		...
		if (n == _Vzd({152,145,148,153,152}) or n:find("slot", 1, true)) and (ch:IsA(_Vzd({107,148,145,137,138,151})) or ch:IsA("Model")) then
			return ch
		end
	end
	return nil
...
function scanSlotMachines()
	...
	for _, slot in ipairs(root:GetChildren()) do
		local sh = slot:FindFirstChild(_Vzd({120,145,148,153,109,134,147,137,145,138}))
		if sh then
...
do local _z984=(3*6); if _z984<0 and _Vj() then _z984=_z984+1 end end
...
function snoSlotHandle(handle)
	if not handle or not handle:IsA(_Vzd({103,134,152,138,117,134,151,153})) then return end
	local me = hrp()
	if not me then return end
	local origin = me.Position
	pcall(function()
...
function setAutoSpinCoins(on)
	...
	local handles, lights, root = scanSlotMachines()
	if not root then
		notify(HUB_NAME, _Vzd({102,154,153,148,82,120,149,142,147,95,69,156,148,151,144,152,149,134,136,138,83,120,145,148,153,152,69,146,142,152,152,142,147,140}), 3)
	else
		notify(HUB_NAME, _Vzd({102,154,153,148,82,120,149,142,147,69,116,115,69,161,69}) .. #handles .. _Vzd({69,141,134,147,137,145,138,152,69,161,69}) .. #lights .. " lights", 2.5)
	end
...
local KNOWN_TOYS = {
	"Airhorn", _Vzd({102,147,155,142,145,108,151,134,158}), "ArmChairBlue", _Vzd({102,151,146,104,141,134,142,151,103,151,148,156,147,108,151,134,158}), _Vzd({102,151,146,104,141,134,142,151,117,142,147,144}),
	...
	_Vzd({103,148,146,135,103,134,145,145,148,148,147}), _Vzd({103,148,146,135,105,134,151,144,114,134,153,153,138,151}), "BombMissile", "BookManyPages", "BookNormal",
	_Vzd({103,148,148,146,135,148,157}), "BoxCrateWood", "BubbleBlower", _Vzd({103,154,136,144,138,153,117,134,142,147,153}), "Campfire",
	"ChildrensChair", "ChildrensCouch", "ChildrensDesk", "ChildrensShelf", _Vzd({104,141,142,145,137,151,138,147,152,121,134,135,145,138}),
	"ClockAlarm", "ComputerLaptopOld", "CouchBlue", "CouchBrownGray", _Vzd({104,148,154,136,141,105,134,151,144,108,151,134,158}),
...
function getOwnedToyNames()
	...
	scan(LP:FindFirstChild(_Vzd({103,134,136,144,149,134,136,144})))
	scan(char())
	local pg = LP:FindFirstChild("PlayerGui")
	if pg then
...
function resolveSpawnCF(name, opts)
	...
	local dist = opts.dist or (tostring(name):find(_Vzd({117,134,145,145,138,153})) and 3 or 5)
...
function stopTrainDrive(quiet)
	...
	S._trainModel = nil
	S._trainParts = nil
	if not quiet then notify(HUB_NAME, _Vzd({121,151,134,142,147,69,152,153,148,149,149,138,137}), 1) end
end
...
function getTweenedFolder()
	...
	return map:FindFirstChild("AlwaysHereTweenedObjects")
		...
		or map:FindFirstChild(_Vzd({121,156,138,138,147,138,137,116,135,143,138,136,153,152}))
...
function trainNameLooksLikeRock(n)
	...
	return n:find("rock", 1, true) or n:find("stone", 1, true) or n:find(_Vzd({135,148,154,145,137,138,151}), 1, true)
		or n:find("pebble", 1, true) or n:find("ore", 1, true) or n:find("crystal", 1, true)
		or n:find("debris", 1, true) or n:find("rubble", 1, true) or n:find("cliff", 1, true)
		or n:find("terrain", 1, true) or n:find(_Vzd({140,151,148,154,147,137}), 1, true)
end

function trainNameLooksLikeTrain(n)
	n = tostring(n or ""):lower()
	if trainNameLooksLikeRock(n) then return false end
	return n == "train" or n:find(_Vzd({153,151,134,142,147}), 1, true) or n:find("mono", 1, true)
		or n:find("loco", 1, true) or n:find("rail", 1, true) or n:find("cart", 1, true)
		or n:find(_Vzd({152,144,158,153,151,134,142,147}), 1, true) or n:find("subway", 1, true) or n:find("tram", 1, true)
end
...
function trainModelIsBlocked(model)
	...
	if n:find("blob", 1, true) or n:find("ufo", 1, true) or n:find(_Vzd({136,151,138,134,153,154,151,138}), 1, true)
...
function trainScoreContainer(ch)
	...
	if n:find("ufo", 1, true) or n:find("blob", 1, true) or n:find("creature", 1, true) then
		return -1, nil, nil
	end
	...
	local model = ch:IsA("Model") and ch or seat:FindFirstAncestorOfClass("Model") or ch
...
function findFlyingBlueTrain()
	...
	if always then
		local names = {
			_Vzd({121,151,134,142,147}), "BlueTrain", "SkyTrain", "TheTrain", "Monorail",
			"FlyingTrain", _Vzd({113,148,136,148,146,148,153,142,155,138}), _Vzd({103,145,154,138,69,121,151,134,142,147}), "MapTrain", "train",
		}
		for _, name in ipairs(names) do
		...
		for _, d in ipairs(workspace:GetDescendants()) do
			if d:IsA("VehicleSeat") then
				...
				if dist < nd and trainPosIsFindable(d.Position) then
					local m = d:FindFirstAncestorOfClass(_Vzd({114,148,137,138,145}))
...
function _vB2(seatOrModel)
	...
	if trainIsSeat(seatOrModel) or seatOrModel:IsA(_Vzd({103,134,152,138,117,134,151,153})) then
		local model = seatOrModel:FindFirstAncestorOfClass(_Vzd({114,148,137,138,145}))
...
function trainGetSetNetworkOwner()
	...
	return g.setnetworkowner or g.set_network_owner
		...
		or (typeof(setnetworkowner) == _Vzd({139,154,147,136,153,142,148,147}) and setnetworkowner)
		or nil
end
...
function trainApplyFeVelocity(parts, drivePart, dir, speed)
	...
	if typeof(dir) == _Vzd({123,138,136,153,148,151,88}) and dir.Magnitude > 0.05 then
...
do local _z485=(4*7); if _z485<0 and _Vj() then _z485=_z485+1 end end
...
function startTrainDrive()
	...
	local distStr = ""
	if me and seat then
		distStr = " | " .. math.floor((me.Position - seat.Position).Magnitude) .. _Vzd({69,152,153,154,137,152})
	end
	...
	S._trainHornConn = UserInputService.InputBegan:Connect(function(input, gp)
		...
		if input.KeyCode == Enum.KeyCode.H and hornSound and hornSound:IsA(_Vzd({120,148,154,147,137})) then
			pcall(function() hornSound:Play() end)
		end
	end)

	notify(HUB_NAME, _Vzd({121,119,102,110,115,69,105,119,110,123,106,69,116,115,69,161,69,124,102,120,105,69,120,149,134,136,138,84,104,153,151,145,69,161,69,109,69,141,148,151,147,69,161,69,120,153,148,149,69,121,151,134,142,147}), 2.8)
	return true
end
...
function ensureSnowCollisionGroup()
	...
	local ok = pcall(function()
		...
		pcall(function()
			PhysicsService:CollisionGroupSetCollidable(SNOW_CG, _Vzd({105,138,139,134,154,145,153}), true)
		end)
	end)
...
function isLocalHoldingSnowball(partOrModel)
	...
	if partOrModel:IsA(_Vzd({103,134,152,138,117,134,151,153})) and hit(partOrModel) then return true end
...
function stripSnowFarmPinsOnly(ch)
	...
	local function kill(d)
		if not d then return end
		local n = d.Name
		if n == "FarmSnowball" or n == _Vzd({123,116,110,105,127,132,120,147,148,156,107,145,142,147,140}) or n == _Vzd({103,151,142,147,140,103,148,137,158}) then
			pcall(function() d:Destroy() end)
		end
	end
...
do local _z288=(4*3); if _z288<0 and _Vj() then _z288=_z288+1 end end
	...
	task.spawn(function()
		...
		while S._snowFarmOn do
			...
			if grown >= want then
				...
				notify(HUB_NAME, _Vzd({108,151,148,156,147,69}) .. grown .. " | pins off — hold freely", 2.5)
				break
			end
			if have < want then
...
function explodeGrownSnowballs()
	...
	notify(HUB_NAME, _Vzd({106,157,149,145,148,137,138,137,69}) .. n .. _Vzd({69,152,147,148,156,135,134,145,145,152}), 1.5)
	return n
end
...
do local _z482=(8*8); if _z482<0 and _Vj() then _z482=_z482+1 end end
...
do local _z800=(9*6); if _z800<0 and _Vj() then _z800=_z800+1 end end

function spawnFormOffsets(toy, offsets, scaleMul, opts)
	opts = opts or {}
	toy = toy or _Vzd({117,134,145,145,138,153,113,142,140,141,153,103,151,148,156,147})
	...
	S.formCancel = false
	if not opts.silent then
		notify(HUB_NAME, label .. _Vzd({69,148,147,69,161,69}) .. n .. " attached" .. (fail > 0 and (" | " .. fail .. _Vzd({69,146,142,152,152})) or ""), 2.5)
	end
	return n > 0
end

do local _z993=(8*11); if _z993<0 and _Vj() then _z993=_z993+1 end end
...
do local _z665=(4*11); if _z665<0 and _Vj() then _z665=_z665+1 end end
...
local FORM_BUILDS = {
	...
	{ id = "wings", title = _Vzd({124,142,147,140,152}), tip = _Vzd({116,147,69,158,148,154,151,69,135,134,136,144,69,161,69,139,145,134,149,152,69,154,149,84,137,148,156,147}), offsets = formWingsOffsets, scale = 1.05, anim = "flap" },
	...
	{ id = _Vzd({136,142,151,136,145,138}), title = "Circle", tip = _Vzd({116,151,135,142,153,142,147,140,69,151,142,147,140,69,134,151,148,154,147,137,69,158,148,154}), offsets = formCircleOffsets, scale = 1.0, anim = "orbit" },
	{ id = _Vzd({134,151,151,148,156}), title = _Vzd({102,151,151,148,156}), tip = _Vzd({102,151,151,148,156,69,152,141,134,149,138,69,156,148,151,147,69,142,147,69,139,151,148,147,153}), offsets = formArrowOffsets, scale = 1.0 },
	{ id = _Vzd({136,151,148,152,152}), title = _Vzd({104,151,148,152,152}), tip = _Vzd({117,145,154,152,69,84,69,136,151,148,152,152,69,148,147,69,158,148,154}), offsets = formCrossOffsets, scale = 1.0 },
...
function sparkClearTracked(destroy)
	...
	for _, pe in ipairs(list) do
		...
		if part and part.Parent then
			for _, nm in ipairs({ "VOIDZ_SparkBP", _Vzd({123,116,110,105,127,132,120,149,134,151,144,103,108}) }) do
				local m = part:FindFirstChild(nm)
				if m then pcall(function() m:Destroy() end) end
			end
		end
...
function sparkRegisterPiece(model, offset)
	...
	if model:IsA(_Vzd({114,148,137,138,145})) then
		for _, d in ipairs(model:GetDescendants()) do
			if d:IsA("BasePart") then
				pcall(function()
					d.CanCollide = false
...
function runSparkBurst()
	...
	local positions = calcSparkPositions(S.sparkShape or _Vzd({120,149,141,138,151,138}), amount, radius, S.sparkRot or 0)
...
do local _z991=(7*7); if _z991<0 and _Vj() then _z991=_z991+1 end end
...
function bombBodyOf(model)
	...
	return model:FindFirstChild(_Vzd({117,134,151,153,109,142,153,105,138,153,138,136,153,148,151}))
...
function explodeBombAt(model, worldPos)
	...
	-- FAST path: one hop + explode (old 6x0.03s wait was the lag)
	if body then
		pcall(function()
			body.Anchored = false
			ownBombPart(body)
	...
	if FTAP.BombExplode then
		...
		-- second fire for reliability without waiting
		pcall(function()
...
function startMissileStrike()
	...
	-- Instant explode on spawn (no park delay / no batch wait)
	if folder then
		missileState.conn = folder.ChildAdded:Connect(function(child)
			...
			task.spawn(function()
				...
				local body = bombBodyOf(child)
				if body then ownBombPart(body) end
				explodeBombAt(child, r.Position)
			end)
		end)
	end

	notify(HUB_NAME, _Vzd({114,142,152,152,142,145,138,69,152,153,151,142,144,138,69,116,115,69,161,69,107,102,120,121,69,138,157,149,145,148,137,138}), 1.2)
	...
	task.spawn(function()
		...
		while missileState.running do
			...
			-- drain any parked pending instantly
			if r and #missileState.pending > 0 then
				local pos = r.Position
				for i = #missileState.pending, 1, -1 do
					local m = table.remove(missileState.pending, i)
			...
			-- spam spawn as fast as remotes allow (one at a time, tiny gap)
			if r and me and toysRoom() > 0 then
				local now = os.clock()
				if now - buyCd > 0.35 then
					buyCd = now
					if FTAP.BuyToy then pcall(function() FTAP.BuyToy:InvokeServer(typ) end) end
				end
				spawnToyNow(typ, {
					cf = me.CFrame * CFrame.new(0, 6, -4),
					rot = Vector3.zero,
					skipBuy = true,
					silent = true,
					gap = 0.01,
...
function fireMissilesOnce()
	task.spawn(function()
		...
		local detonated = 0
		-- FAST burst: spawn → explode on target immediately (no park / no long waits)
		for i = 1, n do
			if not missileState then break end
			local me = hrp()
			local rr = rootOf(p)
			if not me or not rr then break end
			local before = snapshotToySet()
			spawnToyNow(typ, {
				cf = me.CFrame * CFrame.new(0, 5 + i * 0.15, -3),
				skipBuy = true,
				silent = true,
				gap = 0.01,
				canTimeout = 0.5,
			})
			local model = waitNewFormToy(before, 1.2)
			if model then
				local body = bombBodyOf(model)
				if body then ownBombPart(body) end
				local pos = rootOf(p) and rootOf(p).Position or rr.Position
				if explodeBombAt(model, pos) then detonated += 1 end
			end
		end
		notify(HUB_NAME, _Vzd({103,148,148,146,69}) .. detonated .. "/" .. n .. " -> @" .. p.Name, 1.2)
	end)
end

local SUSPICIOUS_NAMES = {
	"voidz", "fe6", "rayfield", _Vzd({148,151,142,148,147}), "dex", "infiniteyield", "iy_", "darkdex",
	"flingaura", "skyvelocity", _Vzd({135,151,142,147,140,135,148,137,158}), "kethhook", "hydroxide", _Vzd({152,142,146,149,145,138,69,152,149,158}),
	_Vzd({152,142,146,149,145,138,152,149,158}), "remote spy", "remotespy", "windui", "linoria", _Vzd({152,142,151,142,154,152}), "bloody",
	_Vzd({135,145,142,153,159}), _Vzd({138,147,137,148,151,142,152}), _Vzd({149,148,148,149,141,154,135}), "nameless", "script-ware", _Vzd({152,158,147,134,149,152,138}), "drawing",
	...
	pcall(function()
		...
		if c then
			for _, d in ipairs(c:GetDescendants()) do
				...
				if d:IsA(_Vzd({109,142,140,141,145,142,140,141,153})) then mark(_Vzd({109,142,140,141,145,142,140,141,153,69,106,120,117,69,148,147,69,136,141,134,151,134,136,153,138,151})) end
			...
			if h and h.JumpPower > 80 then mark(_Vzd({111,154,146,149,117,148,156,138,151,69}) .. tostring(h.JumpPower)) end
		end
		local bp = p:FindFirstChild("Backpack")
		if bp then
			for _, t in ipairs(bp:GetChildren()) do
				checkName(t.Name, _Vzd({135,134,136,144,149,134,136,144}))
			end
		end
		...
		for _, n in ipairs({ "FartherReach", _Vzd({105,138,139,134,154,145,153,119,138,134,136,141}), "CurrentReach" }) do
...
function runConsoleCommand(raw)
	...
	elseif cmd == _Vzd({135,151,142,147,140}) then
		local p = findPlayer(rest)
...
do local _z624=(4*7); if _z624<0 and _Vj() then _z624=_z624+1 end end
...
do local _z254=(9*3); if _z254<0 and _Vj() then _z254=_z254+1 end end
...
function setPurpleTint(on)
	...
	cc.Contrast = 0.08
	cc.Brightness = 0.02
	if not bloom then bloom = Instance.new(_Vzd({103,145,148,148,146,106,139,139,138,136,153})); bloom.Name = _Vzd({123,116,110,105,127,132,103,145,148,148,146}); bloom.Parent = Lighting end
	bloom.Intensity = 0.4
	bloom.Size = 20
	bloom.Threshold = 0.9
end
	...
	pcall(function()
		for _, ch in ipairs(part:GetChildren()) do
			if ch:IsA("BodyVelocity") or ch:IsA(_Vzd({103,148,137,158,107,148,151,136,138})) or ch:IsA("BodyAngularVelocity")
				...
				if ch.Name == "VOIDZ_ThrowArm" or ch.Name == _Vzd({120,154,149,138,151,120,153,151,138,147,140,153,141}) then
				...
				elseif tostring(ch.Name):sub(1, 5) == "VOIDZ" or ch.Name == _Vzd({127,138,151,148,108,151,134,155,142,153,158,107,148,151,136,138})
					or ch.Name == "FlingBV" or ch.Name == "SpinAV" or ch.Name == _Vzd({108,151,134,155,103,107})
...
function resolveReleaseRoot(part)
	...
	if model then
		...
		if hrp and hrp:IsA(_Vzd({103,134,152,138,117,134,151,153})) then return hrp end
	end
	return part
end
...
function applyReleaseEffects(part)
	...
	if wantThrow then
		...
		if plr then
			task.spawn(function()
				pcall(function() snoPlayer(plr) end)
				applyVel(root, power, 0.5, true)
			end)
		end
		effectParts[root] = true
...
function boostGrabAlign(grabModel, on)
	...
	local ap = drag:FindFirstChildOfClass(_Vzd({102,145,142,140,147,117,148,152,142,153,142,148,147})) or drag:FindFirstChild("AlignPosition", true)
...
function _isPalletPart(part)
	...
	for _ = 1, 14 do
		...
		if n:find(_Vzd({149,134,145,145,138,153}), 1, true) or n == _Vzd({149,134,145,145,138,153,145,142,140,141,153,135,151,148,156,147}) then return true end
	...
	if model and model.Parent and tostring(model.Parent.Name):find("SpawnedInToys", 1, true) then
		local n = tostring(model.Name):lower()
		if n:find(_Vzd({149,134,145,145,138,153}), 1, true) or n:find(_Vzd({136,151,134,153,138}), 1, true) or n:find("platform", 1, true) then
			return true
		end
...
function resolvePalletBase(palletPart)
	...
	return base or (palletPart:IsA(_Vzd({103,134,152,138,117,134,151,153})) and palletPart or nil)
end
...
function clearPalletLockForces(root)
	if not root then return end
	pcall(function()
		for _, n in ipairs({ _Vzd({123,116,110,105,127,132,117,134,145,145,138,153,113,148,136,144}), _Vzd({123,116,110,105,127,132,117,134,145,145,138,153,113,148,136,144,108}), "VOIDZ_PalletBV", "VOIDZ_PalletWeld", _Vzd({123,116,110,105,127,132,117,134,145,145,138,153,102,145,142,140,147}) }) do
...
function pinRootToPalletCenter(root, base, plr)
	...
	pcall(function()
		...
		local bp = root:FindFirstChild(_Vzd({123,116,110,105,127,132,117,134,145,145,138,153,113,148,136,144}))
		if not bp then
			bp = Instance.new("BodyPosition")
...
function pinEveryoneOnPallet(base)
	...
	if myR and rootOnPallet(myR, base) then
		...
		pcall(function()
			...
			local bp = myR:FindFirstChild(_Vzd({123,116,110,105,127,132,117,134,145,145,138,153,113,148,136,144}))
			if not bp then
				bp = Instance.new("BodyPosition")
	...
	pcall(function()
		for _, m in ipairs(workspace:GetChildren()) do
			if m:IsA("Model") and not Players:GetPlayerFromCharacter(m) then
				...
				if hum and r and r:IsA(_Vzd({103,134,152,138,117,134,151,153})) and rootOnPallet(r, base) then
...
do local _z129=(4*5); if _z129<0 and _Vj() then _z129=_z129+1 end end
...
do local _z188=(4*9); if _z188<0 and _Vj() then _z188=_z188+1 end end
...
function onGrabPartsAdded(child)
	...
	task.spawn(function()
		...
		local model = grabbed:FindFirstAncestorOfClass(_Vzd({114,148,137,138,145})) or grabbed.Parent
			...
			while child.Parent and not released do
				...
				if (S.toggles.palletSilentAim or S.toggles.shurikenSilentAim) and not released then
					...
					local isShuriken = part and part.Parent and (
						...
						or (part:FindFirstAncestorOfClass(_Vzd({114,148,137,138,145})) and tostring(part:FindFirstAncestorOfClass(_Vzd({114,148,137,138,145})).Name):lower():find("kunai", 1, true))
					)
		...
		if S.grabSpin and S.toggles.spinWhileHold then
			local av = Instance.new(_Vzd({103,148,137,158,102,147,140,154,145,134,151,123,138,145,148,136,142,153,158}))
			av.Name = "SpinAV"
			av.MaxTorque = Vector3.new(math.huge, math.huge, math.huge)
			av.AngularVelocity = Vector3.new(0, S.grabSpinSpeed or 80, 0)
			av.Parent = grabbed
...
function isGrabLinkedToPlayer(plr)
	...
	-- cheap: only check core body parts, not every descendant
	for _, n in ipairs({ "HumanoidRootPart", "Torso", "UpperTorso", "Head", "LowerTorso" }) do
		local bp = c:FindFirstChild(n)
		if bp then
			for _, d in ipairs(bp:GetChildren()) do
				if d:IsA("WeldConstraint") or d:IsA("Weld") then
					local p0, p1 = d.Part0, d.Part1
					local other = (p0 == bp and p1) or (p1 == bp and p0)
					if other and (other:IsDescendantOf(their) or (other.Parent and other.Parent.Name == "GrabPart")) then
						return true
					end
				end
			end
...
-- LIGHT sever (old 6x Struggle + full workspace scan every tick caused huge lag)
function severAttackingGrabLinks(plr)
	...
	if gucciDestroyAttackingGrabs then pcall(gucciDestroyAttackingGrabs, c) end
	-- only top-level GrabParts children (not GetDescendants of workspace)
	for _, child in ipairs(workspace:GetChildren()) do
		if child.Name == "GrabParts" and grabPartsIsAttackingUs and grabPartsIsAttackingUs(child, c) then
			pcall(function() child:Destroy() end)
		end
...
function protectSelfFromCounterKickback()
	...
	pcall(function()
		...
		for _, d in ipairs(me:GetChildren()) do
			if d:IsA("BodyVelocity") or d:IsA("BodyAngularVelocity") or d:IsA(_Vzd({113,142,147,138,134,151,123,138,145,148,136,142,153,158})) then
				local n = d.Name
				if n == "VOIDZ_Counter" or n == "FlingAuraVelocity" or n == "SkyVelocity"
					or n == "VOIDZ_BV" or (isFlingMoverName and isFlingMoverName(n)) then
					d:Destroy()
				end
			end
		end
	...
	if h then
		pcall(function()
			h.PlatformStand = false
			if not (isOnBlobman and isOnBlobman()) then h.Sit = false end
		end)
	end
end

-- Who is grabbing us? Check every limb + FirePlayerPart + attacking GrabParts.
function findGrabAttacker()
	local c = char()
	if not c then return nil end
	local function playerFromOwner(po)
		...
		if typeof(val) == "Instance" and val:IsA("Player") then
			...
			grabber = Players:FindFirstChild(tostring(val))
		end
		if grabber and grabber ~= LP and validP(grabber) then return grabber end
		return nil
	end
	for _, bp in ipairs(c:GetChildren()) do
		if bp:IsA("BasePart") then
			local g = playerFromOwner(bp:FindFirstChild("PartOwner"))
			if g then return g end
			local fpp = bp:FindFirstChild("FirePlayerPart")
			if fpp then
				g = playerFromOwner(fpp:FindFirstChild("PartOwner"))
				if g then return g end
			end
		end
	end
...
function counterAttackPlayer(plr, part)
	...
	local me = hrp()
	if not r or not me then return end
	...
	pcall(function() sno(r, me.Position) end)

	local away = (r.Position - me.Position)
	if away.Magnitude < 0.5 then away = me.CFrame.LookVector end
	away = Vector3.new(away.X, 0, away.Z)
	if away.Magnitude < 0.05 then away = Vector3.new(0, 0, -1) end
	away = away.Unit

	local ktype = S.kickType or _Vzd({120,144,158,69,102,147,136,141,148,151})
	if ktype == "Phoenix" then ktype = _Vzd({120,144,158,69,102,147,136,141,148,151}) end
	...
	local ok, err = pcall(function()
		if mode == "Freeze" then
			...
			r.AssemblyLinearVelocity = Vector3.zero
			if createBringBody then createBringBody(r, r.CFrame) end
		elseif mode == "Death" then
			if destroyGrabOnTargetOnly then destroyGrabOnTargetOnly(r) end
			local hum = plr.Character and plr.Character:FindFirstChildOfClass("Humanoid")
			if hum then
			...
			if skyVel then skyVel(r) end
		elseif mode == "Kick" then
			if destroyGrabOnTargetOnly then destroyGrabOnTargetOnly(r) end
			if kickPlayer then kickPlayer(plr, ktype, true)
			else applyVel(r, force, 0.15) end
		elseif mode == _Vzd({107,145,142,147,140}) then
			if destroyGrabOnTargetOnly then destroyGrabOnTargetOnly(r) end
			if flingPlayer then flingPlayer(plr, force, true, true)
			else applyVel(r, force, 0.8) end
		elseif mode == "Void" then
			if destroyGrabOnTargetOnly then destroyGrabOnTargetOnly(r) end
			if voidPlayer then voidPlayer(plr, true)
			else r.AssemblyLinearVelocity = Vector3.new(0, -1e5, 0) end
		elseif mode == "Ragdoll" then
			if destroyGrabOnTargetOnly then destroyGrabOnTargetOnly(r) end
		...
		elseif mode == "Sky" then
			...
			applyVel(r, force, 0.5)
		end
	end)
	if not ok then
		warn("[VOIDZ] counterAttack", mode, err)
	end

	S._counterKickbackUntil = os.clock() + 0.35
	protectSelfFromCounterKickback()
	task.spawn(function()
		for _ = 1, 6 do
...
function revengeFromGrabParts(grabModel)
	...
	local plr = findGrabAttacker()
	if plr then
		task.spawn(counterAttackPlayer, plr)
	end
end

function runAutoAttackerTick()
	if not (S.autoCounter or S.toggles.autoCounter) then return end
	local attacker = findGrabAttacker()
	if not attacker then
		if not (isLocalBeingHeldFlag() or isLocalVictimGrabbed()) then return end
		return
	end
	task.spawn(counterAttackPlayer, attacker)
end

doAntiGrabHard = function()
	-- Blitz anti-grab / Bloody Gucci — no workspace GrabParts thrash, no Sit=false.
	if isLocalActivelyGrabbing() and not isLocalBeingHeldFlag() then return end
...
bind("antiGrabChild", workspace.ChildAdded:Connect(function(child)
	...
	local function burst()
		...
		if S.autoCounter or S.toggles.autoCounter then
			local a = findGrabAttacker()
			if a then task.spawn(counterAttackPlayer, a) end
		end
	end
	task.defer(burst)
	task.delay(0.04, burst)
	task.delay(0.12, burst)
end))

function isLocalVictimGrabbed()
	-- cheap checks only (old full GetDescendants every 0.08s lagged hard)
	local held = LP:FindFirstChild(_Vzd({110,152,109,138,145,137})) or LP:FindFirstChild(_Vzd({110,152,109,138,145,137}))
	if held and held.Value == true then return true end
	local c = char()
	if not c then return false end
	for _, n in ipairs({ "Head", "HumanoidRootPart", "Torso", "UpperTorso" }) do
		local bp = c:FindFirstChild(n)
		if bp and bp:FindFirstChild("PartOwner") then return true end
	end
			...
			if part:IsA("BasePart") then
				for _, ch in ipairs(part:GetChildren()) do
					...
					if ch:IsA("BodyVelocity") or ch:IsA("BodyPosition") or ch:IsA("BodyForce")
						or ch:IsA(_Vzd({103,148,137,158,102,147,140,154,145,134,151,123,138,145,148,136,142,153,158}))
...
function getGrabbingScript()
	...
	local ps = LP:FindFirstChild("PlayerScripts")
	if ps then
		for _, n in ipairs({ _Vzd({108,151,134,135,135,142,147,140,120,136,151,142,149,153}), "CharacterAndBeamMove", "CharacterAndBeam", "BeamMove" }) do
	...
	pcall(function()
		...
		for k, v in pairs(env) do
			if type(k) == "string" and type(v) == _Vzd({147,154,146,135,138,151}) then
				local lk = k:lower()
				if lk == "distance" or lk == "grabdistance" or lk == _Vzd({145,142,147,138,137,142,152,153,134,147,136,138})
					or lk == "maxdistance" or lk == _Vzd({151,138,134,136,141}) or lk == _Vzd({151,134,147,140,138}) then
					env[k] = amount
				end
			end
		end
...
function enableFurtherReachGamepass()
	pcall(function()
		...
		if old and not (old:IsA(_Vzd({103,148,148,145,123,134,145,154,138})) or old:IsA("NumberValue") or old:IsA(_Vzd({110,147,153,123,134,145,154,138}))) then
			old:Destroy()
			old = nil
		end
		if not old then
		...
		for _, name in ipairs({ "DefaultReach", "CurrentReach" }) do
			...
			if nv:IsA("NumberValue") or nv:IsA(_Vzd({110,147,153,123,134,145,154,138})) then
...
function installFurtherGrabHook()
	...
	local function watchGrabParts(ch)
		...
		if not (ch:IsA("Model") or ch:IsA(_Vzd({107,148,145,137,138,151}))) then return end
		task.spawn(function()
...
do local _z374=(7*6); if _z374<0 and _Vj() then _z374=_z374+1 end end
...
function setLineExtend(on)
	...
	local msg = ok
		and (_Vzd({120,136,151,148,145,145,69,137,142,152,153,134,147,136,138,69,116,115,69,161,69}) .. tostring(amt) .. _Vzd({69,77,156,141,138,138,145,69,156,141,142,145,138,69,141,148,145,137,142,147,140,78}))
...
local function startFovCircle()
	...
	-- Xeno / partial UNC: Drawing may be nil — skip FOV rings, aim still works
	if not voidzHasDrawing or not voidzHasDrawing() then
		return
	end
...
setSilentAim = function(on)
	...
	if on and not silentHooked and voidzHookMeta and voidzGetNcm then
		silentHooked = true
		local old
		old = voidzHookMeta(game, "__namecall", function(self, ...)
			local method = voidzGetNcm()
...
do
	local g = voidzEnv()
	if type(g) == "table" then
		AK = type(g.VOIDZ_ANTIKICK) == _Vzd({153,134,135,145,138}) and g.VOIDZ_ANTIKICK or {}
		g.VOIDZ_ANTIKICK = AK
	end
end
...
local function isRealKickSignal(text, source)
	...
	if source == _Vzd({117,145,134,158,138,151,95,112,142,136,144}) then return true end
	...
	if low:find(_Vzd({134,147,153,142,82,144,142,136,144}), 1, true) and (low:find("grace", 1, true) or low:find("active", 1, true) or low:find("off", 1, true)) then
		return false
	end
	...
	if low:find(_Vzd({145,148,148,149,69,144,142,136,144}), 1, true) or low:find(_Vzd({144,142,136,144,69,153,158,149,138}), 1, true) then return false end
	...
	if low:find(_Vzd({156,134,152,69,144,142,136,144,138,137}), 1, true) and not low:find(_Vzd({158,148,154,69,156,138,151,138,69,144,142,136,144,138,137}), 1, true) then
		return false
	end
	if low:find("kicked player", 1, true) or low:find(_Vzd({144,142,136,144,142,147,140}), 1, true) then return false end
	...
	local phrases = {
		...
		_Vzd({144,142,136,144,138,137,69,139,151,148,146,69,153,141,142,152,69,138,157,149,138,151,142,138,147,136,138}),
		...
		_Vzd({138,151,151,148,151,69,136,148,137,138,95,69,87,92,94}),
...
local function installNamecallKickHook()
	if not voidzHookMeta or not voidzGetNcm then return false end
	local gak = voidzEnv()
	if type(gak) == _Vzd({153,134,135,145,138}) and gak.VOIDZ_AK_HOOKED then return true end
	local ok = pcall(function()
		local old
		old = voidzHookMeta(game, "__namecall", function(self, ...)
			local method = voidzGetNcm()
			...
			if (m == "Kick" or m == "kick") and self == LP then
				...
				local low = msg:lower()
				if low:find(_Vzd({155,148,142,137,159}), 1, true) or low:find("saved your butt", 1, true) then
					return old(self, ...)
				end
		...
		local g2 = voidzEnv()
		if type(g2) == _Vzd({153,134,135,145,138}) then g2.VOIDZ_AK_HOOKED = true end
	end)
	return ok
end
...
setAntiKick = function(on, quiet)
	...
	bindKickScanners()
	print(_Vzd({128,123,116,110,105,127,130,69,134,147,153,142,82,144,142,136,144,69,116,115,69,161,69,152,136,134,147,152,69,136,148,147,152,148,145,138,69,80,69,119,148,135,145,148,157,69,144,142,136,144,69,122,110,69,161,69}) .. GRACE_SEC .. _Vzd({152,69,140,151,134,136,138}))
	if not quiet then
...
function isVoiceBanSignal(text)
	...
	if text == "" then return false end
	local keys = {
		"voice chat", "voicechat", "voice-chat", _Vzd({155,148,142,136,138,69,135,134,147}), "voiceban",
		_Vzd({135,134,147,147,138,137,69,139,151,148,146,69,155,148,142,136,138}), "banned from using voice", _Vzd({153,138,146,149,148,151,134,151,142,145,158,69,135,134,147,147,138,137,69,139,151,148,146,69,155,148,142,136,138}),
		...
		_Vzd({136,148,146,146,154,147,142,153,158,69,152,153,134,147,137,134,151,137,152}), _Vzd({155,148,142,136,138,69,146,148,137,138,151,134,153,142,148,147}), "inappropriate language",
		_Vzd({155,148,142,136,138,69,136,141,134,153,69,152,154,152,149,138,147,152,142,148,147}), _Vzd({137,142,152,134,135,145,138,137,69,155,148,142,136,138}), _Vzd({155,148,142,136,138,69,149,151,142,155,142,145,138,140,138,152}),
		_Vzd({136,148,146,146,154,147,142,136,134,153,142,148,147,69,135,134,147}), _Vzd({153,138,157,153,69,134,147,137,69,155,148,142,136,138}), _Vzd({136,141,134,153,69,134,147,137,69,155,148,142,136,138}),
	}
...
function isVoiceRelatedRemote(inst)
	...
	if n == _Vzd({155,148,142,136,138,135,134,147}) or n == _Vzd({135,134,147,155,148,142,136,138}) or n == "voicechatban" or n == "banfromvoice" then
		return true
	end
	if n:find(_Vzd({155,148,142,136,138,135,134,147}), 1, true) or n:find(_Vzd({155,148,142,136,138,132,135,134,147}), 1, true)
		or n:find(_Vzd({155,148,142,136,138,136,141,134,153,135,134,147}), 1, true) or n:find(_Vzd({151,138,149,148,151,153,155,148,142,136,138}), 1, true)
...
function installAntiVoiceBan(quiet)
	...
	do
		local g = voidzEnv()
		if type(g) == _Vzd({153,134,135,145,138}) then g.VOIDZ_ANTIVOICE = true end
	end

	-- 1) Block Player:Kick with voice ban text only (skip if Xeno lacks hookfunction)
	pcall(function()
		if not voidzHookFn or not voidzNewCC then return end
		local oldKick
		oldKick = voidzHookFn(LP.Kick, voidzNewCC(function(self, ...)
			if S.toggles.antiVoiceBan == false then
				return oldKick(self, ...)
			end
			local msg = tostring((...) or "")
			if isVoiceBanSignal(msg) then
				warn(_Vzd({128,123,116,110,105,127,130,69,135,145,148,136,144,138,137,69,155,148,142,136,138,69,112,142,136,144,95}), msg:sub(1, 80))
				return
			end
			return oldKick(self, ...)
		end))
	...
	pcall(function()
		if not voidzHookMeta or not voidzGetNcm or not voidzNewCC then return end
		if S._antiVoiceNamecall then return end
		local old
		old = voidzHookMeta(game, "__namecall", voidzNewCC(function(self, ...)
	...
	pcall(function()
		...
		task.spawn(function()
			...
			S.conns.antiVoiceCore = pg.DescendantAdded:Connect(function(d)
				if S.toggles.antiVoiceBan == false then return end
				task.defer(function()
					if d:IsA(_Vzd({121,138,157,153,113,134,135,138,145})) or d:IsA(_Vzd({121,138,157,153,103,154,153,153,148,147})) then
...
local function makeScroll(parent)
	...
	lay.Padding = UDim.new(0, mob and 8 or 7)
	lay.SortOrder = Enum.SortOrder.LayoutOrder
	lay.Parent = sc
	pad(sc, mob and 12 or 12, mob and 12 or 12, mob and 18 or 16, mob and 12 or 12)
	return sc
end
...
local function indexFeature(kind, title, tip, extra)
	...
	if tab == "home" and kind == _Vzd({152,138,136,153,142,148,147}) then return end
...
local function section(parent, text, order)
	...
	bar.Position = UDim2.fromOffset(2, mob and 12 or 11)
	bar.BackgroundColor3 = C.accent
	bar.BorderSizePixel = 0
	bar.Parent = wrap
	corner(bar, 1)
	local l = Instance.new(_Vzd({121,138,157,153,113,134,135,138,145}))
	l.BackgroundTransparency = 1
	l.Size = UDim2.new(1, -22, 1, 0)
	l.Position = UDim2.fromOffset(16, 0)
	l.Font = Enum.Font.GothamBold
	l.TextSize = mob and 11 or 10
...
local function makeButton(parent, opts)
	...
	local mob = isMobileMode()
	local h = opts.h or (mob and 44 or 36)
	local wrap = Instance.new("Frame")
	...
	wrap.Parent = parent
	corner(wrap, 10)
	local bStroke = stroke(wrap, opts.danger and (C.dangerStroke or C.danger) or C.strokeSoft, 1, opts.danger and 0.2 or 0.4)
	if opts.danger then
		grad(wrap, mix3(C.dangerText, C.danger, 0.22), C.danger, 18)
	else
		glass(wrap)
	end
	local hasGear = opts.settings ~= nil
	local b = Instance.new("TextButton")
	b.Size = UDim2.new(1, hasGear and (mob and -38 or -32) or 0, 1, 0)
	b.BackgroundTransparency = 1
	b.BorderSizePixel = 0
	b.Font = Enum.Font.GothamBold
	b.TextSize = mob and 14 or 13
	...
	b.MouseEnter:Connect(function()
		tween(wrap, { BackgroundColor3 = opts.danger and Color3.fromRGB(90, 32, 52) or C.card2 }, 0.12)
		if bStroke then tween(bStroke, { Transparency = 0.12, Color = opts.danger and (C.dangerStroke or C.danger) or C.accent }, 0.12) end
	end)
	b.MouseLeave:Connect(function()
		tween(wrap, { BackgroundColor3 = opts.danger and C.danger or C.card }, 0.16)
		if bStroke then tween(bStroke, { Transparency = opts.danger and 0.2 or 0.4, Color = opts.danger and (C.dangerStroke or C.danger) or C.strokeSoft }, 0.16) end
	end)
	...
	if hasGear then
		...
		local gs = mob and 34 or 28
		gear.Size = UDim2.fromOffset(gs, gs)
		gear.Position = UDim2.new(1, -gs, 0.5, -gs / 2)
		gear.BackgroundColor3 = C.card
		gear.Text = "*"
		gear.TextSize = mob and 14 or 12
		gear.TextColor3 = C.accent2
		gear.Font = Enum.Font.GothamBold
		gear.Parent = wrap
		corner(gear, 8)
		stroke(gear, C.strokeSoft, 1, 0.5)
...
local function makeToggle(parent, opts)
	...
	local id = opts.id
	local mob = isMobileMode()
	local rowH = opts.desc and (mob and 56 or 46) or (mob and 46 or 38)
	...
	local pillW, pillH = mob and 46 or 40, mob and 24 or 20
	local knobS = mob and 18 or 16
	...
	knob.Parent = pill
	corner(knob, knobS / 2)
	stroke(knob, Color3.new(1, 1, 1), 1, 0.82)
	...
	local function render()
		...
		if on then
			tween(pill, { BackgroundColor3 = C.accent }, 0.14)
			tween(knob, { Position = knobOn, BackgroundColor3 = Color3.new(1, 1, 1) }, 0.14)
			if pillStroke then tween(pillStroke, { Color = C.accent2, Transparency = 0.15 }, 0.14) end
			rowStroke.Color = C.accent
			rowStroke.Transparency = 0.42
		else
			tween(pill, { BackgroundColor3 = C.bg }, 0.14)
			tween(knob, { Position = knobOff, BackgroundColor3 = C.muted }, 0.14)
			if pillStroke then tween(pillStroke, { Color = C.strokeSoft, Transparency = 0.4 }, 0.14) end
...
local function makeSlider(parent, opts)
	...
	local row = Instance.new("Frame")
	row.LayoutOrder = opts.order or 0
	row.Size = UDim2.new(1, -6, 0, mob and 58 or 50)
	row.BackgroundColor3 = C.card
	row.BorderSizePixel = 0
	row.Parent = parent
	corner(row, 10)
	...
	label.Text = opts.title or _Vzd({120,145,142,137,138,151})
	label.Parent = row
	local val = Instance.new(_Vzd({121,138,157,153,113,134,135,138,145}))
	val.BackgroundTransparency = 1
	val.Size = UDim2.new(0.3, -10, 0, mob and 16 or 14)
	val.Position = UDim2.new(0.68, 0, 0, 8)
	val.Font = Enum.Font.GothamBold
	val.TextSize = mob and 12 or 11
	val.TextColor3 = C.accent2
	val.TextXAlignment = Enum.TextXAlignment.Right
	val.Text = tostring(value)
	val.Parent = row
	local track = Instance.new("Frame")
	track.Size = UDim2.new(1, -24, 0, mob and 8 or 6)
	track.Position = UDim2.fromOffset(12, mob and 38 or 32)
	track.BackgroundColor3 = C.bg
	track.BorderSizePixel = 0
	track.Parent = row
	corner(track, 4)
	local fill = Instance.new(_Vzd({107,151,134,146,138}))
	local frac0 = (value - min) / math.max(max - min, 1)
	fill.Size = UDim2.new(frac0, 0, 1, 0)
	fill.BackgroundColor3 = C.accent
	fill.BorderSizePixel = 0
	fill.Parent = track
	corner(fill, 4)
	grad(fill, C.accent2, C.accent, 0)
	local thumb = Instance.new("Frame")
	local th = mob and 14 or 12
	thumb.Size = UDim2.fromOffset(th, th)
	thumb.AnchorPoint = Vector2.new(0.5, 0.5)
	thumb.Position = UDim2.new(frac0, 0, 0.5, 0)
	thumb.BackgroundColor3 = Color3.new(1, 1, 1)
	thumb.BorderSizePixel = 0
	thumb.ZIndex = 2
	thumb.Parent = track
	corner(thumb, th / 2)
	stroke(thumb, C.accent, 1, 0.25)
...
local function makeInput(parent, opts)
	...
	row.Parent = parent
	corner(row, 10)
	stroke(row, C.strokeSoft, 1, 0.55)
	glass(row)
...
local function makeDropdown(parent, opts)
	...
	btn.Parent = row
	corner(btn, 7)
	stroke(btn, C.strokeSoft, 1, 0.45)
	local open = false
	...
	if #options > 4 then
		...
		searchBox.PlaceholderText = _Vzd({120,138,134,151,136,141,83,83,83})
		searchBox.Text = ""
...
local function makePlayerSearchList(sc, opts, orderFn)
	...
	clearRow.Size = UDim2.new(1, -6, 0, 30)
	clearRow.BackgroundColor3 = C.card
	clearRow.BorderSizePixel = 0
	clearRow.Parent = sc
	corner(clearRow, 8)
	stroke(clearRow, C.strokeSoft, 1, 0.55)
	glass(clearRow)
	...
	clearInfo.TextColor3 = C.text
	clearInfo.TextXAlignment = Enum.TextXAlignment.Left
	clearInfo.Text = _Vzd({120,138,145,138,136,153,142,148,147,69,84,69,145,148,148,149,69,146,134,151,144,152})
	...
	listBox.Parent = sc
	corner(listBox, 10)
	stroke(listBox, C.strokeSoft, 1, 0.5)
	local listSc = Instance.new(_Vzd({120,136,151,148,145,145,142,147,140,107,151,134,146,138}))
	...
	function refresh()
		for _, ch in ipairs(listSc:GetChildren()) do
			if ch:IsA(_Vzd({121,138,157,153,103,154,153,153,148,147})) then
				ch:Destroy()
			end
		end
		...
		for _, lab in ipairs(playerLabels(q)) do
			...
			if p and p.Parent then
				...
				b.Text = (isLoop and "  ●  " or (isSel and "  ›  " or _Vzd({69,69,69,69,69}))) .. lab
				b.AutoButtonColor = false
				b.Parent = listSc
				corner(b, 7)
				if isLoop or isSel then
					pcall(function() stroke(b, C.accent or Color3.fromRGB(155, 70, 255), isLoop and 1.2 or 1, 0.25) end)
				end
				b.MouseEnter:Connect(function()
					if not isSel and not isLoop then tween(b, { BackgroundColor3 = C.card2 }, 0.1) end
				end)
				b.MouseLeave:Connect(function()
					if not isSel and not isLoop then tween(b, { BackgroundColor3 = baseCol }, 0.12) end
				end)
				b.MouseButton1Click:Connect(function()
					S.selected = p
					S.loopTarget = p
	...
	multiClearBtn.MouseButton1Click:Connect(function()
		...
		notify(HUB_NAME, _Vzd({114,134,151,144,152,69,136,145,138,134,151,138,137,69,161,69,149,142,136,144,69,152,153,142,145,145,69}) .. who, 1.2)
	end)
...
local function makeAuraBlock(parent, order, meta)
	...
	corner(holder, 10)
	stroke(holder, C.strokeSoft, 1, 0.55)
	glass(holder)
	local lay = Instance.new(_Vzd({122,110,113,142,152,153,113,134,158,148,154,153}))
	...
	tLabel.Text = _Vzd({121,134,151,140,138,153})
	tLabel.Parent = settings

	local targets = { "Players", _Vzd({116,135,143,138,136,153,152}), _Vzd({117,145,134,158,138,151,152,69,134,147,137,69,116,135,143,138,136,153,152}) }
	...
	local rLabel = Instance.new(_Vzd({121,138,157,153,113,134,135,138,145}))
	...
	rTrack.BackgroundColor3 = C.card2
	rTrack.Text = ""
	rTrack.AutoButtonColor = false
	rTrack.Parent = settings
	corner(rTrack, 4)
	local rFill = Instance.new(_Vzd({107,151,134,146,138}))
	...
	pTrack.BackgroundColor3 = C.card2
	pTrack.Text = ""
	...
	if id == _Vzd({147,138,153,148,156,147}) then
...
local TAB_DEFS = {
	...
	{ id = _Vzd({136,148,146,135,134,153}), icon = "CB", label = "Combat" },
	{ id = "blobman", icon = "BM", label = "Blobman" },
	{ id = "player", icon = "PL", label = "Player" },
	{ id = "grab", icon = "GR", label = "Grab" },
	{ id = "auras", icon = "AU", label = "Auras" },
	{ id = _Vzd({152,138,151,155,138,151}), icon = "SV", label = _Vzd({120,138,151,155,138,151}) },
	{ id = "loop", icon = "LP", label = "Loops" },
	{ id = "anti", icon = "PR", label = "Protect" },
	{ id = "move", icon = "MV", label = _Vzd({114,148,155,138,146,138,147,153}) },
	{ id = "visuals", icon = "VI", label = _Vzd({123,142,152,154,134,145,152}) },
	{ id = "toys", icon = "TY", label = "Toys" },
	{ id = "explosions", icon = "EX", label = "Explosions" },
	{ id = _Vzd({156,148,151,145,137}), icon = "WD", label = "World" },
	{ id = "auto", icon = "AT", label = "Auto" },
	{ id = "console", icon = "MS", label = "Misc" },
	{ id = "trans", icon = "TR", label = _Vzd({121,151,134,147,152}) },
	{ id = _Vzd({152,148,154,147,137,152}), icon = "SD", label = _Vzd({120,148,154,147,137,152}) },
	{ id = "fun", icon = "FN", label = "Fun" },
...
_TAB_BUILDERS["home"] = function(sc, n)
		...
		grad(hero, mix3(C.accentDim, C.card, 0.15), C.card, 16)
		local rail = Instance.new(_Vzd({107,151,134,146,138}))
		...
		hs.TextSize = 12
		hs.TextColor3 = C.text
		hs.TextXAlignment = Enum.TextXAlignment.Left
		hs.TextYAlignment = Enum.TextYAlignment.Top
		hs.TextWrapped = true
		hs.Text = "One key. Every tool.  RightShift hide  ·  search the header or pick a tab."
		hs.Parent = hero

		section(sc, _Vzd({118,122,110,104,112,69,114,102,117}), n())
		for _, item in ipairs({
			{ "Combat", "Click a player, then fling / kick / kill" },
			{ "Blobman", _Vzd({120,142,153,69,158,148,154,151,69,135,145,148,135,81,69,149,142,136,144,69,134,69,147,134,146,138,81,69,113,148,148,149,69,108,151,134,135}) },
			{ "Protect", "Anti-Grab walks while held  ·  Gucci = invis" },
			{ "More", "Auras · Loops · Movement · Toys · World" },
		}) do
			local card = Instance.new("Frame")
			card.LayoutOrder = n()
			...
			card.Parent = sc
			corner(card, 10)
			stroke(card, C.strokeSoft, 1, 0.58)
			glass(card)
			local k = Instance.new("TextLabel")
			k.BackgroundTransparency = 1
			k.Size = UDim2.new(1, -20, 0, 16)
			k.Position = UDim2.fromOffset(12, 7)
			k.Font = Enum.Font.GothamBold
			k.TextSize = 11
			k.TextColor3 = C.accent2
			k.TextXAlignment = Enum.TextXAlignment.Left
			k.Text = item[1]
			k.Parent = card
			local v = Instance.new("TextLabel")
			v.BackgroundTransparency = 1
			v.Size = UDim2.new(1, -20, 0, 16)
			v.Position = UDim2.fromOffset(12, 24)
		...
		section(sc, _Vzd({120,121,102,121,122,120}), n())
		...
		local st = Instance.new(_Vzd({121,138,157,153,113,134,135,138,145}))
		st.LayoutOrder = n()
		...
		st.TextWrapped = true
		st.Text = _Vzd({103,154,142,145,137,69,69}) .. BUILD
			.. "\nPlace  " .. tostring(game.PlaceId)
			.. "\nKey    VOIDZHUB"
		st.Parent = sc
		corner(st, 10)
		pad(st, 12, 12, 12, 12)
		stroke(st, C.strokeSoft, 1, 0.55)
		glass(st)
		S.homeStatus = st
end
_TAB_BUILDERS[_Vzd({136,148,146,135,134,153})] = function(sc, n)
		...
		makeDropdown(sc, {
			...
			tip = "Bloody: Sky Anchor / Float / Silent — needs Head.PartOwner then SkyVelocity",
		})
		...
		makeButton(sc, {
			order = n(), title = "Kill Them", danger = true, tip = _Vzd({112,142,145,145,69,152,138,145,138,136,153,138,137}),
			callback = function()
		...
		makeButton(sc, { order = n(), title = "Burn Them", danger = true, tip = _Vzd({102,149,149,145,158,69,139,142,151,138,69,155,142,134,69,152,153,134,153,154,152,69,153,148,158}), callback = function()
			runOnTarget("Burn", function(p) applyStatusToPlayer("fire", p) end)
		end })
		makeButton(sc, { order = n(), title = _Vzd({117,148,142,152,148,147,69,121,141,138,146}), danger = true, tip = _Vzd({102,149,149,145,158,69,149,148,142,152,148,147,69,155,142,134,69,141,154,151,153,69,149,134,151,153,152}), callback = function()
			runOnTarget("Poison", function(p) local r = rootOf(p); if r then applyMapPoison(r) end end)
		end })
		makeButton(sc, { order = n(), title = "Freeze Them", tip = _Vzd({120,138,153,69,124,134,145,144,120,149,138,138,137,69,80,69,111,154,146,149,117,148,156,138,151,69,153,148,69,85}), callback = function()
			runOnTarget("Freeze", function(p)
				local h = p.Character and p.Character:FindFirstChildOfClass("Humanoid")
				if h then h.WalkSpeed = 0; h.JumpPower = 0; h.JumpHeight = 0 end
			end)
		end })
		makeButton(sc, { order = n(), title = _Vzd({122,147,139,151,138,138,159,138,69,121,141,138,146}), tip = "Restore WalkSpeed 16 + JumpPower 50", callback = function()
...
_TAB_BUILDERS[_Vzd({135,145,148,135,146,134,147})] = function(sc, n)
		...
		makeButton(sc, {
			...
			callback = function()
				spawnToyBurst(_Vzd({104,151,138,134,153,154,151,138,103,145,148,135,146,134,147}), 3)
			end,
		})
		makeButton(sc, {
			...
			callback = function()
				...
				releaseBlobmanSeatAfterFeaturesOff(true)
				for _, id in ipairs({ "blobGrabLoop", "blobGrabAllLoop", "blobExtractPlotsLoop", "blobKickLoop", _Vzd({137,138,152,153,151,148,158,120,138,151,155,138,151}), "blobControlOn" }) do
		...
		section(sc, "GRAB", n())
		makeButton(sc, {
			order = n(), title = _Vzd({103,145,148,135,69,108,151,134,135,69,120,138,145,138,136,153,138,137,69,77,148,147,136,138,78}),
			danger = true,
			...
			callback = function()
				...
				notify(HUB_NAME, _Vzd({103,145,148,135,69,108,151,134,135,69,82,99,69}) .. playerLabel(p), 1.2)
		...
		makeToggle(sc, {
			order = n(), id = _Vzd({135,145,148,135,108,151,134,135,113,148,148,149}), title = _Vzd({113,148,148,149,69,108,151,134,135,69,120,138,145,138,136,153,138,137}),
			tip = "Sit Blobman, pick a player, loop CreatureGrab. Parks only if far so you stay seated.",
			callback = function(on)
				local p = blobTarget() or S.selected or S.loopTarget
		...
		makeToggle(sc, {
			order = n(), id = "blobGrabAllLoop", title = "Loop Grab All",
			tip = _Vzd({103,145,148,135,82,140,151,134,135,69,138,155,138,151,158,148,147,138,69,154,147,153,142,145,69,116,107,107,83}),
			callback = function(on)
				local ok = setBlobGrabAllLoop(on)
				if on and not ok then
					S.toggles.blobGrabAllLoop = false
					if S._toggleRenderers and S._toggleRenderers.blobGrabAllLoop then
						pcall(S._toggleRenderers.blobGrabAllLoop)
					end
				end
			end,
		})
		makeButton(sc, {
			...
			danger = true,
			tip = "One pass while mounted (kit fire)",
			callback = function()
		...
		makeButton(sc, {
			...
			danger = true,
			tip = _Vzd({116,147,138,82,152,141,148,153,69,135,145,148,135,69,140,151,134,135,69,152,138,145,138,136,153,138,137,69,142,147,69,149,145,148,153}),
			callback = function()
				local p = blobTarget()
				if not p then notify(HUB_NAME, _Vzd({117,142,136,144,69,134,69,149,145,134,158,138,151,69,142,147,69,153,141,138,69,145,142,152,153,69,134,135,148,155,138}), 1.5); return end
		...
		makeToggle(sc, {
			order = n(), id = "blobExtractPlotsLoop", title = _Vzd({113,148,148,149,69,106,157,153,151,134,136,153,69,102,145,145,69,110,147,69,117,145,148,153,152}),
			tip = _Vzd({112,138,138,149,69,138,157,153,151,134,136,153,142,147,140,69,138,155,138,151,158,148,147,138,69,142,147,69,149,145,148,153,152,69,154,147,153,142,145,69,116,107,107}),
			callback = function(on)
		...
		makeToggle(sc, {
			...
			tip = "Sit blob → grab all → fling.",
			callback = function(on)
				if on and not requirePremium(_Vzd({105,138,152,153,151,148,158,69,120,138,151,155,138,151})) then
					S.toggles.destroyServer = false
					S.toggles.blobDestroyServer = false
					if S._toggleRenderers and S._toggleRenderers.destroyServer then
						pcall(S._toggleRenderers.destroyServer)
					end
					return
				end
				S.toggles.blobDestroyServer = on
				if on then
					setMassToggle(_Vzd({137,138,152,153,151,148,158,120,151,155}), true, destroyServerLoop)
				else
					stopMass("destroySrv")
					S.toggles.destroyServer = false
				end
		...
		makeToggle(sc, {
			...
			tip = _Vzd({112,138,138,149,69,144,142,136,144,142,147,140,69,152,138,145,138,136,153,138,137,69,156,142,153,141,69,103,145,148,135,146,134,147,69,144,142,136,144,69,154,147,153,142,145,69,116,107,107}),
			callback = function(on)
				...
				startLoop("blobKickLoop", 0.6, function()
					...
					if t and t.Parent then kickPlayer(t, _Vzd({103,145,148,135,146,134,147}), true) end
				end)
			end,
		})
		makeButton(sc, {
			...
			callback = function()
				...
				if controlState and controlState.running then stopControl(true) end
				releaseBlobmanSeatAfterFeaturesOff(true)
				for _, id in ipairs({ "blobGrabLoop", "blobGrabAllLoop", "blobExtractPlotsLoop", "blobKickLoop", "destroyServer", _Vzd({135,145,148,135,104,148,147,153,151,148,145,116,147}) }) do
					if S._toggleRenderers and S._toggleRenderers[id] then
						pcall(S._toggleRenderers[id])
					end
				end
		...
		makeToggle(sc, {
			order = n(), id = "antiBlobman", title = _Vzd({102,147,153,142,69,103,145,148,135,146,134,147,69,84,69,121,151,134,142,147,69,120,138,134,153}),
			...
			callback = function(on)
				...
				notify(HUB_NAME, _Vzd({102,147,153,142,69,135,145,148,135,84,153,151,134,142,147,69,152,138,134,153,69}) .. (on and "ON" or "OFF"), 1.2)
			end,
		})
end
_TAB_BUILDERS["auras"] = function(sc, n)
		...
		makeToggle(sc, {
			...
			callback = function(on)
				...
				notify(HUB_NAME, _Vzd({102,154,151,134,69,151,134,147,140,138,69}) .. (on and "extended" or _Vzd({147,138,134,151,135,158,69,148,147,145,158})), 1.5)
			end,
		})
		makeSlider(sc, {
			order = n(),
...
_TAB_BUILDERS["server"] = function(sc, n)
		...
		makeToggle(sc, {
			order = n(), id = _Vzd({145,134,140,120,138,151,155,138,151}), title = _Vzd({113,134,140,69,120,138,151,155,138,151}),
		...
		makeButton(sc, {
			...
			callback = function()
				for _, name in ipairs({ _Vzd({145,134,140,120,151,155}), "softLag", _Vzd({141,134,151,137,113,134,140}), "destroySrv", _Vzd({137,138,152,153,151,148,158,109,158,135}), "blobSrv" }) do
					stopMass(name)
				end
		...
		makeSlider(sc, {
			...
			stateKey = _Vzd({146,134,152,152,104,158,136,145,138,105,138,145,134,158}),
		...
		makeToggle(sc, {
			order = n(), id = _Vzd({146,134,152,152,132,139,145,142,147,140}), title = _Vzd({113,148,148,149,69,121,141,151,148,156,69,102,145,145}), danger = true,
		...
		makeToggle(sc, {
			order = n(), id = _Vzd({146,134,152,152,132,135,151,142,147,140}), title = _Vzd({113,148,148,149,69,103,151,142,147,140,69,102,145,145}),
		...
		makeToggle(sc, {
			order = n(), id = _Vzd({146,134,152,152,132,139,142,151,138}), title = _Vzd({113,148,148,149,69,103,154,151,147,69,102,145,145}), danger = true,
		...
		section(sc, _Vzd({113,116,116,117,69,123,116,114,110,121,69,77,114,102,117,82,124,110,105,106,78}), n())
								...
								if validP(p) and p.Character then
									...
									if r then
										pcall(function()
											...
											local ao = bananaPrimary:FindFirstChildOfClass(_Vzd({102,145,142,140,147,116,151,142,138,147,153,134,153,142,148,147}))
											if ao then ao.Enabled = false end
											local head = LP.Character and LP.Character:FindFirstChild("Head")
											local parkY = head and (head.Position.Y + 500) or 500
		...
		makeButton(sc, {
			...
			callback = function()
				for _, name in ipairs({ "lagSrv", "softLag", _Vzd({141,134,151,137,113,134,140}), _Vzd({137,138,152,153,151,148,158,120,151,155}), "destroyHyb", _Vzd({135,145,148,135,120,151,155}), "kill", "fling", "kick", _Vzd({135,151,142,147,140}), "ragdoll", "fire", "banana", "paint", "vomit" }) do
					stopMass(name)
				end
				S.toggles.lagServer = false
				S.toggles.destroyServer = false
...
_TAB_BUILDERS["grab"] = function(sc, n)
		...
		makeToggle(sc, {
			order = n(), id = _Vzd({152,149,142,147,124,141,142,145,138,109,148,145,137}), title = _Vzd({120,149,142,147,69,124,141,142,145,138,69,109,148,145,137,142,147,140}),
		...
		makeSlider(sc, {
			order = n(), title = "Zero-G Force", min = 0, max = 100000, default = 50000, step = 1000, stateKey = _Vzd({140,151,134,135,127,138,151,148,108,107,148,151,136,138}),
		})
		makeToggle(sc, {
		...
		makeToggle(sc, {
			order = n(), id = "grabFollowOn", title = _Vzd({110,153,138,146,69,107,148,145,145,148,156,69,107,138,138,153}),
		...
		makeToggle(sc, {
			order = n(), id = _Vzd({152,154,149,138,151,120,153,151}), title = _Vzd({120,154,149,138,151,69,121,141,151,148,156,69,116,147,69,119,138,145,138,134,152,138}),
		...
		makeToggle(sc, {
			...
			callback = function(on)
				...
				if on then
					installGrabWatch()
					for _, ch in ipairs(workspace:GetChildren()) do
						if ch.Name == _Vzd({108,151,134,135,117,134,151,153,152}) then onGrabPartsAdded(ch) end
					end
				end
			end,
		})
		makeToggle(sc, {
			order = n(), id = _Vzd({147,148,136,145,142,149,108,151,134,135}), title = _Vzd({109,148,145,137,69,121,141,151,148,154,140,141,69,124,134,145,145,152}),
		...
		makeToggle(sc, {
			order = n(), id = _Vzd({144,142,145,145,108,151,134,135}), title = _Vzd({112,142,145,145,69,124,141,148,69,126,148,154,69,109,148,145,137}),
		...
		makeToggle(sc, {
			order = n(), id = _Vzd({151,134,140,137,148,145,145,108,151,134,135}), title = _Vzd({119,134,140,137,148,145,145,69,108,151,134,135}),
		...
		makeToggle(sc, {
			...
			tip = _Vzd({104,134,146,149,139,142,151,138,69,139,142,151,138,69,153,148,154,136,141,69,156,141,142,145,138,69,141,148,145,137,142,147,140}),
		...
		makeToggle(sc, {
			order = n(), id = "radioactiveGrab", title = _Vzd({119,134,137,142,148,134,136,153,142,155,138,69,108,151,134,135}),
		...
		makeToggle(sc, {
			...
			callback = function(on)
				if on then setMassToggle(_Vzd({139,145,142,147,140,116,135,143}), true, massFlingObjectsLoop) else stopMass(_Vzd({139,145,142,147,140,116,135,143})) end
			end,
		})
		makeButton(sc, {
		...
		makeToggle(sc, { order = n(), id = _Vzd({134,154,153,148,108,151,134,135,115,138,134,151,138,152,153}), title = "Auto-Grab Closest", tip = "CreateGrabLine + SNO nearest", callback = function(on)
...
_TAB_BUILDERS["anti"] = function(sc, n)
		...
		makeToggle(sc, {
			order = n(), id = "antiGucci", title = "Gucci (Invisible)",
			tip = "Ungrabbable + server invis. You can still grab and jump.",
			desc = "Invis / ungrabbable. Grab and jump still work.",
			callback = function(on)
				S.toggles.antiGucci = on == true
				S.antiWanted = S.antiWanted or {}
				S.antiWanted.antiGucci = on == true
				installAntis()
				stopLoop(_Vzd({140,154,136,136,142,107,151,138,138}))
				if on then
					pcall(resolveFTAP)
					S._gucciQueryOff = false
					gucciApplyNotGrabbableAlways()
					S._gucciQueryOff = true
					if S._gucciHB then pcall(function() S._gucciHB:Disconnect() end) S._gucciHB = nil end
					local ok, err = pcall(startBloodyTrainGucci)
					if not ok then
						warn(_Vzd({128,123,116,110,105,127,130,69,140,154,136,136,142}), err)
						notify(HUB_NAME, "Gucci sit error — warped back if we could", 2)
					else
						notify(HUB_NAME, _Vzd({108,154,136,136,142,69,116,115,69,161,69,142,147,155,142,152,142,135,145,138,69,80,69,154,147,140,151,134,135,135,134,135,145,138}), 1.8)
					end
				else
					if S._gucciHB then pcall(function() S._gucciHB:Disconnect() end) S._gucciHB = nil end
					S._gucciQueryOff = false
					stopBloodyTrainGucci()
					notify(HUB_NAME, "Gucci OFF", 1.2)
				end
			end,
		})
		makeToggle(sc, {
			order = n(), id = "antiGrab", title = "Anti-Grab (Walk Held)",
			tip = _Vzd({124,134,145,144,69,156,141,142,145,138,69,140,151,134,135,135,138,137,83,69,120,149,134,146,69,140,151,134,135,152,69,136,134,147,76,153,69,137,151,134,140,69,158,148,154,83}),
			desc = _Vzd({124,134,145,144,69,139,151,138,138,69,156,141,138,147,69,153,141,138,158,69,140,151,134,135,69,158,148,154,83}),
			callback = function(on)
				S.antiWanted = S.antiWanted or {}
				...
				stopLoop("antiGrab")
				installAntis()
				notify(HUB_NAME, _Vzd({102,147,153,142,82,108,151,134,135,69}) .. (on and "ON (walk)" or "OFF"), 1.5)
				if on then
					pcall(resolveFTAP)
					pcall(gucciApplyNotGrabbableAlways)
					if FTAP.Struggle then pcall(function() FTAP.Struggle:FireServer(LP) end) end
					startLoop("antiGrab", 0.05, function()
						if not S.toggles.antiGrab then return end
						if antiGrabTick then antiGrabTick() end
					end)
					if isLocalBeingHeldFlag() then walkWhileGrabbedTick() end
				else
					local r = hrp()
					if r then
						pcall(function()
		...
		makeButton(sc, {
			order = n(),
			title = _Vzd({107,148,151,136,138,69,102,147,153,142,82,108,151,134,135,69,103,154,151,152,153}),
			tip = _Vzd({116,147,138,82,152,141,148,153,69,156,134,145,144,69,80,69,120,153,151,154,140,140,145,138,69,80,69,119,134,140,137,148,145,145,85}),
			callback = function()
				installAntis()
		...
		section(sc, "COUNTER", n())
		makeToggle(sc, {
			order = n(), id = "autoCounter", title = _Vzd({102,154,153,148,69,102,153,153,134,136,144,138,151}),
			tip = "Instantly counter whoever grabs you (PartOwner + GrabParts). Repeats through spam grabs.",
			callback = function(on)
				S.autoCounter = on
				...
				if on then
					...
					notify(HUB_NAME, "Auto Attacker ON · " .. tostring(S.counterMode or _Vzd({119,138,149,154,145,152,142,148,147})), 1.8)
				else
					S._autoAttackBusy = false
					notify(HUB_NAME, _Vzd({102,154,153,148,69,102,153,153,134,136,144,138,151,69,116,107,107}), 1.2)
				end
			end,
		})
		makeDropdown(sc, {
			order = n(), title = _Vzd({102,153,153,134,136,144,69,114,148,137,138}),
			options = { "Repulsion", _Vzd({107,145,142,147,140}), "Kick", "Sky", "Freeze", _Vzd({105,138,134,153,141}), "Void", "Ragdoll", "Bring" },
			default = S.counterMode or _Vzd({119,138,149,154,145,152,142,148,147}),
			callback = function(v)
				S.counterMode = v
		...
		makeSlider(sc, {
			order = n(), title = "Attack Force", min = 2000, max = 100000, default = 12000, step = 1000,
			stateKey = "revengeForce",
			tip = _Vzd({107,148,151,136,138,69,139,148,151,69,119,138,149,154,145,152,142,148,147,69,84,69,107,145,142,147,140,69,84,69,120,144,158,83,69,112,142,136,144,69,154,152,138,152,69,112,142,136,144,69,121,158,149,138,69,137,151,148,149,137,148,156,147,83}),
		})
		section(sc, "STATUS", n())
		makeToggle(sc, {
			...
			callback = function(on)
				...
				stopLoop(_Vzd({134,147,153,142,103,154,151,147}))
				installAntis()
		...
		makeToggle(sc, {
			...
			callback = function(on)
				...
				stopLoop(_Vzd({134,147,153,142,117,134,142,147,153}))
				if on then startLoop(_Vzd({134,147,153,142,117,134,142,147,153}), 0.15, antiPaintTick) end
			end,
		})
		makeToggle(sc, {
			order = n(), id = "antiBanana", title = _Vzd({102,147,153,142,82,103,134,147,134,147,134,69,84,69,120,145,142,149}),
			tip = "Disables peels + removes FEET/legs (Motor6D detach) when slipped so sit cannot stick.",
			callback = function(on)
				S.toggles.antiBanana = on == true
				stopLoop("antiBanana")
				if on then startLoop(_Vzd({134,147,153,142,103,134,147,134,147,134}), 0.08, antiBananaTick) end
			end,
		})
		makeToggle(sc, { order = n(), id = _Vzd({134,147,153,142,123,148,142,137}), title = _Vzd({102,147,153,142,82,123,148,142,137}), tip = "Rescue if you fall too low", callback = function(on)
			stopLoop("antiVoid")
			if on then
		...
		makeToggle(sc, { order = n(), id = "antiSit", title = _Vzd({102,147,153,142,82,120,142,153,69,84,69,120,138,134,153,69,121,151,134,149}), tip = "Force unsit (skips Blobman and Gucci train occupancy)", callback = function(on)
			stopLoop("antiSit")
			if on then startLoop("antiSit", 0.1, function()
				if gucciProtectOn and gucciProtectOn() then return end
				if S._gucciSeatBreaking then return end
		...
		makeToggle(sc, { order = n(), id = _Vzd({134,147,153,142,119,134,140,137,148,145,145}), title = "Anti-Ragdoll", tip = _Vzd({105,142,152,134,135,145,138,69,151,134,140,137,148,145,145,69,152,153,134,153,138,152}), callback = function(on)
			...
			if on then startLoop(_Vzd({134,147,153,142,119,134,140}), 0.1, function()
		...
		makeToggle(sc, {
			...
			tip = _Vzd({121,141,151,138,134,153,69,107,106,69,149,151,148,153,138,136,153,69,80,69,141,138,134,145,83,69,104,141,134,153,95,69,84,156,134,151,82,146,148,137,138}),
			danger = true,
			callback = function(on)
				if on then
					if not requirePremium(_Vzd({124,102,119,69,114,116,105,106})) then
						S.toggles.warMode = false
						if S._toggleRenderers and S._toggleRenderers.warMode then pcall(S._toggleRenderers.warMode) end
						return
					end
					startWarMode()
				else
				...
				S.toggles.warMode = on == true and isPremiumTier()
				if S._toggleRenderers and S._toggleRenderers.warMode then
					pcall(S._toggleRenderers.warMode)
				end
			end,
		...
		makeButton(sc, {
			...
			danger = true,
			tip = "One-shot FE protect + house escape.",
			callback = function()
				if not requirePremium("WAR BURST") then return end
				if not warModeOn() then startWarMode() end
				voidzWarProtectBurst()
				warHouseEscapeFE("burst-btn")
				notify(HUB_NAME, _Vzd({124,102,119,69,103,122,119,120,121,69,139,142,151,138,137}), 1.1)
			end,
		})
end
_TAB_BUILDERS["player"] = function(sc, n)
		...
		makeToggle(sc, {
			order = n(), id = _Vzd({140,148,137,114,148,137,138}), title = _Vzd({108,148,137,69,114,148,137,138}),
		...
		makeToggle(sc, {
			...
			callback = function(on)
				...
				stopLoop(_Vzd({134,154,153,148,109,138,134,145}))
		...
		makeToggle(sc, {
			order = n(), id = "bigTorso", title = _Vzd({103,142,140,69,121,148,151,152,148}),
		...
		makeToggle(sc, {
			order = n(), id = _Vzd({149,145,134,158,138,151,107,145,148,134,153}), title = _Vzd({107,145,148,134,153,69,84,69,109,148,155,138,151}),
		...
		makeButton(sc, {
			...
			callback = function()
				...
				if S.conns.infJump then pcall(function() S.conns.infJump:Disconnect() end) S.conns.infJump = nil end
				for _, id in ipairs({"god","autoHeal",_Vzd({149,145,134,158,138,151,120,149,142,147}),_Vzd({134,147,153,142,105,151,148,156,147}),"highJump",_Vzd({152,154,149,138,151,120,149,138,138,137}),"noclip"}) do stopLoop(id) end
				local h = hum()
				if h then
					h.WalkSpeed = 16
					h.JumpPower = 50
...
_TAB_BUILDERS["loop"] = function(sc, n)
		...
		makeButton(sc, { order = n(), title = _Vzd({107,145,142,147,140,69,120,138,145,138,136,153,138,137}), danger = true, tip = "SNO + velocity fling", callback = function()
		...
		makeToggle(sc, { order = n(), id = "wlFriends", title = _Vzd({124,141,142,153,138,145,142,152,153,69,107,151,142,138,147,137,152}), tip = _Vzd({120,144,142,149,69,139,151,142,138,147,137,152,69,142,147,69,134,154,151,134,152,84,146,134,152,152}), callback = function(on)
		...
		local loops = {
			{ id = "loopFling", title = _Vzd({112,138,138,149,69,121,141,151,148,156,142,147,140}), tip = _Vzd({102,147,147,148,158,142,147,140,69,136,148,147,152,153,134,147,153,69,139,145,142,147,140,69,161,69,146,134,149,82,156,142,137,138}), waitRespawn = true, fn = function(p)
				blitzLoopHit(p, "fling", S.flingPower or 8000)
			end },
			{ id = "loopKick", title = _Vzd({112,138,138,149,69,112,142,136,144,142,147,140}), tip = _Vzd({112,142,136,144,69,161,69,146,134,149,82,156,142,137,138}), waitRespawn = true, fn = function(p) blitzLoopHit(p, "kick", S.kickType) end },
			...
			{ id = _Vzd({145,148,148,149,119,134,140,137,148,145,145}), title = _Vzd({112,138,138,149,69,119,134,140,137,148,145,145,142,147,140}), tip = _Vzd({120,149,134,146,69,151,134,140,137,148,145,145,69,161,69,146,134,149,82,156,142,137,138}), waitRespawn = true, fn = function(p) blitzLoopHit(p, "ragdoll") end },
			{ id = _Vzd({145,148,148,149,103,151,142,147,140}), title = _Vzd({112,138,138,149,69,103,151,142,147,140,142,147,140}), tip = _Vzd({103,151,142,147,140,69,153,148,69,158,148,154,69,161,69,146,134,149,82,156,142,137,138}), fn = function(p)
				local home = S.loopWait.loopBring and S.loopWait.loopBring.home
				blitzLoopHit(p, _Vzd({135,151,142,147,140}), home)
			end },
			{ id = "loopTp", title = _Vzd({113,148,148,149,69,121,138,145,138,149,148,151,153,69,121,148}), tip = _Vzd({120,153,134,158,69,148,147,69,153,141,138,146}), stay = true, fn = function(p) blitzLoopHit(p, "tp") end },
			{ id = "loopSky", title = "Loop Sky Launch", tip = _Vzd({120,144,158,69,145,134,154,147,136,141,69,161,69,146,134,149,82,156,142,137,138}), waitRespawn = true, fn = function(p) blitzLoopHit(p, "sky") end },
			{ id = _Vzd({145,148,148,149,123,148,142,137}), title = _Vzd({113,148,148,149,69,123,148,142,137}), tip = _Vzd({123,148,142,137,69,152,145,134,146,69,161,69,146,134,149,82,156,142,137,138}), waitRespawn = true, fn = function(p) blitzLoopHit(p, "void") end },
			{ id = _Vzd({145,148,148,149,120,149,142,147}), title = _Vzd({113,148,148,149,69,120,149,142,147}), tip = _Vzd({120,149,142,147,69,153,141,138,146,69,161,69,146,134,149,82,156,142,137,138}), waitRespawn = true, fn = function(p) blitzLoopHit(p, "spin") end },
			{ id = _Vzd({145,148,148,149,120,115,116}), title = "Loop Network Own", tip = _Vzd({120,115,116,69,152,149,134,146,69,161,69,146,134,149,82,156,142,137,138}), fn = function(p) blitzLoopHit(p, "sno") end },
			{ id = _Vzd({145,148,148,149,108,151,134,135}), title = _Vzd({113,148,148,149,69,108,151,134,135,69,113,142,147,138}), tip = _Vzd({104,151,138,134,153,138,108,151,134,135,113,142,147,138,69,152,149,134,146,69,161,69,146,134,149,82,156,142,137,138}), fn = function(p) blitzLoopHit(p, "grab") end },
			{ id = _Vzd({145,148,148,149,109,134,151,137,107,145,142,147,140}), title = _Vzd({113,148,148,149,69,109,134,151,137,69,107,145,142,147,140}), tip = "Max fling | map-wide", waitRespawn = true, fn = function(p)
				blitzLoopHit(p, "hardfling", 20000)
			end },
			{ id = "loopBlobKick", title = _Vzd({113,148,148,149,69,103,145,148,135,146,134,147,69,112,142,136,144}), tip = _Vzd({103,145,148,135,69,144,142,136,144,69,161,69,146,134,149,82,156,142,137,138}), waitRespawn = true, fn = function(p) blitzLoopHit(p, "blobkick") end },
			{ id = _Vzd({145,148,148,149,108,151,134,135,112,142,136,144}), title = _Vzd({113,148,148,149,69,108,151,134,135,69,112,142,136,144}), tip = _Vzd({108,151,134,135,69,144,142,136,144,69,161,69,146,134,149,82,156,142,137,138}), waitRespawn = true, fn = function(p) blitzLoopHit(p, "grabkick") end },
			{ id = "loopStackKick", title = _Vzd({113,148,148,149,69,120,153,134,136,144,69,112,142,136,144}), tip = _Vzd({120,153,134,136,144,69,144,142,136,144,69,161,69,146,134,149,82,156,142,137,138}), waitRespawn = true, fn = function(p) blitzLoopHit(p, "stackkick") end },
			{ id = "loopSilentKick", title = _Vzd({113,148,148,149,69,120,142,145,138,147,153,69,112,142,136,144}), tip = _Vzd({120,142,145,138,147,153,69,144,142,136,144,69,161,69,146,134,149,82,156,142,137,138}), waitRespawn = true, fn = function(p) blitzLoopHit(p, "silent") end },
			{ id = _Vzd({145,148,148,149,107,142,151,138}), title = _Vzd({113,148,148,149,69,107,142,151,138}), tip = "Burn them | map-wide", waitRespawn = true, fn = function(p) blitzLoopHit(p, "fire") end },
			{ id = "loopPoison", title = _Vzd({113,148,148,149,69,117,148,142,152,148,147}), tip = _Vzd({117,148,142,152,148,147,69,161,69,146,134,149,82,156,142,137,138}), fn = function(p) blitzLoopHit(p, "poison") end },
			{ id = _Vzd({145,148,148,149,103,134,147,134,147,134}), title = _Vzd({113,148,148,149,69,103,134,147,134,147,134}), tip = _Vzd({120,145,142,149,69,161,69,146,134,149,82,156,142,137,138}), fn = function(p) blitzLoopHit(p, "banana") end },
			{ id = _Vzd({145,148,148,149,117,134,142,147,153}), title = _Vzd({113,148,148,149,69,117,134,142,147,153}), tip = _Vzd({117,134,142,147,153,69,161,69,146,134,149,82,156,142,137,138}), fn = function(p) blitzLoopHit(p, "paint") end },
			{ id = "loopBringFling", title = _Vzd({113,148,148,149,69,103,151,142,147,140,80,107,145,142,147,140}), tip = _Vzd({103,151,142,147,140,69,153,141,138,147,69,139,145,142,147,140,69,161,69,146,134,149,82,156,142,137,138}), waitRespawn = true, fn = function(p)
				local home = S.loopWait.loopBringFling and S.loopWait.loopBringFling.home
				blitzLoopHit(p, "bringfling", home)
			end },
			{ id = _Vzd({145,148,148,149,113,148,147,140,103,151,142,147,140}), title = _Vzd({113,148,148,149,69,113,148,147,140,69,119,138,134,136,141,69,103,151,142,147,140}), tip = _Vzd({114,134,157,69,151,138,134,136,141,69,80,69,135,151,142,147,140}), fn = function(p)
				if S.toggles.lineExtend then applyLineExtendDistance(S.extendAmount or 80) end
				local home = S.loopWait[_Vzd({145,148,148,149,113,148,147,140,103,151,142,147,140})] and S.loopWait[_Vzd({145,148,148,149,113,148,147,140,103,151,142,147,140})].home
				blitzLoopHit(p, _Vzd({145,148,147,140,135,151,142,147,140}), home)
			end },
			{ id = _Vzd({145,148,148,149,120,149,134,146,120,115,116}), title = "Loop Spam SNO Parts", tip = "Own every part | map-wide", fn = function(p) blitzLoopHit(p, "spamsno") end },
			{ id = "loopDestroyGrab", title = "Loop Destroy Their Grab", tip = _Vzd({105,138,152,153,151,148,158,108,151,134,135,113,142,147,138,69,161,69,146,134,149,82,156,142,137,138}), fn = function(p) blitzLoopHit(p, "destroygrab") end },
		}
		...
		for _, L in ipairs(loops) do
			makeToggle(sc, {
				...
				callback = function(on)
					...
					if not on then
						kickNoclipSelf(false)
						if S._activeLoopId == L.id then S._activeLoopId = nil end
				...
				startLoop(L.id, interval, function()
					...
					kickNoclipSelf(true)
					for _, p in ipairs(targets) do
					if not S.loops[L.id] then break end
					if not p or not p.Parent then continue end
					pcall(armLoopTargetRespawn, p)
					...
					local spawned = S._loopSpawnAt and S._loopSpawnAt[p.Name]
					if spawned and (os.clock() - spawned) < 0.45 then
						continue
					end
					if spawned and w.waiting then
						w.waiting = false
						task.wait(0.2)
					end
					w.waiting = false

					-- Kill/fling loops must still hit after they respawn in a house.
					if not L.waitRespawn and not plotBypass and isInSafePlot(p) then
					...
					if not isAliveP(p) then continue end
					pcall(L.fn, p)
					end
					kickNoclipSelf(false)
					if not L.stay then
						local home = (S.loopWait[L.id] and S.loopWait[L.id].home) or homeCF
						if home then pcall(function() teleportSelf(home) end) end
					end
				end)
				end,
			})
...
_TAB_BUILDERS["move"] = function(sc, n)
		...
		makeToggle(sc, { order = n(), id = "speed", title = _Vzd({124,134,145,144,120,149,138,138,137,69,116,155,138,151,151,142,137,138}), tip = _Vzd({119,138,82,134,149,149,145,142,138,152,69,138,155,138,151,158,69,139,151,134,146,138,69,77,107,121,102,117,69,151,138,152,138,153,152,69,152,149,138,138,137,78}), callback = function() end })
		makeSlider(sc, { order = n(), title = "WalkSpeed", min = 16, max = 300, default = 50, stateKey = _Vzd({156,134,145,144,120,149,138,138,137}) })
		makeToggle(sc, { order = n(), id = _Vzd({152,149,138,138,137,104,107,151,134,146,138}), title = _Vzd({104,107,151,134,146,138,69,120,149,138,138,137,69,103,148,148,152,153}), tip = _Vzd({106,157,153,151,134,69,104,107,151,134,146,138,69,149,154,152,141,69,145,142,144,138,69,146,154,145,153}), callback = function() end })
		...
		local MAP_POSITIONS = {
			...
			{ name = _Vzd({108,151,138,138,147,69,120,134,139,138,82,109,148,154,152,138}), pos = Vector3.new(-584, -6, 93) },
			{ name = _Vzd({104,141,142,147,138,152,138,69,120,134,139,138,82,109,148,154,152,138}), pos = Vector3.new(579, 124, -94) },
		...
		makeToggle(sc, {
			order = n(), id = _Vzd({145,148,148,149,121,117}), title = _Vzd({113,148,148,149,69,121,138,145,138,149,148,151,153,69,153,148,69,120,138,145,138,136,153,138,137}),
...
_TAB_BUILDERS["toys"] = function(sc, n)
		...
		section(sc, _Vzd({120,117,102,124,115,69,112,106,126,103,110,115,105,120}), n())
		makeToggle(sc, { order = n(), id = _Vzd({149,134,145,145,138,153,118}), title = _Vzd({118,69,98,69,120,149,134,156,147,69,117,134,145,145,138,153}), tip = _Vzd({117,134,145,145,138,153,113,142,140,141,153,103,151,148,156,147,69,155,142,134,69,114,138,147,154,121,148,158,152}), desc = _Vzd({121,148,140,140,145,138,69,148,147,84,148,139,139}), callback = setPalletQ })
		makeToggle(sc, { order = n(), id = _Vzd({153,134,135,120,149,134,156,147}), title = _Vzd({121,102,103,69,98,69,120,149,134,156,147,69,120,138,145,138,136,153,138,137,69,121,148,158}), tip = _Vzd({120,149,134,156,147,69,152,138,145,138,136,153,138,137,69,153,148,158,69,148,147,69,121,102,103}), callback = function(on)
			pcall(function() ContextActionService:UnbindAction("VOIDZ_TabToy") end)
			if not on then return end
		...
		for _, toy in ipairs({
			_Vzd({117,134,145,145,138,153,113,142,140,141,153,103,151,148,156,147}), "CreatureBlobman", "BombMissile", "Campfire", "NinjaKunai",
			"NinjaShuriken", "FoodBanana", "DiceSmall", "SprayCanWD", "BallSnowball",
			_Vzd({126,148,154,105,138,136,148,158}), _Vzd({108,145,134,152,152,103,148,157,108,151,134,158}), _Vzd({107,148,148,137,103,151,138,134,137}), "InstrumentDrumSnare",
		}) do
		...
		local formInfo = Instance.new(_Vzd({121,138,157,153,113,134,135,138,145}))
		...
		makeToggle(sc, { order = n(), id = "autoPallet", title = _Vzd({102,154,153,148,69,117,134,145,145,138,153,69,117,134,153,141}), tip = _Vzd({118,154,138,154,138,137,69,149,134,145,145,138,153,152,69,156,141,142,145,138,69,156,134,145,144,142,147,140}), callback = function(on)
			stopLoop("autoPallet")
		...
		makeButton(sc, { order = n(), title = "Destroy All My Toys", danger = true, tip = _Vzd({105,138,152,153,151,148,158,121,148,158,69,148,147,69,120,149,134,156,147,138,137,110,147,121,148,158,152,69,139,148,145,137,138,151}), callback = function()
			...
			notify(HUB_NAME, _Vzd({105,138,152,153,151,148,158,138,137,69}) .. n .. _Vzd({69,153,148,158,152}), 2)
		end })
		...
		makeButton(sc, { order = n(), title = "Count My Toys", tip = _Vzd({120,149,134,156,147,138,137,110,147,121,148,158,152,69,136,141,142,145,137,151,138,147}), callback = function()
			notify(HUB_NAME, _Vzd({121,148,158,152,95,69}) .. countMyToys() .. _Vzd({69,161,69,117,134,145,145,138,153,152,95,69}) .. countMyToys(_Vzd({117,134,145,145,138,153,113,142,140,141,153,103,151,148,156,147})), 2)
		end })
		...
		unInfo.Text = _Vzd({69,69,104,145,142,136,144,69,134,69,147,134,146,138,69,82,99,69,151,138,134,145,69,120,115,116,69,135,151,142,147,140,69,161,69,141,138,145,137,69,142,147,69,139,151,148,147,153,69,148,139,69,158,148,154,69,154,147,153,142,145,69,158,148,154,69,140,151,134,135,69,142,153})
		unInfo.Parent = sc
		corner(unInfo, 8)
		pad(unInfo, 6, 6, 6, 6)
		makeButton(sc, {
		...
		unSc.Parent = unBox
		local ul = Instance.new(_Vzd({122,110,113,142,152,153,113,134,158,148,154,153})); ul.Padding = UDim.new(0, 3); ul.Parent = unSc
		pad(unSc, 4, 4, 4, 4)
		S.unownedList = unSc
end
_TAB_BUILDERS["explosions"] = function(sc, n)
		...
		exNote.Text = "  FAST: spawn → hop onto player → BombExplode (no long park wait)\n  Auto strike spams as fast as remotes allow · toy limit 100/200"
		exNote.Parent = sc
		corner(exNote, 8)
		pad(exNote, 6, 6, 6, 6)
		...
		makeDropdown(sc, {
			...
			options = { "BombMissile", _Vzd({107,142,151,138,156,148,151,144,114,142,152,152,142,145,138}), "BombBalloon", "BombDarkMatter" },
		...
		makeButton(sc, {
			...
			danger = true,
			tip = _Vzd({120,149,134,156,147,69,115,69,146,142,152,152,142,145,138,152,69,134,147,137,69,138,157,149,145,148,137,138,69,153,141,138,146,69,148,147,69,153,141,138,69,153,134,151,140,138,153,69,147,148,156}),
		...
		makeButton(sc, {
			...
			callback = function()
				local n = 0
				for _, name in ipairs({ "BombMissile", _Vzd({107,142,151,138,156,148,151,144,114,142,152,152,142,145,138}), "BombBalloon", "BombDarkMatter" }) do
...
_TAB_BUILDERS[_Vzd({156,148,151,145,137})] = function(sc, n)
		...
		makeToggle(sc, { order = n(), id = _Vzd({134,154,151,134,132,147,138,153,148,156,147}), title = _Vzd({115,138,153,156,148,151,144,69,116,156,147,138,151,152,141,142,149,69,102,154,151,134}), tip = _Vzd({116,117,69,136,148,147,153,142,147,154,148,154,152,69,120,115,116}), callback = function(on) setAura("netown", on) end })
		makeToggle(sc, { order = n(), id = "aura_fling", title = _Vzd({116,135,143,138,136,153,84,117,145,134,158,138,151,69,107,145,142,147,140,69,102,154,151,134}), tip = "Use * in Auras for target mode", callback = function(on) setAura(_Vzd({139,145,142,147,140}), on) end })
		...
		makeToggle(sc, { order = n(), id = "deleteTouch", title = "Delete Touched Parts", tip = _Vzd({105,138,152,153,151,148,158,69,149,134,151,153,152,69,158,148,154,69,153,148,154,136,141}), callback = function(on)
		...
		makeButton(sc, { order = n(), title = "Bring All Nearby Objects", callback = function()
			...
			local n = 0
			for _, p in ipairs(workspace:GetDescendants()) do
				if p:IsA(_Vzd({103,134,152,138,117,134,151,153})) and not p.Anchored and (p.Position-me.Position).Magnitude < 80 then
			...
			notify(HUB_NAME, _Vzd({103,151,148,154,140,141,153,69})..n.._Vzd({69,149,134,151,153,152}), 2)
		end })
end
_TAB_BUILDERS["visuals"] = function(sc, n)
		section(sc, _Vzd({120,121,106,102,113,121,109}), n())
		makeToggle(sc, {
		...
		makeDropdown(sc, { order = n(), title = _Vzd({106,120,117,69,105,138,149,153,141,69,114,148,137,138}), options = { "AlwaysOnTop", "Occluded" }, default = _Vzd({102,145,156,134,158,152,116,147,121,148,149}),
		...
		makeToggle(sc, { order = n(), id = _Vzd({147,148,107,148,140}), title = _Vzd({115,148,69,107,148,140}), callback = function(on) if on then Lighting.FogEnd=1e9 else Lighting.FogEnd=100000 end end })
		makeToggle(sc, { order = n(), id = "night", title = "Night Mode", callback = function(on) Lighting.ClockTime = on and 0 or 14 end })
		makeToggle(sc, { order = n(), id = "day", title = _Vzd({105,134,158,69,114,148,137,138}), callback = function(on) if on then Lighting.ClockTime = 14 end end })
		...
		makeToggle(sc, {
			...
			title = _Vzd({88,151,137,69,117,138,151,152,148,147,69,114,148,137,138}),
...
_TAB_BUILDERS["auto"] = function(sc, n)
		...
		makeToggle(sc, { order = n(), id = "antiafk", title = _Vzd({102,147,153,142,82,102,107,112}), callback = function(on)
		...
		makeToggle(sc, {
			order = n(), id = _Vzd({134,154,153,148,107,145,142,147,140,115,138,134,151,138,152,153}), title = _Vzd({102,154,153,148,69,107,145,142,147,140,69,115,138,134,151,138,152,153}),
			tip = _Vzd({107,145,142,147,140,69,147,138,134,151,138,152,153,69,149,145,134,158,138,151,69,142,147,69,134,154,151,134,69,151,134,147,140,138,69,77,153,141,151,148,153,153,145,138,137,81,69,147,148,69,145,134,140,69,136,151,134,152,141,69,148,147,69,147,142,145,69,151,148,148,153,78}),
			callback = function(on)
				stopLoop("autoFlingN")
				if on then
					startLoop(_Vzd({134,154,153,148,107,145,142,147,140,115}), 0.45, function()
						local me = hrp()
						if not me then return end
						local range = tonumber(S.auraRange) or 50
						...
						if best then pcall(flingPlayer, best, S.flingPower or 8000, true, true) end
					end)
				end
			end,
		})
		makeToggle(sc, {
			order = n(), id = "autoKickNearest", title = _Vzd({102,154,153,148,69,112,142,136,144,69,115,138,134,151,138,152,153}),
			tip = _Vzd({112,142,136,144,69,147,138,134,151,138,152,153,69,142,147,69,134,154,151,134,69,151,134,147,140,138,69,154,152,142,147,140,69,136,154,151,151,138,147,153,69,112,142,136,144,69,121,158,149,138,69,77,148,147,138,69,149,145,134,158,138,151,69,149,138,151,69,153,142,136,144,78}),
			callback = function(on)
				stopLoop("autoKickN")
				if on then
					startLoop(_Vzd({134,154,153,148,112,142,136,144,115}), 0.55, function()
						local me = hrp()
						if not me then return end
						local range = tonumber(S.auraRange) or 50
						local best, bd = nil, range
						...
						if best then
							local kt = S.kickType or _Vzd({120,144,158,69,102,147,136,141,148,151})
							if kt == "Phoenix" then kt = _Vzd({120,144,158,69,102,147,136,141,148,151}) end
							pcall(kickPlayer, best, kt, true)
						end
					end)
				end
			end,
		...
		makeToggle(sc, {
			order = n(), id = _Vzd({134,154,153,148,119,138,143,148,142,147}), title = _Vzd({102,154,153,148,69,119,138,143,148,142,147,69,142,139,69,112,142,136,144,138,137}),
		...
		makeToggle(sc, { order = n(), id = "autoClaim", title = _Vzd({102,154,153,148,69,104,145,134,142,146,69,117,145,148,153}), tip = _Vzd({104,145,142,136,144,69,149,145,148,153,69,152,142,140,147,152,69,84,69,136,145,134,142,146,69,137,138,153,138,136,153,148,151,152,69,147,138,134,151,135,158}), callback = function(on)
		...
		makeButton(sc, {
			...
			callback = function()
				task.spawn(function()
					...
					notify(HUB_NAME, _Vzd({120,149,142,147,69,148,147,136,138,69,137,148,147,138}), 1.5)
				end)
			end,
		})
		makeToggle(sc, {
		...
		makeToggle(sc, {
			order = n(), id = "autoLagServer", title = _Vzd({113,134,140,69,120,138,151,155,138,151}),
		...
		makeToggle(sc, {
			...
			callback = function(on)
				if on and not requirePremium("Destroy Hybrid") then
					S.toggles.autoDestroyHybrid = false
					if S._toggleRenderers and S._toggleRenderers.autoDestroyHybrid then
						pcall(S._toggleRenderers.autoDestroyHybrid)
					end
					return
				end
				if on then setMassToggle(_Vzd({137,138,152,153,151,148,158,109,158,135}), true, destroyServerHybridLoop) else stopMass("destroyHyb") end
			end,
		})
end
_TAB_BUILDERS["console"] = function(sc, n)
		...
		makeToggle(sc, {
			order = n(), id = _Vzd({134,147,153,142,112,142,145,145}), title = _Vzd({102,147,153,142,82,112,142,145,145,69,77,124,134,153,138,151,69,84,69,102,136,142,137,78}),
		...
		makeSlider(sc, { order = n(), title = _Vzd({124,134,145,144,120,149,138,138,137}), min = 16, max = 300, default = 50, stateKey = _Vzd({156,134,145,144,120,149,138,138,137}) })
		makeToggle(sc, {
		...
		makeSlider(sc, { order = n(), title = "CFrame Mult", min = 1, max = 8, default = 2, stateKey = _Vzd({152,149,138,138,137,114,154,145,153}) })
		makeToggle(sc, {
			order = n(), id = "infjump", title = _Vzd({110,147,139,142,147,142,153,138,69,111,154,146,149}),
		...
		makeButton(sc, { order = n(), title = _Vzd({104,148,149,158,69,111,148,135,110,137}), callback = function()
			if setclipboard then setclipboard(game.JobId) end; notify(HUB_NAME, _Vzd({104,148,149,142,138,137}), 1)
		end })
		...
		local function consPrint(line, col)
			...
			local kids = {}
			for _, ch in ipairs(S.consoleOut:GetChildren()) do
				if ch:IsA(_Vzd({121,138,157,153,113,134,135,138,145})) then kids[#kids + 1] = ch end
			end
			while #kids > 80 do
				kids[1]:Destroy()
				table.remove(kids, 1)
		...
		makeInput(sc, { order = n(), id = _Vzd({136,148,147,152,148,145,138,110,147,149,154,153}), placeholder = _Vzd({136,146,137,69,161,69,141,138,145,149,69,161,69,139,145,142,147,140,69,147,134,146,138,69,161,69,152,136,134,147}) })
		makeButton(sc, {
		...
		makeButton(sc, { order = n(), title = "Delete my toys", danger = true, callback = function()
			notify(HUB_NAME, _Vzd({104,145,138,134,151,138,137,69}) .. destroyAllMyToys(), 1.2)
		end })
end
_TAB_BUILDERS[_Vzd({136,148,147,153,151,148,145})] = function(sc, n)
		...
		makeButton(sc, {
			order = n(),
			title = _Vzd({104,148,147,153,151,148,145,69,145,148,148,144,69,153,134,151,140,138,153}),
...
_TAB_BUILDERS[_Vzd({153,151,134,147,152})] = function(sc, n)
		...
		local function translateHttp(text, from, to)
			...
			local ok, res = pcall(function()
				local url = "https://api.mymemory.translated.net/get?q="
					...
					.. _Vzd({75,145,134,147,140,149,134,142,151,98}) .. tostring(from) .. "|" .. tostring(to)
			...
			if low:find(_Vzd({149,145,138,134,152,138,69,152,138,145,138,136,153}), 1, true) or low:find("mymemory", 1, true)
				or low:find(_Vzd({142,147,155,134,145,142,137}), 1, true) or low:find("quota", 1, true)
				or low:find(_Vzd({153,141,138,69,149,138,148,149,145,138,69,134,151,138,69,140,151,138,138,153,142,147,140}), 1, true)
		...
		makeToggle(sc, {
			order = n(), id = _Vzd({134,154,153,148,121,151,134,147,152,145,134,153,138}), title = _Vzd({102,154,153,148,69,121,151,134,147,152,145,134,153,138,69,104,141,134,153}),
			tip = _Vzd({120,141,148,156,152,69,153,151,134,147,152,145,134,153,142,148,147,152,69,142,147,69,141,154,135,69,145,148,140,69,80,69,147,148,153,142,139,158,69,148,147,145,158,83,69,115,138,155,138,151,69,149,148,152,153,152,69,142,147,153,148,69,140,134,146,138,69,136,141,134,153,83}),
			callback = function(on)
				S.toggles.autoTranslate = on == true
				notify(HUB_NAME, _Vzd({102,154,153,148,82,153,151,134,147,152,145,134,153,138,69}) .. (on and _Vzd({116,115,69,77,141,154,135,69,148,147,145,158,78}) or "OFF"), 1.5)
			end,
		})
		makeDropdown(sc, {
			order = n(), title = _Vzd({121,151,134,147,152,145,134,153,138,69,121,148}),
			options = { "English", "Spanish", "French", _Vzd({108,138,151,146,134,147}), "Portuguese", "Japanese", "Korean", _Vzd({104,141,142,147,138,152,138}), _Vzd({119,154,152,152,142,134,147}), _Vzd({102,151,134,135,142,136}), "Italian", "Hindi" },
...
_TAB_BUILDERS["sounds"] = function(sc, n)
		...
		local SOUND_TRIGGERS = {
			...
			{ label = "Daddy",     word = _Vzd({105,134,137,137,158}) },
			{ label = "Baby",      word = "Baby" },
			{ label = "UwU",       word = "UwU" },
			{ label = "OwO",       word = "OwO" },
			{ label = ":)",        word = ":)" },
			{ label = ":D",        word = ":D" },
			{ label = _Vzd({109,148,148,151,134,158}),    word = _Vzd({109,148,148,151,134,158}) },
			{ label = "Yay",       word = "Yay" },
			{ label = "Weee",      word = "Weee" },
			{ label = "Fun",       word = "Fun" },
			{ label = "Mountain",  word = "Mountain" },
			{ label = "Yodel",     word = "Yodel" },
			{ label = _Vzd({126,148,137,138,145,142,147,140}),  word = "Yodeling" },
...
_TAB_BUILDERS["fun"] = function(sc, n)
		...
		local function doHoldAndUse(itemName)
			local me = hrp()
			if not me then notify(HUB_NAME, _Vzd({115,148,69,136,141,134,151,134,136,153,138,151}), 1.5) return false end
			local char = LP.Character
			...
			if not holdPart then
				holdPart = model:WaitForChild(_Vzd({109,148,145,137,117,134,151,153}), 3)
			end
		...
		local paintNote = Instance.new(_Vzd({121,138,157,153,113,134,135,138,145}))
		...
		makeButton(sc, {
			...
			callback = function()
				...
				pcall(function()
					...
					if not can then can = workspace:FindFirstChild(_Vzd({120,149,151,134,158,104,134,147,124,105}), true) end
					if can then
					...
					task.spawn(function()
						...
						while S.toggles.autoBreakPlot do
							...
							if p and validP(p) and p.Character then
								...
								if r then
									pcall(function()
										if FTAP.BuyToy then FTAP.BuyToy:InvokeServer(_Vzd({103,148,146,135,114,142,152,152,142,145,138})) end
										if FTAP.SpawnToy then
											for i = 1, 3 do
		...
		local SPARK_SHAPES = { _Vzd({120,149,141,138,151,138}), "Ring", "Halo", "Fountain", _Vzd({120,149,142,151,134,145}), "Cone", "Cylinder", _Vzd({117,145,134,147,138}) }
		S.sparkShape = S.sparkShape or "Sphere"
		makeDropdown(sc, {
			order = n(),
		...
		makeSlider(sc, {
			order = n(), title = _Vzd({103,154,151,152,153,69,113,142,139,138,153,142,146,138}), min = 2, max = 15, default = S.sparkLifetime or 6, step = 1,
		...
		makeToggle(sc, {
			order = n(), id = "sparkAura", title = _Vzd({120,149,134,151,144,145,138,151,69,102,154,151,134,69,77,107,148,145,145,148,156,78}),
		...
		local ANIMS = {
			...
			{ label = _Vzd({105,134,147,136,138,88}),        asset = "rbxassetid://180435792" },
		...
		makeButton(sc, {
			...
			callback = function()
				...
				if not animator then
					notify(HUB_NAME, _Vzd({115,148,69,102,147,142,146,134,153,148,151,69,107,148,154,147,137}), 1.2)
					return
				end
		...
		section(sc, _Vzd({113,110,114,103,120}), n())
		...
		limbNote.Text = " Bloody-style: Motor6D Part0=nil then Destroy (rip limbs). [U] = your limbs.\n Steal = detach from them + attach to YOU."
		limbNote.Parent = sc
		corner(limbNote, 8)
		pad(limbNote, 6, 6, 6, 6)

		local LIMB_JOINTS = { "Right Shoulder", "Left Shoulder", _Vzd({119,142,140,141,153,69,109,142,149}), _Vzd({113,138,139,153,69,109,142,149}), "Neck", "RootJoint" }
		local LIMB_PARTS = { _Vzd({119,142,140,141,153,69,102,151,146}), _Vzd({113,138,139,153,69,102,151,146}), _Vzd({119,142,140,141,153,69,113,138,140}), _Vzd({113,138,139,153,69,113,138,140}), "Head", "Torso" }
		...
		local function breakLimbs(character, flingPower)
			...
			-- Bloody/Endoris style: Motor6D Part0 = nil then Destroy (not just Destroy)
			if voidzDetachMotorJoints then
				voidzDetachMotorJoints(character, function() return true end)
			else
				for _, d in ipairs(character:GetDescendants()) do
					if d:IsA(_Vzd({114,148,153,148,151,91,105})) then
						pcall(function() d.Part0 = nil; d:Destroy() end)
					end
				end
			end
		...
		local STEAL_LIMBS = {
			...
			{ label = _Vzd({119,142,140,141,153,69,113,138,140}), part = _Vzd({119,142,140,141,153,69,113,138,140}) },
		...
		for _, entry in ipairs(STEAL_LIMBS) do
			makeButton(sc, {
				...
				callback = function()
					...
					if ok then
						notify(HUB_NAME, "Stole " .. entry.label .. _Vzd({69,139,151,148,146,69}) .. playerLabel(p), 1.2)
					else
		...
		makeToggle(sc, {
			...
			callback = function(on)
				...
				if on then
					startLoop(_Vzd({144,138,138,149,120,153,148,145,138,147}), 0.15, function()
						local myRoot = hrp()
					...
					if p ~= LP and isAliveP(p) and not isInSafePlot(p) and not isWL(p) then
						...
						if r then
							pcall(function()
								...
								for _, partName in ipairs({_Vzd({119,142,140,141,153,69,102,151,146}),_Vzd({113,138,139,153,69,102,151,146}),_Vzd({119,142,140,141,153,69,113,138,140}),"Left Leg","Head"}) do
...
_TAB_BUILDERS["settings"] = function(sc, n)
		...
		makeDropdown(sc, {
			...
			tip = _Vzd({117,104,69,140,138,153,152,69,152,136,151,148,145,145,69,156,141,138,138,145,69,161,69,114,148,135,142,145,138,69,140,138,153,152,69,148,147,82,152,136,151,138,138,147,69,135,154,153,153,148,147,152}),
			options = { "PC", "Mobile" },
		...
		makeSlider(sc, {
			...
			callback = function(v)
				...
				-- debounce-ish: save scale with other settings
				pcall(saveVoidzSettings)
			end,
		})
		makeToggle(sc, {
		...
		section(sc, "LOAD CHAT", n())
		local loadNote = Instance.new("TextLabel")
		loadNote.LayoutOrder = n()
		loadNote.Size = UDim2.new(1, -6, 0, 28)
		loadNote.BackgroundTransparency = 1
		loadNote.Font = Enum.Font.Gotham
		loadNote.TextSize = 10
		loadNote.TextColor3 = C.muted
		loadNote.TextXAlignment = Enum.TextXAlignment.Left
		loadNote.TextWrapped = true
		loadNote.Text = " What the hub says in chat/notify on auto-load. Saved to file (persists)."
		loadNote.Parent = sc
		makeInput(sc, {
			order = n(),
			id = "loadChatBox",
			placeholder = "e.g. 💀 VoIdZ HuB LoAdEd 💀",
			default = (S.loadChatMessage and S.loadChatMessage ~= "" and S.loadChatMessage) or VOIDZ_DEFAULT_LOAD_CHAT,
			tip = _Vzd({104,154,152,153,148,146,69,134,154,153,148,82,145,148,134,137,69,146,138,152,152,134,140,138,69,77,149,154,135,145,142,136,69,136,141,134,153,69,80,69,147,148,153,142,139,158,78}),
		})
		makeButton(sc, {
			order = n(),
			title = "Save Load Message",
			tip = _Vzd({124,151,142,153,138,152,69,155,148,142,137,159,132,141,154,135,132,152,138,153,153,142,147,140,152,83,143,152,148,147,69,148,147,69,153,141,142,152,69,138,157,138,136,154,153,148,151}),
			callback = function()
				local box = S.loadChatBox
				local txt = box and box.Text or ""
				S.loadChatMessage = tostring(txt or "")
				if saveVoidzSettings() then
					notify(HUB_NAME, _Vzd({120,134,155,138,137,69,145,148,134,137,69,146,138,152,152,134,140,138}), 1.5)
				end
			end,
		})
		makeButton(sc, {
			order = n(),
			title = "Preview Load Message",
			tip = _Vzd({120,141,148,156,152,69,147,148,153,142,139,158,69,148,147,145,158,69,77,137,148,138,152,69,147,148,153,69,152,149,134,146,69,149,154,135,145,142,136,69,136,141,134,153,78}),
			callback = function()
				local box = S.loadChatBox
				if box then S.loadChatMessage = box.Text end
				notify(HUB_NAME, voidzLoadChatMessage(), 2.5)
			end,
		})
		makeButton(sc, {
			order = n(),
			title = "Reset Load Message",
			callback = function()
				S.loadChatMessage = ""
				if S.loadChatBox then S.loadChatBox.Text = VOIDZ_DEFAULT_LOAD_CHAT end
				saveVoidzSettings()
				notify(HUB_NAME, "Load message reset to default", 1.5)
			end,
		})
		...
		local wlLabel = Instance.new(_Vzd({121,138,157,153,113,134,135,138,145}))
		...
		makeToggle(sc, {
			order = n(), id = "kb_toggleUI", title = _Vzd({121,148,140,140,145,138,69,109,154,135,69,128,119,142,140,141,153,120,141,142,139,153,130}),
		...
		makeToggle(sc, {
			order = n(), id = _Vzd({144,135,132,139,145,142,147,140,115,138,134,151}), title = _Vzd({107,145,142,147,140,69,115,138,134,151,69,128,121,130}),
		...
		makeSlider(sc, {
			order = n(), title = _Vzd({113,134,140,69,110,147,153,138,147,152,142,153,158}), min = 1, max = 500, default = S.lagIntensity or 150,
			...
			callback = function()
				local ok, json = pcall(function()
					return game.HttpService:JSONEncode({
						...
						hubScale = S.hubScale,
						loadChatMessage = S.loadChatMessage,
					})
				end)
				if ok and json then
				...
				if ok and type(data) == "table" then
					for k, v in pairs(data) do
						...
						elseif k == _Vzd({137,138,155,142,136,138}) and type(v) == "string" then
							S.device = v
						elseif k == "loadChatMessage" and type(v) == "string" then
							S.loadChatMessage = v
							if S.loadChatBox then S.loadChatBox.Text = v end
						elseif type(v) == "number" then
							S[k] = v
							if k == "hubScale" and applyHubScale then
								pcall(applyHubScale, v, true)
							end
						end
					end
					pcall(saveVoidzSettings)
					notify(HUB_NAME, "Config imported", 2)
				else
...
local function switchTab(id)
	...
	for tid, btn in pairs(S.tabs) do
		...
		local badgeTx = S.tabBadgeTx and S.tabBadgeTx[tid]
		local ts = S.tabStrokes and S.tabStrokes[tid]
		local bg = S.tabBadgeGlows and S.tabBadgeGlows[tid]
		local rail = S.tabRails and S.tabRails[tid]
		if tid == id then
			btn:SetAttribute("activeTab", true)
			btn.BackgroundColor3 = C.accentDim
			btn.BackgroundTransparency = 0.28
			if lab then lab.TextColor3 = C.text; lab.Font = Enum.Font.GothamBold end
			if badge then badge.BackgroundColor3 = C.accent; badge.BackgroundTransparency = 0.15 end
			if badgeTx then badgeTx.TextColor3 = Color3.new(1, 1, 1) end
			if ts then tween(ts, { Color = C.accent, Transparency = 0.45, Thickness = 1 }, 0.16) end
			if bg then tween(bg, { Color = C.accent2, Transparency = 0.35, Thickness = 1 }, 0.16) end
			if rail then tween(rail, { BackgroundTransparency = 0 }, 0.16) end
		else
			btn:SetAttribute(_Vzd({134,136,153,142,155,138,121,134,135}), nil)
			btn.BackgroundColor3 = C.bg
			btn.BackgroundTransparency = 1
			if lab then lab.TextColor3 = C.text; lab.Font = Enum.Font.GothamMedium end
			if badge then badge.BackgroundColor3 = C.bg; badge.BackgroundTransparency = 0.15 end
			if badgeTx then badgeTx.TextColor3 = C.accent2 end
			if ts then tween(ts, { Color = C.strokeSoft, Transparency = 1, Thickness = 0 }, 0.16) end
			if bg then tween(bg, { Color = C.accent, Transparency = 0.75, Thickness = 0.8 }, 0.16) end
			if rail then tween(rail, { BackgroundTransparency = 1 }, 0.16) end
		end
	end
...
local function ensureMouseModal(on)
	...
	if not m or not m.Parent or m.Parent ~= sg then
		if m then pcall(function() m:Destroy() end) end
		m = Instance.new(_Vzd({121,138,157,153,103,154,153,153,148,147}))
...
function applyHubScale(pct, animate)
	...
	if not us or not us.Parent then
		us = root:FindFirstChild(_Vzd({123,116,110,105,127,132,122,110,120,136,134,145,138}))
		if not us then
...
local function installKeybindHandler()
	bind("keybinds", UserInputService.InputBegan:Connect(function(input, gp)
		...
		elseif S.toggles.kb_pallet and pressed(S.keybinds and S.keybinds.pallet or "Q") then
			if S.toggles.palletQ or S.toggles.kb_pallet then
				...
				if FTAP.SpawnToy then
					...
					spawnToy(_Vzd({117,134,145,145,138,153,113,142,140,141,153,103,151,148,156,147}))
				end
			end
		...
		elseif pressed("U") then
			local char = LP.Character
			if char then
				local LIMB_PARTS = { _Vzd({119,142,140,141,153,69,102,151,146}), _Vzd({113,138,139,153,69,102,151,146}), _Vzd({119,142,140,141,153,69,113,138,140}), _Vzd({113,138,139,153,69,113,138,140}), "Head" }
...
do local _z544=(2*3); if _z544<0 and _Vj() then _z544=_z544+1 end end
...
function buildMain()
	...
	sg.Name = _Vzd({123,116,110,105,127,132,109,122,103})
	sg.ResetOnSpawn = false
	sg.IgnoreGuiInset = true
	sg.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
	...
	toast.Size = UDim2.fromOffset(mobile and 268 or 252, 54)
	...
	toast.Parent = sg
	corner(toast, 12)
	stroke(toast, C.accent, 1, 0.4)
	glass(toast)
	local toastRail = Instance.new("Frame")
	toastRail.Size = UDim2.new(0, 3, 1, -16)
	toastRail.Position = UDim2.fromOffset(8, 8)
	toastRail.BackgroundColor3 = C.accent
	toastRail.BorderSizePixel = 0
	toastRail.Parent = toast
	corner(toastRail, 2)
	local tt = Instance.new("TextLabel")
	tt.BackgroundTransparency = 1
	tt.Size = UDim2.new(1, -28, 0, 16)
	tt.Position = UDim2.fromOffset(18, 8)
	...
	tb.Position = UDim2.fromOffset(18, 26)
	...
	S.root = root
	corner(root, 18)
	local rootStroke = stroke(root, C.accent, 1.4, 0.12)
	panelWash(root, 132)
	...
	pcall(function()
		...
		if cam then
			S._hubScaleViewConn = cam:GetPropertyChangedSignal(_Vzd({123,142,138,156,149,148,151,153,120,142,159,138})):Connect(function()
				if not S.root or not S.root.Parent then return end
	...
	local headerH = mobile and 52 or 56
	local footerH = mobile and 24 or 26
	...
	topGrad.Rotation = 0
	topGrad.Parent = top
	local headerLine = Instance.new(_Vzd({107,151,134,146,138}))
	...
	logoMark.Size = UDim2.fromOffset(mobile and 28 or 30, mobile and 28 or 30)
	logoMark.Position = UDim2.fromOffset(14, (headerH - (mobile and 28 or 30)) / 2)
	...
	local logoMarkTx = Instance.new(_Vzd({121,138,157,153,113,134,135,138,145}))
	logoMarkTx.BackgroundTransparency = 1
	logoMarkTx.Size = UDim2.fromScale(1, 1)
	logoMarkTx.Font = Enum.Font.GothamBlack
	logoMarkTx.TextSize = mobile and 14 or 15
	...
	logo.Position = UDim2.fromOffset(mobile and 50 or 54, 0)
	logo.Font = Enum.Font.GothamBlack
	logo.Size = UDim2.new(0, mobile and 120 or 132, 1, mobile and -16 or -18)
	logo.TextSize = mobile and 13 or 14
	logo.TextColor3 = C.text
	logo.TextXAlignment = Enum.TextXAlignment.Left
	logo.TextYAlignment = Enum.TextYAlignment.Bottom
	logo.Text = _Vzd({123,116,110,105,127})
	logo.ZIndex = 7
	logo.Parent = header

	local verL = Instance.new(_Vzd({121,138,157,153,113,134,135,138,145}))
	verL.BackgroundTransparency = 1
	verL.Position = UDim2.fromOffset(mobile and 50 or 54, headerH - (mobile and 18 or 20))
	verL.Size = UDim2.fromOffset(90, 14)
	verL.Font = Enum.Font.GothamMedium
	verL.TextSize = 9
	verL.TextColor3 = C.accent2
	verL.TextXAlignment = Enum.TextXAlignment.Left
	verL.Text = "HUB  ·  v" .. (BUILD:match(_Vzd({77,128,74,137,74,83,130,80,78,73})) or BUILD)
	verL.ZIndex = 7
	verL.Parent = header

	-- Global feature search (mock center search)
	local searchW = mobile and math.min(160, math.floor(S.mainW * 0.28)) or 300
	local searchBox = Instance.new(_Vzd({121,138,157,153,103,148,157}))
	searchBox.Size = UDim2.fromOffset(searchW, 32)
	searchBox.Position = UDim2.new(0.5, -searchW / 2, 0.5, -16)
	searchBox.BackgroundColor3 = C.bg
	...
	searchBox.TextColor3 = C.text
	searchBox.PlaceholderColor3 = C.muted
	searchBox.PlaceholderText = _Vzd({120,138,134,151,136,141,69,153,148,148,145,152})
	searchBox.Text = ""
	searchBox.ClearTextOnFocus = false
	searchBox.TextXAlignment = Enum.TextXAlignment.Left
	searchBox.ZIndex = 8
	...
	corner(searchBox, 10)
	stroke(searchBox, C.strokeSoft, 1, 0.42)
	pad(searchBox, 0, 12, 0, 12)
	S.globalSearch = searchBox

	local searchDrop = Instance.new("Frame")
	searchDrop.Name = _Vzd({120,138,134,151,136,141,105,151,148,149})
	searchDrop.Visible = false
	searchDrop.Size = UDim2.fromOffset(searchW + 20, 0)
	searchDrop.AutomaticSize = Enum.AutomaticSize.Y
	searchDrop.BackgroundColor3 = C.bg2
	searchDrop.BorderSizePixel = 0
	searchDrop.ZIndex = 40
	searchDrop.ClipsDescendants = true
	searchDrop.Parent = root
	corner(searchDrop, 12)
	stroke(searchDrop, C.accent, 1, 0.4)
	local searchDropLay = Instance.new("UIListLayout")
	searchDropLay.SortOrder = Enum.SortOrder.LayoutOrder
	searchDropLay.Padding = UDim.new(0, 2)
	searchDropLay.Parent = searchDrop
	pad(searchDrop, 6, 6, 6, 6)
	...
	local function refreshSearchDrop()
		hideSearchDrop()
		...
		for i, row in ipairs(hits) do
			...
			sub.Position = UDim2.fromOffset(8, 16)
			sub.Font = Enum.Font.Gotham
			sub.TextSize = 9
			sub.TextColor3 = C.muted
			sub.TextXAlignment = Enum.TextXAlignment.Left
			sub.Text = tabLabelOf(e.tab) .. " · " .. (e.kind or "item")
			sub.ZIndex = 42
			sub.Parent = b
			b.MouseButton1Click:Connect(function()
				searchBox.Text = ""
			...
			b.MouseEnter:Connect(function() tween(b, { BackgroundColor3 = C.accentDim }, 0.1) end)
			b.MouseLeave:Connect(function() tween(b, { BackgroundColor3 = C.card }, 0.1) end)
		end
		searchDrop.Visible = true
		searchDrop.Position = UDim2.new(0.5, -(searchW + 20) / 2, 0, headerH + 4)
	end
	searchBox:GetPropertyChangedSignal("Text"):Connect(refreshSearchDrop)
	searchBox.FocusLost:Connect(function()
		task.delay(0.15, function()
			if not searchBox:IsFocused() then hideSearchDrop() end
		end)
	end)

	local statusBg = Instance.new("Frame")
	statusBg.Size = UDim2.fromOffset(58, 24)
	statusBg.Position = UDim2.new(1, mobile and -108 or -108, 0.5, -12)
	...
	statusDot.BackgroundColor3 = FTAP.ok and C.success or C.warn
	...
	S.status = status
	S.statusDot = statusDot

	local close = Instance.new("TextButton")
	local closeS = mobile and 30 or 28
	close.Size = UDim2.fromOffset(closeS, closeS)
	close.Position = UDim2.new(1, -(closeS + 12), 0.5, -closeS / 2)
	...
	local sideW = mobile and 108 or 142
	local gap = mobile and 8 or 10
	local contentTop = headerH + gap
	local contentBot = footerH + gap
	local side = Instance.new(_Vzd({120,136,151,148,145,145,142,147,140,107,151,134,146,138}))
	side.Size = UDim2.new(0, sideW, 1, -(contentTop + contentBot))
	...
	side.Parent = root
	corner(side, 14)
	stroke(side, C.strokeSoft, 1, 0.62)

	local sideLay = Instance.new("UIListLayout")
	sideLay.Padding = UDim.new(0, mobile and 2 or 3)
	...
	content.Parent = root
	corner(content, 14)
	stroke(content, C.strokeSoft, 1, 0.45)
	panelWash(content, 25)

	local footer = Instance.new(_Vzd({107,151,134,146,138}))
	footer.Size = UDim2.new(1, -gap * 2, 0, footerH)
	footer.Position = UDim2.new(0, gap, 1, -(footerH + 6))
	...
	footL.Size = UDim2.new(0.62, 0, 1, 0)
	footL.Position = UDim2.fromOffset(6, 0)
	footL.Font = Enum.Font.Gotham
	footL.TextSize = mobile and 9 or 10
	footL.TextColor3 = C.text
	footL.TextXAlignment = Enum.TextXAlignment.Left
	footL.Text = "VOIDZHUB  ·  full access"
	footL.ZIndex = 6
	footL.Parent = footer
	...
	footR.TextColor3 = C.text
	footR.TextXAlignment = Enum.TextXAlignment.Right
	footR.Text = _Vzd({103,154,142,145,137,69}) .. (BUILD:match(_Vzd({77,128,74,137,74,83,130,80,78,73})) or BUILD)
	footR.ZIndex = 6
	footR.Parent = footer
	...
	task.spawn(function()
		while footer.Parent do
			...
			footR.Text = n .. " online  ·  " .. (BUILD:match("([%d%.]+)$") or BUILD)
			task.wait(2)
		end
	end)

	for i, def in ipairs(TAB_DEFS) do
		local btnH = mobile and 32 or 30
		...
		btn.Parent = side
		corner(btn, 8)
		local btnStroke = stroke(btn, C.strokeSoft, 0, 1)
		local tabRail = Instance.new("Frame")
		tabRail.Size = UDim2.new(0, 2, 0, mobile and 14 or 12)
		tabRail.Position = UDim2.fromOffset(3, (btnH - (mobile and 14 or 12)) / 2)
		...
		btn.MouseEnter:Connect(function()
			if not btn:GetAttribute("activeTab") then
				tween(btn, { BackgroundTransparency = 0.62, BackgroundColor3 = C.card }, 0.12)
			end
		end)
		btn.MouseLeave:Connect(function()
			if not btn:GetAttribute(_Vzd({134,136,153,142,155,138,121,134,135})) then
				tween(btn, { BackgroundTransparency = 1, BackgroundColor3 = C.bg }, 0.12)
			end
		end)
		local badge = Instance.new("Frame")
		local bs = mobile and 20 or 18
		badge.Size = UDim2.fromOffset(bs, bs)
		badge.Position = UDim2.fromOffset(10, btnH / 2 - bs / 2)
		...
		badge.Parent = btn
		corner(badge, 6)
		local badgeGlow = stroke(badge, C.strokeSoft, 1, 0.7)
		...
		badgeTx.TextSize = mobile and 8 or 8
		...
		local lab = Instance.new(_Vzd({121,138,157,153,113,134,135,138,145}))
		lab.BackgroundTransparency = 1
		lab.Size = UDim2.new(1, -(bs + 18), 1, 0)
		lab.Position = UDim2.fromOffset(bs + 16, 0)
		lab.Font = Enum.Font.GothamMedium
		lab.TextSize = mobile and 11 or 11
		...
		S.tabBadgeTx = S.tabBadgeTx or {}
		S.tabBadgeTx[def.id] = badgeTx
		...
		S.tabRails = S.tabRails or {}
		S.tabRails[def.id] = tabRail
	...
	glass(prem)
	local premMark = Instance.new(_Vzd({107,151,134,146,138}))
	...
	LP.CharacterAdded:Connect(function()
		...
		if w.antiGrab == true and S.toggles.antiGrab then
			stopLoop("antiGrab")
			startLoop(_Vzd({134,147,153,142,108,151,134,135}), 0.1, antiGrabTick)
		...
		for _, key in ipairs({ "antiBurn", "antiBanana", _Vzd({134,147,153,142,123,148,142,137}), "antiFling", _Vzd({134,147,153,142,106,157,149,145,148,137,138}), _Vzd({134,147,153,142,120,142,153}), "antiRagdoll", "god" }) do
	...
	task.spawn(function()
		while S.gui and S.gui.Parent do
			...
			if S.status then S.status.Text = FTAP.ok and "FTAP" or "…" end
			if S.statusDot then S.statusDot.BackgroundColor3 = FTAP.ok and C.success or C.warn end
			if S.homeStatus then
				S.homeStatus.Text = _Vzd({103,154,142,145,137,69,69}) .. BUILD
					.. "\nPlace  " .. tostring(game.PlaceId)
					.. "\nFTAP   " .. (FTAP.ok and "linked" or _Vzd({152,136,134,147,147,142,147,140}))
					.. "  ·  " .. tostring(#Players:GetPlayers()) .. _Vzd({69,149,145,134,158,138,151,152})
			end
		end
	end)
	...
	notify(HUB_NAME, _Vzd({116,147,145,142,147,138,69,115,142,140,140,134,69,161,69}) .. (FTAP.ok and _Vzd({107,121,102,117,69,113,142,147,144,138,137,69,109,138,145,145,69,126,138,134,141}) or _Vzd({120,136,134,147,69,119,138,146,148,153,138,152,69,126,148,154,69,105,154,146,135,134,152,152,83,83,83})), 3)
...
local function buildKey()
	...
	dim.Size = UDim2.fromScale(1, 1)
	dim.BackgroundColor3 = Color3.fromRGB(4, 0, 10)
	dim.BackgroundTransparency = 1
	dim.Parent = sg
	tween(dim, { BackgroundTransparency = 0.08 }, 0.4)
	local dimG = Instance.new("UIGradient")
	dimG.Color = ColorSequence.new({
		ColorSequenceKeypoint.new(0, Color3.fromRGB(40, 8, 70)),
		ColorSequenceKeypoint.new(0.5, Color3.fromRGB(4, 0, 10)),
		ColorSequenceKeypoint.new(1, Color3.fromRGB(50, 6, 28)),
	})
	dimG.Rotation = 125
	dimG.Parent = dim
	...
	wm.Position = UDim2.fromScale(0.5, 0.48)
	wm.Size = UDim2.new(1, 0, 0, 140)
	...
	for i = 1, 16 do
		...
		ln.BackgroundTransparency = 0.92
		ln.BorderSizePixel = 0
		ln.Size = UDim2.new(1, 0, 0, 1)
		ln.Position = UDim2.new(0, 0, i / 16, 0)
		ln.Parent = dim
	end
	...
	card.Parent = sg
	corner(card, 20)
	local cardStroke = stroke(card, C.accent, 1.5, 0.18)
	grad(card, mix3(C.accentDim, C.bg2, 0.2), C.bg, 128)
	tween(card, { Size = UDim2.fromOffset(400, 318) }, 0.5, Enum.EasingStyle.Back)
	...
	topG.Color = ColorSequence.new({
		ColorSequenceKeypoint.new(0, C.accent),
		ColorSequenceKeypoint.new(0.5, Color3.fromRGB(255, 70, 110)),
		ColorSequenceKeypoint.new(1, C.accent2 or C.accent),
	})
	topG.Parent = top
	local rail = Instance.new("Frame")
	rail.Size = UDim2.new(0, 3, 1, -24)
	rail.Position = UDim2.fromOffset(10, 12)
	rail.BackgroundColor3 = C.accent
	rail.BorderSizePixel = 0
	rail.Parent = card
	corner(rail, 2)
	...
	mark.Position = UDim2.new(0.5, -24, 0, 22)
	mark.BackgroundColor3 = C.accent
	mark.BorderSizePixel = 0
	mark.Parent = card
	corner(mark, 14)
	stroke(mark, C.accent2, 1.4, 0.25)
	...
	title.Position = UDim2.fromOffset(20, 80)
	title.Font = Enum.Font.GothamBlack
	title.TextSize = 20
	title.TextColor3 = C.text
	title.Text = "VOIDZ ACCESS"
	title.Parent = card

	local sub = Instance.new("TextLabel")
	sub.BackgroundTransparency = 1
	sub.Size = UDim2.new(1, -40, 0, 16)
	sub.Position = UDim2.fromOffset(20, 106)
	sub.Font = Enum.Font.Gotham
	sub.TextSize = 12
	sub.TextColor3 = C.muted
	sub.Text = "Enter key  ·  one shot  ·  full hub"
	sub.Parent = card

	local box = Instance.new("TextBox")
	box.Size = UDim2.new(1, -56, 0, 42)
	box.Position = UDim2.fromOffset(28, 138)
	box.BackgroundColor3 = C.bg
	box.BorderSizePixel = 0
	box.Font = Enum.Font.Code
	box.TextSize = 16
	box.TextColor3 = C.text
	box.PlaceholderText = _Vzd({123,116,110,105,127,109,122,103})
	box.PlaceholderColor3 = C.muted
	box.Text = ""
	box.ClearTextOnFocus = false
	box.Parent = card
	corner(box, 11)
	local boxStroke = stroke(box, C.strokeSoft, 1, 0.35)
	pad(box, 0, 14, 0, 14)
	box.Focused:Connect(function()
		tween(boxStroke, { Color = C.accent, Transparency = 0.05 }, 0.12)
	end)
	box.FocusLost:Connect(function()
		tween(boxStroke, { Color = C.strokeSoft, Transparency = 0.35 }, 0.16)
	end)

	local status = Instance.new("TextLabel")
	status.BackgroundTransparency = 1
	status.Size = UDim2.new(1, -40, 0, 14)
	status.Position = UDim2.fromOffset(20, 188)
	status.Font = Enum.Font.Code
	status.TextSize = 11
	...
	unlock.Position = UDim2.fromOffset(28, 214)
	unlock.BackgroundColor3 = C.accent
	...
	unlock.Parent = card
	corner(unlock, 11)
	stroke(unlock, C.accent2, 1.2, 0.25)
	unlock.MouseEnter:Connect(function()
		tween(unlock, { BackgroundColor3 = mix3(C.accent2, C.accent, 0.25) }, 0.12)
	end)
	unlock.MouseLeave:Connect(function()
		tween(unlock, { BackgroundColor3 = C.accent }, 0.16)
	end)
	task.spawn(function()
		while card.Parent do
			if S.hubOpen then break end
			tween(cardStroke, { Transparency = 0.05 }, 1.1)
			task.wait(1.1)
			if not card.Parent then break end
			tween(cardStroke, { Transparency = 0.35 }, 1.1)
			task.wait(1.1)
		end
	end)
	...
	local function pickDeviceThenMain()
		...
		overlay.BackgroundColor3 = Color3.fromRGB(4, 0, 10)
		...
		pick.Size = UDim2.fromOffset(380, 228)
		pick.BackgroundColor3 = C.bg2
		pick.BorderSizePixel = 0
		pick.Parent = overlay
		corner(pick, 18)
		stroke(pick, C.accent, 1.4, 0.2)
		grad(pick, mix3(C.accentDim, C.bg2, 0.25), C.bg, 120)
		local pt = Instance.new("TextLabel")
		pt.BackgroundTransparency = 1
		pt.Size = UDim2.new(1, -24, 0, 28)
		pt.Position = UDim2.fromOffset(12, 18)
		...
		ps.Size = UDim2.new(1, -24, 0, 36)
		ps.Position = UDim2.fromOffset(12, 48)
		ps.Font = Enum.Font.Gotham
		ps.TextSize = 12
		ps.TextColor3 = C.muted
		ps.TextWrapped = true
		ps.Text = "PC = keybinds  ·  Mobile = bigger tap targets\nThen VOIDZ boots in."
		ps.Parent = pick
		...
		pc.Size = UDim2.new(0.42, 0, 0, 52)
		pc.Position = UDim2.new(0.06, 0, 1, -72)
		pc.BackgroundColor3 = C.accent
		pc.Text = "PC"
		pc.TextColor3 = Color3.new(1, 1, 1)
		pc.Font = Enum.Font.GothamBlack
		pc.TextSize = 16
		pc.AutoButtonColor = false
		pc.Parent = pick
		corner(pc, 12)
		stroke(pc, C.accent2, 1.2, 0.3)
		local mob = Instance.new("TextButton")
		mob.Size = UDim2.new(0.42, 0, 0, 52)
		mob.Position = UDim2.new(0.52, 0, 1, -72)
	...
	local function try()
		...
		if tier then
			...
			sub.Text = "VOIDZHUB · full hub"
			pickDeviceThenMain()
		else
			status.TextColor3 = C.dangerText or C.danger
			status.Text = _Vzd({99,69,142,147,155,134,145,142,137})
			box.Text = ""
			task.spawn(function()
				local base = card.Position
				for _, d in ipairs({ 8, -10, 7, -5, 0 }) do
					card.Position = UDim2.new(base.X.Scale, d, base.Y.Scale, base.Y.Offset)
					task.wait(0.03)
				end
				card.Position = base
			end)
	...
	do
		local g = voidzEnv()
		if type(g) == "table" then
			g.VOIDZ_UNLOAD = unload
			g.VOIDZ_LATE = Late
			g.VOIDZ_BUILD = BUILD
			g.VOIDZ_EXECUTOR = voidzExecutorName
			if Late.openHub then g.VOIDZ_OPEN_HUB = Late.openHub end
		end
	end
	...
	if type(Late.openHub) ~= _Vzd({139,154,147,136,153,142,148,147}) then
		...
		pcall(function()
			local g = voidzEnv()
			if type(g) == _Vzd({153,134,135,145,138}) then g.VOIDZ_OPEN_HUB = Late.openHub end
		end)
	end
...
local function emergencyKeyUI(errMsg)
	pcall(function()
		...
		t.Parent = f
		local b = Instance.new(_Vzd({121,138,157,153,113,134,135,138,145}))
		b.BackgroundTransparency = 1
...
local function showImmediateKeyUI()
	...
	local topBar = Instance.new(_Vzd({107,151,134,146,138}))
	...
	box.ZIndex = 3
	box.Parent = boxHost
	local bc = Instance.new(_Vzd({122,110,104,148,151,147,138,151}))
	...
	status.TextXAlignment = Enum.TextXAlignment.Left
	status.Text = _Vzd({106,147,153,138,151,69,123,116,110,105,127,109,122,103})
	status.ZIndex = 2
	status.Parent = card
	...
	local function tryUnlock()
		...
		if not tier then
			...
			status.Text = "Invalid key · VOIDZHUB"
...
-- Control matches Blitz: SNO Head, horizontal BV, stick under at Y-10.

-- VOIDZ HUB | v1.4.29 | 2026-08-23
-- hi im voidz

...
local BUILD = _Vzd({87,85,87,91,82,85,93,82,87,88,82,86,83,89,83,92})
...
-- Gucci walk and Anti-Grab freeze fight each other. Keep only one on.
function voidzExclusiveGrabDefense(keep)
	if keep == "gucci" then
		if S.toggles.antiGrab then
			...
			if S._toggleRenderers and S._toggleRenderers.antiGrab then
				pcall(S._toggleRenderers.antiGrab)
			end
			notify(HUB_NAME, "Anti-Grab off · Gucci walk", 1.2)
		end
	elseif keep == "antiGrab" then
		if S.toggles.antiGucci then
			S.toggles.antiGucci = false
			S._gucciQueryOff = false
			pcall(stopBloodyTrainGucci)
			pcall(gucciClearNonBody)
			local r = hrp()
			if r then
				pcall(function()
			...
			if S._toggleRenderers and S._toggleRenderers.antiGucci then
				pcall(S._toggleRenderers.antiGucci)
			end
			notify(HUB_NAME, "Gucci off · Anti-Grab freeze", 1.2)
		end
	end
end
...
function stroke(i, col, th, transparency)
	local s = Instance.new(_Vzd({122,110,120,153,151,148,144,138}))
...
function refreshThemeVisuals()
	...
	for _, d in ipairs(S.gui:GetDescendants()) do
		if d:IsA(_Vzd({108,154,142,116,135,143,138,136,153})) then
...
function resolveFTAP()
	FTAP.ok = false
	pcall(function()
		local ge = ReplicatedStorage:FindFirstChild(_Vzd({108,151,134,135,106,155,138,147,153,152}))
...
function createKickPhysical(part, mode)
	...
	if mode == "Float Pin" then mode = _Vzd({107,145,148,134,153}) end
...
function clearBringBodyOnPlayer(p)
	...
	for _, d in ipairs(p.Character:GetDescendants()) do
		if d:IsA("BodyPosition") or d:IsA("BodyVelocity") or d:IsA("BodyAngularVelocity")
			...
			local n = d.Name
			if n == _Vzd({103,151,142,147,140,103,148,137,158}) or n == "VOIDZ_BV" or n == "FlingAuraVelocity"
...
function bringPlayer(p, destCF, quiet)
	...
	for _ = 0, 50 do
		...
		local sv = r:FindFirstChild(_Vzd({120,144,158,123,138,145,148,136,142,153,158}))
		if sv then sv:Destroy() end
		forceUnsit(p)
		if ownsPlayer(p) then
			pcall(function()
...
function parkStatusToy(model, primary)
	if not primary or not primary:IsA(_Vzd({103,134,152,138,117,134,151,153})) then return end
...
function applyMapPaint(targetRoot)
	...
	pcall(function()
		...
		if not paint and map then
			paint = map:FindFirstChild(_Vzd({117,134,142,147,153,117,145,134,158,138,151,117,134,151,153}), true)
		end
	end)
	if paint and paint:IsA(_Vzd({103,134,152,138,117,134,151,153})) then
		pcall(function()
			paint.Anchored = true
...
local KICK_TYPES = {
	...
	"Blobman", _Vzd({103,145,148,135,109,148,155,138,151}), "GrabKick", "StackKick",
}
...
function kickPhysicalMode(ktype)
	...
	if ktype == _Vzd({120,142,145,138,147,153}) then return "Silent" end
...
function ensureBlobman(quiet)
	...
	if not S._blobLastSpawnAt or (now - S._blobLastSpawnAt) >= 3.5 then
		...
		pcall(function()
			if FTAP.SpawnToy then
				FTAP.SpawnToy:InvokeServer(_Vzd({104,151,138,134,153,154,151,138,103,145,148,135,146,134,147}), me.CFrame * CFrame.new(0, 0, -6), Vector3.zero)
			end
		end)
	end
...
function resolveBlobModelFromSeat(seat)
	...
	for _ = 1, 8 do
		...
		if n:find("blob", 1, true)
			...
			or cur:FindFirstChild(_Vzd({103,145,148,135,146,134,147,120,138,134,153,102,147,137,116,156,147,138,151,120,136,151,142,149,153}), true)
...
function findBlobDetector(blob, prefer)
	...
	local names = prefer == "Right"
		...
		or { _Vzd({113,138,139,153,105,138,153,138,136,153,148,151}), "RightDetector" }
...
function fireCreatureGrabBoth(kit, targetRoot)
	...
	local CG = scr and scr:FindFirstChild(_Vzd({104,151,138,134,153,154,151,138,108,151,134,135})) or kit.creatureGrab
	if CG then
		pcall(function()
...
destroyServerHybridLoop = function(keep)
	...
	while keep() do
		...
		if me and FTAP.SpawnToy then
			for _, toy in ipairs({ "Missile", "VoidBomb", "Firework", _Vzd({103,148,146,135,103,134,145,145,148,148,147}), _Vzd({120,147,148,156,135,134,145,145}) }) do
				pcall(function()
...
local AURA_META = {
	...
	{ id = "bring", title = _Vzd({103,151,142,147,140,69,115,138,134,151,135,158}), tip = "Bring nearby players in front of you (proximity)." },
	{ id = "void", title = "Phase Through Floor", tip = _Vzd({105,142,152,134,135,145,138,69,136,148,145,145,142,152,142,148,147,69,80,69,137,151,148,149,69,82,69,153,141,138,158,69,149,141,134,152,138,69,153,141,151,148,154,140,141,69,153,141,138,69,140,151,148,154,147,137,83}) },
...
function voidzRemoveFeetForAntiBanana(c)
	...
	for _, name in ipairs({
		...
		_Vzd({113,138,139,153,107,148,148,153}), "RightFoot", "LeftLowerLeg", "RightLowerLeg", "LeftUpperLeg", "RightUpperLeg",
	}) do
...
function isFlingMoverName(n)
	n = tostring(n or "")
	return n == "FlingAuraVelocity" or n == _Vzd({120,144,158,123,138,145,148,136,142,153,158}) or n == "KickAuraP" or n == "KickAuraP1"
		or n == _Vzd({112,142,136,144,102,154,151,134,108}) or n == "VOIDZ_BV" or n == "VOIDZ_VoidBV" or n == "BringBody"
	...
	if hot or S._afStripN >= 2 then
		...
		pcall(function()
			for _, d in ipairs(r:GetChildren()) do
				local n = d.Name
				if n == "KickAuraP" or n == _Vzd({112,142,136,144,102,154,151,134,117,86}) or n == "KickAuraG" or n == _Vzd({120,144,158,123,138,145,148,136,142,153,158})
					or n == "FlingAuraVelocity" or n == "VOIDZ_BV" or n == _Vzd({103,151,142,147,140,103,148,137,158}) then
					d:Destroy()
				end
			end
		end)
...
function plotHasOwner(plot)
	...
	local sign = plot:FindFirstChild(_Vzd({117,145,148,153,120,142,140,147})) or plot:FindFirstChild("PlotSign", true)
	if not sign then
...
function collectHouseSpots()
	...
	if plots then
		for _, plot in ipairs(plots:GetChildren()) do
			if plot:IsA(_Vzd({114,148,137,138,145})) or plot:IsA(_Vzd({107,148,145,137,138,151})) then
...
local WAR_LOOP_IDS = {
	"warProtect", _Vzd({156,134,151,108,154,136,136,142}), "warAntiKill", "warAntiFling",
...
function installWarModeChatCommands()
	...
	local function handle(msg)
		...
		if m == _Vzd({154,147,156,134,151,82,146,148,137,138}) or m == "unwarmode" or m == _Vzd({154,147,156,134,151}) or m == "war-off" or m == "waroff" then
		...
		if child.Name == "GrabParts" and grabPartsIsAttackingUs(child, c) then
			...
			for _, d in ipairs(child:GetDescendants()) do
				if d:IsA("WeldConstraint") or d:IsA("Weld") or d:IsA(_Vzd({102,145,142,140,147,117,148,152,142,153,142,148,147}))
					or d:IsA(_Vzd({102,145,142,140,147,116,151,142,138,147,153,134,153,142,148,147})) or d:IsA("Motor6D") or d:IsA("RigidConstraint")
	...
	for _, d in ipairs(workspace:GetDescendants()) do
		if d:IsA("VehicleSeat") or d:IsA("Seat") then
			if isBlobSeat and isBlobSeat(d) then
				...
				local prefer = n:find(_Vzd({153,151,134,142,147}), 1, true) or n:find("alwayshere", 1, true) or d:IsA("VehicleSeat")
				if prefer then
...
-- Blitz Anti-Grab while held: Velocity=0, Anchored=true, Struggle(LP), RagdollRemote(HRP, 0).
-- That is the whole feature. No workspace scans, no Sit=false, no Gucci walk.
	...
	if h then
		pcall(function()
			for _, d in ipairs(h:GetChildren()) do
				if d:IsA("Weld") or d:IsA(_Vzd({124,138,145,137,104,148,147,152,153,151,134,142,147,153})) then
					local p0, p1 = d.Part0, d.Part1
	...
	S.conns.gucciChar = LP.CharacterAdded:Connect(function()
		...
		task.delay(0.7, function()
			if gucciProtectOn() then
				local rr = hrp()
				if rr then
					S._gucciHomeCF = rr.CFrame
					S._gucciWalkCF = rr.CFrame
				end
				pcall(bloodyTrainGucciSit)
				pcall(gucciApplyNotGrabbableAlways)
			end
...
function gucciAntiTick()
	...
	-- Anti-grab: Blitz freeze-struggle only
	if antiGrabProtectOn() then
		blitzAntiGrabWhileHeld()
		return
	end
...
function installAntis()
		...
		isHeld.Changed:Connect(function(held)
			...
			S._blitzHeldHB = RunService.Heartbeat:Connect(function()
				...
				if not isLocalBeingHeldFlag() then
					...
					if r.Parent and S._blitzGrabAnchored then r.Anchored = false; S._blitzGrabAnchored = false end
					S._gucciSeatBrokeThisHold = false
					return
				end
				-- Anti-Grab = Blitz freeze-struggle. Gucci = Bloody walk-free (never Anchor).
				if S.toggles.antiGrab then
					blitzAntiGrabWhileHeld()
				elseif r.Anchored then
					r.Anchored = false
			...
			if S.toggles.antiGucci and not S.toggles.antiGrab then
				bloodyGucciSeatBreak()
			end
	...
	bind("gucciAntiHB", RunService.Heartbeat:Connect(function(dt)
		...
		-- GUCCI: Bloody train occupancy stays ON the whole time (SS invis), not only while held.
		...
		-- ANTI-GRAB: Blitz freeze-struggle only. Gucci walk would unanchor and fight it.
...
-- Who is grabbing us? (cheap single scan)
function findGrabAttacker()
	...
	for _, n in ipairs({ "Head", "HumanoidRootPart", "Torso", "UpperTorso" }) do
		...
		if po and po.Value ~= nil then
			...
			if grabber and grabber ~= LP and validP(grabber) then
				return grabber
			end
		end
	end
...
function counterAttackPlayer(plr, part)
	...
	-- 1) light sever once (no 6x spam)
	severAttackingGrabLinks(plr)
	protectSelfFromCounterKickback()

	-- 2) short wait only if still welded (max 3 frames, not 8)
	for _ = 1, 3 do
...
function runAutoAttackerTick()
	...
	if S._autoAttackBusy then return end
	local held = (isLocalBeingHeldFlag and isLocalBeingHeldFlag()) or (isLocalVictimGrabbed and isLocalVictimGrabbed())
	if not held then return end
	local attacker = findGrabAttacker()
	if not attacker then return end
	S._autoAttackBusy = true
	pcall(counterAttackPlayer, attacker)
...
bind("antiGrabChild", workspace.ChildAdded:Connect(function(child)
	...
	if isLocalActivelyGrabbing() and not isLocalBeingHeldFlag() then return end
	if S.toggles.antiGrab and isLocalBeingHeldFlag() then
		blitzAntiGrabWhileHeld()
	end
end))
...
function setupDragExtend(grabModel)
	...
	local ap = drag:FindFirstChildOfClass(_Vzd({102,145,142,140,147,117,148,152,142,153,142,148,147})) or drag:FindFirstChild("AlignPosition")
...
_TAB_BUILDERS["home"] = function(sc, n)
		...
		st.TextWrapped = true
		st.Text = _Vzd({103,154,142,145,137,95,69}) .. BUILD
...
_TAB_BUILDERS["combat"] = function(sc, n)
		...
		makeToggle(sc, {
			order = n(), id = _Vzd({149,145,148,153,102,146,135,154,152,141}), title = _Vzd({102,146,135,154,152,141,69,116,147,69,117,145,148,153,69,106,157,142,153}),
...
_TAB_BUILDERS["blobman"] = function(sc, n)
		...
		makeButton(sc, {
			...
			callback = function()
				...
				releaseBlobmanSeatAfterFeaturesOff(true)
				for _, id in ipairs({ "blobGrabLoop", "blobGrabAllLoop", _Vzd({135,145,148,135,106,157,153,151,134,136,153,117,145,148,153,152,113,148,148,149}), "blobKickLoop", _Vzd({137,138,152,153,151,148,158,120,138,151,155,138,151}), _Vzd({135,145,148,135,104,148,147,153,151,148,145,116,147}) }) do
									...
									if r then
										pcall(function()
											if not bananaModel or not bananaModel.Parent then
												bananaModel, bananaPrimary = ensureToy(_Vzd({107,148,148,137,103,134,147,134,147,134}))
											end
											...
											local holdRF = holdPart and holdPart:FindFirstChild(_Vzd({109,148,145,137,110,153,138,146,119,138,146,148,153,138,107,154,147,136,153,142,148,147}))
...
_TAB_BUILDERS["anti"] = function(sc, n)
		...
		makeToggle(sc, {
			...
			tip = "Bloody: sit AlwaysHere train so others see you there (SS invis). Local weld stripped so you still walk. Turns Anti-Grab off.",
			desc = "Server occupancy on the train. Do not combine with Anti-Grab.",
			callback = function(on)
				S.toggles.antiGucci = on == true
				S.antiWanted = S.antiWanted or {}
				S.antiWanted.antiGucci = on == true
				if on then voidzExclusiveGrabDefense("gucci") end
				installAntis()
				stopLoop("gucciFree")
				if on then
					pcall(resolveFTAP)
		...
		makeToggle(sc, {
			order = n(), id = "antiGrab", title = "Anti-Grab (Freeze Break)",
			tip = "Blitz: while IsHeld, freeze HRP + Struggle + Ragdoll0. Unanchor when released. Turns Gucci off.",
			desc = _Vzd({107,151,138,138,159,138,82,152,153,151,154,140,140,145,138,83,69,105,148,69,147,148,153,69,136,148,146,135,142,147,138,69,156,142,153,141,69,108,154,136,136,142,83}),
			callback = function(on)
				S.antiWanted = S.antiWanted or {}
				S.antiWanted.antiGrab = on == true
				S.toggles.antiGrab = on == true
				if on then voidzExclusiveGrabDefense("antiGrab") end
				stopLoop("antiGrab")
				installAntis()
				...
				if on then
					pcall(resolveFTAP)
					if FTAP.Struggle then pcall(function() FTAP.Struggle:FireServer(LP) end) end
					startLoop("antiGrab", 0.1, function()
						if not S.toggles.antiGrab then return end
						if antiGrabTick then antiGrabTick() end
					end)
					if isLocalBeingHeldFlag() then blitzAntiGrabWhileHeld() end
				else
					local r = hrp()
					if r then
						pcall(function()
		...
		makeButton(sc, {
			...
			tip = "Blitz freeze-struggle once (Anchor + Struggle + Ragdoll0)",
			callback = function()
				installAntis()
		...
		makeToggle(sc, {
			...
			tip = "When someone grabs you, counter with Attack Mode (fixed + low lag).",
			callback = function(on)
				S.autoCounter = on
				...
				if on then
					installAntis()
					startLoop(_Vzd({134,154,153,148,107,145,142,147,140}), 0.18, function()
		...
		makeToggle(sc, { order = n(), id = _Vzd({134,147,153,142,120,142,153}), title = _Vzd({102,147,153,142,82,120,142,153,69,84,69,120,138,134,153,69,121,151,134,149}), tip = "Force unsit (skips Blobman and Gucci train occupancy)", callback = function(on)
			stopLoop("antiSit")
...
_TAB_BUILDERS["loop"] = function(sc, n)
		...
		local loops = {
			{ id = _Vzd({145,148,148,149,107,145,142,147,140}), title = _Vzd({112,138,138,149,69,121,141,151,148,156,142,147,140}), tip = "Annoying constant fling | map-wide", waitRespawn = true, fn = function(p)
			...
			{ id = _Vzd({145,148,148,149,103,151,142,147,140,107,145,142,147,140}), title = _Vzd({113,148,148,149,69,103,151,142,147,140,80,107,145,142,147,140}), tip = "Bring then fling | map-wide", waitRespawn = true, fn = function(p)
...
_TAB_BUILDERS["toys"] = function(sc, n)
		...
		makeButton(sc, { order = n(), title = "Spawn Pallet Stack (5)", tip = _Vzd({90,69,149,134,145,145,138,153,152,69,152,153,134,136,144,138,137,69,77,152,138,151,142,134,145,78}), callback = function()
			spawnToyBurst(_Vzd({117,134,145,145,138,153,113,142,140,141,153,103,151,148,156,147}), 5)
		end })
...
_TAB_BUILDERS["world"] = function(sc, n)
		...
		makeToggle(sc, { order = n(), id = _Vzd({134,154,151,134,132,139,145,142,147,140}), title = "Object/Player Fling Aura", tip = "Use * in Auras for target mode", callback = function(on) setAura(_Vzd({139,145,142,147,140}), on) end })
		...
		makeToggle(sc, { order = n(), id = _Vzd({137,138,145,138,153,138,121,148,154,136,141}), title = _Vzd({105,138,145,138,153,138,69,121,148,154,136,141,138,137,69,117,134,151,153,152}), tip = _Vzd({105,138,152,153,151,148,158,69,149,134,151,153,152,69,158,148,154,69,153,148,154,136,141}), callback = function(on)
...
_TAB_BUILDERS["auto"] = function(sc, n)
		...
		makeToggle(sc, {
			order = n(), id = _Vzd({134,154,153,148,113,134,140,120,138,151,155,138,151}), title = "Lag Server",
		...
		makeToggle(sc, {
			order = n(), id = _Vzd({134,154,153,148,105,138,152,153,151,148,158,109,158,135,151,142,137}), title = "Destroy Hybrid (no Blobman)",
...
_TAB_BUILDERS["console"] = function(sc, n)
		...
		makeToggle(sc, {
			...
			callback = function(on)
				...
				pcall(function()
					local script = LP.PlayerScripts:FindFirstChild(_Vzd({104,141,134,151,134,136,153,138,151,102,147,137,103,138,134,146,114,148,155,138}))
...
_TAB_BUILDERS[_Vzd({152,148,154,147,137,152})] = function(sc, n)
		...
		local quickSounds = { "Mommy", _Vzd({105,134,137,137,158}), "Oof", "Kill", "Yay", "UwU", "Hello", "Ayo", "Bruh", "Lol", "Noob" }
...
_TAB_BUILDERS["fun"] = function(sc, n)
		...
		local SPARK_TOYS = {
			"Firework", "BombBalloon", _Vzd({103,134,145,145,148,148,147}), "BallSnowball", _Vzd({105,142,152,136,148,104,148,145,148,151,103,134,145,145}),
			"BubbleBlower", _Vzd({103,148,148,146,135,148,157}), "BellSmall", "DiceSmall", _Vzd({104,134,147,137,158,104,148,151,147}),
		}
		...
		makeSlider(sc, {
			order = n(), title = _Vzd({102,146,148,154,147,153}), min = 4, max = 28, default = S.sparkAmount or 16, step = 1,
...
local function installKeybindHandler()
	bind(_Vzd({144,138,158,135,142,147,137,152}), UserInputService.InputBegan:Connect(function(input, gp)
		...
		elseif pressed("U") then
			local char = LP.Character
			if char then
				local LIMB_PARTS = { "Right Arm", "Left Arm", _Vzd({119,142,140,141,153,69,113,138,140}), _Vzd({113,138,139,153,69,113,138,140}), "Head" }
...
function buildMain()
	...
	local function hideSearchDrop()
		...
		for _, c in ipairs(searchDrop:GetChildren()) do
			if c:IsA("GuiObject") and not c:IsA(_Vzd({122,110,113,142,152,153,113,134,158,148,154,153})) and not c:IsA(_Vzd({122,110,117,134,137,137,142,147,140})) then
				c:Destroy()
			end
		end
	end
	...
	LP.CharacterAdded:Connect(function()
		...
		for _, key in ipairs({ _Vzd({134,147,153,142,103,154,151,147}), "antiBanana", _Vzd({134,147,153,142,123,148,142,137}), "antiFling", _Vzd({134,147,153,142,106,157,149,145,148,137,138}), "antiSit", "antiRagdoll", "god" }) do
...
-- Gucci: warp back to saved home CF, don't overwrite it with the train.

-- VOIDZ HUB | v1.4.7 | 2026-08-23
-- hi im voidz

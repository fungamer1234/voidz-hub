...
local BUILD = _Vzd({87,85,87,91,82,85,92,82,87,93,82,86,83,87,83,89,89})
			...
			for _, d in ipairs(ch:GetDescendants()) do
				...
				if d:IsA("BasePart") then
					...
					if (n:find("beam", 1, true) or n:find("line", 1, true) or n:find("rope", 1, true)
						or n == "grabline" or n == "string") and d.Transparency > 0.85 then
						d.Transparency = 0
...
function findSlotsFolder()
	...
	for _, ch in ipairs(workspace:GetChildren()) do
		...
		if (n == "slots" or n:find("slot", 1, true)) and (ch:IsA(_Vzd({107,148,145,137,138,151})) or ch:IsA(_Vzd({114,148,137,138,145}))) then
			return ch
		end
	end
	return nil
...
local KNOWN_TOYS = {
	...
	_Vzd({103,138,137,103,145,134,147,144,138,153,103,145,154,138}), "BedFramedOrange", "BedFuton", _Vzd({103,138,145,145,103,142,140}), "BellSmall",
...
-- soft=true: only strip farm pins (safe while YOU are grabbing — no velocity nuke)
function freeOneSnowballModel(ch, soft)
	...
	for _, d in ipairs(ch:GetDescendants()) do
		-- always strip farm pin movers
		if d.Name == "FarmSnowball" or d.Name == "VOIDZ_SnowFling" then
			pcall(function() d:Destroy() end)
		elseif d:IsA("BodyPosition") or d:IsA("BodyVelocity") or d:IsA(_Vzd({103,148,137,158,102,147,140,154,145,134,151,123,138,145,148,136,142,153,158}))
			or d:IsA(_Vzd({103,148,137,158,107,148,151,136,138})) or d:IsA(_Vzd({102,145,142,140,147,117,148,152,142,153,142,148,147})) or d:IsA("LinearVelocity") then
			local n = d.Name
			if n == "FarmSnowball" or n:find("Farm", 1, true) or n:find("Snow", 1, true)
				or n == "BringBody" or (not holding) then
				pcall(function() d:Destroy() end)
			end
		end
		if d:IsA("BasePart") then
			pcall(function()
				...
				d.CanQuery = true
				d.Massless = false
				-- NEVER zero velocity / thrash physics while held — that force-drops the grab
				if not holding then
	...
	for _, d in ipairs(ch:GetChildren()) do
		if d.Name == "FarmSnowball" or ((d:IsA("BodyPosition") or d:IsA("BodyVelocity")) and not holding) then
			pcall(function() d:Destroy() end)
		end
	end
end
	...
	if folder then
		for _, ch in ipairs(folder:GetChildren()) do
			if ch.Name == "BallSnowball" then
				-- if already holding this ball, soft free only
...
-- After farm: keep ownership while you grab so the server does not force-drop the line
function startSnowGrabAssist()
	...
	S._snowGrabAssistConn = RunService.Heartbeat:Connect(function()
		...
		local hp = S.heldParts
		local gm = S.grabMap
		local function assistPart(part)
			...
			local isSnow = (model and model.Name == "BallSnowball")
				or part.Name == "SoundPart"
				or (model and tostring(model.Name):find("Snow", 1, true))
			if not isSnow then return end
			anyHold = true
			-- strip farm pin if it somehow returns
			pcall(function()
				local f = part:FindFirstChild("FarmSnowball")
				if f then f:Destroy() end
				if model then
					for _, d in ipairs(model:GetDescendants()) do
						if d.Name == "FarmSnowball" then d:Destroy() end
					end
				end
			end)
			setSnowPartCollisionGroup(part, false)
			if model then setSnowballNoCollideEachOther(model, false) end
			-- light SNO so ownership stays with you (drop = no owner)
			if FTAP.SetNetworkOwner then
				sno(part, me.Position)
			end
		end
		if type(hp) == "table" then
			for part, _ in pairs(hp) do assistPart(part) end
		end
		if type(gm) == "table" then
			for _, part in pairs(gm) do
				if typeof(part) == "Instance" then assistPart(part) end
			end
		end
		if not anyHold then
			-- idle: still strip any leftover FarmSnowball on your toys once in a while
			local now = os.clock()
			if now - (S._snowAssistIdleT or 0) > 0.4 then
...
function stopSnowFarm(quiet)
	...
	-- soft re-strip leftover pins only (skip balls you are already holding)
	task.delay(0.15, function()
		if S._snowFarmOn then return end
		local folder = getMyToyFolder and getMyToyFolder()
		if not folder then return end
		for _, ch in ipairs(folder:GetChildren()) do
			if ch.Name == "BallSnowball" and not isLocalHoldingSnowball(ch) then
	...
	if not quiet then notify(HUB_NAME, "Snowball farm OFF | grab ready (hold ownership assist on)", 1.8) end
end
...
function holdGrownSnowball(sound, model)
	...
	-- keep no ball-vs-ball collide while parked with other growers; free restores Default
	...
	-- ALWAYS free this ball when hold ends (farm off or done) so YOU can grab it
	if model then freeOneSnowballModel(model) end
	if sound and sound.Parent then
		pcall(function()
			local f = sound:FindFirstChild("FarmSnowball")
			if f then f:Destroy() end
			sound.CanTouch = true
			sound.CanCollide = true
			sound.CanQuery = true
...
function farmSnowballLoop(model)
	...
	while S._snowFarmOn and model and model.Parent do
		-- re-apply every cycle (new parts / server resets group)
		setSnowballNoCollideEachOther(model, true)
	...
	task.spawn(function()
		...
		while S._snowFarmOn do
			...
			if grown >= want then
				...
				notify(HUB_NAME, "Grown " .. grown .. " | grab them now (SNO assist on)", 2.5)
				break
			end
			if have < want then
...
local SUSPICIOUS_NAMES = {
	...
	"blitz", "endoris", _Vzd({149,148,148,149,141,154,135}), _Vzd({147,134,146,138,145,138,152,152}), "script-ware", "synapse", "drawing",
	"esp", "chams", "aimbot", "silentaim", _Vzd({139,151,138,138,136,134,146}), "noclip", "flybv", _Vzd({135,148,137,158,155,138,145,148,136,142,153,158}),
}
...
function onGrabPartsAdded(child)
	...
	task.spawn(function()
		...
		-- Snowballs: strip farm pin + keep SNO so grab does not force-drop after ~1s
		do
			local snowModel = model
			if snowModel and (snowModel.Name == "BallSnowball" or tostring(snowModel.Name):find("Snow", 1, true)) then
				pcall(function()
					freeOneSnowballModel(snowModel, true)
					setSnowballNoCollideEachOther(snowModel, false)
				end)
				task.spawn(function()
					local me = hrp()
					for _ = 1, 12 do
						if not child.Parent then break end
						if grabbed and grabbed.Parent and me then
							sno(grabbed, me.Position)
							if snowModel then
								for _, d in ipairs(snowModel:GetDescendants()) do
									if d.Name == "FarmSnowball" then pcall(function() d:Destroy() end) end
								end
							end
						end
						RunService.Heartbeat:Wait()
			...
			if part:IsA("BasePart") then
				for _, ch in ipairs(part:GetChildren()) do
					if ch:IsA("BodyVelocity") or ch:IsA("BodyPosition") or ch:IsA("BodyForce")
						...
						local n = ch.Name
						if n ~= _Vzd({123,116,110,105,127,132,107,145,158}) and n ~= _Vzd({123,116,110,105,127,132,107,145,158,108}) then
							pcall(function() ch:Destroy() end)
						end
					end
...
_TAB_BUILDERS["server"] = function(sc, n)
		...
		makeButton(sc, {
			...
			callback = function()
				for _, name in ipairs({ "lagSrv", "softLag", "hardLag", "destroySrv", "destroyHyb", "blobSrv", "kill", "fling", "kick", _Vzd({135,151,142,147,140}), _Vzd({151,134,140,137,148,145,145}), "fire", "banana", "paint", "vomit" }) do
					stopMass(name)
				end
				S.toggles.lagServer = false
				S.toggles.destroyServer = false
...
_TAB_BUILDERS["loop"] = function(sc, n)
		...
		local loops = {
			...
			{ id = _Vzd({145,148,148,149,108,151,134,135,112,142,136,144}), title = "Loop Grab Kick", tip = "Grab kick | map-wide", waitRespawn = true, fn = function(p) kickPlayer(p, _Vzd({108,151,134,135,112,142,136,144}), true) end },
...
_TAB_BUILDERS["fun"] = function(sc, n)
		...
		local SPARK_SHAPES = { _Vzd({120,149,141,138,151,138}), "Ring", "Spiral", "Cone", "Cylinder", "Plane" }
		...
		local SPARK_TOYS = { "Firework", "Balloon", _Vzd({120,147,148,156,135,134,145,145}), "BombBalloon", "CandyCorn", "WaterBomb", "Pumpkin", "Present" }
		...
		makeButton(sc, {
			...
			callback = function()
				...
				if not p or not validP(p) or not p.Character then
					notify(HUB_NAME, _Vzd({117,142,136,144,69,134,69,149,145,134,158,138,151,69,139,142,151,152,153}), 1.2)
					return
				end
				local r = rootOf(p)
				if not r then return end

...
local BUILD = _Vzd({87,85,87,91,82,85,92,82,87,93,82,86,83,87,83,89,90})
...
function installInstantEscape()
	...
	task.spawn(function()
		local isHeld = LP:FindFirstChild(_Vzd({110,152,109,138,145,137})) or LP:WaitForChild(_Vzd({110,152,109,138,145,137}), 20)
		if not isHeld then return end
...
_TAB_BUILDERS["server"] = function(sc, n)
		...
		makeButton(sc, {
			...
			callback = function()
				for _, name in ipairs({ "lagSrv", _Vzd({152,148,139,153,113,134,140}), _Vzd({141,134,151,137,113,134,140}), "destroySrv", "destroyHyb", "blobSrv", "kill", "fling", "kick", "bring", "ragdoll", "fire", "banana", "paint", "vomit" }) do
					stopMass(name)
				end
				S.toggles.lagServer = false
				S.toggles.destroyServer = false
...
_TAB_BUILDERS["trans"] = function(sc, n)
		...
		makeDropdown(sc, {
			...
			options = { _Vzd({106,147,140,145,142,152,141}), "Spanish", "French", "German", "Portuguese", "Japanese", "Korean", _Vzd({104,141,142,147,138,152,138}), "Russian", "Arabic", _Vzd({110,153,134,145,142,134,147}), "Hindi" },
...
_TAB_BUILDERS["fun"] = function(sc, n)
		...
		local SPARK_SHAPES = { "Sphere", "Ring", "Spiral", "Cone", "Cylinder", _Vzd({117,145,134,147,138}) }
		...
		local SPARK_TOYS = { _Vzd({107,142,151,138,156,148,151,144}), "Balloon", "Snowball", "BombBalloon", "CandyCorn", "WaterBomb", "Pumpkin", "Present" }

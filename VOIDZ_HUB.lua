...
_TAB_BUILDERS["anti"] = function(sc, n)
		...
		makeToggle(sc, {
			...
			tip = _Vzd({124,141,138,147,69,152,148,146,138,148,147,138,69,140,151,134,135,152,69,158,148,154,81,69,136,148,154,147,153,138,151,69,156,142,153,141,69,102,153,153,134,136,144,69,114,148,137,138,69,77,139,142,157,138,137,69,80,69,145,148,156,69,145,134,140,78,83}),
			callback = function(on)
				S.autoCounter = on
...
_TAB_BUILDERS[_Vzd({149,145,134,158,138,151})] = function(sc, n)
		...
		makeToggle(sc, {
			...
			callback = function(on)
				...
				stopLoop("superSpeed")
				if on then startLoop(_Vzd({152,154,149,138,151,120,149,138,138,137}), 0.1, function()
		...
		makeButton(sc, {
			...
			callback = function()
				...
				if S.conns.infJump then pcall(function() S.conns.infJump:Disconnect() end) S.conns.infJump = nil end
				for _, id in ipairs({"god","autoHeal","playerSpin",_Vzd({134,147,153,142,105,151,148,156,147}),"highJump","superSpeed",_Vzd({147,148,136,145,142,149})}) do stopLoop(id) end
				local h = hum()
				if h then
					h.WalkSpeed = 16
					h.JumpPower = 50
...
_TAB_BUILDERS["loop"] = function(sc, n)
		...
		local loops = {
			...
			{ id = "loopGrabKick", title = _Vzd({113,148,148,149,69,108,151,134,135,69,112,142,136,144}), tip = "Grab kick | map-wide", waitRespawn = true, fn = function(p) kickPlayer(p, _Vzd({108,151,134,135,112,142,136,144}), true) end },
			...
			{ id = "loopPoison", title = _Vzd({113,148,148,149,69,117,148,142,152,148,147}), tip = "Poison | map-wide", fn = function(p) applyStatusToPlayer(_Vzd({149,148,142,152,148,147}), p) end },
...
_TAB_BUILDERS["toys"] = function(sc, n)
		...
		makeToggle(sc, { order = n(), id = _Vzd({134,154,153,148,117,134,145,145,138,153}), title = _Vzd({102,154,153,148,69,117,134,145,145,138,153,69,117,134,153,141}), tip = _Vzd({118,154,138,154,138,137,69,149,134,145,145,138,153,152,69,156,141,142,145,138,69,156,134,145,144,142,147,140}), callback = function(on)
			stopLoop("autoPallet")
...
_TAB_BUILDERS["fun"] = function(sc, n)
		...
		local SPARK_SHAPES = { "Sphere", "Ring", "Halo", _Vzd({107,148,154,147,153,134,142,147}), "Spiral", "Cone", "Cylinder", "Plane" }
		S.sparkShape = S.sparkShape or "Sphere"
		makeDropdown(sc, {
			order = n(),
		...
		local SPARK_TOYS = {
			...
			"BubbleBlower", "Boombox", _Vzd({103,138,145,145,120,146,134,145,145}), "DiceSmall", _Vzd({104,134,147,137,158,104,148,151,147}),
		}
		S.sparkToyName = S.sparkToyName or "Firework"
		makeDropdown(sc, {
			order = n(),
...
function buildMain()
	...
	LP.CharacterAdded:Connect(function()
		...
		for _, key in ipairs({ "antiBurn", _Vzd({134,147,153,142,103,134,147,134,147,134}), _Vzd({134,147,153,142,123,148,142,137}), "antiFling", _Vzd({134,147,153,142,106,157,149,145,148,137,138}), "antiSit", "antiRagdoll", "god" }) do
...
local function showImmediateKeyUI()
	...
	local function paintOutline(host, col, th)
		...
		local names = { _Vzd({123,116,110,105,127,132,116,113,132,121}), _Vzd({123,116,110,105,127,132,116,113,132,103}), "VOIDZ_OL_L", "VOIDZ_OL_R" }
	...
	local function setOutlineColor(host, col)
		for _, n in ipairs({ "VOIDZ_OL_T", _Vzd({123,116,110,105,127,132,116,113,132,103}), "VOIDZ_OL_L", _Vzd({123,116,110,105,127,132,116,113,132,119}) }) do

...
if getgenv and getgenv().BATMAN_LOADED then
	pcall(function() if getgenv().BATMAN_UNLOAD then getgenv().BATMAN_UNLOAD() end end)
end
if getgenv then getgenv().BATMAN_LOADED = true end
...
local ACCESS_KEY = "WAYNE"
local HUB_NAME = "BATMAN HUB"
local BUILD = "2026-07-24-batman"
local GuiService = game:GetService("GuiService")

-- 7 themes (Batman = dark knight blue)
local THEMES = {
	Purple = {
	...
	Batman = {
		bg = Color3.fromRGB(4, 4, 4), bg2 = Color3.fromRGB(8, 8, 8),
		card = Color3.fromRGB(12, 12, 12), card2 = Color3.fromRGB(20, 20, 20),
		stroke = Color3.fromRGB(200, 170, 30), strokeSoft = Color3.fromRGB(110, 90, 18),
		accent = Color3.fromRGB(220, 190, 40), accent2 = Color3.fromRGB(255, 220, 60),
		accentDim = Color3.fromRGB(130, 110, 22), text = Color3.fromRGB(240, 240, 235),
		muted = Color3.fromRGB(120, 120, 110), danger = Color3.fromRGB(40, 10, 10),
		dangerText = Color3.fromRGB(200, 60, 60), dangerStroke = Color3.fromRGB(150, 35, 40),
		success = Color3.fromRGB(200, 190, 50), warn = Color3.fromRGB(255, 210, 50),
		black = Color3.fromRGB(0, 0, 0), tip = Color3.fromRGB(8, 8, 4),
	},
	Red = {
...
function applyThemeColors(name)
	local t = THEMES[name] or THEMES.Batman
	if not t then return end
	for k, v in pairs(t) do C[k] = v end
end
applyThemeColors("Batman")
...
function stroke(i, col, th, transparency)
	...
	s.Parent = i
	s:SetAttribute("BATMAN_Stroke", true)
	return s
end
...
function refreshThemeVisuals()
	...
	local oldName = S._prevTheme or "Batman"
	local newName = S.theme or "Batman"
	...
	for _, d in ipairs(S.gui:GetDescendants()) do
		...
		if d:IsA("UIStroke") and d:GetAttribute("BATMAN_Stroke") then
...
function applyTheme(name)
	S._prevTheme = S.theme or "Batman"
...
function openOptionPanel(opts)
	opts = opts or {}
	if not S.gui then return end
	local old = S.gui:FindFirstChild("BATMAN_OptionPanel")
	if old then old:Destroy() end
	local dim = Instance.new("TextButton")
	dim.Name = "BATMAN_OptionPanel"
...
function resolveFTAP()
	...
	if S.status then
		S.status.Text = FTAP.ok and "ALFRED:ON" or "ALFRED:…"
	end
	return FTAP.ok
end
...
function applyVel(part, power, up)
	...
	pcall(function()
		local old = part:FindFirstChild("BATMAN_BV") or part:FindFirstChild("FlingAuraVelocity")
		if old then old:Destroy() end
			...
			for _, ch in ipairs(part:GetChildren()) do
				if ch:IsA("BodyPosition") or ch:IsA("BodyVelocity") or ch:IsA("BodyForce")
					...
					if n == "SkyVelocity" then
						-- keep launch
					elseif n == "BringBody" or n == "BATMAN_BV" or n == "FlingAuraVelocity"
						or n == "KickAuraP" or n == "KickAuraP1" or n == "KickAuraVelocity"
						or n == "FollowBP" or n == "FreezeBP" or n == "BATMAN_ControlBV"
						or n == "BATMAN_ControlHold" then
						ch:Destroy()
					end
				end
			end
...
function launchTarget(p, mode)
	...
	if mode == "void" then
		pcall(function()
			local bv = r:FindFirstChild("BATMAN_BV")
...
function voidPlayer(p, quiet)
	...
	for _ = 1, 20 do
		...
		-- BodyVelocity slam
		local bv = r:FindFirstChild("BATMAN_VoidBV")
		if not bv then
			bv = Instance.new("BodyVelocity")
			bv.Name = "BATMAN_VoidBV"
			bv.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
			bv.Parent = r
		end
...
function ensureFreezePart()
	...
	freezePart.Name = "BATMAN_FreezeCam"
...
function installControlKeyC(on, quiet)
	...
	pcall(function() ContextActionService:UnbindAction("BATMAN_ControlK") end)
	pcall(function() ContextActionService:UnbindAction("BATMAN_ControlC") end)
...
function parkStatusToy(model, primary)
	...
	pcall(function()
		...
		local bp = primary:FindFirstChild("BATMAN_StatusPark")
		if not bp then
			bp = Instance.new("BodyPosition")
			bp.Name = "BATMAN_StatusPark"
...
function massRagdollLoop(keep)
	...
	local bp = primary:FindFirstChild("BATMAN_RagdollPark")
	if not bp then
		bp = Instance.new("BodyPosition")
		bp.Name = "BATMAN_RagdollPark"
...
function massFireLoop(keep)
	...
	local head = LP.Character and LP.Character:FindFirstChild("Head")
	local parkY = head and (head.Position.Y + 500) or 500
	local bp = primary:FindFirstChild("BATMAN_FirePark")
	if not bp then
		bp = Instance.new("BodyPosition")
		bp.Name = "BATMAN_FirePark"
		bp.MaxForce = Vector3.new(12500, 12500, 12500)
		bp.P = 12500
		bp.Parent = primary
	end
...
function massRagdollOnce()
	...
	local bp = primary:FindFirstChild("BATMAN_RagdollPark")
	if not bp then
		bp = Instance.new("BodyPosition"); bp.Name = "BATMAN_RagdollPark"
...
function massFireOnce()
	...
	local bp = primary:FindFirstChild("BATMAN_FirePark")
	if not bp then
		bp = Instance.new("BodyPosition"); bp.Name = "BATMAN_FirePark"
		bp.MaxForce = Vector3.new(12500, 12500, 12500); bp.P = 12500; bp.Parent = primary
	end
	bp.Position = Vector3.new(0, parkY, 0); sno(primary)
...
function zeroGNearbyObjects(seconds)
	...
	local function tryPart(inst)
		...
		pcall(function()
			sno(inst, me.Position)
			local old = inst:FindFirstChild("BATMAN_ZeroG")
			if old then old:Destroy() end
			local bf = Instance.new("BodyForce")
			bf.Name = "BATMAN_ZeroG"
...
function runPlotExitAmbush(p)
	...
	plotBypass = false
	if not ok then warn("[BATMAN] plot exit", err) end
end
...
function installPlotWatch()
	...
	print("[BATMAN] plot ambush watch installed")
end
...
function restoreBuriedModel(model)
	...
	clearAuraEffect(root, { "BATMAN_BuryBV", "BATMAN_BuryBP" })
end
...
function cleanupAura(id)
	for _, p in ipairs(Players:GetPlayers()) do
		...
		elseif id == "freeze" then
			clearAuraEffect(root, { "BATMAN_FreezeBP" })
		elseif id == "orbit" then
			clearAuraEffect(root, { "BATMAN_OrbitBP" })
		elseif id == "fling" then
			clearAuraEffect(root, { "FlingAuraVelocity", "BATMAN_BV" })
		end
	end
end
...
function applyVelBurst(part, power, up)
	...
	pcall(function()
		...
		local old = part:FindFirstChild("BATMAN_BV")
		if old then old:Destroy() end
		local bv = Instance.new("BodyVelocity")
		bv.Name = "BATMAN_BV"
		bv.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
...
function tick_ragdoll(cfg, serverWide)
	...
	local function ensurePeel()
		...
		local head = LP.Character and LP.Character:FindFirstChild("Head")
		local parkY = head and (head.Position.Y + 500) or 500
		local bp = pp:FindFirstChild("BATMAN_AuraRagdollPark")
		if not bp then
			bp = Instance.new("BodyPosition")
			bp.Name = "BATMAN_AuraRagdollPark"
			bp.MaxForce = Vector3.new(12500, 12500, 12500)
			bp.P = 12500
			bp.Parent = pp
		end
...
function tick_stomp(cfg, serverWide)
	...
	local function buryModel(model, root)
		...
		pcall(function()
			...
			local bv = root:FindFirstChild("BATMAN_BuryBV")
			if not bv then
				bv = Instance.new("BodyVelocity")
				bv.Name = "BATMAN_BuryBV"
				bv.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
				bv.Parent = root
			end
			...
			local bp = root:FindFirstChild("BATMAN_BuryBP")
			if not bp then
				bp = Instance.new("BodyPosition")
				bp.Name = "BATMAN_BuryBP"
				bp.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
				bp.D = 2000
				bp.P = 1e5
				bp.Parent = root
...
function tick_orbit(cfg, serverWide)
	...
	eachAuraTarget(cfg, function(p, r)
		...
		pcall(function()
			local bp = r:FindFirstChild("BATMAN_OrbitBP")
			if not bp then
				bp = Instance.new("BodyPosition")
				bp.Name = "BATMAN_OrbitBP"
				bp.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
				bp.D = 800
				bp.P = 12000
				bp.Parent = r
...
function tick_freeze(cfg, serverWide)
	...
	eachAuraTarget(cfg, function(p, r)
		pcall(function()
			r.AssemblyLinearVelocity = Vector3.zero
			r.AssemblyAngularVelocity = Vector3.zero
			local bp = r:FindFirstChild("BATMAN_FreezeBP") or Instance.new("BodyPosition")
			bp.Name = "BATMAN_FreezeBP"
			bp.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
			bp.P = 5e4
			bp.D = 1e3
...
function tick_launch(cfg, serverWide)
	...
	eachAuraTarget(cfg, function(p, r, power)
		pcall(function()
			...
			local bv = r:FindFirstChild("BATMAN_LaunchBV")
			if not bv then
				bv = Instance.new("BodyVelocity")
				bv.Name = "BATMAN_LaunchBV"
...
function tick_flatten(cfg, serverWide)
	...
	eachAuraTarget(cfg, function(p, r, power)
		pcall(function()
			...
			r.AssemblyAngularVelocity = Vector3.zero
			local bp = r:FindFirstChild("BATMAN_FlattenBP")
			if not bp then
				bp = Instance.new("BodyPosition")
				bp.Name = "BATMAN_FlattenBP"
				bp.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
				bp.D = 2500
				bp.P = 1e5
				bp.Parent = r
...
local function isWaterishPart(part)
	if not part or not part:IsA("BasePart") then return false end
	if part.Name == "BATMAN_WaterWalk" then return false end
...
setWaterWalk = function(on)
	...
	for _, d in ipairs(workspace:GetChildren()) do
		if d.Name == "BATMAN_WaterWalk" then pcall(function() d:Destroy() end) end
	end
...
function clearFormWear(doDestroy)
	...
	for _, pe in ipairs(pieces) do
		local part = pe.part
		if part and part.Parent then
			for _, nm in ipairs({ "BATMAN_FormBP", "BATMAN_FormBG" }) do
...
function ensureFormMovers(part)
	...
	local bp = part:FindFirstChild("BATMAN_FormBP")
	if not bp then
		bp = Instance.new("BodyPosition")
		bp.Name = "BATMAN_FormBP"
		bp.MaxForce = Vector3.new(1e9, 1e9, 1e9)
		bp.P = 120000
		bp.D = 2500
		bp.Parent = part
	end
	local bg = part:FindFirstChild("BATMAN_FormBG")
	if not bg then
		bg = Instance.new("BodyGyro")
		bg.Name = "BATMAN_FormBG"
		bg.MaxTorque = Vector3.new(1e9, 1e9, 1e9)
		bg.P = 120000
		bg.D = 2500
...
function clearBringForces(model)
	...
	for _, d in ipairs(model:GetDescendants()) do
		if d:IsA("BasePart") then
			for _, nm in ipairs({ "BATMAN_BringBP", "BATMAN_BringBG", "BATMAN_BringBV" }) do
...
function ensureBringMovers(part)
	if not part or not part.Parent then return nil, nil end
	local bp = part:FindFirstChild("BATMAN_BringBP")
	if not bp then
		bp = Instance.new("BodyPosition")
		bp.Name = "BATMAN_BringBP"
		bp.MaxForce = Vector3.new(1e9, 1e9, 1e9)
		bp.P = 1e5
		bp.D = 2500
		bp.Parent = part
	end
	local bg = part:FindFirstChild("BATMAN_BringBG")
	if not bg then
		bg = Instance.new("BodyGyro")
		bg.Name = "BATMAN_BringBG"
		bg.MaxTorque = Vector3.new(1e9, 1e9, 1e9)
		bg.P = 5e4
		bg.D = 800
		bg.Parent = part
...
function setPalletQ(on)
	pcall(function() ContextActionService:UnbindAction("BATMAN_PalletQ") end)
...
function setAccentTint(on)
	local cc = Lighting:FindFirstChild("BATMAN_CC")
	local bloom = Lighting:FindFirstChild("BATMAN_Bloom")
	if not on then
		if cc then cc:Destroy() end
		if bloom then bloom:Destroy() end
		return
	end
	if not cc then cc = Instance.new("ColorCorrectionEffect"); cc.Name = "BATMAN_CC"; cc.Parent = Lighting end
	cc.TintColor = Color3.fromRGB(200, 160, 255)
	cc.Saturation = 0.15
	cc.Contrast = 0.08
	cc.Brightness = 0.02
	if not bloom then bloom = Instance.new("BloomEffect"); bloom.Name = "BATMAN_Bloom"; bloom.Parent = Lighting end
	bloom.Intensity = 0.4
	bloom.Size = 20
	bloom.Threshold = 0.9
end
	...
	pcall(function()
		for _, ch in ipairs(part:GetChildren()) do
			if ch:IsA("BodyVelocity") or ch:IsA("BodyForce") or ch:IsA("BodyAngularVelocity")
				...
				if ch.Name == "BATMAN_ThrowArm" or ch.Name == "SuperStrength" then
					if ch:IsA("BodyVelocity") and ch.MaxForce.Magnitude > 1 then
						ch:Destroy()
					end
				elseif tostring(ch.Name):sub(1, 5) == "BATMAN" or ch.Name == "ZeroGravityForce"
...
function ensureThrowArm(part)
	...
	local bv = part:FindFirstChild("BATMAN_ThrowArm")
	if bv and bv:IsA("BodyVelocity") then return bv end
	local ok, created = pcall(function()
		local b = Instance.new("BodyVelocity")
		b.Name = "BATMAN_ThrowArm"
...
function clearPalletLockForces(root)
	if not root then return end
	pcall(function()
		for _, n in ipairs({ "BATMAN_PalletLock", "BATMAN_PalletLockG", "BATMAN_PalletBV", "BATMAN_PalletWeld", "BATMAN_PalletAlign" }) do
...
function pinRootToPalletCenter(root, base, plr)
	...
	pcall(function()
		...
		local bp = root:FindFirstChild("BATMAN_PalletLock")
		if not bp then
			bp = Instance.new("BodyPosition")
			bp.Name = "BATMAN_PalletLock"
			bp.MaxForce = Vector3.new(1e9, 1e9, 1e9)
			bp.P = 2e5
			bp.D = 3000
			bp.Parent = root
		...
		local bg = root:FindFirstChild("BATMAN_PalletLockG")
		if not bg then
			bg = Instance.new("BodyGyro")
			bg.Name = "BATMAN_PalletLockG"
			bg.MaxTorque = Vector3.new(1e9, 1e9, 1e9)
			bg.P = 1e5
			bg.D = 800
			bg.Parent = root
		...
		bg.CFrame = top
		local bv = root:FindFirstChild("BATMAN_PalletBV")
		if not bv then
			bv = Instance.new("BodyVelocity")
			bv.Name = "BATMAN_PalletBV"
...
function pinEveryoneOnPallet(base)
	...
	if myR and rootOnPallet(myR, base) then
		...
		pcall(function()
			...
			local bp = myR:FindFirstChild("BATMAN_PalletLock")
			if not bp then
				bp = Instance.new("BodyPosition")
				bp.Name = "BATMAN_PalletLock"
				bp.MaxForce = Vector3.new(1e9, 1e9, 1e9)
			...
			local bv = myR:FindFirstChild("BATMAN_PalletBV")
			if not bv then
				bv = Instance.new("BodyVelocity")
				bv.Name = "BATMAN_PalletBV"
				bv.MaxForce = Vector3.new(1e9, 1e9, 1e9)
		...
		local function doRelease()
			...
			if part then
				...
				task.defer(function()
					...
					local arm = r and r:FindFirstChild("BATMAN_ThrowArm")
...
function installGrabWatch()
	...
	print("[BATMAN] grab-release watch installed")
end
	...
	if r then
		...
		for _, d in ipairs(r:GetChildren()) do
			if (d:IsA("BodyVelocity") or d:IsA("BodyAngularVelocity") or d:IsA("BodyForce"))
				and d.Name ~= "BATMAN_Fly" and d.Name ~= "BATMAN_FlyG" and d.Name ~= "BringBody" then
				pcall(function() d:Destroy() end)
			end
		end
	end
			...
			if part:IsA("BasePart") then
				for _, ch in ipairs(part:GetChildren()) do
					if ch:IsA("BodyVelocity") or ch:IsA("BodyPosition") or ch:IsA("BodyForce")
						...
						local n = ch.Name
						if n ~= "BATMAN_Fly" and n ~= "BATMAN_FlyG" then
...
function installInstantEscape()
	...
	pcall(function() ContextActionService:UnbindAction("BATMAN_InstantEscape") end)
...
function setupDragExtend(grabModel)
	...
	if grabModel:GetAttribute("BATMAN_ReachSetup") then return end
	local drag = grabModel:FindFirstChild("DragPart")
	if not drag or not drag:IsA("BasePart") then return end
	grabModel:SetAttribute("BATMAN_ReachSetup", true)
...
function tickDragExtend(grabModel)
	...
	local drag = grabModel:FindFirstChild("DragPart")
	local bp = drag and drag:FindFirstChild("BATMAN_ScrollDrag")
	local cam = workspace.CurrentCamera
...
function setLineExtend(on)
	...
	pcall(function()
		for _, ch in ipairs(workspace:GetChildren()) do
			if ch.Name == "GrabParts" then
				ch:SetAttribute("BATMAN_ReachSetup", nil)
				local drag = ch:FindFirstChild("DragPart")
				if drag then
					local bp = drag:FindFirstChild("BATMAN_ScrollDrag")
...
local AK = (getgenv and getgenv().BATMAN_ANTIKICK) or {}
if getgenv then getgenv().BATMAN_ANTIKICK = AK end
...
local function doVoidzRejoin(reason)
	if not antiKickReady() then
		print("[BATMAN] anti-kick ignored (off/grace):", tostring(reason):sub(1, 80))
		return
	end
	...
	local fancy = "✦ BATMAN · anti-kick checked… rejoining ✦"
	...
	task.spawn(function()
		...
		pcall(function()
			if queue_on_teleport then
				queue_on_teleport("print('[BATMAN] anti-kick rejoin')")
			end
			if syn and syn.queue_on_teleport then
				syn.queue_on_teleport("print('[BATMAN] anti-kick rejoin')")
			end
		end)
		...
		for i, fn in ipairs(tries) do
			local ok = pcall(fn)
			print("[BATMAN] rejoin try", i, ok, tostring(reason or ""):sub(1, 60))
			if ok then break end
			task.wait(0.15)
		end
		-- Preemptive self-kick so YOU disconnect cleanly before game AC finishes
		-- (message contains BATMAN so namecall hook allows it)
		task.delay(0.35, function()
			pcall(function()
				LP:Kick("BATMAN anti-kick · rejoining before AC")
			end)
		end)
...
local function onKickSignal(text, source)
	...
	AK.seen[key] = t
	print("[BATMAN] kick signal from", source, "→", tostring(text):sub(1, 100))
	doVoidzRejoin(text)
end

local function installNamecallKickHook()
	if not hookmetamethod or not getnamecallmethod then return false end
	if getgenv and getgenv().BATMAN_AK_HOOKED then return true end
	local ok = pcall(function()
		local old
		old = hookmetamethod(game, "__namecall", function(self, ...)
			local method = getnamecallmethod()
			...
			if (m == "Kick" or m == "kick") and self == LP then
				...
				if antiKickReady() then
					...
					print("[BATMAN] Player:Kick intercepted:", msg)
					task.defer(function()
		...
		if getgenv then getgenv().BATMAN_AK_HOOKED = true end
	end)
	return ok
end
...
setAntiKick = function(on, quiet)
	...
	bindKickScanners()
	print("[BATMAN] anti-kick ON · scans console + Roblox kick UI · " .. GRACE_SEC .. "s grace")
	if not quiet then
	...
	task.delay(GRACE_SEC, function()
		if AK.enabled and AK.gen == gen then
			...
			print("[BATMAN] anti-kick active — console + RobloxPromptGui + Player:Kick")
		end
	end)
end
...
installAntiKickOnLoad = function()
	...
	print("[BATMAN] anti-kick infrastructure ready (not enabled)")
end
...
local function unload()
	...
	pcall(function()
		local ak = getgenv and getgenv().BATMAN_ANTIKICK
		if ak then
			ak.enabled = false
			ak.readyAt = math.huge
			ak.rejoining = false
	...
	pcall(function() ContextActionService:UnbindAction("BATMAN_PalletQ") end)
	pcall(function() ContextActionService:UnbindAction("BATMAN_TabToy") end)
	pcall(function() ContextActionService:UnbindAction("BATMAN_InstantEscape") end)
	pcall(function() ContextActionService:UnbindAction("BATMAN_ControlK") end)
	...
	if getgenv then getgenv().BATMAN_LOADED = nil; getgenv().BATMAN_UNLOAD = nil end
	print("[BATMAN] unloaded")
end
if getgenv then getgenv().BATMAN_UNLOAD = unload end
...
local TAB_DEFS = {
	-- core
	{ id = "home", icon = "B07", label = "Home" },
	{ id = "combat", icon = "B06", label = "Combat" },
	{ id = "player", icon = "B05", label = "Player" },
	{ id = "grab", icon = "B04", label = "Grab" },
	{ id = "auras", icon = "B03", label = "Auras" },
	{ id = "server", icon = "B02", label = "Server" },
	{ id = "loop", icon = "B01", label = "Loops" },
	-- self
	{ id = "anti", icon = "B08", label = "Protect" },
	{ id = "move", icon = "B09", label = "Move" },
	{ id = "visuals", icon = "B10", label = "Visuals" },
	-- world / toys
	{ id = "toys", icon = "B11", label = "Toys" },
	{ id = "explosions", icon = "B12", label = "Blasts" },
	{ id = "world", icon = "B13", label = "World" },
	{ id = "auto", icon = "B14", label = "Auto" },
	{ id = "console", icon = "B15", label = "Misc" },
	{ id = "trans", icon = "B16", label = "Trans" },
	{ id = "sounds", icon = "B17", label = "Sounds" },
	{ id = "fun", icon = "B18", label = "Fun" },
	{ id = "settings", icon = "B19", label = "Config" },
}
								...
								if validP(p) and p.Character then
									...
									if r then
										pcall(function()
											...
											local head = LP.Character and LP.Character:FindFirstChild("Head")
											local parkY = head and (head.Position.Y + 500) or 500
											local bp = bananaPrimary:FindFirstChild("BATMAN_VomitPark")
											if not bp then
												bp = Instance.new("BodyPosition")
												bp.Name = "BATMAN_VomitPark"
												bp.MaxForce = Vector3.new(12500, 12500, 12500)
												bp.P = 12500
...
_TAB_BUILDERS["anti"] = function(sc, n)
		...
		S.toggles.antiKick = (getgenv and getgenv().BATMAN_ANTIKICK and getgenv().BATMAN_ANTIKICK.enabled) == true
		makeToggle(sc, {
...
_TAB_BUILDERS["player"] = function(sc, n)
		...
		makeToggle(sc, {
			...
			callback = function(on)
				local r = hrp()
				if not r then return end
				local bp = r:FindFirstChild("BATMAN_FloatBP")
				if on then
					if not bp then
						bp = Instance.new("BodyPosition")
						bp.Name = "BATMAN_FloatBP"
						bp.MaxForce = Vector3.new(1e9, 1e9, 1e9)
						bp.P = 8000
						bp.D = 1500
						bp.Parent = r
...
_TAB_BUILDERS["toys"] = function(sc, n)
		...
		makeToggle(sc, { order = n(), id = "tabSpawn", title = "TAB = Spawn Selected Toy", tip = "Spawn selected toy on TAB", callback = function(on)
			pcall(function() ContextActionService:UnbindAction("BATMAN_TabToy") end)
			if not on then return end
			ContextActionService:BindAction("BATMAN_TabToy", function(_, state)
...
_TAB_BUILDERS["auto"] = function(sc, n)
		...
		S.toggles.autoRejoin = (getgenv and getgenv().BATMAN_ANTIKICK and getgenv().BATMAN_ANTIKICK.enabled) == true
		makeToggle(sc, {
...
_TAB_BUILDERS["console"] = function(sc, n)
		...
		consPrint("BATMAN console · type help", C.accent2)
			...
			for i = #stolenParts, 1, -1 do
				...
				if part and part.Parent then
					...
					for _, d in ipairs(part:GetDescendants()) do
						if d:IsA("WeldConstraint") and d.Name == "BATMAN_StolenLimb" then
							pcall(function() d:Destroy() end)
						end
					end
					-- remove BodyPosition
...
_TAB_BUILDERS["settings"] = function(sc, n)
		...
		makeButton(sc, {
			...
			callback = function()
				...
				S.hubScale = 100; S.device = "PC"; S.theme = "Batman"
				applyTheme("Batman")
...
local function buildMain()
	...
	local old = parent:FindFirstChild("BATMAN_HUB"); if old then old:Destroy() end
	...
	status.Text = FTAP.ok and "ALFRED" or "…"
	status.ZIndex = 7
	...
	for i, def in ipairs(TAB_DEFS) do
		...
		if not ok then warn("[BATMAN] tab "..def.id, err) end
	...
	LP.CharacterAdded:Connect(function()
		...
		local fp = workspace:FindFirstChild("BATMAN_FreezePart")
		if fp then
			unfreezeCam()
			local anyCam = MASS.bring or MASS.kick or MASS.kill or MASS.fling or MASS.ragdoll or MASS.fire or MASS.vomit
			if anyCam then
	...
	task.spawn(function()
		while S.gui and S.gui.Parent do
			...
			if S.status then S.status.Text = FTAP.ok and "ALFRED:ON" or "ALFRED:…" end
			if S.homeStatus then
				S.homeStatus.Text = " Build: " .. BUILD
					.. "\n Place: " .. tostring(game.PlaceId)
					.. "\n ALFRED: " .. (FTAP.ok and "linked" or "scanning")
	...
	notify(HUB_NAME, "Online · " .. (FTAP.ok and "ALFRED Online" or "Scanning Batcave…"), 3)
	print("[BATMAN HUB]", BUILD, "ALFRED", FTAP.ok)
end
...
local function buildKey()
	local parent = getUiParent()
	local old = parent:FindFirstChild("BATMAN_KEY"); if old then old:Destroy() end
	local sg = Instance.new("ScreenGui")
	sg.Name = "BATMAN_KEY"
	sg.ResetOnSpawn = false
	sg.IgnoreGuiInset = true
	...
	sub.Text = "ALFRED PROTOCOL · ENTER ACCESS CODE"
	sub.Parent = card
	...
	local function showDeviceSplash(device, onDone)
		...
		local parent = getUiParent()
		local old = parent:FindFirstChild("BATMAN_SPLASH")
		if old then old:Destroy() end
		local splash = Instance.new("ScreenGui")
		splash.Name = "BATMAN_SPLASH"
		...
		tag.Text = (S.theme or "Batman") .. " theme · ALFRED protocol · " .. BUILD
	...
	Late.unload = unload
	if getgenv then
		getgenv().BATMAN_UNLOAD = unload
	end
end
_voidzLateInit()

print("[BATMAN HUB] loading", BUILD)
task.spawn(resolveFTAP)

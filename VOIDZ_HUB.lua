...
local BUILD = _Vzd({87,85,87,91,82,85,92,82,87,93,82,86,83,87,83,86,94})
...
function modelBlueScore(model)
	...
	for _, d in ipairs(model:GetDescendants()) do
		if d:IsA(_Vzd({103,134,152,138,117,134,151,153})) and d.Transparency < 0.9 and d.Size.Magnitude > 1 then
			total += 1
...
function _vB1(inst)
	...
	for _, k in ipairs({ "train", "monorail", "tram", "metro", "skytrain", _Vzd({145,148,136,148,146,148,153,142,155,138}), "railcar" }) do
...
function findFlyingBlueTrain()
		...
		for _, ch in ipairs(always:GetChildren()) do
			...
			if ch:IsA("Model") or ch:IsA("Folder") then
				...
				if not bestSeat or bestScore < 20 then
					local root = ch:IsA("Model") and (ch.PrimaryPart or ch:FindFirstChildWhichIsA(_Vzd({103,134,152,138,117,134,151,153}), true))
	...
	if always then
		...
		for _, ch in ipairs(always:GetChildren()) do
			if ch:IsA(_Vzd({114,148,137,138,145})) then
...
function startTrainDrive()
	...
	S._trainModel = model
	local drivePart = _vB2(seat) or (seat:IsA(_Vzd({103,134,152,138,117,134,151,153})) and seat) or _vB2(model)
	...
	notify(HUB_NAME, _Vzd({103,145,154,138,69,153,151,134,142,147,69,220,69}) .. where .. " · WASD fly · Space/Ctrl up-down · H horn", 3.2)
	return true
end
...
function startSnowFarm()
	...
	task.spawn(function()
		...
		while S._snowFarmOn do
			...
			if grown >= want then
				stopSnowFarm(true)
				notify(HUB_NAME, _Vzd({108,151,148,156,147,69}) .. grown .. _Vzd({69,220,69,139,145,142,147,140,69,148,151,69,138,157,149,145,148,137,138}), 2.5)
				break
			end
			if have < want then
...
function spawnFormOffsets(toy, offsets, scaleMul, opts)
	...
	if room < 2 then
		S.formBuilding = false
		notify(HUB_NAME, "Toy limit full (" .. getToyLimit() .. _Vzd({78,69,220,69,137,138,145,138,153,138,69,153,148,158,152,69,139,142,151,152,153}), 2.5)
		return false
	end
	local want = #offsets
...
local FORM_BUILDS = {
	...
	{ id = "star", title = "Star", tip = _Vzd({120,153,134,151,69,148,147,69,158,148,154,151,69,135,134,136,144,69,220,69,152,149,142,147,152}), offsets = formStarOffsets, scale = 1.0, anim = "spin" },
...
function startMissileStrike()
	...
	if not FTAP.BombExplode then
		notify(HUB_NAME, _Vzd({115,148,69,103,148,146,135,106,157,149,145,148,137,138,69,220,69,146,142,152,152,142,145,138,152,69,156,142,145,145,69,152,149,134,156,147,69,135,154,153,69,147,148,153,69,137,138,153,148,147,134,153,138}), 2.5)
	end
...
function runConsoleCommand(raw)
	...
	elseif cmd == "exec" or cmd == "lua" then
		...
		consoleLog(_Vzd({154,147,144,147,148,156,147,69,136,146,137,69,220,69,153,158,149,138,69,141,138,145,149}), C.danger)
	end
end
	...
	startLoop("watch_" .. name .. actionName, 0.25, function()
		...
		if not validP(p) then
			...
			if actionName:lower():find("kick") or actionName:lower():find("kill") then
				notify(HUB_NAME, _Vzd({107,142,147,142,152,141,138,137,69,220,69}) .. tostring(name), 2)
			end
			return
		end
		attackerFn(p)
...
function _setPalletCage(on)
	...
	notify(HUB_NAME, _Vzd({117,134,145,145,138,153,69,145,148,136,144,69,116,115,69,220,69,141,148,145,137,69,134,69,149,134,145,145,138,153}), 1.5)
end
	...
	task.spawn(function()
		...
		S.conns.escapeHeld = isHeld.Changed:Connect(function(v)
			if v == true and (S.escapeSpace ~= false) then
				notify(HUB_NAME, _Vzd({108,151,134,135,135,138,137,69,220,69,120,149,134,136,138,69,153,148,69,138,152,136,134,149,138}), 1.5)
			end
		end)
	end)
end
...
setAntiKick = function(on, quiet)
	...
	if not quiet then
		notify(HUB_NAME, _Vzd({102,147,153,142,82,144,142
...
_TAB_BUILDERS["anti"] = function(sc, n)
		...
		makeToggle(sc, {
			...
			tip = _Vzd({105,138,153,138,136,153,69,144,142,136,144,69,138,134,151,145,158,69,183,69,152,138,145,139,82,144,142,136,144,69,80,69,151,138,143,148,142,147,69,103,106,107,116,119,106,69,140,134,146,138,69,102,104,69,139,142,147,142,152,141,138,152}),
		...
		makeToggle(sc, {
			...
			desc = _Vzd({116,115,69,183,69,139,151,138,138,82,146,148,155,138,69,156,141,142,145,138,69,140,151,134,135,135,138,137,69,220,69,147,148,69,139,151,138,138,159,138,69,220,69,151,138,136,148,146,146,138,147,137,138,137}),
			callback = function(on)
		...
		makeToggle(sc, {
			order = n(), id = _Vzd({134,147,153,142,121,151,134,142,147}), title = "Anti Train / Blobman Seat",
...
_TAB_BUILDERS["player"] = function(sc, n)
		...
		makeToggle(sc, {
			...
			tip = _Vzd({111,154,146,149,119,138,150,154,138,152,153,69,183,69,139,148,151,136,138,69,143,154,146,149,69,138,155,138,151,158,69,153,142,146,138,69,77,141,148,145,137,69,152,149,134,136,138,69,153,148,69,139,145,158,78}),
			callback = function(on)
...
_TAB_BUILDERS["loop"] = function(sc, n)
		...
		makeDropdown(sc, {
			...
			callback = function(v) S.kickType = v; notify(HUB_NAME, _Vzd({112,142,136,144,69,153,158,149,138,69,183,69}) .. v, 1) end,
		})
		...
		local loops = {
			...
			{ id = "loopKick", title = _Vzd({112,138,138,149,69,112,142,136,144,142,147,140}), tip = _Vzd({112,142,136,144,69,220,69,146,134,149,82,156,142,137,138}), waitRespawn = true, fn = function(p) kickPlayer(p, S.kickType, true) end },
			...
			{ id = "loopRagdoll", title = "Keep Ragdolling", tip = _Vzd({120,149,134,146,69,151,134,140,137,148,145,145,69,220,69,146,134,149,82,156,142,137,138}), waitRespawn = true, fn = function(p) ragdoll(p, true) end },
			...
			{ id = _Vzd({145,148,148,149,120,144,158}), title = _Vzd({113,148,148,149,69,120,144,158,69,113,134,154,147,136,141}), tip = _Vzd({120,144,158,69,145,134,154,147,136,141,69,220,69,146,134,149,82,156,142,137,138}), waitRespawn = true, fn = function(p) kickPlayer(p, "Sky", true) end },
			{ id = "loopVoid", title = _Vzd({113,148,148,149,69,123,148,142,137}), tip = _Vzd({123,148,142,137,69,152,145,134,146,69,220,69,146,134,149,82,156,142,137,138}), waitRespawn = true, fn = function(p) voidPlayer(p, true) end },
			...
			{ id = "loopGrab", title = _Vzd({113,148,148,149,69,108,151,134,135,69,113,142,147,138}), tip = _Vzd({104,151,138,134,153,138,108,151,134,135,113,142,147,138,69,152,149,134,146,69,220,69,146,134,149,82,156,142,137,138}), fn = function(p) if FTAP.CreateGrabLine then local t=rootOf(p); pcall(function() FTAP.CreateGrabLine:FireServer(t,t.CFrame) end) end end },
			{ id = "loopHardFling", title = "Loop Hard Fling", tip = _Vzd({114,134,157,69,139,145,142,147,140,69,220,69,146,134,149,82,156,142,137,138}), waitRespawn = true, fn = function(p)
				local r = rootOf(p); if not r then return end
				clearTargetMovers(p.Character)
			...
			{ id = "loopStackKick", title = "Loop Stack Kick", tip = _Vzd({120,153,134,136,144,69,144,142,136,144,69,220,69,146,134,149,82,156,142,137,138}), waitRespawn = true, fn = function(p) kickPlayer(p, "StackKick", true) end },
			...
			{ id = "loopPaint", title = _Vzd({113,148,148,149,69,117,134,142,147,153}), tip = _Vzd({117,134,142,147,153,69,220,69,146,134,149,82,156,142,137,138}), fn = function(p) applyStatusToPlayer(_Vzd({149,134,142,147,153}), p) end },
			{ id = "loopBringFling", title = "Loop Bring+Fling", tip = _Vzd({103,151,142,147,140,69,153,141,138,147,69,139,145,142,147,140,69,220,69,146,134,149,82,156,142,137,138}), waitRespawn = true, fn = function(p)
				bringPlayer(p, nil, true)
				local r = rootOf(p); if not r then return end
				clearTargetMovers(p.Character)
				applyVel(r, S.flingPower or 600, 0.3)
...
_TAB_BUILDERS["move"] = function(sc, n)
		...
		makeToggle(sc, { order = n(), id = "fly", title = "Fly", tip = _Vzd({103,148,137,158,123,138,145,148,136,142,153,158,69,80,69,103,148,137,158,108,158,151,148,69,139,145,158}), desc = _Vzd({124,102,120,105,69,220,69,120,149,134,136,138,69,220,69,120,141,142,139,153}), callback = setFly })
...
_TAB_BUILDERS["toys"] = function(sc, n)
		...
		for _, def in ipairs(FORM_BUILDS) do
			makeButton(sc, {
				...
				tip = def.tip or (_Vzd({107,148,151,146,69,220,69}) .. def.title),
				callback = function()
		...
		makeButton(sc, { order = n(), title = "Count My Toys", tip = _Vzd({120,149,134,156,147,138,137,110,147,121,148,158,152,69,136,141,142,145,137,151,138,147}), callback = function()
			notify(HUB_NAME, "Toys: " .. countMyToys() .. _Vzd({69,220,69,117,134,145,145,138,153,152,95,69}) .. countMyToys("PalletLightBrown"), 2)
		end })
		...
		makeToggle(sc, {
			...
			callback = function(on)
				...
				if on then
					...
					notify(HUB_NAME, _Vzd({88,151,137,69,149,138,151,152,148,147,69,116,115,69,220,69,137,142,152,153,69}) .. tostring(dist), 1.2)
				else
					pcall(function()
			...
			callback = function()
				...
				if #pool > 0 then
					...
					if r then
						...
						notify(HUB_NAME, _Vzd({120,134,139,138,69,121,117,69,220,69}) .. pick.name, 1.5)
					end
				else
...
_TAB_BUILDERS["fun"] = function(sc, n)
		...
		local SPARK_SHAPES = { "Sphere", "Ring", _Vzd({120,149,142,151,134,145}), "Cone", "Cylinder", "Plane" }
		...
		makeButton(sc, {
			...
			callback = function()
				...
				notify(HUB_NAME, _Vzd({120,149,134,151,144,145,138,151,69,135,154,151,152,153,69,77}) .. (S.sparkShape or "Sphere") .. " x " .. amount .. ") . server", 1.2)
			end,
		})
		makeToggle(sc, {
...
makeButton(sc, {
	...
	callback = function()
		...
		if not root then
			notify(HUB_NAME, _Vzd({103,145,154,138,69,153,151,134,142,147,69,147,148,153,69,145,148,134,137,138,137,69,158,138,153,69,220,69,156,134,142,153,69,134,69,135,142,153}), 2)
			return
		end
		pcall(function() me.CFrame = root.CFrame * CFrame.new(0, 6, 0) end)
		notify(HUB_NAME, _Vzd({116,147,69,153,151,134,142,147,69,220,69,149,151,138,152,152,69,105,151,142,155,138,69,103,145,154,138,69,121,151,134,142,147}), 1.5)
	end,
})
...
makeSlider(sc, {
	...
	step = 1,
	stateKey = "ballSize",
	tip = _Vzd({120,138,151,155,138,151,69,140,151,148,156,152,69,156,141,142,145,138,69,151,148,145,145,142,147,140,69,148,147,69,152,147,148,156,69,220,69,152,153,148,149,69,156,141,138,147,69,120,142,159,138,69,99,98,69,153,141,142,152}),
})
...
_TAB_BUILDERS["settings"] = function(sc, n)
		...
		makeDropdown(sc, {
			...
			tip = _Vzd({117,104,69,140,138,153,152,69,152,136,151,148,145,145,69,156,141,138,138,145,69,220,69,114,148,135,142,145,138,69,140,138,153,152,69,148,147,82,152,136,151,138,138,147,69,135,154,153,153,148,147,152}),
			options = { "PC", "Mobile" },
		...
		makeButton(sc, {
			...
			callback = function()
				notify(HUB_NAME, _Vzd({122,147,145,148,134,137,142,147,140,75}), 2)
...
function buildMain()
	...
	task.spawn(function()
		while S.gui and S.gui.Parent do
			...
			if S.status then S.status.Text = FTAP.ok and _Vzd({107,121,102,117,95,116,115}) or _Vzd({107,121,102,117,95,75}) end
	...
	notify(HUB_NAME, _Vzd({116,147,145,142,147,138,69,115,142,140,140,134,69,220,69}) .. (FTAP.ok and _Vzd({107,121,102,117,69,113,142,147,144,138,137,69,109,138,145,145,69,126,138,134,141}) or _Vzd({120,136,134,147,69,119,138,146,148,153,138,152,69,126,148,154,69,105,154,146,135,134,152,152,75})), 3)
...
local function buildKey()
	...
	unlock.Text = _Vzd({122,115,113,116,104,112,69,183})
	...
	local function tryUnlock()
		...
		task.spawn(function()
			...
			local function ensureCore()
				status.Text = _Vzd({113,148,134,137,142,147,140,69,141,154,135,69,136,148,151,138,75})
...
task.spawn(function()
	...
	Late._initDone = true
	print(_Vzd({128,123,116,110,105,127,69,109,122,103,130,69,145,134,153,138,69,142,147,142,153,69,148,144,69,220,69,148,149,138,147,109,154,135,69,151,138,134,137,158}))
	pcall(resolveFTAP)

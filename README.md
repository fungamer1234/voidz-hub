...
Build: `2026-08-23-1.4.30` · Key: `VOIDZHUB` (unlocks everything)  
...
## Recent (1.4.30) — Gucci walk + real invis

Gucci was re-sitting the train and warping you back to the toggle spot, so you could not walk. Sit is once. Invis is the same Character Invisibility as Visuals (body under the map, camera on the surface).

## Recent (1.4.29) — control like Blitz

Control Selected / look / C was fighting itself (full-force BV, grab-line spam, TP beside them). It now does Blitz `controlCreature`: SNO their Head, horizontal BodyVelocity from your WASD, sit under them at Y-10, camera on their Humanoid.

## Recent (1.4.28) — Gucci no low-Y lift

Gucci was TPing you back up whenever you dropped below Y=5 (void/water). That snap is gone. Train sit still warps you home so you don't stay on the train.
...
Kill/fling/kick loops were sticking on the corpse (`Ragdolled` still counted as valid) or skipping forever if they respawned in a house. They now wait for a live character, then hit again.

## Recent (1.4.25) — key + splash

Key gate is a full-void access panel (watermark, pulse border, shake on bad key). Boot splash is chromatic VOIDZ + orbit rings + load bar. Same key `VOIDZHUB`.
...
Home copy, Protect labels, aura sliders follow the theme, Gucci CanQuery tick is cheaper.

## Recent (1.4.13) — Gucci grab + jump work

Gucci was leaving you seated (`Sit`) with `CanTouch` off and Ragdoll0/BV every frame — no grabbing, jump floated.

- `CanQuery` off (ungrabbable). `CanTouch` on so you can grab.
- Locally unsit (no Jump) so grab + jump feel normal.
- No idle BodyVelocity / RagdollRemote. Cam only pinned if you actually fall.
...
Unsit after the train sit was killing invis (server saw you at play again). Gucci now **keeps Occupant on the train**, warps you home the same frame, strips the local weld, `CanQuery` off. No Jump/Sit=false. Cam stays at standing height.

## Recent (1.4.10) — Gucci no void death / cam height

Train sit was leaving you (and CamPart) at void Y. Sit is **1 frame**, then warp to last standing height (at least Y=8). If you dip below Y=5, lift. CamPart stays at standing height on HRP.

## Recent (1.4.9) — Gucci sits the train once

Bloody sits, unsits, warps you back, and **never sits again** that life. We were re-sitting every 1.6s, which TPed you to the train. That’s gone.

## Recent (1.4.8) — Anti-Grab walk / Gucci train

- **Anti-Grab** is the walk-while-held that felt right (normal WalkSpeed, Struggle, no freeze).
- **Gucci** is Bloody V2 train occupancy (SS invis). They no longer turn each other off.
...
Walk-while-grabbed uses normal `WalkSpeed` (16). Extra CFrame shove / 1.55x boost is gone. Turn on **WalkSpeed Override** if you want it faster.

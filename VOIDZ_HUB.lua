...
local BUILD = _Vzd({87,85,87,91,82,85,92,82,87,93,82,86,83,87,83,87,88})
...
function _vB1(inst)
	...
	for _, k in ipairs({ "train", _Vzd({146,148,147,148,151,134,142,145}), "tram", "metro", "skytrain", _Vzd({145,148,136,148,146,148,153,142,155,138}), "railcar" }) do
...
function findFlyingBlueTrain()
	...
	local function considerSeat(seat, modelBonus)
		...
		if model then
			...
			if path:find(_Vzd({153,151,134,142,147}), 1, true) then sc += 10 end
	...
	if always then
		for _, ch in ipairs(always:GetChildren()) do
			...
			local bonus = 0
			if n:find(_Vzd({153,151,134,142,147}), 1, true) then bonus += 20 end
...
function _vB3(seat)
	if not seat then return end
	local model = seat:FindFirstAncestorOfClass("Model") or S._trainModel
	local me = hrp()
	local origin = (me and me.Position) or (seat:IsA("BasePart") and seat.Position) or Vector3.zero
	local parts = {}
	if seat:IsA("BasePart") then table.insert(parts, seat) end
	if model then
		for _, d in ipairs(model:GetDescendants()) do
...
function startTrainDrive()
	...
	for attempt = 1, 10 do
		...
		if attempt == 3 or attempt == 6 then
			local always = getTweenedFolder()
			if always then
				for _, ch in ipairs(always:GetChildren()) do
					local root = ch:IsA("Model") and (ch.PrimaryPart or ch:FindFirstChildWhichIsA("BasePart", true))
					if root and root:IsA("BasePart") and root.Position.Y > 25 then
						pcall(function() me.CFrame = root.CFrame * CFrame.new(0, 10, 0) end)
						task.wait(0.35)
						break
					end
				end
	...
	if (me.Position - drivePart.Position).Magnitude > 20 then
		pcall(function() me.CFrame = drivePart.CFrame * CFrame.new(0, 6, 0) end)
		task.wait(0.2)
	end
	...
	task.wait(0.1)
	_vB3(seat:IsA("BasePart") and seat or drivePart)

	local function unanchorTrain()
		local m = model or (drivePart and drivePart:FindFirstAncestorOfClass("Model"))
	...
	S._trainSnoConn = RunService.Heartbeat:Connect(function()
		if not S.trainDriving or not drivePart.Parent then return end
		local origin = drivePart.Position
		if seat and seat.Parent and seat:IsA(_Vzd({103,134,152,138,117,134,151,153})) then sno(seat, origin) end
		sno(drivePart, origin)
		if model then
			for _, d in ipairs(model:GetDescendants()) do
				if d:IsA("BasePart") and (d == drivePart or d == seat or math.random() < 0.12) then
					sno(d, origin)
	...
	notify(HUB_NAME, "Blue train | " .. where .. " | WASD fly | Space/Ctrl up-down | H horn", 3.2)
	return true
end

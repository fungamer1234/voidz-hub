...
local BUILD = _Vzd({87,85,87,91,82,85,93,82,87,87,82,86,83,88,83,86})
...
-- Loop grab: Blitz pattern — MUST be seated; if not, wait (no force sit)
function setBlobGrabLoop(on, p)
	...
	-- Mount up to a few times at start, then loop only fires when seated
	task.spawn(function()
		for _ = 1, 6 do
	...
	startLoop(id, 0.10, function()
		...
		pcall(function()
			...
			if S._blobGrabTargetUserId then
				for _, pl in ipairs(Players:GetPlayers()) do
					if pl.UserId == S._blobGrabTargetUserId then target = pl; break end
				end
			end
			target = target or Players:FindFirstChild(S._blobGrabTargetName or "") or S.blobTarget or S.selected
...
function tick_telekinesis(cfg, serverWide)
	...
	local me = hrp()
	if not me then return end
	local shape = S.tkShape or _Vzd({121,148,151,147,134,137,148})
	local cam = workspace.CurrentCamera
	local mouseHit = nil
	pcall(function()
...
local AURA_META = {
	...
	{ id = _Vzd({151,138,149,138,145}), title = _Vzd({117,154,152,141,69,115,138,134,151,135,158,69,102,156,134,158}), tip = "Push nearby players away (proximity)." },
...
function antiStickyTick()
	...
	if not c then return end
	for _, d in ipairs(c:GetDescendants()) do
		if d:IsA(_Vzd({103,134,152,138,117,134,151,153})) and d.Massless then
		...
		if child.Name == _Vzd({108,151,134,135,117,134,151,153,152}) and grabPartsIsAttackingUs and grabPartsIsAttackingUs(child, c) then
			...
			for _, d in ipairs(child:GetDescendants()) do
				if d:IsA("WeldConstraint") or d:IsA("Weld") or d:IsA("AlignPosition")
					or d:IsA("AlignOrientation") or d:IsA(_Vzd({119,142,140,142,137,104,148,147,152,153,151,134,142,147,153})) then
					pcall(function() d:Destroy() end)
				end
			end
...
function hardRestartGrabBeamScript(force)
	...
	pcall(function()
		...
		for _, root in ipairs(roots) do
			for _, name in ipairs({ _Vzd({104,141,134,151,134,136,153,138,151,102,147,137,103,138,134,146,114,148,155,138}), _Vzd({104,141,134,151,134,136,153,138,151,102,147,137,103,138,134,146}), "BeamMove" }) do
...
function gucciStripThrowMovers(c)
	...
	for _, d in ipairs(c:GetDescendants()) do
		...
		elseif d:IsA("BodyVelocity") or d:IsA(_Vzd({103,148,137,158,102,147,140,154,145,134,151,123,138,145,148,136,142,153,158})) or d:IsA(_Vzd({103,148,137,158,107,148,151,136,138}))
			...
			pcall(function() d:Destroy() end)
		elseif n == "SkyVelocity" or n == _Vzd({103,151,142,147,140,103,148,137,158}) or n == _Vzd({112,142,136,144,102,154,151,134,117}) or n == "KickAuraP1"
...
function gucciForceFreeMove()
	...
	-- non-body limbs + strip throw movers; KEEP their GrabParts / beam visual
	...
	bv.MaxForce = Vector3.new(1e9, 2e5, 1e9)

	if md.Magnitude > 0.04 then
		local dir = md.Unit
		local push = dir * (spd * 1.85)
		pcall(function()
			r.CFrame = r.CFrame + dir * 0.95
			local y = math.clamp(r.AssemblyLinearVelocity.Y, -100, 75)
		...
		local v = r.AssemblyLinearVelocity
		r.AssemblyLinearVelocity = Vector3.new(v.X * 0.06, v.Y, v.Z * 0.06)
	end
	...
	if space then
		if not S._gucciSpaceHeld then
			...
			local v = r.AssemblyLinearVelocity
			r.AssemblyLinearVelocity = Vector3.new(v.X, math.max(v.Y, 62), v.Z)
		end
	else
...
-- Blitz Anti-Grab while held: Anchor + Struggle + RagdollRemote(0). That's their "can't keep grab".
function blitzAntiGrabWhileHeld()
	...
	if not isLocalBeingHeldFlag() then
		if r.Anchored and not S.toggles.antiExplode then
			-- only unanchor if we anchored for grab break (not explosion pin)
	...
	-- also strip kit
	if gucciServerUngrab then pcall(gucciServerUngrab) end
end
...
function bloodyFindGucciSeat()
	...
	if gucciFindFreedomSeat then
		local s = gucciFindFreedomSeat()
...
function bloodyGucciSeatBreak()
	...
	local h = hum()
	if not r or not h then return end
	if isOnBlobman and isOnBlobman() then return end
...
function bloodyGucciPulse()
	...
	if not c or not r then return end
	if isLocalActivelyGrabbing() and not isLocalBeingHeldFlag() then return end
	-- NEVER anchor — that is Blitz anti-grab freeze and fights Bloody Gucci
	pcall(function()
		r.Anchored = false
		S._blitzGrabAnchored = false
		if h then
			h.PlatformStand = false
			h.AutoRotate = true
			if not (isOnBlobman and isOnBlobman()) and not S._gucciSeatBreaking then
				h.Sit = false
			end
		end
	end)
...
function gucciFreeTick()
	...
	if isLocalBeingHeldFlag() or isGucciVictim(c) then
...
function antiGrabTick()
	...
	-- Blitz Anti-Grab exact path
	blitzAntiGrabWhileHeld()
	gucciServerUngrab()
	gucciDenyGrabLatch()
	-- massless false (Blitz enforceMasslessFalse spirit)
	local c = char()
	if c then
		for _, p in ipairs(c:GetDescendants()) do
			if p:IsA("BasePart") and p.Massless and p.Name ~= "HumanoidRootPart" then
				-- only force massless false when anti-grab (not gucci nonbody)
				if not S.toggles.antiGucci then
...
function installAntis()
	...
	bind(_Vzd({140,154,136,136,142,102,147,153,142,109,103}), RunService.Heartbeat:Connect(function(dt)
		...
		-- GUCCI: Bloody V2 every heartbeat (seat-break + Struggle/Ragdoll0 + walk). Never Anchor.
...
local TAB_DEFS = {
	...
	{ id = _Vzd({134,154,151,134,152}), icon = "05", label = _Vzd({102,154,151,134,152}) },
...
_TAB_BUILDERS["blobman"] = function(sc, n)
		...
		makeToggle(sc, {
			...
			tip = "Blitz-style: sit Blobman first, then loop fires CreatureGrab (no force-sit).",
			callback = function(on)
				local p = blobTarget()
...
_TAB_BUILDERS["server"] = function(sc, n)
		...
		makeButton(sc, {
			...
			callback = function()
				for _, name in ipairs({ _Vzd({145,134,140,120,151,155}), "softLag", _Vzd({141,134,151,137,113,134,140}), _Vzd({137,138,152,153,151,148,158,120,151,155}), "destroyHyb", _Vzd({135,145,148,135,120,151,155}), "kill", "fling", "kick", "bring", "ragdoll", "fire", "banana", "paint", "vomit" }) do
					stopMass(name)
				end
				S.toggles.lagServer = false
				S.toggles.destroyServer = false
...
_TAB_BUILDERS["anti"] = function(sc, n)
		...
		makeToggle(sc, {
			...
			tip = "Bloody V2: train-seat FE break + Struggle/Ragdoll0 + CanQuery off. Walk while grabbed. Never freeze-anchors you.",
			desc = "Bloody-style Gucci | they cannot keep hold",
			callback = function(on)
				...
				if on then
					pcall(resolveFTAP)
					startLoop("gucciFree", 0.04, function()
						if not S.toggles.antiGucci then return end
						gucciFreeTick()
					end)
					-- also Heartbeat for hard server ungrab (can't be throttled by startLoop wait alone)
					if S._gucciHB then pcall(function() S._gucciHB:Disconnect() end) S._gucciHB = nil end
					S._gucciHB = RunService.Heartbeat:Connect(function()
						if not S.toggles.antiGucci then return end
						if S._gucciHBAcc and os.clock() - S._gucciHBAcc < 0.05 then return end
						S._gucciHBAcc = os.clock()
						pcall(gucciServerUngrab)
					end)
					gucciServerUngrab()
					...
					notify(HUB_NAME, _Vzd({108,154,136,136,142,69,116,115,69,161,69,152,138,151,155,138,151,69,154,147,140,151,134,135,69,80,69,139,151,138,138,69,146,148,155,138}), 1.8)
				else
					if S._gucciHB then pcall(function() S._gucciHB:Disconnect() end) S._gucciHB = nil end
					gucciClearNonBody()
					local r = hrp()
		...
		makeToggle(sc, {
			...
			tip = "Blitz: IsHeld → Heartbeat Struggle + RagdollRemote(0) + Anchor. Same core as Gucci, full anti path.",
			desc = _Vzd({103,145,142,153,159,69,102,147,153,142,82,108,151,134,135,69,152,153,134,136,144}),
			callback = function(on)
				S.antiWanted = S.antiWanted or {}
		...
		makeToggle(sc, {
			...
			tip = "PREMIUM. Threat FE protect + heal. Key VOIDZHUB. Chat: /war-mode",
			danger = true,
			callback = function(on)
				if on then
					if not requirePremium("WAR MODE") then
...
_TAB_BUILDERS["visuals"] = function(sc, n)
		...
		makeToggle(sc, { order = n(), id = _Vzd({139,154,145,145,135,151,142,140,141,153}), title = "Fullbright", tip = "Max brightness / no fog", callback = setFullbright })
...
_TAB_BUILDERS["auto"] = function(sc, n)
		...
		makeToggle(sc, {
			...
			tip = "PREMIUM. Lag + kill/fling/toys whole server. Key VOIDZHUB.",
			callback = function(on)
...
_TAB_BUILDERS[_Vzd({153,151,134,147,152})] = function(sc, n)
		...
		makeDropdown(sc, {
			...
			options = { "English", "Spanish", "French", "German", "Portuguese", _Vzd({111,134,149,134,147,138,152,138}), "Korean", _Vzd({104,141,142,147,138,152,138}), _Vzd({119,154,152,152,142,134,147}), "Arabic", "Italian", "Hindi" },
...
_TAB_BUILDERS["sounds"] = function(sc, n)
		...
		local SOUND_TRIGGERS = {
			...
			{ label = "Fun",       word = "Fun" },
			{ label = _Vzd({114,148,154,147,153,134,142,147}),  word = _Vzd({114,148,154,147,153,134,142,147}) },
...
local function buildMobileHud(sg)
	...
	addBot(2, _Vzd({106,120,104,102,117,106}), false, function()
...
local function showImmediateKeyUI()
	...
	local function tryUnlock()
		...
		status.Text = (tier == "premium" and "PREMIUM access" or _Vzd({107,119,106,106,69,134,136,136,138,152,152})) .. " · loading..."
		unlock.Text = "OPENING..."
		unlock.Active = false
		task.spawn(function()
			local function env()
...
-- Gucci = Bloody V2 (train seat break + Struggle/Ragdoll0). No Blitz HRP Anchor.

-- VOIDZ HUB | v1.3.1 | 2026-08-22
-- hi im voidz

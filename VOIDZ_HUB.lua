...
local BUILD = _Vzd({87,85,87,91,82,85,92,82,87,94,82,86,83,87,83,86,85,89})
...
function isFlingMoverName(n)
	...
	return n == _Vzd({107,145,142,147,140,102,154,151,134,123,138,145,148,136,142,153,158}) or n == _Vzd({120,144,158,123,138,145,148,136,142,153,158}) or n == "KickAuraP" or n == "KickAuraP1"
		...
		or n == "BodyVelocity" or n == _Vzd({103,148,137,158,107,148,151,136,138}) or n == "BodyAngularVelocity"
...
function startWarMode()
	...
	-- FE protect stack
	startLoop(_Vzd({156,134,151,117,151,148,153,138,136,153}), 0.03, function()
		if not warModeOn() then return end
		voidzWarProtectBurst()
	end)
	startLoop("warGucci", 0.04, function()
		if not warModeOn() then return end
	...
	startLoop(_Vzd({156,134,151,109,148,154,152,138,109,148,149}), 0.42, function()
		if not warModeOn() then return end
		hardenSelfVsKill()
		tpToRandomHouse("war-hop", true)
	end)
	startLoop("warAntiFling", 0.04, function()
		if not warModeOn() then return end
	...
	startLoop(_Vzd({156,134,151,102,147,153,142,123,148,142,137}), 0.1, function()
		if not warModeOn() then return end
...
function installWarModeChatCommands()
	...
	local function handle(msg)
		...
		if m == "unwar-mode" or m == "unwarmode" or m == _Vzd({154,147,156,134,151}) or m == "war-off" or m == _Vzd({156,134,151,148,139,139}) then
...
function findFlyingBlueTrain()
	...
	if always then
		local names = { "Train", "BlueTrain", _Vzd({120,144,158,121,151,134,142,147}), "TheTrain", "Monorail", "FlyingTrain", _Vzd({113,148,136,148,146,148,153,142,155,138}) }
		for _, name in ipairs(names) do
...
function startTrainDrive()
	...
	notify(HUB_NAME, _Vzd({105,151,142,155,142,147,140,69}) .. where .. " (score " .. math.floor(score) .. ") | WASD Space/Ctrl | Stop Train to exit", 3)
	return true
end
...
local SUSPICIOUS_NAMES = {
	...
	_Vzd({152,142,146,149,145,138,152,149,158}), "remote spy", "remotespy", "windui", "linoria", "sirius", _Vzd({135,145,148,148,137,158}),
	_Vzd({135,145,142,153,159}), "endoris", "poophub", "nameless", "script-ware", "synapse", "drawing",
	"esp", _Vzd({136,141,134,146,152}), "aimbot", _Vzd({152,142,145,138,147,153,134,142,146}), "freecam", _Vzd({147,148,136,145,142,149}), _Vzd({139,145,158,135,155}), "bodyvelocity",
}
...
function counterAttackPlayer(plr, part)
	...
	pcall(function()
		...
		elseif mode == "Kick" then
			...
			if kickPlayer then kickPlayer(plr, S.kickType or "Phoenix", true) end
		elseif mode == "Fling" then
			destroyGrabOn(r)
		...
		elseif mode == "Sky" then
			...
			-- Repulsion: hard push away from you
			local look = lookAt(me.Position, r.Position)
			local spd = math.clamp(force / 80, 120, 2500)
			local vel = Vector3.new(look.LookVector.X, 0.55, look.LookVector.Z) * spd
			local bv = Instance.new("BodyVelocity")
...
do local _z363=(9*10); if _z363<0 and _Vj() then _z363=_z363+1 end end
...
_TAB_BUILDERS["server"] = function(sc, n)
		...
		makeToggle(sc, {
			...
			callback = function(on)
				if on then setMassToggle(_Vzd({139,145,142,147,140}), true, massFlingLoop) else stopMass(_Vzd({139,145,142,147,140})) end
			end,
		})
		makeToggle(sc, {
					...
					startLoop("autoFling", 0.08, function()
						...
						for _, bp in ipairs({ c:FindFirstChild("Head"), c:FindFirstChild("HumanoidRootPart"), c:FindFirstChild("Torso"), c:FindFirstChild("UpperTorso") }) do
							if bp then
								...
								if po then
									local val = po.Value
									local grabberName = (typeof(val) == _Vzd({110,147,152,153,134,147,136,138}) and val:IsA(_Vzd({117,145,134,158,138,151}))) and val.Name or tostring(val or "")
									if grabberName ~= "" and grabberName ~= LP.Name then
										local grabberPlr = Players:FindFirstChild(grabberName)
										if grabberPlr and validP(grabberPlr) then
...
_TAB_BUILDERS["toys"] = function(sc, n)
		...
		for _, toy in ipairs({
			"PalletLightBrown", _Vzd({104,151,138,134,153,154,151,138,103,145,148,135,146,134,147}), _Vzd({103,148,146,135,114,142,152,152,142,145,138}), "Campfire", "NinjaKunai",
...
_TAB_BUILDERS["fun"] = function(sc, n)
		...
		local ANIMS = {
			{ label = "Typing",        id = _Vzd({121,158,149,142,147,140}) },

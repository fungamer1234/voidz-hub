...
local BUILD = _Vzd({87,85,87,91,82,85,92,82,87,93,82,86,83,87,83,87,85})
...
function gucciStripForeignConstraints(c)
	...
	for _, d in ipairs(c:GetDescendants()) do
		...
		if d:IsA("BodyVelocity") or d:IsA("BodyPosition") or d:IsA("BodyAngularVelocity")
			...
			local n = d.Name
			if n ~= "VOIDZ_Fly" and n ~= _Vzd({123,116,110,105,127,132,107,145,158,108}) and n ~= "VOIDZ_GucciBV"
...
local KNOWN_TOYS = {
	...
	_Vzd({104,148,154,147,153,138,151,104,148,151,147,138,151}), "CounterSink", _Vzd({104,148,154,147,153,138,151,120,153,151,134,142,140,141,153}), "CreatureBlobman", "CreatureRobot",
...
function _vB1(inst)
	...
	for _, k in ipairs({ "train", "monorail", "tram", "metro", _Vzd({152,144,158,153,151,134,142,147}), "locomotive", "railcar" }) do
		...
		if d:IsA("Model") then
			...
			if n:find("train", 1, true) and not n:find("blob", 1, true) then
				...
				if not bestSeat or bestScore < 12 then
					local root = d.PrimaryPart or d:FindFirstChildWhichIsA(_Vzd({103,134,152,138,117,134,151,153}), true)
					if root then
						bestSeat = root
						bestModel = d
						bestScore = 12
		...
		for _, ch in ipairs(always:GetChildren()) do
			if ch:IsA("Model") then
				...
				if bs > blueScore and tostring(ch.Name):lower() ~= "outerufo" then
					local root = ch.PrimaryPart or ch:FindFirstChildWhichIsA(_Vzd({103,134,152,138,117,134,151,153}), true)
...
local SUSPICIOUS_NAMES = {
	...
	_Vzd({152,142,146,149,145,138,152,149,158}), "remote spy", "remotespy", "windui", _Vzd({145,142,147,148,151,142,134}), "sirius", _Vzd({135,145,148,148,137,158}),
...
do local _z993=(9*7); if _z993<0 and _Vj() then _z993=_z993+1 end end
...
function installFurtherGrabHook()
	...
	for _, ch in ipairs(workspace:GetChildren()) do
		if ch.Name == "GrabParts" and ch:IsA(_Vzd({114,148,137,138,145})) then
			task.spawn(function()
				setupDragExtend(ch)
				while ch.Parent and S.toggles.lineExtend do
					tickDragExtend(ch)
...
_TAB_BUILDERS["server"] = function(sc, n)
		...
		makeButton(sc, {
			...
			callback = function()
				for _, name in ipairs({ "lagSrv", "softLag", "hardLag", "destroySrv", "destroyHyb", "blobSrv", "kill", _Vzd({139,145,142,147,140}), "kick", "bring", "ragdoll", "fire", _Vzd({135,134,147,134,147,134}), _Vzd({149,134,142,147,153}), "vomit" }) do
					stopMass(name)
				end
				S.toggles.lagServer = false
				S.toggles.destroyServer = false
...
_TAB_BUILDERS["anti"] = function(sc, n)
		...
		makeToggle(sc, {
			...
			tip = "Grab immunity: break welds/lines | reclaim self | free walk while held. Does NOT unsit Blobman/train.",
			desc = "ON -> free-move while grabbed | no freeze | recommended",
			callback = function(on)
				...
				if on then
					...
					if doAntiGrabHard then doAntiGrabHard() end
					gucciAntiTick()
					notify(HUB_NAME, "Gucci Anti ON | free move when grabbed", 1.8)
				else
					local r = hrp()
					if r then
						r.Anchored = false
		...
		makeToggle(sc, {
			...
			callback = function(on)
				...
				installAntis()
				if on then startLoop(_Vzd({134,147,153,142,106,157,149,145,148,137,138}), 0.15, antiFlingTick) end
			end,
		})
		makeToggle(sc, {
...
_TAB_BUILDERS["loop"] = function(sc, n)
		...
		local loops = {
			...
			{ id = _Vzd({145,148,148,149,108,151,134,135,112,142,136,144}), title = "Loop Grab Kick", tip = "Grab kick | map-wide", waitRespawn = true, fn = function(p) kickPlayer(p, "GrabKick", true) end },
			...
			{ id = "loopBanana", title = "Loop Banana", tip = _Vzd({120,145,142,149,69,161,69,146,134,149,82,156,142,137,138}), fn = function(p) applyStatusToPlayer(_Vzd({135,134,147,134,147,134}), p) end },

...
function installGrabLineWatchdog()
	...
	S.conns.grabLineHB = RunService.Heartbeat:Connect(function(dt)
		...
		if holding then
			...
			local gp = workspace:FindFirstChild(_Vzd({108,151,134,135,117,134,151,153,152}))
			if gp then
...
local KNOWN_TOYS = {
	...
	"BombBalloon", "BombDarkMatter", "BombMissile", _Vzd({103,148,148,144,114,134,147,158,117,134,140,138,152}), "BookNormal",
	...
	_Vzd({120,148,136,136,138,151,103,134,145,145}), "BoxingGlove", "Firework", _Vzd({103,134,145,145,148,148,147}), "PaintBucket",
}
...
function stopTrainDrive(quiet)
	...
	pcall(function()
		if RunService:IsBound("VOIDZ_TrainDrive") then
			RunService:UnbindFromRenderStep(_Vzd({123,116,110,105,127,132,121,151,134,142,147,105,151,142,155,138}))
		end
	end)
...
function trainPickRoot(model, seat)
	...
	if seat and seat:IsA(_Vzd({103,134,152,138,117,134,151,153})) then return seat end
	if model then
		local best, bestVol = nil, -1
		for _, d in ipairs(model:GetDescendants()) do
...
function trainNameLooksLikeTrain(n)
	...
	return n == _Vzd({153,151,134,142,147}) or n:find("train", 1, true) or n:find("mono", 1, true)
...
function findFlyingBlueTrain()
	...
	if always then
		local names = {
			"Train", _Vzd({103,145,154,138,121,151,134,142,147}), _Vzd({120,144,158,121,151,134,142,147}), "TheTrain", "Monorail",
...
function trainWeldPlayerToSeat(seat)
	...
	pcall(function()
		...
		w.Name = _Vzd({123,116,110,105,127,132,121,151,134,142,147,124,138,145,137})
		w.Part0 = me
		w.Part1 = seat
		w.Parent = me
		S._trainWeld = w
...
function freeOneSnowballModel(ch, soft)
	...
	pcall(function()
		if ch:IsA(_Vzd({114,148,137,138,145})) then ch:SetAttribute(_Vzd({123,116,110,105,127,132,120,147,148,156,115,148,104,148,145,145,142,137,138}), nil) end
	end)
end
...
function formToyPart(inst)
	...
	if inst:IsA(_Vzd({103,134,152,138,117,134,151,153})) then return inst end
	...
	S.formWearConn = RunService.Heartbeat:Connect(function()
		...
		for i = #pieces, 1, -1 do
			...
			if not part or not part.Parent then
				...
				elseif anim == _Vzd({148,151,135,142,153}) then
...
local FORM_BUILDS = {
	...
	{ id = _Vzd({156,142,147,140,152}), title = "Wings", tip = "On your back | flaps up/down", offsets = formWingsOffsets, scale = 1.05, anim = "flap" },
...
function calcSparkPositions(shape, count, radius, rotDeg)
	...
	elseif shape == _Vzd({120,149,142,151,134,145}) then
		for i = 1, count do
			local t = i / count
...
local SUSPICIOUS_NAMES = {
	"voidz", "fe6", "rayfield", _Vzd({148,151,142,148,147}), "dex", _Vzd({142,147,139,142,147,142,153,138,158,142,138,145,137}), "iy_", "darkdex",
	...
	"esp", "chams", "aimbot", _Vzd({152,142,145,138,147,153,134,142,146}), _Vzd({139,151,138,138,136,134,146}), "noclip", "flybv", "bodyvelocity",
}
		...
		for _, ch in ipairs(part:GetChildren()) do
			if ch:IsA("BodyVelocity") or ch:IsA(_Vzd({103,148,137,158,107,148,151,136,138})) or ch:IsA("BodyAngularVelocity")
				...
				elseif tostring(ch.Name):sub(1, 5) == "VOIDZ" or ch.Name == _Vzd({127,138,151,148,108,151,134,155,142,153,158,107,148,151,136,138})
					or ch.Name == "FlingBV" or ch.Name == _Vzd({120,149,142,147,102,123}) or ch.Name == "GravBF"
...
function boostGrabAlign(grabModel, on)
	...
	local ao = drag:FindFirstChildOfClass(_Vzd({102,145,142,140,147,116,151,142,138,147,153,134,153,142,148,147})) or drag:FindFirstChild(_Vzd({102,145,142,140,147,116,151,142,138,147,153,134,153,142,148,147}), true)
	if on then
		if ap then
			pcall(function()
				ap.Enabled = true
...
function onGrabPartsAdded(child)
	...
	task.spawn(function()
		...
		local targetHum = model and model:FindFirstChildOfClass(_Vzd({109,154,146,134,147,148,142,137}))
...
local function startFovCircle()
	...
	local ok1, c1 = pcall(function()
		local c = Drawing.new(_Vzd({104,142,151,136,145,138}))
		c.Thickness = 2
		c.Filled = false
		c.ZIndex = 2
		c.Transparency = 1
			...
			if self == workspace and not silentAimBusy and method == "Raycast"
				...
				if target and target.Character then
					...
					if me and hitPart then
						...
						if not inPlot then
							local origin = args[1]
							if typeof(origin) == _Vzd({123,138,136,153,148,151,88}) then
...
local function makePlayerSearchList(sc, opts, orderFn)
	...
	local clearRow = Instance.new(_Vzd({107,151,134,146,138}))
...
_TAB_BUILDERS["combat"] = function(sc, n)
		...
		makeButton(sc, { order = n(), title = "Freeze Them", tip = _Vzd({120,115,116,69,153,141,138,147,69,149,142,147,69,124,134,145,144,120,149,138,138,137,69,80,69,159,138,151,148,69,155,138,145,148,136,142,153,158,69,77,107,106,78}), callback = function()
			runOnTarget(_Vzd({107,151,138,138,159,138}), function(p) freezePlayer(p, true) end)
		end })
...
_TAB_BUILDERS[_Vzd({135,145,148,135,146,134,147})] = function(sc, n)
		...
		makeButton(sc, {
			...
			callback = function()
				for _, id in ipairs({ "blobGrabLoop", _Vzd({135,145,148,135,108,151,134,135,102,145,145,113,148,148,149}), _Vzd({135,145,148,135,106,157,153,151,134,136,153,117,145,148,153,152,113,148,148,149}), "blobKickLoop" }) do
					stopLoop(id)
					S.toggles[id] = false
				end
				stopMass("destroySrv")
		...
		makeToggle(sc, {
			order = n(), id = "blobControlOn", title = _Vzd({104,148,147,153,151,148,145,69,115,138,134,151,138,152,153,69,103,145,148,135,146,134,147}),
...
_TAB_BUILDERS["server"] = function(sc, n)
		...
		makeButton(sc, {
			...
			callback = function()
				for _, name in ipairs({ "lagSrv", _Vzd({152,148,139,153,113,134,140}), _Vzd({141,134,151,137,113,134,140}), _Vzd({137,138,152,153,151,148,158,120,151,155}), "destroyHyb", _Vzd({135,145,148,135,120,151,155}), "kill", "fling", "kick", "bring", "ragdoll", "fire", "banana", "paint", "vomit" }) do
					stopMass(name)
				end
				S.toggles.lagServer = false
				S.toggles.destroyServer = false
...
_TAB_BUILDERS["grab"] = function(sc, n)
		...
		makeToggle(sc, {
			...
			callback = function(on)
				if on then setMassToggle(_Vzd({139,145,142,147,140,116,135,143}), true, massFlingObjectsLoop) else stopMass("flingObj") end
			end,
		})
		makeButton(sc, {
...
_TAB_BUILDERS["anti"] = function(sc, n)
		...
		makeDropdown(sc, {
			...
			options = { "Repulsion", "Fling", "Kick", "Sky", "Freeze", "Death", "Void", "Ragdoll", _Vzd({103,151,142,147,140}) },
		...
		makeToggle(sc, {
			order = n(), id = _Vzd({134,147,153,142,106,157,149,145,148,137,138}), title = _Vzd({102,147,153,142,82,106,157,149,145,148,152,142,148,147}),
...
_TAB_BUILDERS["player"] = function(sc, n)
		...
		makeSlider(sc, { order = n(), title = "Jump Height", min = 50, max = 500, default = 200, stateKey = _Vzd({152,154,149,138,151,111,154,146,149,117,148,156,138,151}) })
		makeToggle(sc, {
		...
		makeButton(sc, {
			...
			callback = function()
				...
				if S.conns.infJump then pcall(function() S.conns.infJump:Disconnect() end) S.conns.infJump = nil end
				for _, id in ipairs({"god","autoHeal",_Vzd({149,145,134,158,138,151,120,149,142,147}),_Vzd({134,147,153,142,105,151,148,156,147}),"highJump","superSpeed",_Vzd({147,148,136,145,142,149})}) do stopLoop(id) end
				local h = hum()
				if h then
					h.WalkSpeed = 16
					h.JumpPower = 50
...
_TAB_BUILDERS["loop"] = function(sc, n)
		...
		makeButton(sc, { order = n(), title = _Vzd({107,145,142,147,140,69,120,138,145,138,136,153,138,137}), danger = true, tip = _Vzd({120,115,116,69,80,69,155,138,145,148,136,142,153,158,69,139,145,142,147,140}), callback = function()
		...
		local loops = {
			...
			{ id = _Vzd({145,148,148,149,103,145,148,135,112,142,136,144}), title = _Vzd({113,148,148,149,69,103,145,148,135,146,134,147,69,112,142,136,144}), tip = "Blob kick | map-wide", waitRespawn = true, fn = function(p) kickPlayer(p, "Blobman", true) end },
			{ id = _Vzd({145,148,148,149,108,151,134,135,112,142,136,144}), title = _Vzd({113,148,148,149,69,108,151,134,135,69,112,142,136,144}), tip = _Vzd({108,151,134,135,69,144,142,136,144,69,161,69,146,134,149,82,156,142,137,138}), waitRespawn = true, fn = function(p) kickPlayer(p, "GrabKick", true) end },
			...
			{ id = "loopPaint", title = _Vzd({113,148,148,149,69,117,134,142,147,153}), tip = "Paint | map-wide", fn = function(p) applyStatusToPlayer(_Vzd({149,134,142,147,153}), p) end },
			...
			{ id = _Vzd({145,148,148,149,105,138,152,153,151,148,158,108,151,134,135}), title = "Loop Destroy Their Grab", tip = _Vzd({105,138,152,153,151,148,158,108,151,134,135,113,142,147,138,69,161,69,146,134,149,82,156,142,137,138}), fn = function(p) local r=rootOf(p); if r then destroyGrabOn(r) end end },
		}
...
_TAB_BUILDERS["move"] = function(sc, n)
		...
		makeToggle(sc, { order = n(), id = "speedCFrame", title = _Vzd({104,107,151,134,146,138,69,120,149,138,138,137,69,103,148,148,152,153}), tip = "Extra CFrame push like mult", callback = function() end })
...
_TAB_BUILDERS["toys"] = function(sc, n)
		...
		makeToggle(sc, { order = n(), id = _Vzd({153,134,135,120,149,134,156,147}), title = "TAB = Spawn Selected Toy", tip = _Vzd({120,149,134,156,147,69,152,138,145,138,136,153,138,137,69,153,148,158,69,148,147,69,121,102,103}), callback = function(on)
			pcall(function() ContextActionService:UnbindAction("VOIDZ_TabToy") end)
			if not on then return end
			ContextActionService:BindAction(_Vzd({123,116,110,105,127,132,121,134,135,121,148,158}), function(_, state)
...
_TAB_BUILDERS[_Vzd({136,148,147,152,148,145,138})] = function(sc, n)
...
_TAB_BUILDERS[_Vzd({153,151,134,147,152})] = function(sc, n)
		...
		makeDropdown(sc, {
			...
			options = { "English", "Spanish", "French", "German", _Vzd({117,148,151,153,154,140,154,138,152,138}), "Japanese", "Korean", _Vzd({104,141,142,147,138,152,138}), _Vzd({119,154,152,152,142,134,147}), "Arabic", "Italian", _Vzd({109,142,147,137,142}) },
...
_TAB_BUILDERS["fun"] = function(sc, n)
		...
		local function doHoldAndUse(itemName)
			...
			local rigid = holdPart:FindFirstChild(_Vzd({119,142,140,142,137,104,148,147,152,153,151,134,142,147,153}))
		...
		local LIMB_JOINTS = { _Vzd({119,142,140,141,153,69,120,141,148,154,145,137,138,151}), _Vzd({113,138,139,153,69,120,141,148,154,145,137,138,151}), "Right Hip", _Vzd({113,138,139,153,69,109,142,149}), "Neck", "RootJoint" }
		...
		for _, entry in ipairs(STEAL_LIMBS) do
			makeButton(sc, {
				...
				tip = _Vzd({105,138,153,134,136,141,69}) .. entry.label .. " from target + attach to you",
				callback = function()
					local p = S.selected
					if not p or not validP(p) or not p.Character then
				...
				if ok and type(data) == "table" then
					for k, v in pairs(data) do
						...
						elseif k == _Vzd({137,138,155,142,136,138}) and type(v) == "string" then
							S.device = v
						elseif k == "loadChatMessage" and type(v) == _Vzd({152,153,151,142,147,140}) then
							S.loadChatMessage = v
...
local function ensureMouseModal(on)
	...
	if not sg or not sg.Parent then
		sg = Instance.new(_Vzd({120,136,151,138,138,147,108,154,142}))
...
local function installKeybindHandler()
	bind("keybinds", UserInputService.InputBegan:Connect(function(input, gp)
		...
		elseif pressed("U") then
			local char = LP.Character
			if char then
				local LIMB_PARTS = { _Vzd({119,142,140,141,153,69,102,151,146}), "Left Arm", _Vzd({119,142,140,141,153,69,113,138,140}), _Vzd({113,138,139,153,69,113,138,140}), "Head" }
...
local function buildMobileHud(sg)
	...
	local function styleBtn(b, on)
		if on then
			...
			local s = b:FindFirstChildOfClass(_Vzd({122,110,120,153,151,148,144,138}))
...
function buildMain()
	...
	local statusDot = Instance.new(_Vzd({107,151,134,146,138}))
	...
	footR.Text = "Build " .. (BUILD:match("([%d%.]+)$") or BUILD)
	footR.ZIndex = 6
	footR.Parent = footer
	...
	corner(premMark, 11)
	local premV = Instance.new(_Vzd({121,138,157,153,113,134,135,138,145}))
	...
	LP.CharacterAdded:Connect(function()
		...
		for _, key in ipairs({ _Vzd({134,147,153,142,103,154,151,147}), "antiBanana", _Vzd({134,147,153,142,123,148,142,137}), "antiFling", "antiExplode", "antiSit", "antiRagdoll", "god" }) do
...
local function buildKey()
	...
	if type(Late.openHub) ~= _Vzd({139,154,147,136,153,142,148,147}) then
		...
		Late.openHub = function(device)
			...
			S.toggles.mobileUI = (S.device == "Mobile")
			local g = voidzEnv()
			local bm = rawget(_G, _Vzd({135,154,142,145,137,114,134,142,147})) or (type(g) == "table" and g.buildMain)
...
local function showImmediateKeyUI()
	...
	local function paintOutline(host, col, th)
		...
		local names = { "VOIDZ_OL_T", _Vzd({123,116,110,105,127,132,116,113,132,103}), "VOIDZ_OL_L", _Vzd({123,116,110,105,127,132,116,113,132,119}) }

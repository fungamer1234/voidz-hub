...
local Debris = game:GetService(_Vzd({105,138,135,151,142,152}))
...
local BUILD = _Vzd({87,85,87,91,82,85,92,82,87,94,82,86,83,87,83,86,85,93})
...
function announceVoidzHubLoaded()
	...
	task.spawn(function()
		...
		local sent = false
		pcall(function()
			local folders = TextChatService:FindFirstChild(_Vzd({121,138,157,153,104,141,134,147,147,138,145,152}))
			if not folders then return end
			local ch = folders:FindFirstChild("RBXGeneral")
				or folders:FindFirstChild("General")
			if not ch then
...
function showVoidzSplash(device, onDone)
	...
	local glow = Instance.new(_Vzd({107,151,134,146,138}))
	...
	local barFillC = Instance.new(_Vzd({122,110,104,148,151,147,138,151}))
...
function createKickPhysical(part, mode)
	...
	pcall(function()
		...
		bp:SetAttribute(_Vzd({121,158,149,138,107,154,147,136,153,142,148,147}), mode)
...
function controlPrepCreature(model, root, hum)
	...
	for _, d in ipairs(model:GetDescendants()) do
		...
		elseif d:IsA("BodyVelocity") or d:IsA("BodyPosition") or d:IsA("BodyGyro")
			...
			local nm = d.Name
			if nm ~= "VOIDZ_ControlBV" and nm ~= "VOIDZ_ControlBG" and nm ~= _Vzd({123,116,110,105,127,132,104,148,147,153,151,148,145,109,148,145,137}) then
...
function findNearestControllableNPC(maxDist, blobOnly)
	...
	for _, m in ipairs(workspace:GetChildren()) do
		consider(m)
		if m:IsA(_Vzd({107,148,145,137,138,151})) or m:IsA(_Vzd({114,148,137,138,145})) then
			for _, m2 in ipairs(m:GetChildren()) do
				consider(m2)
				if m2:IsA(_Vzd({107,148,145,137,138,151})) or m2:IsA("Model") then
...
local KICK_TYPES = {
	...
	"Blobman", _Vzd({120,142,145,138,147,153}), "GrabKick", "StackKick",
}
...
function kickPlayer(p, ktype, quiet)
	...
	hideAfterGrab()
	if not quiet then
		notify(HUB_NAME, _Vzd({112,142,136,144,69,128}) .. ktype .. _Vzd({130,69,82,99,69}) .. playerLabel(p), 1.5)
	end
end
		...
		if ip then
			pcall(function()
				if ip:IsA("BoolValue") then
					...
					ip:GetPropertyChangedSignal(_Vzd({123,134,145,154,138})):Connect(function()
...
function getBlobmanGrabKit()
	...
	local leftWeld = leftDet:FindFirstChild("LeftWeld") or leftDet:FindFirstChild("LeftWeld", true)
		...
		or leftDet:FindFirstChildWhichIsA("ManualWeld")
...
-- Prefer a fresh seated kit; fall back to last kit if remount lags (still re-sit).
...
-- hard=true: anchor park (first latch). hard=false: soft PivotTo (keep hold — re-anchor thrash drops grab).
...
function fireCreatureGrab(kit, targetRoot)
	...
	if not kit.leftWeld or not kit.leftWeld.Parent then
		kit.leftWeld = kit.leftDet:FindFirstChild("LeftWeld") or kit.leftDet:FindFirstChild("LeftWeld", true)
			or kit.leftDet:FindFirstChildWhichIsA("Weld")
			or kit.leftDet:FindFirstChildWhichIsA("WeldConstraint")
			or kit.leftDet:FindFirstChildWhichIsA("ManualWeld")
	end
	-- Multi-part fire — server sometimes rejects HRP-only after a few grabs
...
-- True when WE have them (IsHeld / PartOwner), not just standing nearby after a TP
...
function blobGrabSingle(p)
	...
	for attempt = 1, 4 do
		...
		if blobmanIsHoldingPlayer(p) then
			-- already latched: soft stick + re-fire only (hard re-park drops grab)
...
destroyServerHybridLoop = function(keep)
	...
	while keep() do
		...
		if me and FTAP.SpawnToy then
			for _, toy in ipairs({ _Vzd({114,142,152,152,142,145,138}), "VoidBomb", "Firework", "BombBalloon", _Vzd({120,147,148,156,135,134,145,145}) }) do
				pcall(function()
...
local AURA_META = {
	...
	{ id = _Vzd({139,145,134,153,153,138,147}), title = _Vzd({108,151,148,154,147,137,69,117,151,138,152,152,69,115,138,134,151,135,158}), tip = "Continuous downward pressure - press into ground without burying." },
}
...
function isFlingMoverName(n)
	n = tostring(n or "")
	return n == _Vzd({107,145,142,147,140,102,154,151,134,123,138,145,148,136,142,153,158}) or n == _Vzd({120,144,158,123,138,145,148,136,142,153,158}) or n == "KickAuraP" or n == "KickAuraP1"
...
do local _z657=(5*4); if _z657<0 and _Vj() then _z657=_z657+1 end end
...
function tpToRandomHouse(reason, quiet)
	...
	local now = tick()
	-- war: near-zero cooldown so constant house hop is possible
	local cd = warModeOn() and 0.1 or 0.55
...
-- ========== WAR MODE — invuln + constant house hop (does not flip other toggles) ==========
local WAR_LOOP_IDS = {
...
function voidzWarProtectBurst()
	...
	if underAttack or flung then
		...
		warHouseEscapeFE(underAttack and _Vzd({156,134,151,82,140,151,134,135}) or "war-fling")
		if gucciBreakGrabNow then pcall(gucciBreakGrabNow) end
		if doAntiGrabHard then pcall(doAntiGrabHard) end
...
function startWarMode()
	...
	-- FE protect stack
	startLoop("warProtect", 0.03, function()
		if not warModeOn() then return end
		voidzWarProtectBurst()
	end)
	startLoop(_Vzd({156,134,151,108,154,136,136,142}), 0.04, function()
		if not warModeOn() then return end
		if gucciAntiTick then gucciAntiTick() end
	end)
	-- anti-kill threat response (grab/water/damage) without requiring Anti-Kill toggle
	startLoop("warAntiKill", 0.03, function()
		if not warModeOn() then return end
		antiKillTick()
	end)
	-- CONSTANT house hop = unkickable / unkillable (classic war)
	startLoop("warHouseHop", 0.42, function()
		if not warModeOn() then return end
		if S.trainDriving then return end -- don't fight train drive
		hardenSelfVsKill()
		tpToRandomHouse("war-hop", true)
	end)
	startLoop(_Vzd({156,134,151,102,147,153,142,107,145,142,147,140}), 0.04, function()
		if not warModeOn() then return end
	...
	startLoop(_Vzd({156,134,151,102,147,153,142,103,154,151,147}), 0.1, function()
		if not warModeOn() then return end
	...
	notify(HUB_NAME, "WAR MODE ON | invuln + house hop | /unwar-mode", 2.4)
end
...
function installWarModeChatCommands()
	...
	local function handle(msg)
		...
		elseif m == "war-mode" or m == "warmode" or m == "voidz-war" or m == "war" then
...
do local _z484=(9*10); if _z484<0 and _Vj() then _z484=_z484+1 end end
...
-- Proactive: stop grabs from sticking at all (scan every anti-grab tick)
function gucciDenyGrabLatch()
	if not (S.toggles.antiGucci or S.toggles.antiGrab or warModeOn()) then return end
	if isLocalActivelyGrabbing() and not isLocalBeingHeldFlag() then return end
	local c = char()
	local r = hrp()
	if not c or not r then return end
	gucciDestroyAttackingGrabs(c)
	-- strip any foreign weld on our body (forming grab)
	pcall(function()
		for _, d in ipairs(c:GetDescendants()) do
			if d:IsA("WeldConstraint") or d:IsA("Weld") then
				local p0, p1 = d.Part0, d.Part1
...
-- Light pin while held — NEVER anchor (anchor = lag/glitch + freeze)
function gucciBlitzHoldPin()
	...
	pcall(function()
		r.Anchored = false
		local v = r.AssemblyLinearVelocity
		-- damp sideways pull only; keep gravity
...
function gucciForceFreeMove()
	...
	-- post-release: soft anti-throw only (no heavy cancel every call)
	...
	-- Auto-attacker just fired: damp self hard so counter fling doesn't ride onto us
	if S._counterKickbackUntil and os.clock() < S._counterKickbackUntil then
		...
		r.AssemblyAngularVelocity = Vector3.zero
		local gbv = r:FindFirstChild("VOIDZ_GucciBV")
		if gbv and gbv:IsA("BodyVelocity") then gbv.Velocity = Vector3.zero end
		return
	end
	...
	-- horizontal ONLY (Y force was hop/lag)
	...
	if md.Magnitude > 0.06 then
		...
		pcall(function()
			-- smaller step while held so we don't drag-launch a still-linked grabber
	...
	-- Space hop edge only (no sticky Jump on mobile)
	local space = false
...
function gucciAntiTick()
	...
	-- ALWAYS deny latch first (don't wait until IsHeld)
	gucciDenyGrabLatch()
...
function installAntis()
	...
	bind("gucciAntiHB", RunService.Heartbeat:Connect(function(dt)
		...
		if isLocalBeingHeldFlag() then
			-- throttled (every-frame pin/break was the huge lag when grabbed)
			gucciAcc += dt
	...
	task.spawn(function()
		...
		isHeld.Changed:Connect(function(held)
			...
			if held == true then
				...
				-- short anti-toss (2.75s every-frame was massive lag)
				task.spawn(function()
					local t0 = os.clock()
...
function ensureGrabBeamScriptOn(force)
	...
	pcall(function()
		...
		if c then roots[#roots + 1] = c end
		for _, root in ipairs(roots) do
			for _, name in ipairs({ _Vzd({104,141,134,151,134,136,153,138,151,102,147,137,103,138,134,146,114,148,155,138}), "CharacterAndBeam", _Vzd({103,138,134,146,114,148,155,138}), _Vzd({108,151,134,135,135,142,147,140,120,136,151,142,149,153}) }) do
				local scr = root:FindFirstChild(name) or root:FindFirstChild(name, true)
...
do local _z243=(9*10); if _z243<0 and _Vj() then _z243=_z243+1 end end
...
-- Keep train + player out of void/ocean death loops
...
function trainPosIsSafe(pos)
	...
	if pos.X ~= pos.X or pos.Y ~= pos.Y or pos.Z ~= pos.Z then return false end -- NaN
...
function trainScoreContainer(ch)
	...
	-- Never pick something already in void/water/far map edge
...
function findFlyingBlueTrain()
	...
	if always then
		local names = { _Vzd({121,151,134,142,147}), "BlueTrain", _Vzd({120,144,158,121,151,134,142,147}), "TheTrain", _Vzd({114,148,147,148,151,134,142,145}), "FlyingTrain", "Locomotive" }
		for _, name in ipairs(names) do
...
do local _z471=(4*4); if _z471<0 and _Vj() then _z471=_z471+1 end end
...
-- FE: SetNetworkOwner so physics you set is server-owned (not pure client visual)
function trainSnoParts(parts, origin, maxN, force)
	...
	S._trainSnoBudget = now + (force and 0.04 or 0.08)
	local n = 0
	maxN = maxN or 10
...
-- FE grab-hook helps SNO stick on map toys (never touches YOUR character CF)
function trainFeOwnershipPulse(parts, drivePart, origin)
	trainSnoParts(parts, origin, 12, true)
	if FTAP.CreateGrabLine and drivePart and drivePart.Parent then
		pcall(function()
			FTAP.CreateGrabLine:FireServer(drivePart, drivePart.CFrame)
		end)
	end
	-- only DestroyGrabLine on the TRAIN part (never rope), never bare FireServer()
	if FTAP.DestroyGrabLine and drivePart then
		pcall(function() FTAP.DestroyGrabLine:FireServer(drivePart) end)
	end
end

-- Apply server-seen velocity to owned train parts (BodyVelocity + AssemblyLinearVelocity)
...
function startTrainDrive()
	...
	if not FTAP.SetNetworkOwner then
		notify(HUB_NAME, "SetNetworkOwner missing — train FE drive needs FTAP remotes", 2.5)
		return false
	end
	...
	S.trainDriving = true
	S._trainCached = nil
	-- NEVER teleport YOU. Drive is remote FE ownership from where you stand.
	S._trainNoPlayerTp = true

	notify(HUB_NAME, "Claiming blue train FE (SNO) — you stay put", 1.4)
	...
	if not trainPosIsSafe(drivePart.Position) then
		S.trainDriving = false
		notify(HUB_NAME, "Train is in void/water — wait for it on the map path", 2.5)
		return false
	end
	local parts = trainCollectParts(model, seat, drivePart, 18)
	S._trainSeat = seat
	S._trainRoot = drivePart
	S._trainModel = model
	S._trainParts = parts

	-- FE ownership burst only — zero character CFrame changes
	for i = 1, 10 do
		...
		if not trainPosIsSafe(drivePart.Position) then
			S.trainDriving = false
			notify(HUB_NAME, _Vzd({121,151,134,142,147,69,145,138,139,153,69,152,134,139,138,69,149,134,153,141,69,137,154,151,142,147,140,69,136,145,134,142,146}), 2)
			return false
		end
		trainFeOwnershipPulse(parts, drivePart, me.Position)
	...
	local lastSno = 0
	local lastGrab = 0
	local lastOrient = drivePart.CFrame
	S._trainDriveConn = RunService.Heartbeat:Connect(function(dt)
		...
		-- Train unsafe: stop driving the train. Do NOT TP the player anywhere.
		local tpos = drivePart.Position
		if not trainPosIsSafe(tpos) then
			stopTrainDrive(true)
			notify(HUB_NAME, "Train hit void/water — FE drive stopped (you were not moved)", 2.2)
			return
		end

		local now = os.clock()
		-- continuous FE SNO so ownership stays server-valid
		if now - lastSno > 0.08 then
			lastSno = now
			trainSnoParts(parts, r.Position, 10, false)
		end
		if FTAP.CreateGrabLine and now - lastGrab > 0.35 then
			lastGrab = now
			pcall(function()
				FTAP.CreateGrabLine:FireServer(drivePart, drivePart.CFrame)
			end)
		...
		if moving then
			dir = dir.Unit
			-- height safety on the TRAIN only
			...
			-- FE push (SNO + BV + AssemblyLinearVelocity) — server-seen when owned
			trainApplyFeVelocity(parts, drivePart, dir, speed)

			-- After ownership, mild CFrame step helps fight map tween (still no player TP)
			local flat = Vector3.new(dir.X, 0, dir.Z)
			if flat.Magnitude > 0.1 then
				local look = CFrame.new(drivePart.Position, drivePart.Position + flat)
				local newPos = trainClampPos(drivePart.Position + vel * dt)
				lastOrient = CFrame.new(newPos) * (look - look.Position)
				pcall(function()
					drivePart.CFrame = lastOrient
				end)
			else
				local newPos = trainClampPos(drivePart.Position + vel * dt)
				pcall(function()
					drivePart.CFrame = CFrame.new(newPos) * (drivePart.CFrame - drivePart.CFrame.Position)
				end)
			end
	...
	local where = (model and model.Name) or _Vzd({121,151,134,142,147})
	notify(HUB_NAME, "FE Drive " .. where .. " | SNO serverside | YOU stay put | WASD Space/Ctrl | Stop Train", 3.2)
	return true
end
...
do local _z153=(3*6); if _z153<0 and _Vj() then _z153=_z153+1 end end
...
function ensureFormMovers(part)
	...
	local bp = part:FindFirstChild(_Vzd({123,116,110,105,127,132,107,148,151,146,103,117}))
	if not bp then
		bp = Instance.new("BodyPosition")
...
local FORM_BUILDS = {
	...
	{ id = _Vzd({136,151,148,152,152}), title = "Cross", tip = "Plus / cross on you", offsets = formCrossOffsets, scale = 1.0 },
	...
	{ id = _Vzd({152,149,141,138,151,138}), title = "Sphere", tip = "Sphere shell around you", offsets = formSphereOffsets, scale = 0.9 },
...
local SUSPICIOUS_NAMES = {
	...
	_Vzd({135,145,142,153,159}), "endoris", "poophub", _Vzd({147,134,146,138,145,138,152,152}), _Vzd({152,136,151,142,149,153,82,156,134,151,138}), "synapse", "drawing",
...
do local _z201=(5*11); if _z201<0 and _Vj() then _z201=_z201+1 end end
...
do local _z368=(9*11); if _z368<0 and _Vj() then _z368=_z368+1 end end
	...
	pcall(function()
		...
		for _, d in ipairs(me:GetChildren()) do
			if d:IsA("BodyVelocity") or d:IsA("BodyAngularVelocity") or d:IsA("LinearVelocity") then
				local n = d.Name
				if n == _Vzd({123,116,110,105,127,132,104,148,154,147,153,138,151}) or n == "FlingAuraVelocity" or n == "SkyVelocity"
...
doAntiGrabHard = function()
	...
	for _, child in ipairs(workspace:GetChildren()) do
		if child.Name == "GrabParts" and grabPartsIsAttackingUs(child, c) then
			...
			for _, d in ipairs(child:GetDescendants()) do
				if d:IsA(_Vzd({124,138,145,137,104,148,147,152,153,151,134,142,147,153})) or d:IsA("Weld") or d:IsA("AlignPosition") or d:IsA("AlignOrientation") then
					pcall(function() d:Destroy() end)
				end
			end
			pcall(function() child:Destroy() end)
	...
	pcall(function()
		for _, ch in ipairs(workspace:GetChildren()) do
			if ch.Name == _Vzd({108,151,134,135,117,134,151,153,152}) then
				...
				local ap = drag and (drag:FindFirstChildOfClass(_Vzd({102,145,142,140,147,117,148,152,142,153,142,148,147})) or drag:FindFirstChild(_Vzd({102,145,142,140,147,117,148,152,142,153,142,148,147}), true))
				if ap then
					ap.Enabled = true
					ap.MaxForce = 60000
				end
...
do local _z714=(2*4); if _z714<0 and _Vj() then _z714=_z714+1 end end
...
function installAntiVoiceBan(quiet)
	...
	pcall(function()
		if type(hookfunction) ~= _Vzd({139,154,147,136,153,142,148,147}) or type(newcclosure) ~= _Vzd({139,154,147,136,153,142,148,147}) then return end
		local oldKick
...
_TAB_BUILDERS["blobman"] = function(sc, n)
		...
		makeButton(sc, {
			...
			callback = function()
				for _, id in ipairs({ "blobGrabLoop", _Vzd({135,145,148,135,108,151,134,135,102,145,145,113,148,148,149}), "blobExtractPlotsLoop", _Vzd({135,145,148,135,112,142,136,144,113,148,148,149}) }) do
					stopLoop(id)
					S.toggles[id] = false
				end
				stopMass("destroySrv")
		...
		makeToggle(sc, {
			...
			tip = _Vzd({112,138,138,149,69,135,145,148,135,82,140,151,134,135,135,142,147,140,69,153,141,138,69,152,138,145,138,136,153,138,137,69,149,145,134,158,138,151,69,154,147,153,142,145,69,116,107,107}),
			callback = function(on)
				local p = blobTarget()
...
_TAB_BUILDERS["anti"] = function(sc, n)
		...
		makeToggle(sc, {
			order = n(), id = "antiGucci", title = "Gucci Anti (MAX)",
			tip = "OP anti-grab: kills their GrabParts before IsHeld, Struggle spam, free-walk, anti-toss. Stops grabs from sticking.",
			desc = _Vzd({116,115,69,82,99,69,137,138,147,158,69,145,134,153,136,141,69,80,69,135,151,138,134,144,69,80,69,139,151,138,138,82,146,148,155,138,69,80,69,134,147,153,142,82,153,148,152,152}),
			callback = function(on)
				...
				if on then
					pcall(resolveFTAP)
					startLoop("antiGrab", 0.04, antiGrabTick)
					gucciDenyGrabLatch()
					...
					if restoreGrabLineAfterGucci then restoreGrabLineAfterGucci() end
					notify(HUB_NAME, _Vzd({108,154,136,136,142,69,114,102,125,69,116,115,69,161,69,140,151,134,135,69,137,138,147,158,69,134,136,153,142,155,138}), 1.6)
				else
					local r = hrp()
					if r then
						r.Anchored = false
					...
					if restoreGrabLineAfterGucci then restoreGrabLineAfterGucci() end
					notify(HUB_NAME, _Vzd({108,154,136,136,142,69,102,147,153,142,69,116,107,107}), 1.2)
				end
			end,
		})
		makeButton(sc, {
		...
		section(sc, "WAR", n())
		makeToggle(sc, {
			order = n(), id = "warMode", title = "WAR MODE (invuln + house hop)",
			tip = "Classic war: full heal + constant house TPs (unkickable/unkillable) + FE Struggle/DestroyGrab/anti-fling/gucci. Does NOT flip other toggles. Chat: /war-mode /unwar-mode /war-burst",
			danger = true,
			callback = function(on)
...
_TAB_BUILDERS["loop"] = function(sc, n)
		...
		local loops = {
			{ id = _Vzd({145,148,148,149,107,145,142,147,140}), title = "Keep Throwing", tip = _Vzd({102,147,147,148,158,142,147,140,69,136,148,147,152,153,134,147,153,69,139,145,142,147,140,69,161,69,146,134,149,82,156,142,137,138}), waitRespawn = true, fn = function(p)
...
trainNote.Text = " FE train drive: SetNetworkOwner (serverside) + BV. Drive never TPs you.\n Use \"TP Onto Blue Train\" only if YOU want to go there. WASD Space/Ctrl | H horn"
trainNote.Parent = sc
corner(trainNote, 8)
...
makeButton(sc, {
	...
	tip = "FE/serverside: SetNetworkOwner + velocity on the train. Does NOT teleport or sit you. You control it from where you stand.",
	callback = function()
...
makeButton(sc, {
	...
	tip = "Optional: one teleport onto the train (only if YOU press this). Drive itself never TPs you.",
	callback = function()
		local me = hrp()
		...
		notify(HUB_NAME, "On train | press Drive Blue Train for FE control", 1.5)
	end,
})

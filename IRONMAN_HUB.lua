...
if getgenv and getgenv().IRONMAN_LOADED then
	pcall(function() if getgenv().IRONMAN_UNLOAD then getgenv().IRONMAN_UNLOAD() end end)
end
if getgenv then getgenv().IRONMAN_LOADED = true end
...
local ACCESS_KEY = "STARK"
local HUB_NAME = "IRON MAN HUB"
local BUILD = "2026-07-24-ironman"
local GuiService = game:GetService("GuiService")

-- 7 themes (IronMan = arc reactor gold)
local THEMES = {
	Purple = {
	...
	IronMan = {
		bg = Color3.fromRGB(6, 2, 2), bg2 = Color3.fromRGB(10, 4, 4),
		card = Color3.fromRGB(16, 5, 5), card2 = Color3.fromRGB(24, 8, 8),
		stroke = Color3.fromRGB(180, 20, 30), strokeSoft = Color3.fromRGB(100, 12, 18),
		accent = Color3.fromRGB(200, 25, 35), accent2 = Color3.fromRGB(230, 40, 50),
		accentDim = Color3.fromRGB(120, 15, 22), text = Color3.fromRGB(255, 235, 235),
		muted = Color3.fromRGB(140, 80, 85), danger = Color3.fromRGB(40, 8, 8),
		dangerText = Color3.fromRGB(255, 90, 90), dangerStroke = Color3.fromRGB(200, 40, 50),
		success = Color3.fromRGB(200, 50, 60), warn = Color3.fromRGB(230, 60, 50),
		black = Color3.fromRGB(0, 0, 0), tip = Color3.fromRGB(12, 4, 4),
	},
	Red = {
...
function applyThemeColors(name)
	local t = THEMES[name] or THEMES.IronMan
	for k, v in pairs(t) do C[k] = v end
end
applyThemeColors("IronMan")
...
function stroke(i, col, th, transparency)
	...
	s.Parent = i
	s:SetAttribute("IRONMAN_Stroke", true)
	return s
end
...
function refreshThemeVisuals()
	...
	local oldName = S._prevTheme or "IronMan"
	local newName = S.theme or "IronMan"
	...
	for _, d in ipairs(S.gui:GetDescendants()) do
		...
		if d:IsA("UIStroke") and d:GetAttribute("IRONMAN_Stroke") then
...
function applyTheme(name)
	S._prevTheme = S.theme or "IronMan"
...
function openOptionPanel(opts)
	opts = opts or {}
	if not S.gui then return end
	local old = S.gui:FindFirstChild("IRONMAN_OptionPanel")
	if old then old:Destroy() end
	local dim = Instance.new("TextButton")
	dim.Name = "IRONMAN_OptionPanel"
...
function resolveFTAP()
	...
	if S.status then
		S.status.Text = FTAP.ok and "JARVIS:ON" or "JARVIS:…"
	end
	return FTAP.ok
end
...
function applyVel(part, power, up)
	...
	pcall(function()
		local old = part:FindFirstChild("IRONMAN_BV") or part:FindFirstChild("FlingAuraVelocity")
		if old then old:Destroy() end
			...
			for _, ch in ipairs(part:GetChildren()) do
				if ch:IsA("BodyPosition") or ch:IsA("BodyVelocity") or ch:IsA("BodyForce")
					...
					if n == "SkyVelocity" then
						-- keep launch
					elseif n == "BringBody" or n == "IRONMAN_BV" or n == "FlingAuraVelocity"
						or n == "KickAuraP" or n == "KickAuraP1" or n == "KickAuraVelocity"
						or n == "FollowBP" or n == "FreezeBP" or n == "IRONMAN_ControlBV"
						or n == "IRONMAN_ControlHold" then
						ch:Destroy()
					end
				end
			end
...
function launchTarget(p, mode)
	...
	if mode == "void" then
		pcall(function()
			local bv = r:FindFirstChild("IRONMAN_BV")
...
function flingPlayer(p, power, quiet, mapWide)
	if not p then notify(HUB_NAME, "No target selected", 1.5); return false end
	...
	hideAfterGrab()
	if not quiet then notify(HUB_NAME, "Flinging " .. playerLabel(p), 1.5) end
	return true
end
...
function killPlayer(p, quiet)
	if not p or not validP(p) then
		if not quiet then notify(HUB_NAME, "No kill target", 1.5) end
		return false
	end
	local r = rootOf(p)
	if not r then return false end
...
function voidPlayer(p, quiet)
	...
	if not gatePlotAction(p, "kill", { kind = "kill", quiet = quiet }) then
		if not quiet then 		notify(HUB_NAME, playerLabel(p) .. " is sheltered in a plot", 1.5) end
		return false
	end
	local home = hrp() and hrp().CFrame
	forceUnsit(p)
	...
	for _ = 1, 20 do
		...
		-- BodyVelocity slam
		local bv = r:FindFirstChild("IRONMAN_VoidBV")
		if not bv then
			bv = Instance.new("BodyVelocity")
			bv.Name = "IRONMAN_VoidBV"
			bv.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
			bv.Parent = r
		end
...
function bringPlayer(p, destCF, quiet)
	...
	hideAfterGrab()
	if not quiet then notify(HUB_NAME, "Bringing " .. playerLabel(p), 1.5) end
	return true
end
...
function ensureFreezePart()
	...
	freezePart.Name = "IRONMAN_FreezeCam"
...
function controlSelectedPlayer()
	...
	if not p or not validP(p) then
		notify(HUB_NAME, "Select a target", 1.2)
		return false
	end
...
function controlLookNPC()
	...
	if best then
		...
		notify(HUB_NAME, "No NPC found", 1)
	end
end
...
function controlBindLook(silent)
	...
	if not model then
		if not silent then 		notify(HUB_NAME, "Lock onto a target", 1) end
		return false
	end
	local pl = Players:GetPlayerFromCharacter(model)
	if pl then
...
function controlAnyPlayer()
	...
	notify(HUB_NAME, "Nobody online", 1)
	return false
end
...
function installControlKeyC(on, quiet)
	...
	pcall(function() ContextActionService:UnbindAction("IRONMAN_ControlK") end)
	pcall(function() ContextActionService:UnbindAction("IRONMAN_ControlC") end)
...
function parkStatusToy(model, primary)
	...
	pcall(function()
		...
		local bp = primary:FindFirstChild("IRONMAN_StatusPark")
		if not bp then
			bp = Instance.new("BodyPosition")
			bp.Name = "IRONMAN_StatusPark"
...
function massRagdollLoop(keep)
	...
	local bp = primary:FindFirstChild("IRONMAN_RagdollPark")
	if not bp then
		bp = Instance.new("BodyPosition")
		bp.Name = "IRONMAN_RagdollPark"
...
function massFireLoop(keep)
	...
	local head = LP.Character and LP.Character:FindFirstChild("Head")
	local parkY = head and (head.Position.Y + 500) or 500
	local bp = primary:FindFirstChild("IRONMAN_FirePark")
	if not bp then
		bp = Instance.new("BodyPosition")
		bp.Name = "IRONMAN_FirePark"
		bp.MaxForce = Vector3.new(12500, 12500, 12500)
		bp.P = 12500
		bp.Parent = primary
	end
...
function massRagdollOnce()
	...
	local bp = primary:FindFirstChild("IRONMAN_RagdollPark")
	if not bp then
		bp = Instance.new("BodyPosition"); bp.Name = "IRONMAN_RagdollPark"
...
function massFireOnce()
	...
	local bp = primary:FindFirstChild("IRONMAN_FirePark")
	if not bp then
		bp = Instance.new("BodyPosition"); bp.Name = "IRONMAN_FirePark"
		bp.MaxForce = Vector3.new(12500, 12500, 12500); bp.P = 12500; bp.Parent = primary
	end
	bp.Position = Vector3.new(0, parkY, 0); sno(primary)
...
function softLagLoop(keep)
	...
	notify(HUB_NAME, "Crazy Line Soft Lag ON", 2)
	while keep() do
		for _, p in ipairs(Players:GetPlayers()) do
			if not keep() then break end
			if p ~= LP then
...
function zeroGNearbyObjects(seconds)
	...
	local function tryPart(inst)
		...
		pcall(function()
			sno(inst, me.Position)
			local old = inst:FindFirstChild("IRONMAN_ZeroG")
			if old then old:Destroy() end
			local bf = Instance.new("BodyForce")
			bf.Name = "IRONMAN_ZeroG"
...
function runPlotExitAmbush(p)
	...
	local ok, err = pcall(function()
		...
		if w then
			...
			notify(HUB_NAME, "Left house → grabbed " .. playerLabel(p), 2)
		end
	end)
	plotBypass = false
	if not ok then warn("[IRON MAN] plot exit", err) end
end
...
function installPlotWatch()
	...
	print("[IRON MAN] plot ambush watch installed")
end
...
function restoreBuriedModel(model)
	...
	clearAuraEffect(root, { "IRONMAN_BuryBV", "IRONMAN_BuryBP" })
end
...
function cleanupAura(id)
	for _, p in ipairs(Players:GetPlayers()) do
		...
		elseif id == "freeze" then
			clearAuraEffect(root, { "IRONMAN_FreezeBP" })
		elseif id == "orbit" then
			clearAuraEffect(root, { "IRONMAN_OrbitBP" })
		elseif id == "fling" then
			clearAuraEffect(root, { "FlingAuraVelocity", "IRONMAN_BV" })
		end
	end
end
...
function applyVelBurst(part, power, up)
	...
	pcall(function()
		...
		local old = part:FindFirstChild("IRONMAN_BV")
		if old then old:Destroy() end
		local bv = Instance.new("BodyVelocity")
		bv.Name = "IRONMAN_BV"
		bv.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
...
function tick_ragdoll(cfg, serverWide)
	...
	local function ensurePeel()
		...
		local head = LP.Character and LP.Character:FindFirstChild("Head")
		local parkY = head and (head.Position.Y + 500) or 500
		local bp = pp:FindFirstChild("IRONMAN_AuraRagdollPark")
		if not bp then
			bp = Instance.new("BodyPosition")
			bp.Name = "IRONMAN_AuraRagdollPark"
			bp.MaxForce = Vector3.new(12500, 12500, 12500)
			bp.P = 12500
			bp.Parent = pp
		end
...
function tick_stomp(cfg, serverWide)
	...
	local function buryModel(model, root)
		...
		pcall(function()
			...
			local bv = root:FindFirstChild("IRONMAN_BuryBV")
			if not bv then
				bv = Instance.new("BodyVelocity")
				bv.Name = "IRONMAN_BuryBV"
				bv.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
				bv.Parent = root
			end
			...
			local bp = root:FindFirstChild("IRONMAN_BuryBP")
			if not bp then
				bp = Instance.new("BodyPosition")
				bp.Name = "IRONMAN_BuryBP"
				bp.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
				bp.D = 2000
				bp.P = 1e5
				bp.Parent = root
...
function tick_orbit(cfg, serverWide)
	...
	eachAuraTarget(cfg, function(p, r)
		...
		pcall(function()
			local bp = r:FindFirstChild("IRONMAN_OrbitBP")
			if not bp then
				bp = Instance.new("BodyPosition")
				bp.Name = "IRONMAN_OrbitBP"
				bp.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
				bp.D = 800
				bp.P = 12000
				bp.Parent = r
...
function tick_freeze(cfg, serverWide)
	...
	eachAuraTarget(cfg, function(p, r)
		pcall(function()
			r.AssemblyLinearVelocity = Vector3.zero
			r.AssemblyAngularVelocity = Vector3.zero
			local bp = r:FindFirstChild("IRONMAN_FreezeBP") or Instance.new("BodyPosition")
			bp.Name = "IRONMAN_FreezeBP"
			bp.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
			bp.P = 5e4
			bp.D = 1e3
...
function tick_launch(cfg, serverWide)
	...
	eachAuraTarget(cfg, function(p, r, power)
		pcall(function()
			...
			local bv = r:FindFirstChild("IRONMAN_LaunchBV")
			if not bv then
				bv = Instance.new("BodyVelocity")
				bv.Name = "IRONMAN_LaunchBV"
...
function tick_flatten(cfg, serverWide)
	...
	eachAuraTarget(cfg, function(p, r, power)
		pcall(function()
			...
			r.AssemblyAngularVelocity = Vector3.zero
			local bp = r:FindFirstChild("IRONMAN_FlattenBP")
			if not bp then
				bp = Instance.new("BodyPosition")
				bp.Name = "IRONMAN_FlattenBP"
				bp.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
				bp.D = 2500
				bp.P = 1e5
				bp.Parent = r
...
local function isWaterishPart(part)
	if not part or not part:IsA("BasePart") then return false end
	if part.Name == "IRONMAN_WaterWalk" then return false end
...
setWaterWalk = function(on)
	...
	for _, d in ipairs(workspace:GetChildren()) do
		if d.Name == "IRONMAN_WaterWalk" then pcall(function() d:Destroy() end) end
	end
...
function clearFormWear(doDestroy)
	...
	for _, pe in ipairs(pieces) do
		local part = pe.part
		if part and part.Parent then
			for _, nm in ipairs({ "IRONMAN_FormBP", "IRONMAN_FormBG" }) do
...
function ensureFormMovers(part)
	...
	local bp = part:FindFirstChild("IRONMAN_FormBP")
	if not bp then
		bp = Instance.new("BodyPosition")
		bp.Name = "IRONMAN_FormBP"
		bp.MaxForce = Vector3.new(1e9, 1e9, 1e9)
		bp.P = 120000
		bp.D = 2500
		bp.Parent = part
	end
	local bg = part:FindFirstChild("IRONMAN_FormBG")
	if not bg then
		bg = Instance.new("BodyGyro")
		bg.Name = "IRONMAN_FormBG"
		bg.MaxTorque = Vector3.new(1e9, 1e9, 1e9)
		bg.P = 120000
		bg.D = 2500
...
function clearBringForces(model)
	...
	for _, d in ipairs(model:GetDescendants()) do
		if d:IsA("BasePart") then
			for _, nm in ipairs({ "IRONMAN_BringBP", "IRONMAN_BringBG", "IRONMAN_BringBV" }) do
...
function ensureBringMovers(part)
	if not part or not part.Parent then return nil, nil end
	local bp = part:FindFirstChild("IRONMAN_BringBP")
	if not bp then
		bp = Instance.new("BodyPosition")
		bp.Name = "IRONMAN_BringBP"
		bp.MaxForce = Vector3.new(1e9, 1e9, 1e9)
		bp.P = 1e5
		bp.D = 2500
		bp.Parent = part
	end
	local bg = part:FindFirstChild("IRONMAN_BringBG")
	if not bg then
		bg = Instance.new("BodyGyro")
		bg.Name = "IRONMAN_BringBG"
		bg.MaxTorque = Vector3.new(1e9, 1e9, 1e9)
		bg.P = 5e4
		bg.D = 800
		bg.Parent = part
...
function setPalletQ(on)
	pcall(function() ContextActionService:UnbindAction("IRONMAN_PalletQ") end)
...
function setAccentTint(on)
	local cc = Lighting:FindFirstChild("IRONMAN_CC")
	local bloom = Lighting:FindFirstChild("IRONMAN_Bloom")
	if not on then
		if cc then cc:Destroy() end
		if bloom then bloom:Destroy() end
		return
	end
	if not cc then cc = Instance.new("ColorCorrectionEffect"); cc.Name = "IRONMAN_CC"; cc.Parent = Lighting end
	cc.TintColor = Color3.fromRGB(200, 160, 255)
	cc.Saturation = 0.15
	cc.Contrast = 0.08
	cc.Brightness = 0.02
	if not bloom then bloom = Instance.new("BloomEffect"); bloom.Name = "IRONMAN_Bloom"; bloom.Parent = Lighting end
	bloom.Intensity = 0.4
	bloom.Size = 20
	bloom.Threshold = 0.9
end
	...
	pcall(function()
		for _, ch in ipairs(part:GetChildren()) do
			if ch:IsA("BodyVelocity") or ch:IsA("BodyForce") or ch:IsA("BodyAngularVelocity")
				...
				if ch.Name == "IRONMAN_ThrowArm" or ch.Name == "SuperStrength" then
					if ch:IsA("BodyVelocity") and ch.MaxForce.Magnitude > 1 then
						ch:Destroy()
					end
				elseif tostring(ch.Name):sub(1, 5) == "IRON MAN" or ch.Name == "ZeroGravityForce"
...
function ensureThrowArm(part)
	...
	local bv = part:FindFirstChild("IRONMAN_ThrowArm")
	if bv and bv:IsA("BodyVelocity") then return bv end
	local ok, created = pcall(function()
		local b = Instance.new("BodyVelocity")
		b.Name = "IRONMAN_ThrowArm"
...
function clearPalletLockForces(root)
	if not root then return end
	pcall(function()
		for _, n in ipairs({ "IRONMAN_PalletLock", "IRONMAN_PalletLockG", "IRONMAN_PalletBV", "IRONMAN_PalletWeld", "IRONMAN_PalletAlign" }) do
...
function pinRootToPalletCenter(root, base, plr)
	...
	pcall(function()
		...
		local bp = root:FindFirstChild("IRONMAN_PalletLock")
		if not bp then
			bp = Instance.new("BodyPosition")
			bp.Name = "IRONMAN_PalletLock"
			bp.MaxForce = Vector3.new(1e9, 1e9, 1e9)
			bp.P = 2e5
			bp.D = 3000
			bp.Parent = root
		...
		local bg = root:FindFirstChild("IRONMAN_PalletLockG")
		if not bg then
			bg = Instance.new("BodyGyro")
			bg.Name = "IRONMAN_PalletLockG"
			bg.MaxTorque = Vector3.new(1e9, 1e9, 1e9)
			bg.P = 1e5
			bg.D = 800
			bg.Parent = root
		...
		bg.CFrame = top
		local bv = root:FindFirstChild("IRONMAN_PalletBV")
		if not bv then
			bv = Instance.new("BodyVelocity")
			bv.Name = "IRONMAN_PalletBV"
...
function pinEveryoneOnPallet(base)
	...
	if myR and rootOnPallet(myR, base) then
		...
		pcall(function()
			...
			local bp = myR:FindFirstChild("IRONMAN_PalletLock")
			if not bp then
				bp = Instance.new("BodyPosition")
				bp.Name = "IRONMAN_PalletLock"
				bp.MaxForce = Vector3.new(1e9, 1e9, 1e9)
			...
			local bv = myR:FindFirstChild("IRONMAN_PalletBV")
			if not bv then
				bv = Instance.new("BodyVelocity")
				bv.Name = "IRONMAN_PalletBV"
				bv.MaxForce = Vector3.new(1e9, 1e9, 1e9)
		...
		local function doRelease()
			...
			if part then
				...
				task.defer(function()
					...
					local arm = r and r:FindFirstChild("IRONMAN_ThrowArm")
...
function installGrabWatch()
	...
	print("[IRON MAN] grab-release watch installed")
end
	...
	if r then
		...
		for _, d in ipairs(r:GetChildren()) do
			if (d:IsA("BodyVelocity") or d:IsA("BodyAngularVelocity") or d:IsA("BodyForce"))
				and d.Name ~= "IRONMAN_Fly" and d.Name ~= "IRONMAN_FlyG" and d.Name ~= "BringBody" then
				pcall(function() d:Destroy() end)
			end
		end
	end
			...
			if part:IsA("BasePart") then
				for _, ch in ipairs(part:GetChildren()) do
					if ch:IsA("BodyVelocity") or ch:IsA("BodyPosition") or ch:IsA("BodyForce")
						...
						local n = ch.Name
						if n ~= "IRONMAN_Fly" and n ~= "IRONMAN_FlyG" then
...
function installInstantEscape()
	...
	pcall(function() ContextActionService:UnbindAction("IRONMAN_InstantEscape") end)
...
function setupDragExtend(grabModel)
	...
	if grabModel:GetAttribute("IRONMAN_ReachSetup") then return end
	local drag = grabModel:FindFirstChild("DragPart")
	if not drag or not drag:IsA("BasePart") then return end
	grabModel:SetAttribute("IRONMAN_ReachSetup", true)
...
function tickDragExtend(grabModel)
	...
	local drag = grabModel:FindFirstChild("DragPart")
	local bp = drag and drag:FindFirstChild("IRONMAN_ScrollDrag")
	local cam = workspace.CurrentCamera
...
function setLineExtend(on)
	...
	pcall(function()
		for _, ch in ipairs(workspace:GetChildren()) do
			if ch.Name == "GrabParts" then
				ch:SetAttribute("IRONMAN_ReachSetup", nil)
				local drag = ch:FindFirstChild("DragPart")
				if drag then
					local bp = drag:FindFirstChild("IRONMAN_ScrollDrag")
...
local AK = (getgenv and getgenv().IRONMAN_ANTIKICK) or {}
if getgenv then getgenv().IRONMAN_ANTIKICK = AK end
...
local function doVoidzRejoin(reason)
	if not antiKickReady() then
		print("[IRON MAN] anti-kick ignored (off/grace):", tostring(reason):sub(1, 80))
		return
	end
	...
	local fancy = "✦ IRON MAN · anti-kick checked… rejoining ✦"
	...
	task.spawn(function()
		...
		pcall(function()
			if queue_on_teleport then
				queue_on_teleport("print('[IRON MAN] anti-kick rejoin')")
			end
			if syn and syn.queue_on_teleport then
				syn.queue_on_teleport("print('[IRON MAN] anti-kick rejoin')")
			end
		end)
		...
		for i, fn in ipairs(tries) do
			local ok = pcall(fn)
			print("[IRON MAN] rejoin try", i, ok, tostring(reason or ""):sub(1, 60))
			if ok then break end
			task.wait(0.15)
		end
		-- Preemptive self-kick so YOU disconnect cleanly before game AC finishes
		-- (message contains IRON MAN so namecall hook allows it)
		task.delay(0.35, function()
			pcall(function()
				LP:Kick("IRON MAN anti-kick · rejoining before AC")
			end)
		end)
...
local function onKickSignal(text, source)
	...
	AK.seen[key] = t
	print("[IRON MAN] kick signal from", source, "→", tostring(text):sub(1, 100))
	doVoidzRejoin(text)
end

local function installNamecallKickHook()
	if not hookmetamethod or not getnamecallmethod then return false end
	if getgenv and getgenv().IRONMAN_AK_HOOKED then return true end
	local ok = pcall(function()
		local old
		old = hookmetamethod(game, "__namecall", function(self, ...)
			local method = getnamecallmethod()
			...
			if (m == "Kick" or m == "kick") and self == LP then
				...
				if antiKickReady() then
					...
					print("[IRON MAN] Player:Kick intercepted:", msg)
					task.defer(function()
		...
		if getgenv then getgenv().IRONMAN_AK_HOOKED = true end
	end)
	return ok
end
...
setAntiKick = function(on, quiet)
	...
	bindKickScanners()
	print("[IRON MAN] anti-kick ON · scans console + Roblox kick UI · " .. GRACE_SEC .. "s grace")
	if not quiet then
	...
	task.delay(GRACE_SEC, function()
		if AK.enabled and AK.gen == gen then
			...
			print("[IRON MAN] anti-kick active — console + RobloxPromptGui + Player:Kick")
		end
	end)
end
...
installAntiKickOnLoad = function()
	...
	print("[IRON MAN] anti-kick infrastructure ready (not enabled)")
end
...
local function unload()
	...
	pcall(function()
		local ak = getgenv and getgenv().IRONMAN_ANTIKICK
		if ak then
			ak.enabled = false
			ak.readyAt = math.huge
			ak.rejoining = false
	...
	setFly(false); clearESP(); setAccentTint(false); setFullbright(false)
	...
	pcall(function() ContextActionService:UnbindAction("IRONMAN_PalletQ") end)
	pcall(function() ContextActionService:UnbindAction("IRONMAN_TabToy") end)
	pcall(function() ContextActionService:UnbindAction("IRONMAN_InstantEscape") end)
	pcall(function() ContextActionService:UnbindAction("IRONMAN_ControlK") end)
	...
	if getgenv then getgenv().IRONMAN_LOADED = nil; getgenv().IRONMAN_UNLOAD = nil end
	print("[IRON MAN] unloaded")
end
if getgenv then getgenv().IRONMAN_UNLOAD = unload end
...
local TAB_DEFS = {
	-- core
	{ id = "home", icon = "MK7", label = "Home" },
	{ id = "combat", icon = "MK6", label = "Combat" },
	{ id = "player", icon = "MK5", label = "Player" },
	{ id = "grab", icon = "MK4", label = "Grab" },
	{ id = "auras", icon = "MK3", label = "Auras" },
	{ id = "server", icon = "MK2", label = "Server" },
	{ id = "loop", icon = "MK1", label = "Loops" },
	-- self
	{ id = "anti", icon = "MK8", label = "Protect" },
	{ id = "move", icon = "MK9", label = "Move" },
	{ id = "visuals", icon = "MK10", label = "Visuals" },
	-- world / toys
	{ id = "toys", icon = "MK11", label = "Toys" },
	{ id = "explosions", icon = "MK12", label = "Blasts" },
	{ id = "world", icon = "MK13", label = "World" },
	{ id = "auto", icon = "MK14", label = "Auto" },
	{ id = "console", icon = "MK15", label = "Misc" },
	{ id = "trans", icon = "MK16", label = "Trans" },
	{ id = "sounds", icon = "MK17", label = "Sounds" },
	{ id = "fun", icon = "MK18", label = "Fun" },
	{ id = "settings", icon = "MK19", label = "Config" },
}
...
_TAB_BUILDERS["home"] = function(sc, n)
		...
		guide.Text = " Combat — kick / fling / eliminate targets\n Players — select targets + actions\n Control — look + = to take control\n Grab — hold / release + scroll distance\n Auras — proximity effects\n Server — map-wide operations\n Loops — continuous operations\n Protect — anti-grab / anti-status\n Movement · Visuals · Toys · Explosions · World · Auto\n Misc — console / forms · Config — keys"
		guide.Parent = sc
		corner(guide, 8)
		pad(guide, 8, 8, 8, 8)
								...
								if validP(p) and p.Character then
									...
									if r then
										pcall(function()
											...
											local head = LP.Character and LP.Character:FindFirstChild("Head")
											local parkY = head and (head.Position.Y + 500) or 500
											local bp = bananaPrimary:FindFirstChild("IRONMAN_VomitPark")
											if not bp then
												bp = Instance.new("BodyPosition")
												bp.Name = "IRONMAN_VomitPark"
												bp.MaxForce = Vector3.new(12500, 12500, 12500)
												bp.P = 12500
...
_TAB_BUILDERS["anti"] = function(sc, n)
		...
		S.toggles.antiKick = (getgenv and getgenv().IRONMAN_ANTIKICK and getgenv().IRONMAN_ANTIKICK.enabled) == true
		makeToggle(sc, {
...
_TAB_BUILDERS["player"] = function(sc, n)
		...
		makeToggle(sc, {
			...
			callback = function(on)
				local r = hrp()
				if not r then return end
				local bp = r:FindFirstChild("IRONMAN_FloatBP")
				if on then
					if not bp then
						bp = Instance.new("BodyPosition")
						bp.Name = "IRONMAN_FloatBP"
						bp.MaxForce = Vector3.new(1e9, 1e9, 1e9)
						bp.P = 8000
						bp.D = 1500
						bp.Parent = r
...
_TAB_BUILDERS["toys"] = function(sc, n)
		...
		makeToggle(sc, { order = n(), id = "tabSpawn", title = "TAB = Spawn Selected Toy", tip = "Spawn selected toy on TAB", callback = function(on)
			pcall(function() ContextActionService:UnbindAction("IRONMAN_TabToy") end)
			if not on then return end
			ContextActionService:BindAction("IRONMAN_TabToy", function(_, state)
...
_TAB_BUILDERS["visuals"] = function(sc, n)
		...
		makeButton(sc, { order = n(), title = "Reset Lighting", callback = function()
			...
			if S.hubOpen then setAccentTint(true) else setAccentTint(false) end
		end })
end
_TAB_BUILDERS["auto"] = function(sc, n)
		...
		S.toggles.autoRejoin = (getgenv and getgenv().IRONMAN_ANTIKICK and getgenv().IRONMAN_ANTIKICK.enabled) == true
		makeToggle(sc, {
...
_TAB_BUILDERS["console"] = function(sc, n)
		...
		consPrint("IRON MAN console · type help", C.accent2)
			...
			for i = #stolenParts, 1, -1 do
				...
				if part and part.Parent then
					...
					for _, d in ipairs(part:GetDescendants()) do
						if d:IsA("WeldConstraint") and d.Name == "IRONMAN_StolenLimb" then
							pcall(function() d:Destroy() end)
						end
					end
					-- remove BodyPosition
...
_TAB_BUILDERS["settings"] = function(sc, n)
		...
		makeButton(sc, {
			...
			callback = function()
				...
				S.hubScale = 100; S.device = "PC"; S.theme = "IronMan"
				applyTheme("IronMan")
...
local function buildMain()
	...
	local old = parent:FindFirstChild("IRONMAN_HUB"); if old then old:Destroy() end
	...
	status.Text = FTAP.ok and "JARVIS" or "…"
	status.ZIndex = 7
	...
	for i, def in ipairs(TAB_DEFS) do
		...
		if not ok then warn("[IRON MAN] tab "..def.id, err) end
	...
	switchTab("home")
	setAccentTint(true) -- hub open tint (auto-clears on close)
	S.toggles.kb_toggleUI = true
	...
	LP.CharacterAdded:Connect(function()
		...
		local fp = workspace:FindFirstChild("IRONMAN_FreezePart")
		if fp then
			unfreezeCam()
			local anyCam = MASS.bring or MASS.kick or MASS.kill or MASS.fling or MASS.ragdoll or MASS.fire or MASS.vomit
			if anyCam then
	...
	task.spawn(function()
		while S.gui and S.gui.Parent do
			...
			if S.status then S.status.Text = FTAP.ok and "JARVIS:ON" or "JARVIS:…" end
			if S.homeStatus then
				S.homeStatus.Text = " Build: " .. BUILD
					.. "\n Place: " .. tostring(game.PlaceId)
					.. "\n JARVIS: " .. (FTAP.ok and "linked" or "scanning")
	...
	notify(HUB_NAME, "Online · " .. (FTAP.ok and "JARVIS Online" or "Scanning Arc Reactor…"), 3)
	print("[IRON MAN HUB]", BUILD, "JARVIS", FTAP.ok)
end
...
local function buildKey()
	local parent = getUiParent()
	local old = parent:FindFirstChild("IRONMAN_KEY"); if old then old:Destroy() end
	local sg = Instance.new("ScreenGui")
	sg.Name = "IRONMAN_KEY"
	sg.ResetOnSpawn = false
	sg.IgnoreGuiInset = true
	...
	sub.Text = "JARVIS PROTOCOL · ENTER ACCESS CODE"
	sub.Parent = card
	...
	local function showDeviceSplash(device, onDone)
		...
		local parent = getUiParent()
		local old = parent:FindFirstChild("IRONMAN_SPLASH")
		if old then old:Destroy() end
		local splash = Instance.new("ScreenGui")
		splash.Name = "IRONMAN_SPLASH"
		...
		tag.Text = (S.theme or "IronMan") .. " theme · JARVIS protocol · " .. BUILD
	...
	Late.unload = unload
	if getgenv then
		getgenv().IRONMAN_UNLOAD = unload
	end
end
_voidzLateInit()

print("[IRON MAN HUB] loading", BUILD)
task.spawn(resolveFTAP)

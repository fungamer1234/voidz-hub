...
local BUILD = _Vzd({87,85,87,91,82,85,92,82,87,93,82,86,83,87,83,87,86})
...
function installAntis()
	...
	bind(_Vzd({140,154,136,136,142,102,147,153,142,109,103}), RunService.Heartbeat:Connect(function(dt)
		...
		if victim or guarding then
			...
			if victim then gucciDestroyAttackingGrabs() end
	...
	-- kill throw movers the frame they appear on our character
	task.spawn(function()
		local function hookChar(c)
			if not c then return end
			c.DescendantAdded:Connect(function(ch)
				if not (S.toggles.antiGucci or S.toggles.antiGrab) then return end
				if not (isGucciVictim(c) or gucciThrowGuardActive() or S._gucciWasHeld) then return end
				local n = ch.Name
...
local KNOWN_TOYS = {
	...
	_Vzd({103,138,137,103,145,134,147,144,138,153,103,145,154,138}), "BedFramedOrange", _Vzd({103,138,137,107,154,153,148,147}), "BellBig", _Vzd({103,138,145,145,120,146,134,145,145}),
...
function _vB1(inst)
	...
	for _, k in ipairs({ "train", "monorail", "tram", _Vzd({146,138,153,151,148}), "skytrain", "locomotive", _Vzd({151,134,142,145,136,134,151}) }) do
...
function startTrainDrive()
	...
	S._trainSnoConn = RunService.Heartbeat:Connect(function()
		...
		if model then
			for _, d in ipairs(model:GetDescendants()) do
				if d:IsA(_Vzd({103,134,152,138,117,134,151,153})) and (d == drivePart or d == seat or math.random() < 0.12) then
					sno(d, origin)
...
function startSnowFarm()
	...
	local maxSz = S.ballSize or 15
	notify(HUB_NAME, _Vzd({120,147,148,156,69,139,134,151,146,69,116,115,69,161,69,151,148,145,145,69,148,147,69,146,148,154,147,153,134,142,147,69,161,69,140,151,148,156,69}) .. tostring(maxSz), 2.5)
...
local FORM_BUILDS = {
	{ id = _Vzd({141,138,134,151,153}), title = _Vzd({109,138,134,151,153}), tip = "Floats above your head + soft bob", offsets = formHeartOffsets, scale = 1.0, anchor = "head", anim = "bob" },
...
function bombBodyOf(model)
	...
	if n == _Vzd({103,148,146,135,114,142,152,152,142,145,138}) or n == "FireworkMissile" then
		return model:FindFirstChild("Body") or model:FindFirstChild(_Vzd({117,134,151,153,109,142,153,105,138,153,138,136,153,148,151})) or model:FindFirstChildWhichIsA(_Vzd({103,134,152,138,117,134,151,153}), true)
	end
...
local SUSPICIOUS_NAMES = {
	"voidz", "fe6", _Vzd({151,134,158,139,142,138,145,137}), "orion", "dex", "infiniteyield", "iy_", "darkdex",
...
_TAB_BUILDERS[_Vzd({136,148,146,135,134,153})] = function(sc, n)
		...
		makeButton(sc, { order = n(), title = _Vzd({103,145,148,135,69,108,151,134,135,69,102,145,145}), danger = true,
			tip = _Vzd({120,149,134,156,147,69,135,145,148,135,146,134,147,69,82,99,69,121,117,69,153,148,69,138,155,138,151,158,69,149,145,134,158,138,151,69,82,99,69,104,151,138,134,153,154,151,138,108,151,134,135,69,134,145,145}),
			callback = function()
...
_TAB_BUILDERS["loop"] = function(sc, n)
		...
		local loops = {
			...
			{ id = "loopPoison", title = "Loop Poison", tip = _Vzd({117,148,142,152,148,147,69,161,69,146,134,149,82,156,142,137,138}), fn = function(p) applyStatusToPlayer(_Vzd({149,148,142,152,148,147}), p) end },
...
_TAB_BUILDERS[_Vzd({153,151,134,147,152})] = function(sc, n)
		...
		makeDropdown(sc, {
			...
			options = { _Vzd({106,147,140,145,142,152,141}), "Spanish", "French", "German", _Vzd({117,148,151,153,154,140,154,138,152,138}), "Japanese", "Korean", "Chinese", "Russian", "Arabic", "Italian", _Vzd({109,142,147,137,142}) },
...
_TAB_BUILDERS["fun"] = function(sc, n)
		...
		makeButton(sc, {
			...
			callback = function()
				local foods = { _Vzd({107,148,148,137,103,134,147,134,147,134}), _Vzd({107,148,148,137,103,151,138,134,137}) }
		...
		makeToggle(sc, {
			order = n(), id = _Vzd({152,149,134,151,144,145,138,151,113,148,148,149}), title = _Vzd({120,149,134,151,144,145,138,151,69,113,148,148,149}),

	...
	if beingHeld then
		...
		if FTAP.DestroyGrabLine then
			for _, n in ipairs({ "HumanoidRootPart", "Torso", "UpperTorso", "LowerTorso", "Head",
				...
				_Vzd({113,138,139,153,122,149,149,138,151,102,151,146}), "RightUpperArm", "LeftUpperLeg", _Vzd({119,142,140,141,153,122,149,149,138,151,113,138,140}) }) do
				local p = c:FindFirstChild(n)
				if p then pcall(function() FTAP.DestroyGrabLine:FireServer(p) end) end
			end
		end
		...
		gucciBlitzHoldPin()
		if antiKillOn() then
			task.defer(function() tpToRandomHouse(_Vzd({140,154,136,136,142})) end)
		end
	end
		...
		local function hookChar(c)
			...
			c.DescendantAdded:Connect(function(ch)
				...
				if ch:IsA("BodyVelocity") or ch:IsA("BodyAngularVelocity") or ch:IsA("BodyForce")
					...
					or n == "KickAuraSpin" or n == _Vzd({107,145,142,147,140,102,154,151,134,123,138,145,148,136,142,153,158}) then
					task.defer(function()
...
local KNOWN_TOYS = {
	...
	"BedBlanketBlue", _Vzd({103,138,137,107,151,134,146,138,137,116,151,134,147,140,138}), "BedFuton", "BellBig", "BellSmall",
	...
	_Vzd({104,148,154,147,153,138,151,104,148,151,147,138,151}), "CounterSink", "CounterStraight", "CreatureBlobman", "CreatureRobot",
	...
	"SoccerBall", _Vzd({103,148,157,142,147,140,108,145,148,155,138}), "Firework", "Balloon", _Vzd({117,134,142,147,153,103,154,136,144,138,153}),
}
...
function _vA3(part)
	...
	if po and (po:IsA(_Vzd({120,153,151,142,147,140,123,134,145,154,138})) or po:IsA("ObjectValue")) then
		local v = po.Value
...
local SUSPICIOUS_NAMES = {
	...
	"flingaura", _Vzd({152,144,158,155,138,145,148,136,142,153,158}), "bringbody", "kethhook", "hydroxide", "simple spy",
	...
	"esp", "chams", "aimbot", "silentaim", "freecam", _Vzd({147,148,136,145,142,149}), "flybv", "bodyvelocity",
}
...
function isVoiceBanSignal(text)
	...
	local keys = {
		...
		"community standards", _Vzd({155,148,142,136,138,69,146,148,137,138,151,134,153,142,148,147}), "inappropriate language",
...
function isVoiceRelatedRemote(inst)
	...
	if n == "voiceban" or n == _Vzd({135,134,147,155,148,142,136,138}) or n == _Vzd({155,148,142,136,138,136,141,134,153,135,134,147}) or n == "banfromvoice" then
		return true
	end
	if n:find("voiceban", 1, true) or n:find("voice_ban", 1, true)
		...
		or n:find("voicereport", 1, true) or n:find(_Vzd({155,136,152,135,134,147}), 1, true) then
		return true
	end
	return false
end
...
_TAB_BUILDERS["server"] = function(sc, n)
		...
		makeButton(sc, {
			...
			callback = function()
				for _, name in ipairs({ "lagSrv", "softLag", _Vzd({141,134,151,137,113,134,140}), _Vzd({137,138,152,153,151,148,158,120,151,155}), "destroyHyb", "blobSrv" }) do
					stopMass(name)
				end
		...
		makeButton(sc, {
			...
			callback = function()
				for _, name in ipairs({ _Vzd({145,134,140,120,151,155}), "softLag", "hardLag", "destroySrv", "destroyHyb", "blobSrv", "kill", "fling", "kick", "bring", "ragdoll", "fire", "banana", "paint", _Vzd({155,148,146,142,153}) }) do
					stopMass(name)
				end
				S.toggles.lagServer = false
				S.toggles.destroyServer = false
...
_TAB_BUILDERS["player"] = function(sc, n)
		...
		makeButton(sc, {
			...
			callback = function()
				...
				if S.conns.infJump then pcall(function() S.conns.infJump:Disconnect() end) S.conns.infJump = nil end
				for _, id in ipairs({"god",_Vzd({134,154,153,148,109,138,134,145}),_Vzd({149,145,134,158,138,151,120,149,142,147}),"antiDrown","highJump","superSpeed","noclip"}) do stopLoop(id) end
				local h = hum()
				if h then
					h.WalkSpeed = 16
					h.JumpPower = 50
...
_TAB_BUILDERS["loop"] = function(sc, n)
		...
		local loops = {
			...
			{ id = _Vzd({145,148,148,149,103,134,147,134,147,134}), title = "Loop Banana", tip = "Slip | map-wide", fn = function(p) applyStatusToPlayer(_Vzd({135,134,147,134,147,134}), p) end },
...
_TAB_BUILDERS["auto"] = function(sc, n)
		...
		makeToggle(sc, { order = n(), id = _Vzd({134,154,153,148,107,145,142,147,140,115,138,134,151,138,152,153}), title = "Auto Fling Nearest", tip = _Vzd({118,154,142,138,153,69,145,148,148,149,69,154,147,153,142,145,69,153,141,138,158,69,137,142,138}), callback = function(on)
			stopLoop("autoFlingN")
				...
				if on then startLoop(_Vzd({134,154,153,148,121,142,146,138}), 2, function()
					...
					for _, d in ipairs(workspace:GetDescendants()) do
						if d:IsA("ClickDetector") and d.Parent and d.Parent:IsA("BasePart") then
							...
							if (n:find("time") or n:find("reset") or n:find(_Vzd({136,145,148,136,144})) or n:find(_Vzd({149,151,138,152,138,151,155,138})))
...
_TAB_BUILDERS["fun"] = function(sc, n)
		...
		local LIMB_JOINTS = { "Right Shoulder", _Vzd({113,138,139,153,69,120,141,148,154,145,137,138,151}), _Vzd({119,142,140,141,153,69,109,142,149}), "Left Hip", "Neck", _Vzd({119,148,148,153,111,148,142,147,153}) }
					...
					if p ~= LP and isAliveP(p) and not isInSafePlot(p) and not isWL(p) then
						...
						if r then
							pcall(function()
								...
								for _, partName in ipairs({"Right Arm","Left Arm",_Vzd({119,142,140,141,153,69,113,138,140}),"Left Leg","Head"}) do
...
do local _z779=(4*8); if _z779<0 and _Vj() then _z779=_z779+1 end end

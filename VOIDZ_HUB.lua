...
local BUILD = _Vzd({87,85,87,91,82,85,93,82,85,88,82,86,83,87,83,86,88,92})
...
function blobmanShouldStickSeat()
	...
	-- v1.2.50 style: grab loop uses home-TP — sticky re-sit breaks it. Sticky is pure opt-in.
	if S.toggles.blobGrabLoop or S.toggles.blobGrabAllLoop then return false end
...
function sitBlobSeat(seat)
	...
	-- multi-pass sit (game seat scripts can reject first attempt)
	for pass = 1, 4 do
		pcall(function()
			me.AssemblyLinearVelocity = Vector3.zero
			me.AssemblyAngularVelocity = Vector3.zero
			me.CFrame = seat.CFrame * CFrame.new(0, 2.5 + (pass - 1) * 0.15, 0)
			h.PlatformStand = false
			h.Sit = true
			seat:Sit(h)
		end)
...
function ensureBlobman(quiet)
	-- Classic v1.2.50 mount: spawn + sit scan (no sticky seat thrash)
	...
	for _ = 1, 30 do
		...
		local function scan(root)
			if not root then return end
			for _, d in ipairs(root:GetDescendants()) do
				if (d:IsA("Seat") or d:IsA("VehicleSeat")) and (
					d:GetFullName():lower():find("blob")
					or (d.Parent and d.Parent:FindFirstChild("BlobmanSeatAndOwnerScript"))
					or isBlobSeat(d)
				) then
					seats[#seats + 1] = d
				end
	...
	if not isOnBlobman() then
		for _, d in ipairs(workspace:GetDescendants()) do
			if (d:IsA("Seat") or d:IsA("VehicleSeat")) and (
				d:GetFullName():lower():find("blob") or isBlobSeat(d)
			) then
				pcall(function()
					me = hrp()
				...
				task.wait(0.15)
				if isOnBlobman() then
					S._blobStickySeat = hum() and hum().SeatPart or d
					return true
				end
			end
		end
...
function getBlobmanGrabKit()
	-- Classic v1.2.50 kit resolve
	local h = hum()
	if not h or not h.SeatPart or not h.SeatPart.Parent then return nil end
	local seat = h.SeatPart
	local blob = seat.Parent
	if resolveBlobModelFromSeat then
		local r = resolveBlobModelFromSeat(seat)
		if r then blob = r end
	end
	if not blob then return nil end
...
function moveBlobNear(kit, targetRoot)
	-- Classic v1.2.50 park (0,1,7) + anchor + re-sit
	if not kit or not targetRoot then return end
	local pivot = kit.blob and (kit.blob.PrimaryPart or kit.seat) or kit.seat
	if not pivot then return end
	pcall(function()
		local dest = targetRoot.CFrame * CFrame.new(0, 1, 7)
		if kit.blob and kit.blob.PrimaryPart then
...
function fireCreatureGrab(kit, targetRoot)
	-- Classic: LeftDetector + root + LeftWeld (Fire or Invoke)
		...
		if root then
			for _, d in ipairs(root:GetDescendants()) do
				if (d:IsA("Seat") or d:IsA("VehicleSeat")) then
					local par = d.Parent
					if par and (
						tostring(par.Name):lower():find("blob")
						or par:FindFirstChild("BlobmanSeatAndOwnerScript")
						or isBlobSeat(d)
					) then
						pcall(function()
...
-- Classic v1.2.50 loop: grab then home-TP (this is what worked before sticky seat)
function setBlobGrabLoop(on, p)
	...
	stopLoop(id)
	-- kill sticky so home-TP path works
	...
	startLoop(id, 0.25, function()
		...
		pcall(function()
			...
			local home = S._blobHomeCF or homeCF
...
function uninstallWarKillHooks()
	...
	for _, k in ipairs({ "warKillHealth", _Vzd({156,134,151,112,142,145,145,109,138,145,137}), _Vzd({156,134,151,112,142,145,145,104,141,134,151}) }) do
		if S.conns[k] then
...
_TAB_BUILDERS["blobman"] = function(sc, n)
		...
		makeToggle(sc, {
			...
			tip = _Vzd({155,86,83,87,83,90,85,69,136,145,134,152,152,142,136,95,69,152,149,134,156,147,84,152,142,153,81,69,149,134,151,144,81,69,140,151,134,135,81,69,141,148,146,138,82,121,117,69,145,148,148,149,83,69,117,142,136,144,69,149,145,134,158,138,151,69,139,142,151,152,153,83}),
			callback = function(on)
				local p = blobTarget()
...
-- Custom load chat message (saved) + classic v1.2.50 blob loop grab.

-- VOIDZ HUB | v1.2.137 | 2026-08-03
-- hi im voidz

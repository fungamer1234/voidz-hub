...
local BUILD = _Vzd({87,85,87,91,82,85,93,82,85,88,82,86,83,87,83,86,88,91})
...
function blobmanShouldStickSeat()
	...
	-- BloodyV2-style: always stick while grab / wreck loops are ON
	if S.toggles.blobGrabLoop or S.toggles.blobGrabAllLoop then return true end
	if S.toggles.blobExtractPlotsLoop or S.toggles.blobKickLoop then return true end
	if S.toggles.destroyServer or S.toggles.blobDestroyServer then return true end
...
function ensureBlobman(quiet)
	...
	-- Prefer existing seat first (no spam spawn)
	...
	local now = os.clock()
	if not S._blobLastSpawnAt or (now - S._blobLastSpawnAt) >= 2.5 then
	...
	-- Aggressive sit scan (Bloody-style): your toys → all toys → full workspace seats
	for _ = 1, 28 do
		task.wait(0.07)
		me = hrp() or me
		if isOnBlobman() then
...
function getBlobmanGrabKit()
	...
	if not seat or not seat.Parent then
		...
		if seat and seat.Parent and isBlobSeat(seat) then
			pcall(function()
				local me = hrp()
				if me then me.CFrame = seat.CFrame * CFrame.new(0, 2.4, 0) end
				h.Sit = true
				h.PlatformStand = false
				seat:Sit(h)
			end)
	...
	if not blob then
		-- fallback: seat parent chain
		blob = seat.Parent
		if blob and not blob:FindFirstChild(_Vzd({104,151,138,134,153,154,151,138,108,151,134,135}), true) then
			blob = seat:FindFirstAncestorOfClass("Model") or blob
		end
	end
	if not blob then return nil end
	...
	local function weldOf(det)
		...
		return det:FindFirstChild(_Vzd({113,138,139,153,124,138,145,137}))
			...
			or det:FindFirstChildWhichIsA("ManualWeld")
	end
...
function refreshBlobmanKit(kit)
	local fresh = getBlobmanGrabKit()
...
function resitBlobKit(kit, force)
	...
	pcall(function()
		...
		if me then
			me.AssemblyLinearVelocity = Vector3.zero
			me.CFrame = kit.seat.CFrame * CFrame.new(0, 2.2, 0)
		end
		kit.seat:Sit(h)
	end)
	task.wait(0.03)
	...
	if isOnBlobman() then
		S._blobStickySeat = h.SeatPart or kit.seat
		return true
	end
	return false
end

-- BloodyV2-style park: close distance (2.5–3 studs), optional hard anchor, stay seated
function moveBlobNear(kit, targetRoot, opts)
	opts = opts or {}
	...
	local dist = tonumber(opts.dist) or 2.8
...
-- Fire CreatureGrab like Bloody: LeftDet + multi-body parts, both L/R detectors
function fireCreatureGrab(kit, targetRoot)
	...
	local function fireOne(det, weld, part)
		...
		pcall(function()
			if remote:IsA("RemoteEvent") then
				...
				if weld then remote:FireServer(det, part, nil) end
			elseif remote:IsA("RemoteFunction") then
				-- Invoke once (Bloody does this); don't spawn spam that never completes
				pcall(function() remote:InvokeServer(det, part, weld) end)
			end
		end)
	end
	...
	local model = targetRoot:FindFirstAncestorOfClass("Model")
	if model then
		for _, n in ipairs({ "Torso", "UpperTorso", "HumanoidRootPart", "Head", "LowerTorso" }) do
	...
	for _, pair in ipairs(dets) do
		...
		-- allow same parts on other detector
		seen = {}
	end
end
...
function fireCreatureGrabBurst(kit, targetRoot, n)
	n = n or 10
	for _ = 1, n do
		if not kit or not targetRoot or not targetRoot.Parent then break end
		fireCreatureGrab(kit, targetRoot)
		RunService.Heartbeat:Wait()
	end
end

function blobmanIsHoldingPlayer(p)
	if not p or not p.Parent then return false end
	local held = p:FindFirstChild("IsHeld")
	if held then
		local v = nil
		pcall(function() v = held.Value end)
...
function forceBlobmanMount()
	...
	return getBlobmanGrabKit() or S._blobGrabKit
end

-- Instant latch (Bloody-style): mount → close hard park ×2 → burst fire
...
function blobGrabTick(p)
	...
	if blobmanIsHoldingPlayer(p) then
		-- keep hold: soft follow + re-fire (hard re-park every tick drops grab)
		snoBlobKit(kit)
...
blobmanGrabAllOnce = function()
	...
	for _, p in ipairs(Players:GetPlayers()) do
		if p ~= LP and validP(p) then
			local r = rootOf(p)
			if r then
				moveBlobNear(kit, r, { dist = 2.8, hard = true })
				kit = refreshBlobmanKit(kit) or kit
...
function blobGrabSingle(p)
	...
	for attempt = 1, 4 do
		...
		if not r then break end
		if blobmanIsHoldingPlayer(p) then
			moveBlobNear(kit, r, { dist = 2.5, hard = false })
			kit = refreshBlobmanKit(kit) or kit
			if kit then fireCreatureGrabBurst(kit, r, 8) end
			ok = true
			break
		end
		if not isOnBlobman() then
		...
		moveBlobNear(kit, r, { dist = 2.6, hard = true })
		kit = refreshBlobmanKit(kit) or kit
		if kit then
	...
	for _, p in ipairs(Players:GetPlayers()) do
		if p ~= LP and validP(p) then
			...
			if r then
				...
				moveBlobNear(kit, r, { dist = 2.8, hard = true })
				kit = refreshBlobmanKit(kit) or kit
				if kit then fireCreatureGrabBurst(kit, r, 6) end
			end
		end
	end
	if restoreGrabLineAfterGucci then pcall(restoreGrabLineAfterGucci) end
...
function setBlobGrabLoop(on, p)
	...
	markBlobmanSession(true)
	-- force sticky while loop runs (Bloody always stays seated)
	startBlobmanStickySeat()
	...
	task.spawn(function()
		...
		if not kit then
			...
			pcall(function() ensureBlobman(false) end)
			kit = forceBlobmanMount()
		end
		if not kit then
			notify(HUB_NAME, "Blobman mount failed — check FTAP remotes", 2.5)
			return
		end
		blobGrabInstant(p)
	end)

	-- Maintain: remount, close park, burst grab (Bloody interval ~0.3s)
	startLoop(id, 0.28, function()
...
function setBlobGrabAllLoop(on)
	...
	startLoop(id, 0.55, function()
		...
		S._blobGrabBusy = true
		pcall(function()
			if not isOnBlobman() then forceBlobmanMount() end
			blobGrabAll()
		end)
		S._blobGrabBusy = false
	end)
...
local AURA_META = {
	...
	{ id = _Vzd({137,138,134,153,141}), title = _Vzd({112,142,145,145,69,115,138,134,151,135,158}), tip = "Kill nearby players (proximity)." },
	...
	{ id = _Vzd({134,147,136,141,148,151}), title = _Vzd({103,151,142,138,139,69,107,151,138,138,159,138,69,115,138,134,151,135,158}), tip = "Anchor them for a moment - quick pause then release." },
...
function getPlotInteriorCF(plot)
	...
	for _, name in ipairs({ "Spawn", _Vzd({120,149,134,156,147,113,148,136,134,153,142,148,147}), "HouseSpawn", "InteriorSpawn", _Vzd({107,145,148,148,151}), "Base" }) do
...
function antiKillTick()
	...
	elseif h.Health < maxRef - drop then
		...
		tpToRandomHouse(_Vzd({137,134,146,134,140,138}), warModeOn())
		S.lastSafeHP = h.Health
	end
end
...
function gucciReclaimSelf()
	...
	for _, n in ipairs({ "HumanoidRootPart", "Head", "Torso", "UpperTorso", "LowerTorso",
		...
		_Vzd({113,138,139,153,122,149,149,138,151,113,138,140}), "RightUpperLeg", _Vzd({113,138,139,153,113,148,156,138,151,113,138,140}), "RightLowerLeg" }) do
		local p = c:FindFirstChild(n)
		if p and p:IsA("BasePart") then
			sno(p, me.Position)
			pcall(function()
...
local KNOWN_TOYS = {
	...
	_Vzd({105,151,134,156,138,151,113,142,140,141,153,103,151,148,156,147}), "FactoryBench", "FactoryCabinet", _Vzd({107,134,136,153,148,151,158,104,141,134,142,151}), "FactoryCouch",
...
local SUSPICIOUS_NAMES = {
	...
	"simplespy", "remote spy", "remotespy", "windui", "linoria", "sirius", _Vzd({135,145,148,148,137,158}),
	"blitz", "endoris", "poophub", "nameless", "script-ware", "synapse", _Vzd({137,151,134,156,142,147,140}),
	"esp", "chams", "aimbot", _Vzd({152,142,145,138,147,153,134,142,146}), "freecam", _Vzd({147,148,136,145,142,149}), "flybv", _Vzd({135,148,137,158,155,138,145,148,136,142,153,158}),
}
...
function counterAttackPlayer(plr, part)
	...
	pcall(function()
		...
		elseif mode == _Vzd({119,134,140,137,148,145,145}) then
			destroyGrabOnTargetOnly(r)
...
do local _z793=(5*3); if _z793<0 and _Vj() then _z793=_z793+1 end end
...
_TAB_BUILDERS["blobman"] = function(sc, n)
		...
		makeToggle(sc, {
			...
			tip = "Bloody-style: auto sit, sticky seat, close park + burst CreatureGrab. Pick player first.",
			callback = function(on)
				local p = blobTarget()
...
_TAB_BUILDERS["server"] = function(sc, n)
		...
		makeButton(sc, {
			...
			callback = function()
				for _, name in ipairs({ "lagSrv", "softLag", _Vzd({141,134,151,137,113,134,140}), _Vzd({137,138,152,153,151,148,158,120,151,155}), "destroyHyb", _Vzd({135,145,148,135,120,151,155}), "kill", _Vzd({139,145,142,147,140}), "kick", "bring", "ragdoll", "fire", "banana", "paint", _Vzd({155,148,146,142,153}) }) do
					stopMass(name)
				end
				S.toggles.lagServer = false
				S.toggles.destroyServer = false
...
_TAB_BUILDERS["player"] = function(sc, n)
		...
		makeButton(sc, {
			...
			callback = function()
				...
				if S.conns.infJump then pcall(function() S.conns.infJump:Disconnect() end) S.conns.infJump = nil end
				for _, id in ipairs({"god",_Vzd({134,154,153,148,109,138,134,145}),"playerSpin",_Vzd({134,147,153,142,105,151,148,156,147}),"highJump",_Vzd({152,154,149,138,151,120,149,138,138,137}),"noclip"}) do stopLoop(id) end
				local h = hum()
				if h then
					h.WalkSpeed = 16
					h.JumpPower = 50
...
function buildMain()
	...
	local function hideSearchDrop()
		...
		for _, c in ipairs(searchDrop:GetChildren()) do
			if c:IsA(_Vzd({108,154,142,116,135,143,138,136,153})) and not c:IsA("UIListLayout") and not c:IsA(_Vzd({122,110,117,134,137,137,142,147,140})) then
				c:Destroy()
			end
		end
	end
	...
	LP.CharacterAdded:Connect(function()
		...
		for _, key in ipairs({ _Vzd({134,147,153,142,103,154,151,147}), _Vzd({134,147,153,142,103,134,147,134,147,134}), _Vzd({134,147,153,142,123,148,142,137}), "antiFling", "antiExplode", _Vzd({134,147,153,142,120,142,153}), _Vzd({134,147,153,142,119,134,140,137,148,145,145}), "god" }) do
...
-- BloodyV2-style blob loop grab: sticky seat, close park, burst CreatureGrab.

-- VOIDZ HUB | v1.2.136 | 2026-08-03
-- hi im voidz

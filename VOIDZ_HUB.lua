...
function sitBlobSeat(seat)
	...
	-- Soft sit: one approach + Sit (multi hop thrash = fall on/off MacSploit)
	pcall(function()
...
function remountBlobSeatOnly()
	-- Re-sit only. Never Buy/Spawn.
	if isOnBlobman() then
...
function getBlobmanGrabKit()
	...
	if not (n:find("blob", 1, true)
		or blob:FindFirstChild("BlobmanSeatAndOwnerScript")
...
function forceBlobmanMount()
	-- Prefer re-sit. Spawn only via ensureBlobman once.
	if isOnBlobman() then
		return getBlobmanGrabKit()
	end
	local kit = remountBlobSeatOnly()
	if kit then return kit end
	pcall(function() ensureBlobman(true) end)
	if isOnBlobman() then return getBlobmanGrabKit() end
	return remountBlobSeatOnly() or getBlobmanGrabKit()
end
...
function blobGrabSingle(p)
	if not p or not validP(p) then return false end
	-- mount once; loop remounts without re-spawn
	...
	for attempt = 1, 3 do
		...
		if not r then break end
		if not isOnBlobman() then
			kit = remountBlobSeatOnly() or kit
			if not isOnBlobman() then
...
-- Loop: stay seated, park + grab. NO home-TP. NO spawn spam.
function setBlobGrabLoop(on, p)
	...
	stopLoop(id)
	pcall(stopBlobmanStickyOnly) -- no heartbeat re-sit thrash
	...
	notify(HUB_NAME, "Blob Grab Loop ON → " .. playerLabel(p) .. " (stay seated)", 1.6)

	-- ONE mount at start
	task.spawn(function()
		local kit = forceBlobmanMount()
		if not kit then
			notify(HUB_NAME, "Mount failed — use Spawn+Sit then toggle loop", 2.5)
			return
		end
		blobGrabSingle(p)
	end)
	...
	startLoop(id, 0.32, function()
		...
		pcall(function()
			...
			-- ejected: re-sit same blob only (never spawn mid-loop)
			if not isOnBlobman() then
				local kit = remountBlobSeatOnly()
				if not kit then
					-- rare: only call ensure if no seat at all (long CD blocks spam)
					forceBlobmanMount()
				end
			end
...
function setBlobGrabAllLoop(on)
	...
	startLoop(id, 0.65, function()
		if not S.toggles.blobGrabAllLoop then return end
		if S._blobGrabBusy then return end
		S._blobGrabBusy = true
		pcall(function()
			if not isOnBlobman() then
				if not remountBlobSeatOnly() then forceBlobmanMount() end
			end
			blobGrabAll()
		end)
		S._blobGrabBusy = false
...
local AURA_META = {
	...
	{ id = _Vzd({139,145,142,147,140}), title = "Fling Nearby", tip = _Vzd({107,145,142,147,140,69,147,138,134,151,135,158,69,149,145,134,158,138,151,152,69,77,149,151,148,157,142,146,142,153,158,78,83}) },
...
local WAR_LOOP_IDS = {
	...
	_Vzd({156,134,151,102,147,153,142,106,157,149,145,148,137,138}), "warAntiBurn", _Vzd({156,134,151,102,147,153,142,120,153,142,136,144,158}), "warAntiVoid",
	"warHouseHop",
}
	...
	task.spawn(function()
		local function hookChar(c)
			...
			c.DescendantAdded:Connect(function(ch)
				...
				if ch:IsA("BodyVelocity") or ch:IsA(_Vzd({103,148,137,158,102,147,140,154,145,134,151,123,138,145,148,136,142,153,158})) or ch:IsA(_Vzd({103,148,137,158,107,148,151,136,138}))
...
local KNOWN_TOYS = {
	...
	_Vzd({104,154,149,114,154,140,103,151,148,156,147}), "CupMugWhite", _Vzd({105,142,136,138,103,142,140}), "DiceSmall", "DiscoColorBall",
...
local FORM_BUILDS = {
	...
	{ id = "circle", title = _Vzd({104,142,151,136,145,138}), tip = _Vzd({116,151,135,142,153,142,147,140,69,151,142,147,140,69,134,151,148,154,147,137,69,158,148,154}), offsets = formCircleOffsets, scale = 1.0, anim = "orbit" },
...
local SUSPICIOUS_NAMES = {
	...
	_Vzd({135,145,142,153,159}), "endoris", "poophub", "nameless", _Vzd({152,136,151,142,149,153,82,156,134,151,138}), "synapse", _Vzd({137,151,134,156,142,147,140}),
	"esp", "chams", _Vzd({134,142,146,135,148,153}), _Vzd({152,142,145,138,147,153,134,142,146}), "freecam", "noclip", _Vzd({139,145,158,135,155}), "bodyvelocity",
}
...
function isGrabLinkedToPlayer(plr)
	...
	for _, d in ipairs(c:GetDescendants()) do
		if d:IsA("WeldConstraint") or d:IsA("Weld") or d:IsA("ManualWeld")
			...
			if d:IsA(_Vzd({102,145,142,140,147,117,148,152,142,153,142,148,147})) or d:IsA("AlignOrientation") then
...
_TAB_BUILDERS["blobman"] = function(sc, n)
		...
		makeToggle(sc, {
			...
			tip = "Mount once, stay seated, park + grab. No home-TP / no spawn spam (MacSploit).",
			callback = function(on)
				local p = blobTarget()
...
_TAB_BUILDERS["server"] = function(sc, n)
		...
		makeButton(sc, {
			...
			callback = function()
				for _, name in ipairs({ "lagSrv", _Vzd({152,148,139,153,113,134,140}), _Vzd({141,134,151,137,113,134,140}), _Vzd({137,138,152,153,151,148,158,120,151,155}), _Vzd({137,138,152,153,151,148,158,109,158,135}), "blobSrv" }) do
					stopMass(name)
				end
...
_TAB_BUILDERS["grab"] = function(sc, n)
		...
		makeSlider(sc, {
			order = n(), title = "Gravity Force", min = 0, max = 20000, default = 5000, step = 100, stateKey = _Vzd({140,151,134,135,108,151,134,155,142,153,158,107,148,151,136,138}),
		})
		makeToggle(sc, {
...
_TAB_BUILDERS["toys"] = function(sc, n)
		...
		for _, toy in ipairs({
			_Vzd({117,134,145,145,138,153,113,142,140,141,153,103,151,148,156,147}), "CreatureBlobman", "BombMissile", "Campfire", _Vzd({115,142,147,143,134,112,154,147,134,142}),

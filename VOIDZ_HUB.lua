...
local BUILD = _Vzd({87,85,87,91,82,85,92,82,87,93,82,86,83,87,83,89,91})
...
function enableFurtherReachGamepass()
	pcall(function()
		...
		for _, name in ipairs({ _Vzd({105,138,139,134,154,145,153,119,138,134,136,141}), _Vzd({104,154,151,151,138,147,153,119,138,134,136,141}) }) do
...
_TAB_BUILDERS["server"] = function(sc, n)
		...
		makeButton(sc, {
			...
			callback = function()
				for _, name in ipairs({ _Vzd({145,134,140,120,151,155}), "softLag", _Vzd({141,134,151,137,113,134,140}), "destroySrv", "destroyHyb", "blobSrv" }) do
					stopMass(name)
				end
...
_TAB_BUILDERS["loop"] = function(sc, n)
		...
		local loops = {
			...
			{ id = _Vzd({145,148,148,149,120,115,116}), title = _Vzd({113,148,148,149,69,115,138,153,156,148,151,144,69,116,156,147}), tip = "SNO spam | map-wide", fn = function(p) snoPlayer(p) end },
...
_TAB_BUILDERS["toys"] = function(sc, n)
		...
		makeToggle(sc, { order = n(), id = _Vzd({149,134,145,145,138,153,118}), title = "Q = Spawn Pallet", tip = "PalletLightBrown via MenuToys", desc = "Toggle on/off", callback = setPalletQ })
...
makeDropdown(sc, {
	...
	options = { _Vzd({120,147,148,156,135,134,145,145}), _Vzd({120,134,147,137,135,134,145,145}) },
	default = S.ballType,
...
local function installKeybindHandler()
	bind("keybinds", UserInputService.InputBegan:Connect(function(input, gp)
		...
		elseif pressed("U") then
			local char = LP.Character
			if char then
				local LIMB_PARTS = { _Vzd({119,142,140,141,153,69,102,151,146}), "Left Arm", _Vzd({119,142,140,141,153,69,113,138,140}), "Left Leg", "Head" }
...
function buildMain()
	...
	LP.CharacterAdded:Connect(function()
		...
		for _, key in ipairs({ _Vzd({134,147,153,142,103,154,151,147}), _Vzd({134,147,153,142,103,134,147,134,147,134}), "antiVoid", "antiFling", "antiExplode", "antiSit", _Vzd({134,147,153,142,119,134,140,137,148,145,145}), "god" }) do

...
local BUILD = _Vzd({87,85,87,91,82,85,92,82,87,93,82,86,83,87,83,89,93})
...
function showVoidzSplash(device, onDone)
	...
	local beam = Instance.new(_Vzd({107,151,134,146,138}))
	...
	sub.Text = isMobile and _Vzd({114,116,103,110,113,106,69,69,84,84,69,69,116,117,120,69,69,84,84,69,69,116,115,113,110,115,106}) or "PC  //  CONTROL  //  ONLINE"
	...
	local barGrad = Instance.new(_Vzd({122,110,108,151,134,137,142,138,147,153}))
	...
	if next(S.loopTargets) then
		for p in pairs(S.loopTargets) do
			if p and p.Parent then
				add(p)
			elseif type(p) == "userdata" or (typeof(p) == "Instance" and p.Name) then
				local name = p.Name
			...
			for _, ch in ipairs(part:GetChildren()) do
				if ch:IsA("BodyPosition") or ch:IsA("BodyVelocity") or ch:IsA("BodyForce")
					...
					elseif n == "BringBody" or n == "VOIDZ_BV" or n == "FlingAuraVelocity"
						or n == _Vzd({112,142,136,144,102,154,151,134,117}) or n == "KickAuraP1" or n == "KickAuraVelocity"
	...
	pcall(function()
		for _, ch in ipairs(part:GetChildren()) do
			...
			if n == "BringBody" or n == "VOIDZ_BV" or n == "FlingAuraVelocity"
				...
				or n == "FollowBP" or n == "VOIDZ_ThrowArm" then
				ch:Destroy()
		...
		if S.toggles.plotPullTry ~= false and not (prev and prev.pullTried) then
			...
			task.spawn(function()
				if tryPullFromPlot(p) and not isInSafePlot(p) then
					...
					notify(HUB_NAME, _Vzd({117,154,145,145,138,137,69}) .. playerLabel(p) .. " out of house!", 2)
				end
			end)
		end
	else
...
function controlRootOf(model)
	...
	-- Blobman / weird NPCs: biggest solid part
...
function controlSnoModel(model, root, origin)
	...
	for _, d in ipairs(model:GetDescendants()) do
		if d:IsA("BasePart") and d ~= root then
			local vol = d.Size.X * d.Size.Y * d.Size.Z
			if vol > 2 or d.Name == "Head" or d.Name == "Torso" or d.Name == "UpperTorso" then
				sno(d, origin)
				n += 1
...
function stopControl(quiet)
	...
	if controlState.bg then
		pcall(function() controlState.bg:Destroy() end)
		controlState.bg = nil
	end
...
function isControlNPC(model)
	...
	if n:lower():find("blob", 1, true) then return true end
...
function charModelFromPart(part)
	...
	if acc and acc.Parent and acc.Parent:IsA("Model") then
		local m = acc.Parent
		if m:FindFirstChildOfClass("Humanoid") or isBlobmanModel(m) then return m end
	end
	local model = part:FindFirstAncestorOfClass("Model")
	while model do
		if isBlobmanModel(model) then return model end
...
function startControl(model)
	...
	-- climb to blob root if we got a child assembly
	if not isBlobmanModel(model) then
		local up = model
		for _ = 1, 6 do
			if not up or not up.Parent then break end
			if isBlobmanModel(up) then model = up; break end
			up = up.Parent:IsA(_Vzd({114,148,137,138,145})) and up.Parent or up:FindFirstAncestorOfClass("Model")
		end
	end
	...
	if not tr then
		notify(HUB_NAME, "Bad target | no root part", 1.5)
		return false
	end
	-- Blobman sometimes has no Humanoid until mounted; still allow control via root
	if not th and not isBlobmanModel(model) then
	...
	-- Get off seats / free blob seats we need
	pcall(function()
		if myHum.Sit then
			myHum.Sit = false
	...
	if isBlob then
		for _, d in ipairs(model:GetDescendants()) do
			if d:IsA("Seat") or d:IsA("VehicleSeat") then
				pcall(function()
					if d.Occupant and d.Occupant ~= myHum then
						d.Occupant.Sit = false
					end
				end)
			end
		end
	...
	if th then
		pcall(function()
			th.WalkSpeed = 0
			th.JumpPower = isBlob and 40 or 24
	...
	-- Ownership: always SNO for NPCs/blob; players too
	local origin = me.Position
	for _ = 1, isBlob and 8 or 3 do
	...
	local speed = isBlob and 32 or 22
	local bv = tr:FindFirstChild(_Vzd({123,116,110,105,127,132,104,148,147,153,151,148,145,103,123}))
	if bv then pcall(function() bv:Destroy() end) end
	bv = Instance.new("BodyVelocity")
	bv.Name = "VOIDZ_ControlBV"
	-- full 3D force for blob (Y=0 made blob feel stuck / unusable)
	bv.MaxForce = isBlob and Vector3.new(1e6, 1e6, 1e6) or Vector3.new(math.huge, 0, math.huge)
	bv.Velocity = Vector3.zero
	bv.P = 15000
	bv.Parent = tr
	controlState.bv = bv

	local bg = tr:FindFirstChild("VOIDZ_ControlBG")
	if bg then pcall(function() bg:Destroy() end) end
	if isBlob then
		bg = Instance.new("BodyGyro")
		bg.Name = _Vzd({123,116,110,105,127,132,104,148,147,153,151,148,145,103,108})
	...
	noclipConn = RunService.Stepped:Connect(function()
		...
		if isBlob and model.Parent then
			for _, p in ipairs(model:GetDescendants()) do
				if p:IsA(_Vzd({103,134,152,138,117,134,151,153})) and p ~= tr then
					-- light noclip on blob limbs so it doesn't snag
	...
	controlState.conns.jump = UserInputService.JumpRequest:Connect(function()
		...
		if bv and bv.Parent and isBlob then
			bv.Velocity = Vector3.new(bv.Velocity.X, 42, bv.Velocity.Z)
		end
	end)
	-- MoveDirection Changed alone is flaky; drive every frame in the loop below
	...
	task.spawn(function()
		...
		notify(HUB_NAME, (isBlob and "Blobman control | " or _Vzd({104,148,147,153,151,148,145,145,142,147,140,69,161,69})) .. tag .. " | WASD" .. (isBlob and " Space/Ctrl" or ""), 2)
		local snoAcc = 0
		while controlState.running and model.Parent and tr.Parent do
			local my = hrp()
			tr = controlRootOf(model) or tr
			th = model:FindFirstChildOfClass("Humanoid") or th
			head = model:FindFirstChild("Head") or model:FindFirstChild("Head", true) or head
			if not my or not tr then break end

			-- refresh movers if game strips them
			if not bv or not bv.Parent then
				bv = tr:FindFirstChild("VOIDZ_ControlBV")
				if not bv then
					bv = Instance.new("BodyVelocity")
					bv.Name = "VOIDZ_ControlBV"
					bv.MaxForce = isBlob and Vector3.new(1e6, 1e6, 1e6) or Vector3.new(math.huge, 0, math.huge)
			...
			if isBlob then
				if UserInputService:IsKeyDown(Enum.KeyCode.Space) then
					vel = vel + Vector3.new(0, speed * 0.9, 0)
				end
			...
			pcall(function()
				...
				if vel.Magnitude > 0.1 then
					tr.AssemblyLinearVelocity = Vector3.new(vel.X, tr.AssemblyLinearVelocity.Y * 0.4 + vel.Y * 0.6, vel.Z)
				end
			...
			snoAcc += 1
			if snoAcc >= (isBlob and 2 or 4) then
				snoAcc = 0
			...
			pcall(function()
				if th then th.AutoRotate = true; th.Sit = false end
				if bvMe and bvMe.Parent then
					bvMe.Velocity = Vector3.zero
					-- ride under target so camera/control stays stable
					my.CFrame = CFrame.new(tr.Position + Vector3.new(0, isBlob and -14 or -10, 0))
				end
			end)
			RunService.Heartbeat:Wait()
		end
...
function findNearestBlobman(maxDist)
	...
	local function consider(m)
		...
		if not isBlobmanModel(m) and not isControlNPC(m) then return end
	...
	if not best then
		for _, pl in ipairs(Players:GetPlayers()) do
			local f = workspace:FindFirstChild(pl.Name .. "SpawnedInToys")
			if f then
...
function controlLookNPC()
	...
	local blob = findNearestBlobman(150)
	if blob then
		startControl(blob)
		return true
	end
	...
	notify(HUB_NAME, "No NPC / Blobman nearby", 1.5)
	return false
end

function controlNearestBlobman()
	local blob = findNearestBlobman(200)
	if not blob then
		-- try spawn own then control
...
function toggleControlBind()
	...
	local blob = findNearestBlobman(100)
	if blob then
		startControl(blob)
		return
	end
	notify(HUB_NAME, _Vzd({113,148,148,144,69,134,153,69,152,148,146,138,148,147,138,81,69,149,142,136,144,69,134,69,149,145,134,158,138,151,81,69,148,151,69,152,153,134,147,137,69,147,138,134,151,69,103,145,148,135,146,134,147}), 1.5)
end
...
function kickPlayer(p, ktype, quiet)
	...
	elseif ktype == "Float Pin" then
		ownershipVisit({
			...
			onOwned = function(rr)
				...
				skyVel(rr)
				pcall(function()
					local bp = rr:FindFirstChild(_Vzd({112,142,136,144,102,154,151,134,117}))
	...
	hideAfterGrab()
	if not quiet then
		notify(HUB_NAME, _Vzd({112,142,136,144,69,128}) .. ktype .. "] -> " .. playerLabel(p), 1.5)
	end
end
...
local AURA_META = {
	{ id = "netown", title = _Vzd({116,156,147,69,115,138,134,151,135,158}), tip = "Network own nearby players (proximity)." },
	...
	{ id = "repel", title = _Vzd({117,154,152,141,69,115,138,134,151,135,158,69,102,156,134,158}), tip = "Push nearby players away (proximity)." },
...
function setAntiLag(on)
	...
	pcall(function()
		...
		if beam and (beam:IsA("LocalScript") or beam:IsA("Script")) then
			...
			notify(HUB_NAME, _Vzd({102,147,153,142,82,113,134,140,69}) .. (on and "ON (beam script off)" or "OFF"), 2)
		else
...
local function isWaterishPart(part)
	...
	if n:find(_Vzd({156,134,153,138,151}), 1, true) or n:find("ocean", 1, true) or n:find("sea", 1, true)
...
setWaterWalk = function(on)
	...
	notify(HUB_NAME, _Vzd({124,134,153,138,151,69,152,148,145,142,137,69,161,69}) .. partCount .. " parts | " .. cellCount .. " terrain cells", 2.2)
end
end)()
		...
		if child.Name == "GrabParts" then
			for _, d in ipairs(child:GetDescendants()) do
				if d:IsA("WeldConstraint") or d:IsA("Weld") then
					...
					if not target and p0 and not p0:IsDescendantOf(c) then target = p0 end
					if target then
						local m = target:FindFirstAncestorOfClass("Model")
						local plr = m and Players:GetPlayerFromCharacter(m)
						if plr and plr ~= LP then
							-- grabber side: our body is NOT the held Part1
							if not (p1 and p1:IsDescendantOf(c)) then
								return true
							end
						end
					end
...
function getPlotInteriorCF(plot)
	...
	for _, name in ipairs({ "Spawn", "SpawnLocation", "HouseSpawn", _Vzd({110,147,153,138,151,142,148,151,120,149,134,156,147}), "Floor", "Base" }) do
...
function isGucciVictim(c)
	c = c or char()
	if not c then return false end
	-- Never treat ourselves as victim while we are the grabber (was killing our own grabs)
	...
	-- GrabParts only if weld holds OUR body (Part1), not when we hold them
	for _, child in ipairs(workspace:GetChildren()) do
		if child.Name == _Vzd({108,151,134,135,117,134,151,153,152}) and grabPartsIsAttackingUs(child, c) then
			return true
		end
	...
	for _, p in ipairs(c:GetDescendants()) do
		if p:IsA("BasePart") then
			pcall(function()
				...
				if v.Magnitude > 85 or math.abs(v.Y) > 70 then
					p.AssemblyLinearVelocity = Vector3.new(
						math.clamp(v.X, -maxH, maxH),
						math.clamp(v.Y, -maxDown, maxUp),
						math.clamp(v.Z, -maxH, maxH)
					)
				end
				local a = p.AssemblyAngularVelocity
				if a.Magnitude > 20 then
...
function gucciForceFreeMove()
	...
	if isGucciVictim(c) or gucciThrowGuardActive() then
	...
	if md.Magnitude > 0.05 then
		...
		pcall(function()
			...
			if v.Magnitude > 90 or math.abs(v.Y) > 70 then
				r.AssemblyLinearVelocity = Vector3.new(
					math.clamp(v.X, -40, 40),
					math.clamp(v.Y, -50, 40),
...
function gucciBreakGrabNow()
	...
	-- Never run break logic while we are the one holding someone
	if isLocalActivelyGrabbing() and not isLocalBeingHeldFlag() then
		return
	end

	local beingHeld = isLocalBeingHeldFlag()
	if beingHeld then
	...
	pcall(function()
		r.Anchored = false
		if h and beingHeld then
			h.PlatformStand = false
			h.Sit = false
		end
	end)

	-- Struggle / DestroyGrabLine only when WE are held (they cancel OUR grabs if spammed while holding)
	if beingHeld then
		if FTAP.Struggle then
			for _ = 1, 6 do
				pcall(function() FTAP.Struggle:FireServer(LP) end)
	...
	if beingHeld or gucciThrowGuardActive() then
		...
		if FTAP.RagdollRemote and beingHeld then
			pcall(function() FTAP.RagdollRemote:FireServer(r, 0) end)
		end
		gucciStripThrowMovers(c)
		gucciCancelThrowVelocity()
...
function gucciAntiTick()
	...
	if victim and S.toggles.antiKill then
		tpToRandomHouse("grab")
	end

	if victim then
		gucciArmThrowGuard(1.5)
		gucciBreakGrabNow()
	...
	task.spawn(function()
		...
		isHeld.Changed:Connect(function(held)
			...
			if held == true then
				...
				-- right-click toss fires on release — hard cancel for ~2.5s
				S._gucciWasHeld = false
				gucciArmThrowGuard(2.75)
				gucciCancelThrowVelocity()
				gucciForceFreeMove()
				task.defer(gucciCancelThrowVelocity)
				task.delay(0.03, gucciCancelThrowVelocity)
				task.delay(0.08, gucciCancelThrowVelocity)
				task.delay(0.16, gucciCancelThrowVelocity)
				task.delay(0.3, gucciCancelThrowVelocity)
				task.delay(0.55, gucciCancelThrowVelocity)
				task.delay(1.0, gucciCancelThrowVelocity)
				task.spawn(function()
					local t0 = os.clock()
					while os.clock() - t0 < 2.75 and (S.toggles.antiGucci or S.toggles.antiGrab) do
						gucciCancelThrowVelocity()
						gucciForceFreeMove()
						RunService.Heartbeat:Wait()
					end
...
-- Make grab rope/beam visible again (farm / invis-line / stuck state)
function restoreGrabLineVisuals(root)
	pcall(function()
		local function fixBeam(d)
			if not d then return end
			if d:IsA("Beam") then
				d.Enabled = true
				d.Transparency = NumberSequence.new(0)
				if (d.Width0 or 0) < 0.05 then d.Width0 = 0.2 end
				if (d.Width1 or 0) < 0.05 then d.Width1 = 0.2 end
		...
		local function fixContainer(ch)
			...
			for _, d in ipairs(ch:GetDescendants()) do
				fixBeam(d)
				-- some FTAP builds use thin parts as the rope
				if d:IsA("BasePart") then
					local n = d.Name:lower()
					if (n:find("beam", 1, true) or n:find("line", 1, true) or n:find("rope", 1, true)
						or n == _Vzd({140,151,134,135,145,142,147,138}) or n == "string") and d.Transparency > 0.85 then
						d.Transparency = 0
		...
		if root then
			...
			for _, ch in ipairs(workspace:GetChildren()) do
				local n = ch.Name
				if n == "GrabParts" or n == "GrabLine" or n:find("Grab", 1, true) then
					fixContainer(ch)
				end
			end
			local c = char()
			...
			-- Beam move script must be on or line never draws (unless user anti-lag)
			if not S.toggles.antiLag then
				local ps = LP:FindFirstChild("PlayerScripts")
				if ps then
					local beamScr = ps:FindFirstChild("CharacterAndBeamMove")
						or ps:FindFirstChild("CharacterAndBeamMove", true)
					if beamScr and (beamScr:IsA("LocalScript") or beamScr:IsA("ModuleScript") or beamScr:IsA("Script")) then
						pcall(function() beamScr.Disabled = false end)
					end
				end
			end
		end
...
local KNOWN_TOYS = {
	...
	_Vzd({103,134,145,145,103,134,152,144,138,153,135,134,145,145}), "BallMagicLight", "BallSnowball", "BathroomShower", "BathroomSink",
	...
	_Vzd({104,148,154,136,141,113,142,140,141,153,103,151,148,156,147,108,151,134,158}), "CouchPink", "CouchWhite", "CouchPurple",
	"CounterCorner", "CounterSink", "CounterStraight", "CreatureBlobman", "CreatureRobot",
	"CupMugBrown", "CupMugWhite", _Vzd({105,142,136,138,103,142,140}), "DiceSmall", "DiscoColorBall",
...
function findFlyingBlueTrain()
	...
	if map then
		for _, ch in ipairs(map:GetChildren()) do
			local n = tostring(ch.Name):lower()
			if n:find(_Vzd({153,151,134,142,147}), 1, true) or n:find("mono", 1, true) then
				consider(ch)
			end
		end
	end
...
function runSparkBurst()
	...
	if not anchor then
		notify(HUB_NAME, "No sparkler target / character", 1.5)
		return
	end
...
function bombBodyOf(model)
	...
	if n == "BombDarkMatter" then
		return model:FindFirstChild(_Vzd({117,158,151,134,146,142,137})) or model:FindFirstChild(_Vzd({117,134,151,153,109,142,153,105,138,153,138,136,153,148,151}))
	end
...
local SUSPICIOUS_NAMES = {
	"voidz", "fe6", "rayfield", "orion", "dex", "infiniteyield", "iy_", _Vzd({137,134,151,144,137,138,157}),
...
function onGrabPartsAdded(child)
	...
	-- Always re-show line unless Invisible Line is intentionally ON (farm used to leave beams hidden)
	if not S.toggles.invisLine then
		task.defer(function()
			if child.Parent then restoreGrabLineVisuals(child) end
		end)
		task.delay(0.12, function()
			if child.Parent and not S.toggles.invisLine then restoreGrabLineVisuals(child) end
		end)
	end
	task.spawn(function()
...
do local _z718=(3*10); if _z718<0 and _Vj() then _z718=_z718+1 end end
...
_TAB_BUILDERS["home"] = function(sc, n)
		...
		makeButton(sc, { order = n(), title = _Vzd({113,142,147,144,69,108,134,146,138,69,119,138,146,148,153,138,152}), tip = "Refresh remotes if features stop working", callback = function()
			...
			notify(HUB_NAME, ok and "Remotes linked" or _Vzd({120,153,142,145,145,69,145,148,134,137,142,147,140,69,151,138,146,148,153,138,152,83,83,83}), 2)
		end })
...
_TAB_BUILDERS["combat"] = function(sc, n)
		...
		local function runOnTarget(label, fn)
			...
			if not p then
				notify(HUB_NAME, _Vzd({115,148,69,153,134,151,140,138,153,69,82,69,149,142,136,144,69,124,141,148}), 1.5)
				return
			end
		...
		makeButton(sc, {
			order = n(), title = _Vzd({103,145,148,135,146,134,147,69,106,157,153,151,134,136,153,69,102,145,145,69,107,151,148,146,69,117,145,148,153,152}),
			danger = true,
		...
		makeButton(sc, {
			order = n(), title = _Vzd({121,117,69,121,148,69,121,141,138,146}), tip = "Teleport to the selected player",
			callback = function()
		...
		blobNote.Text = _Vzd({120,149,134,156,147,152,69,135,145,148,135,146,134,147,69,80,69,104,151,138,134,153,154,151,138,108,151,134,135,83,69,124,148,151,144,152,69,142,147,152,142,137,138,69,149,145,148,153,152,69,82,69,135,158,149,134,152,152,138,152,69,141,148,154,152,138,69,149,151,148,153,138,136,153,142,148,147,83})
		blobNote.Parent = sc
		corner(blobNote, 8)
		...
		makeButton(sc, { order = n(), title = _Vzd({103,145,148,135,69,108,151,134,135,69,102,145,145}), danger = true,
...
_TAB_BUILDERS["server"] = function(sc, n)
		...
		makeToggle(sc, {
			...
			tip = _Vzd({120,149,134,156,147,69,103,145,148,135,146,134,147,81,69,151,142,137,138,69,142,153,81,69,140,151,134,135,69,138,155,138,151,158,69,149,145,134,158,138,151,69,148,147,69,153,141,138,69,152,138,151,155,138,151}),
			callback = function(on)
		...
		makeButton(sc, {
			...
			callback = function()
				for _, name in ipairs({ "lagSrv", "softLag", "hardLag", "destroySrv", "destroyHyb", _Vzd({135,145,148,135,120,151,155}) }) do
					stopMass(name)
				end
...
_TAB_BUILDERS["grab"] = function(sc, n)
		...
		makeSlider(sc, {
			order = n(), title = _Vzd({121,141,151,148,156,69,120,153,151,138,147,140,153,141}), min = 0, max = 500, default = 80, stateKey = "grabFlingPower",
		})
		makeToggle(sc, {
		...
		makeToggle(sc, {
			order = n(), id = _Vzd({151,134,140,137,148,145,145,108,151,134,135}), title = "Ragdoll Grab",
...
_TAB_BUILDERS["anti"] = function(sc, n)
		...
		makeToggle(sc, {
			...
			tip = "Grab + throw immunity: free walk while held, cancel right-click toss on release. No blob/train unsit.",
			desc = _Vzd({116,115,69,82,99,69,139,151,138,138,82,146,148,155,138,69,80,69,134,147,153,142,82,153,148,152,152,69,161,69,151,138,136,148,146,146,138,147,137,138,137}),
			callback = function(on)
		...
		makeToggle(sc, {
			order = n(), id = "antiTrain", title = _Vzd({102,147,153,142,69,121,151,134,142,147,69,84,69,103,145,148,135,146,134,147,69,120,138,134,153}),
		...
		makeToggle(sc, { order = n(), id = "antiSit", title = _Vzd({102,147,153,142,82,120,142,153,69,84,69,120,138,134,153,69,121,151,134,149}), tip = "Force unsit", callback = function(on)
		...
		makeToggle(sc, { order = n(), id = "antiRagdoll", title = _Vzd({102,147,153,142,82,119,134,140,137,148,145,145}), tip = "Disable ragdoll states", callback = function(on)
			stopLoop("antiRag")
...
_TAB_BUILDERS["player"] = function(sc, n)
		...
		makeToggle(sc, {
			order = n(), id = _Vzd({140,148,137,114,148,137,138}), title = "God Mode",
...
_TAB_BUILDERS["loop"] = function(sc, n)
		...
		makeButton(sc, { order = n(), title = "Kick Selected", danger = true, tip = _Vzd({122,152,138,152,69,144,142,136,144,69,153,158,149,138,69,134,135,148,155,138}), callback = function()
			local targets = getLoopTargets()
			if #targets == 0 then notify(HUB_NAME, "Pick a player first", 1.2) return end
			for _, p in ipairs(targets) do
				task.spawn(function()
		...
		makeToggle(sc, { order = n(), id = _Vzd({156,145,107,151,142,138,147,137,152}), title = "Whitelist Friends", tip = "Skip friends in auras/mass", callback = function(on)
		...
		local loops = {
			...
			{ id = _Vzd({145,148,148,149,112,142,136,144}), title = "Keep Kicking", tip = "Kick | map-wide", waitRespawn = true, fn = function(p) kickPlayer(p, S.kickType, true) end },
			...
			{ id = "loopVoid", title = _Vzd({113,148,148,149,69,123,148,142,137}), tip = _Vzd({123,148,142,137,69,152,145,134,146,69,161,69,146,134,149,82,156,142,137,138}), waitRespawn = true, fn = function(p) voidPlayer(p, true) end },
			...
			{ id = _Vzd({145,148,148,149,108,151,134,135,112,142,136,144}), title = "Loop Grab Kick", tip = _Vzd({108,151,134,135,69,144,142,136,144,69,161,69,146,134,149,82,156,142,137,138}), waitRespawn = true, fn = function(p) kickPlayer(p, "GrabKick", true) end },
			...
			{ id = _Vzd({145,148,148,149,103,151,142,147,140,107,145,142,147,140}), title = "Loop Bring+Fling", tip = "Bring then fling | map-wide", waitRespawn = true, fn = function(p)
				bringPlayer(p, nil, true)
				local r = rootOf(p); if not r then return end
				clearTargetMovers(p.Character)
				applyVel(r, S.flingPower or 600, 0.3)
...
_TAB_BUILDERS["toys"] = function(sc, n)
		...
		makeToggle(sc, { order = n(), id = _Vzd({149,134,145,145,138,153,118}), title = "Q = Spawn Pallet", tip = "PalletLightBrown via MenuToys", desc = _Vzd({121,148,140,140,145,138,69,148,147,84,148,139,139}), callback = setPalletQ })
		makeToggle(sc, { order = n(), id = "tabSpawn", title = _Vzd({121,102,103,69,98,69,120,149,134,156,147,69,120,138,145,138,136,153,138,137,69,121,148,158}), tip = "Spawn selected toy on TAB", callback = function(on)
			pcall(function() ContextActionService:UnbindAction("VOIDZ_TabToy") end)
			if not on then return end
		...
		makeButton(sc, { order = n(), title = _Vzd({103,145,148,135,146,134,147,69,102,151,146,158,69,77,88,78}), danger = true, callback = function()
		...
		makeButton(sc, { order = n(), title = _Vzd({104,148,154,147,153,69,114,158,69,121,148,158,152}), tip = "SpawnedInToys children", callback = function()
...
_TAB_BUILDERS["explosions"] = function(sc, n)
		...
		makeDropdown(sc, {
			order = n(),
			title = "Account type",
			options = { _Vzd({102,154,153,148,82,137,138,153,138,136,153}), _Vzd({115,148,69,140,134,146,138,149,134,152,152,69,77,86,85,85,78}), "Gamepass (200)" },
...
_TAB_BUILDERS["world"] = function(sc, n)
		...
		makeToggle(sc, { order = n(), id = _Vzd({154,147,134,147,136,141,148,151,102,154,151,134}), title = "Unanchor Aura", tip = "Unanchor + SNO nearby", callback = function(on)
			stopLoop("unanchor")
...
_TAB_BUILDERS["visuals"] = function(sc, n)
		...
		makeDropdown(sc, { order = n(), title = "ESP Depth Mode", options = { "AlwaysOnTop", _Vzd({116,136,136,145,154,137,138,137}) }, default = "AlwaysOnTop",
...
_TAB_BUILDERS["auto"] = function(sc, n)
		...
		makeToggle(sc, { order = n(), id = _Vzd({134,154,153,148,120,115,116}), title = _Vzd({102,154,153,148,69,115,138,153,156,148,151,144,69,116,156,147,69,102,154,151,134}), tip = _Vzd({102,145,142,134,152,69,148,139,69,147,138,153,148,156,147,69,134,154,151,134}), callback = function(on)
...
_TAB_BUILDERS["control"] = function(sc, n)
		...
		makeButton(sc, {
			order = n(),
			title = "Control Blobman",
			tip = "Nearest Blobman (your toys / map). Hard SNO + full 3D move.",
			callback = function() task.spawn(controlNearestBlobman) end,
		})
		makeButton(sc, {
			order = n(),
			title = "Control NPC",
...
_TAB_BUILDERS["fun"] = function(sc, n)
		...
		makeButton(sc, {
			order = n(),
			title = _Vzd({104,148,147,153,151,148,145,69,103,145,148,135,146,134,147}),
			tip = "Nearest Blobman with hard SNO + full 3D WASD/Space/Ctrl",
			callback = function() task.spawn(controlNearestBlobman) end,
		})
		makeButton(sc, {
			order = n(),
		...
		local STEAL_LIMBS = {
			...
			{ label = "Left Leg",  part = _Vzd({113,138,139,153,69,113,138,140}) },
...
_TAB_BUILDERS["settings"] = function(sc, n)
		...
		makeToggle(sc, {
			order = n(), id = "unlockMouse", title = _Vzd({122,147,145,148,136,144,69,114,148,154,152,138}),
...
local function ensureMouseModal(on)
	...
	if not m or not m.Parent then
		...
		m.Position = UDim2.fromOffset(-100, -100)
	...
	-- Modal=true is what actually frees the mouse in first-person games like FTAP
	m.Visible = on == true
	m.Modal = on == true
end

...
local BUILD = _Vzd({87,85,87,91,82,85,92,82,87,94,82,86,83,87,83,86,87,85})
...
local function voidzLoadChatMessage()
	-- original style: skulls + alternating case (💀 VoIdZ HuB LoAdEd 💀)
...
-- Once after key unlock: ONE public chat line (safe send) + local notify
function announceVoidzHubLoaded()
	...
	task.spawn(function()
		-- wait for TextChat channels so public SendAsync works
		for _ = 1, 30 do
		...
		-- PUBLIC chat (single path via voidzChat)
		voidzChat(msg)
		-- also local so you see it if filter/delay
		voidzChatSystem(msg)
...
function showVoidzSplash(device, onDone)
	...
	local grid = Instance.new(_Vzd({107,151,134,146,138}))
...
function applyVel(part, power, up)
	...
	pcall(function()
		local old = part:FindFirstChild(_Vzd({123,116,110,105,127,132,103,123})) or part:FindFirstChild("FlingAuraVelocity")
		if old then old:Destroy() end
		local bv = Instance.new("BodyVelocity")
		bv.Name = _Vzd({107,145,142,147,140,102,154,151,134,123,138,145,148,136,142,153,158})
			...
			for _, ch in ipairs(part:GetChildren()) do
				if ch:IsA("BodyPosition") or ch:IsA("BodyVelocity") or ch:IsA("BodyForce")
					...
					elseif n == "BringBody" or n == "VOIDZ_BV" or n == _Vzd({107,145,142,147,140,102,154,151,134,123,138,145,148,136,142,153,158})
						or n == _Vzd({112,142,136,144,102,154,151,134,117}) or n == _Vzd({112,142,136,144,102,154,151,134,117,86}) or n == "KickAuraVelocity"
...
local KICK_TYPES = {
	...
	_Vzd({103,145,148,135,146,134,147}), _Vzd({120,142,145,138,147,153}), "GrabKick", "StackKick",
}
...
function getBlobmanGrabKit()
	...
	local leftWeld = leftDet:FindFirstChild("LeftWeld") or leftDet:FindFirstChild("LeftWeld", true)
		...
		or leftDet:FindFirstChildWhichIsA(_Vzd({114,134,147,154,134,145,124,138,145,137}))
	...
	if not creatureGrab then
		for _, d in ipairs(blob:GetDescendants()) do
			if d.Name == "CreatureGrab" and (d:IsA(_Vzd({119,138,146,148,153,138,106,155,138,147,153})) or d:IsA(_Vzd({119,138,146,148,153,138,107,154,147,136,153,142,148,147}))) then
				creatureGrab = d
				break
			end
		end
...
function blobmanIsHoldingPlayer(p)
	if not p or not validP(p) then return false end
	local held = p:FindFirstChild(_Vzd({110,152,109,138,145,137}))
...
function isFlingMoverName(n)
	n = tostring(n or "")
	return n == "FlingAuraVelocity" or n == "SkyVelocity" or n == _Vzd({112,142,136,144,102,154,151,134,117}) or n == "KickAuraP1"
...
function installWarModeChatCommands()
	...
	local function handle(msg)
		...
		elseif m == "war-mode" or m == _Vzd({156,134,151,146,148,137,138}) or m == "voidz-war" or m == "war" then
...
function isGrabLinkedToPlayer(plr)
	...
	for _, d in ipairs(c:GetDescendants()) do
		if d:IsA("WeldConstraint") or d:IsA("Weld") or d:IsA("ManualWeld")
			or d:IsA(_Vzd({102,145,142,140,147,117,148,152,142,153,142,148,147})) or d:IsA("AlignOrientation") or d:IsA("RigidConstraint") then
			local p0, p1 = d.Part0, d.Part1
...
_TAB_BUILDERS["anti"] = function(sc, n)
		...
		makeToggle(sc, {
			...
			callback = function(on)
				...
				if on then
					pcall(resolveFTAP)
					startLoop(_Vzd({134,147,153,142,108,151,134,135}), 0.04, antiGrabTick)
					gucciDenyGrabLatch()
...
function buildMain()
	...
	LP.CharacterAdded:Connect(function()
		...
		for _, key in ipairs({ _Vzd({134,147,153,142,103,154,151,147}), "antiBanana", _Vzd({134,147,153,142,123,148,142,137}), _Vzd({134,147,153,142,107,145,142,147,140}), "antiExplode", "antiSit", "antiRagdoll", "god" }) do

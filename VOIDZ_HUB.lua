...
-- Below this size: gentle (won't shatter). At/above: full mountain speed.
S._snowSafeSize = S._snowSafeSize or 4.5

function snowGrowProfile(sz)
	sz = tonumber(sz) or 1
	local safe = S._snowSafeSize or 4.5
	-- amp/ampZ = mountain coverage (studs), wait = time per waypoint (lower = faster)
	if sz < 2.4 then
		-- still breakable — short soft rolls only
		return {
			amp = 7, ampZ = 4, force = 5e4, p = 9000, d = 2000,
			maxSpeed = 28, angMax = 18, wait = 0.4, lifts = 0.25, mode = "baby",
		}
	elseif sz < safe then
		-- almost safe — medium distance, quicker
		return {
			amp = 16, ampZ = 10, force = 3.8e4, p = 12000, d = 1400,
			maxSpeed = 55, angMax = 28, wait = 0.32, lifts = 0.1, mode = "ramp",
		}
	elseif sz < 8 then
		-- SAFE: cover a lot of mountain, go fast
		return {
			amp = 48, ampZ = 34, force = 3.2e4, p = 22000, d = 700,
			maxSpeed = 140, angMax = 80, wait = 0.18, lifts = 0, mode = "fast",
		}
	elseif sz < 12 then
		return {
			amp = 58, ampZ = 40, force = 2.8e4, p = 26000, d = 600,
			maxSpeed = 170, angMax = 100, wait = 0.14, lifts = 0, mode = "turbo",
		}
	else
		-- huge balls: max coverage / speed until target size
		return {
			amp = 70, ampZ = 48, force = 2.5e4, p = 30000, d = 500,
			maxSpeed = 200, angMax = 120, wait = 0.11, lifts = 0, mode = "turbo",
		}
	end
end
...
function snowRollWaypoints(prof)
	...
	-- fast/turbo: long path across the mountain (not just tiny left/right)
	return {
		Vector3.new(ax, 0, 0),
		Vector3.new(ax * 0.7, 0, az),
		Vector3.new(0, 0, az),
		Vector3.new(-ax * 0.7, 0, az * 0.7),
		Vector3.new(-ax, 0, 0),
		Vector3.new(-ax * 0.5, 0, -az),
		Vector3.new(ax * 0.3, 0, -az),
			...
			if _vA3(sound) then
				...
				if bp then
					...
					for _, off in ipairs(points) do
						...
						-- snappy pull when safe size+
						if prof.mode == "fast" or prof.mode == _Vzd({153,154,151,135,148}) then
							bp.P = prof.p
							bp.D = prof.d
							local f = prof.force
							bp.MaxForce = Vector3.new(f * 1.2, f * 1.5, f * 1.2)
							pcall(function()
								local to = bp.Position - sound.Position
								if to.Magnitude > 1 then
									sound.AssemblyLinearVelocity = to.Unit * math.min(prof.maxSpeed, to.Magnitude * 3)
								end
							end)
						end
...
function startSnowFarm()
	...
	local maxSz = S.ballSize or 15
	notify(HUB_NAME, "Snow farm ON | soft under " .. tostring(S._snowSafeSize or 4.5) .. " then FAST mountain rolls to " .. tostring(maxSz), 2.5)
...
local function showImmediateKeyUI()
	...
	local function setOutlineColor(host, col)
		for _, n in ipairs({ _Vzd({123,116,110,105,127,132,116,113,132,121}), _Vzd({123,116,110,105,127,132,116,113,132,103}), _Vzd({123,116,110,105,127,132,116,113,132,113}), "VOIDZ_OL_R" }) do

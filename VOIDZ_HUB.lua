...
function findFlyingBlueTrain()
	...
	if always then
		local names = { "Train", "BlueTrain", _Vzd({120,144,158,121,151,134,142,147}), _Vzd({121,141,138,121,151,134,142,147}), "Monorail", "FlyingTrain", _Vzd({113,148,136,148,146,148,153,142,155,138}) }
		for _, name in ipairs(names) do
...
local SUSPICIOUS_NAMES = {
	...
	"simplespy", _Vzd({151,138,146,148,153,138,69,152,149,158}), "remotespy", _Vzd({156,142,147,137,154,142}), _Vzd({145,142,147,148,151,142,134}), "sirius", "bloody",
	"blitz", "endoris", "poophub", "nameless", _Vzd({152,136,151,142,149,153,82,156,134,151,138}), "synapse", "drawing",
	"esp", _Vzd({136,141,134,146,152}), _Vzd({134,142,146,135,148,153}), _Vzd({152,142,145,138,147,153,134,142,146}), "freecam", "noclip", "flybv", _Vzd({135,148,137,158,155,138,145,148,136,142,153,158}),
}
...
function _isPalletPart(part)
	...
	if model and model.Parent and tostring(model.Parent.Name):find("SpawnedInToys", 1, true) then
		local n = tostring(model.Name):lower()
		if n:find(_Vzd({149,134,145,145,138,153}), 1, true) or n:find(_Vzd({136,151,134,153,138}), 1, true) or n:find(_Vzd({149,145,134,153,139,148,151,146}), 1, true) then
			return true
		end
...
_TAB_BUILDERS["blobman"] = function(sc, n)
		...
		makeButton(sc, {
			...
			callback = function()
				...
				releaseBlobmanSeatAfterFeaturesOff(true)
				for _, id in ipairs({ _Vzd({135,145,148,135,108,151,134,135,113,148,148,149}), "blobGrabAllLoop", _Vzd({135,145,148,135,106,157,153,151,134,136,153,117,145,148,153,152,113,148,148,149}), "blobKickLoop", "destroyServer", "blobControlOn" }) do
		...
		makeButton(sc, {
			...
			callback = function()
				...
				if controlState and controlState.running then stopControl(true) end
				releaseBlobmanSeatAfterFeaturesOff(true)
				for _, id in ipairs({ _Vzd({135,145,148,135,108,151,134,135,113,148,148,149}), _Vzd({135,145,148,135,108,151,134,135,102,145,145,113,148,148,149}), "blobExtractPlotsLoop", "blobKickLoop", _Vzd({137,138,152,153,151,148,158,120,138,151,155,138,151}), _Vzd({135,145,148,135,104,148,147,153,151,148,145,116,147}) }) do
					if S._toggleRenderers and S._toggleRenderers[id] then
						pcall(S._toggleRenderers[id])
					end
				end
...
_TAB_BUILDERS["server"] = function(sc, n)
		...
		makeButton(sc, {
			...
			callback = function()
				for _, name in ipairs({ "lagSrv", "softLag", _Vzd({141,134,151,137,113,134,140}), _Vzd({137,138,152,153,151,148,158,120,151,155}), "destroyHyb", _Vzd({135,145,148,135,120,151,155}), "kill", _Vzd({139,145,142,147,140}), "kick", "bring", "ragdoll", "fire", "banana", "paint", "vomit" }) do
					stopMass(name)
				end
				S.toggles.lagServer = false
				S.toggles.destroyServer = false
...
_TAB_BUILDERS["anti"] = function(sc, n)
		...
		section(sc, "WAR", n())
		makeToggle(sc, {
			order = n(), id = _Vzd({156,134,151,114,148,137,138}), title = _Vzd({124,102,119,69,114,116,105,106,69,77,142,147,155,154,145,147,69,80,69,141,148,154,152,138,69,141,148,149,78}),
